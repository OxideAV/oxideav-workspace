...
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end + DRC + DE + outer metadata walker. r17 wired ASPX_ACPL_2; **r18 wires ASPX_ACPL_1 joint-MDCT residual layer** (Pseudocode 116 stereo path + Tables 47/48 chparam_info/sap_data + §4.3.6.2.7 Pseudocode 5 max_sfb selection). Both `b_enable_mdct_stereo_proc` branches handled. 326 tests. Pending: ASPX_ACPL_3 multichannel (5_X channel walker family is multi-round scope). | — |
...
| **H.266 (VVC)** | 🚧 4:2:0 IDR decodes end-to-end with color. Full intra + inverse transform + chroma reconstruction. Deblock + SAO + ALF + r17 CC-ALF apply (§8.8.5.7) + AlfFixFiltCoeff/AlfClassToFiltMap. **r18 BDPCM intra mode (luma + chroma)** — Tables 69/70/77/78 + 132 init/ctxInc, BdpcmDir mapping (0→ANGULAR18, 1→ANGULAR50), §8.7.3 eqs. 1145-1146 + 1153-1154 transform-skip dequant + accumulation. Gated on `sps_bdpcm_enabled_flag`. 382 lib + 14 integration tests. Pending: P/B inter slices, dual-tree, CCLM, MIP/ISP. | 🚧 Forward CABAC + forward DCT-II + flat quant landed (round-trip bit-identical against decoder). Residual emit + pixel output wiring still pending. |
