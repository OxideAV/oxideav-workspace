...
| **NSF** (NES) | 🚧 ~97% — full 6502 + 2A03 APU + six expansion chips + VRC7/OPLL pipeline incl. envelope ladder + rhythm mode + NSF v1/v2/NSFe + NSFe mixe per-device default mix + §7 VRC7/OPLL AM (1.0 dB tremolo) + VIB (±7-cent) LFO audible + Namco 163 sum/divide multi-channel mixing + YM2413 rhythm noise generator (23-bit x²³+x⁹+1 LFSR); lacks exact §7 LFO depth step arrays (docs gap) | — synthesis only |
...
| **DDS** | ✅ ~99% — header + DXT10 + BC1-7 + BC6H all modes + cubemaps/arrays/volumes + 16-bit/float + packed R11G11B10_FLOAT + R9G9B9E5_SHAREDEXP + R10G10B10A2_UINT + sub-sampled packed R8G8_B8G8/G8R8_G8B8 + 8/16/32-bit plain-integer UINT/SINT HDR uncompressed surfaces + legacy X8B8G8R8/X1R5G5B5/X4R4G4B4/L16/A4L4 mask layouts + daily fuzz | ✅ ~96% — uncompressed + BC1-7 + BC6H + mip chains + cubemap/array |
| **OpenEXR** | 🚧 ~94% — scanline (+PXR24) + tiled + deep + multi-part across all 4 part types + mip/ripmap + B44/B44A scanline decode (edge-aware HALF blocks + pLinear inverse-log) + typed attribute inspectors; PIZ/DWAA/DWAB blocked on docs trace | ✅ ~96% — scanline (+PXR24) + tiled + deep + multi-part mixed write |
| **Farbfeld** | ✅ 100% | ✅ 100% |
