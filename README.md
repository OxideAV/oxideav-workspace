...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures; 0.1.13 public-surface lock | ✅ 100% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §11 intra-pick + §13.4 fitter + r168 real two-pass rate-control (first_pass_analyze MAD+variance + qindex distribution + Vp8TwoPassEncoder driver; 9-test roundtrip) + 0.1.13 public-surface lock (Vp8Encoder/Vp8EncoderConfig/Vp8EncoderStats/LoopFilterMode + 28 DEFAULT_* knobs) |
...
| **WebP** (VP8 + VP8L) | ✅ 100% — RFC 9649 container + VP8L lossless + ALPH + animated + VP8-lossy via sibling crate + API-COMPAT-0.1.2 surface lock + standalone-API e2e (8 tests under `--no-default-features`) + external-oracle e2e (dwebp / webpmux / ffmpeg cross-decode, 3 directions, byte-exact lossless) | ✅ 100% — VP8L lossless encoder (LZ77 + spatial/color transforms + color cache + multi-meta-prefix + Shannon chooser) + VP8-lossy via oxideav-vp8 0.2.1 (RIFF/WEBP wrap + From<Vp8Error> 1-to-1 adapter + lossy roundtrip) |
