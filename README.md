...
| **JPEG XL** | 🚧 ~52% — rounds 1+2+3 against ISO/IEC 18181-1:2024 final core (master @ 9100db6): C.2.1 + H.2 + H.3 + H.4.1+H.4.2 + H.6.4 inverse Palette + H.6.3 inverse RCT (6 modes × 6 perms) + H.6.2 inverse Squeeze + H.5 Self-correcting predictor (WpState). **gray-64x64 (docs) decodes PIXEL-CORRECT vs expected.png** (gradient (x+y)*2 & 0xff). pixel-1x1 regression-free. Round-3 root-cause finding: ANS state init was being read EAGERLY in entropy prelude; cjxl 0.12.0 emits it AFTER inner ModularHeader's transforms (split EntropyStream::read prelude/state-init). 200 tests. FDIS-2021 typo #5 fixed by 2024; typos #1-#4 retained. Decoder rounds 7-11 + encoder rounds 1-6 retired 2026-05-08 under fruits-of-poisonous-tree (`old` branch); v0.0.5/v0.0.6/v0.0.7 yanked. gradient-64x64-lossless + palette-32x32 EOF during multi-channel pixel decode (~12× bits/token over cjxl budget — round-4 #1); grey_8x8 (cjxl 0.11.1) Kraft-sum-overflow — distinct bug | — RETIRED 2026-05-08, re-author after decoder forward progress |
