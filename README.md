...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF atoms (gama/clap/pasp/colr/edts/elst/ctts/cslg/tref) + chan layout + faststart + udta + dref + tkhd matrix → rotation + chapter resolution + gmhd/text/tmcd headers + tmcd-in-stsd timecode parser + multi-hop alias-chain following (`open_with_aliases`, MAX_ALIAS_DEPTH=4 + cycle detection) + ISO BMFF `meta` mode (pitm + iinf v0/1/2/3 + iloc v0/1/2 + idat resolver + xml/bxml + iref typed refs) + styl/ftab/hlit/hclr/drpo text-sample style trailers; rejects fragmented MP4 with hint to `oxideav-mp4` |
| AVI       | ✅ | ✅ | ✅ | OpenDML 2.0 super-index + AVIX continuation + dmlh + vprp + LIST rec clusters + 2-field interlaced (`AVI_INDEX_2FIELD` 12-byte entries) + 02ix mid-`movi` index emit/parse + truncated-head recovery + VBR audio + LIST INFO emit + read accessors (`info_for` / `info_all_for` / multi-value `avi:info.<fourcc>`) + xxpc palette-change tracking; idx1 keyframe seek with O(1) per-stream flags cache |
...
| **AC-4** (Dolby) | 🚧 ~94% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/cfg1/cfg2/cfg3 dispatch + LFE body + SSF/SNF + SAP Tables 181 + 183 a/b/c/d (sap_mode 0/1/3) applied spectrally in 5_X ACPL_1 dispatch + Ls/Rs surround mono walker + ASPX bandwidth-extension trailer capture for 5_X SIMPLE/ASPX (cfg2 wired end-to-end with per-channel `aspx_extend_pcm`); lacks Pseudocode 121 companding gain + cfg0/1/3 ASPX trailer-aware dispatch | — |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~52% — forward CABAC + DCT-II + per-CTU SAO RDO + ALF luma per-CTB filter-set RDO + primary chroma + CC-ALF chained into IDR pipeline + inline per-CTU ALF CABAC walk (two-pass §7.3.11.2: reconstruct, RDO, then ALF bins + residual TBs interleaved per CTU) + PH-level ALF chain emission (`pps_alf_info_in_ph_flag`, `ph_alf_*` family) + quantised APS NAL emission (chroma-ALF + CC-ALF prefix-APS); end-to-end ALF round-trip verified |
...
| **VP8** | ✅ 100% — entire 15-fixture corpus bit-exact | 🚧 ~97% — I+P + B_PRED + SPLIT_MV + alt-ref/golden + Lagrangian RDO + libvpx-shape Trellis + activity AQ + RFC 6386 §15.2 mode/ref deltas (per-MB filter-level varies by ref/mode) + opt-in 4×4 B_PRED RDO + opt-in UV-mode RDO + opt-in joint LF-RDO with real rate term + opt-in SPLIT_MV partition RDO (D + λ·R over 16×8 / 8×16 / 8×8 / 4×4 with `MBSPLIT_PROBS` + `SUB_MV_REF_PROBS` rate model) + opt-in psy-RDO/ARNR |
...
| **JPEG XL** | 🚧 ~84% — ISO/IEC 18181-1:2024 final core. 5 small lossless fixtures decode PIXEL-CORRECT. Modular path complete; VarDCT scaffold (Quantizer + LfCoefficients + F.1 LF dequant + F.2 adaptive smoothing + HfMetadata + DctSelect derivation + HfBlockContext custom branch + HfGlobal C.6.2 dequant-matrix encoding modes + GlobalModular ModularHeader §C.9.1 N=0 gate + F.3.1 single-TOC-entry section chaining on shared BitReader) wired into pipeline; d1 fixture now reaches HfMetadata transforms blocker; lacks HfPass C.7 + PassGroup HF C.8.3 + GetDCTQuantWeights materialisation + IDCT dispatch + XYB / CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **SVG** | ✅ ~96% — full shape set + path + g + defs + gradients + text/tspan + mask + clipPath + use/symbol + svgz + SMIL animate/set/animateTransform at arbitrary `t` (paced + spline calcMode) + CSS3 Selectors L3 cascade + SVG2 §9.3.2 `d`-as-CSS + filter primitive rasterisation graph (13 primitives — feGaussianBlur / feOffset / feFlood / feComposite / feBlend / feMorphology / feColorMatrix / feMerge / feComponentTransfer / feDropShadow / feConvolveMatrix / feTurbulence / feDisplacementMap) | ✅ ~85% — round-trips shape/stroke/fill/gradient/transform/mask/clipPath + PreservedExtras side-channel for `<style>`/`<filter>`/`<animate>`/`<foreignObject>` fragments |
| **PDF** | ✅ ~88% — bytes → Scene via legacy xref + cross-reference streams (`/Type /XRef`) + object-stream resolver (`/Type /ObjStm`) + recursive object parser (FlateDecode + PNG predictors) + content-stream operator parser + `/Prev` chain incremental updates + `/Encrypt` decode all revisions (R=2/3/4 RC4 + AES-128, R=5 Adobe AES-256, R=6 ISO 2.0 AES-256) + per-stream `/Crypt /Identity` opt-out + public-key security handlers `adbe.pkcs7.s3/s4/s5` decode (CMS EnvelopedData with RSAES-PKCS1-v1_5 + SHA-1/256 Algorithm 12; hand-rolled DER+CMS+X.509 parsers; new `rsa` + `sha1` deps) | ✅ ~88% — PDF 1.4/1.5 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + xref-stream encoder + ObjStm encoder + incremental updates + Linearization (Annex F: lin-dict in first 1024 bytes + first-page xref + page-offset hint stream + main xref) + ObjStm+encryption combined path + `/Encrypt` ENCODE all revisions with bounce round-trip; lacks text, JPEG passthrough, public-key encoder |
...
| MSMPEG4 v3 (DIV3) | `mpg4c32.dll` | wmpcdcs8-2001 reference binary | ✅ ICERR_OK 2/2 (I + P) frames; 42.9 dB PSNR-RGB vs ffmpeg | Required: 13 stubs + `Registry::register_data` channel (`_adjust_fdiv`) + x87 ISA (8-deep f64 stack, FLD/FST/FILD/FIST/FADD…/FXCH/FCHS/FNSTSW/FLDCW + FSIN/FCOS/FPREM/FSCALE) + lowercase FOURCC pre-DRV_OPEN + DirectShow GUID handshake (`b4c66e30-0180-11d3-bbc6-006008320064` planted at `[esi+0xb4..0xc8]`, gated on MP43/MP42/MPG4 fccHandler). 12 dB delta vs bit-exact = YUV→BGR matrix difference between mpg4c32 internal converter and ffmpeg swscale (no structural mismatch). Codec maintains reference-frame state across calls. |
