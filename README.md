...
| MSADDS audio | `msadds32.ax` | winxp | ✅ **ReceiveConnection lands S_OK (r60)** — CRT drain r48-56; r57 audio CLSID `{22E24591-…}` (zero new COM stubs); r58 pin topology + Run state machine S_OK (self-contained, no msacm32 delegation); r59 clean-room ASF parser (`extract_wma_amt_from_asf` / `AmtBlueprint`) + ffmpeg WMA1/WMA2 fixtures committed. **r60 disassembled `CompleteConnect @ 0x2057` to crack QueryAccept validation: 3 gates — `wFormatTag ∈ {0x0160, 0x0161}`, `cbSize ≥ 0x29/0x2F`, `memcmp(extradata+4/10, "1A0F78F0-EC8A-11d2-BBBE-006008320064\0", 37)` (the WMA decoder's own component-CLSID stowaway signature ffmpeg cannot produce). `AmtBlueprint::wma_criteria_passing` builds an extradata that satisfies all three.** Next: r61 IMemAllocator handshake (`Receive` currently returns `VFW_E_NOT_COMMITTED`) | Same x87/COM/IBaseFilter scaffolding as MP43 video path; `AmtBlueprint::wma_criteria_passing()` constructor; clean-room QueryAccept validation in `docs/codec/msadds32-query-accept-validation.md` |
