...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~98% — §K.7.2 hierarchical DCT progression (SOF0/1/2 + SOF5 diff, 1/3-comp RGB+YUV incl. P=12 12-bit YUV/grayscale/RGB + 4-comp CMYK/YCCK) + baseline + progressive + lossless (Huffman + arithmetic, incl. SOF11 subsampled YUV-class) + 12-bit + CMYK/YCCK + RTP/JPEG + DNL (SOF Y=0, T.81 §B.2.5) + Annex J hierarchical spatial-lossless decode (1/3/4-component, EXP ×2 bi-linear upsample, bit-exact) + SOF6 differential-progressive + SOF7/SOF15 DCT-terminating + SOF11/SOF15 arithmetic spatial-lossless progression + §C Kraft-inequality DHT over-subscription guard + typed APP0/APP14/ICC views + fuzz | ✅ ~97% — baseline + progressive + lossless (Huffman + arithmetic, incl. SOF11 subsampled) + CMYK + RGB24 + grayscale paths |
...
| **ILBM** (Amiga IFF) | ✅ ~94% — BMHD/CMAP/BODY + EHB/HAM + SHAM/PCHG + ANIM op-0/1/5 + colour-range chunks + RGBN/RGB8 24-bit genlock-RLE bodies + DEEP chunky deep-raster (DGBL/DPEL/DLOC + TVDC + RUNLENGTH/ByteRun1 decode+encode → RGBA) + multi-image/cel-anim DEEP (DCHG timing + per-DBOD frames + DLOC sub-rectangle compositing → demux); lacks DEEP HUFFMAN/JPEG bodies | ✅ ~84% — muxer parity + masking + ANIM op-5 |
