...
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element; decoder body still pending | 🚧 scaffold — Phase-2 writers: section_data + ics_info + pulse_data + tns_data + scale_factor_data + DPCM accumulator pair + r160 §4.4.2.1 raw_data_block frame assembler (SCE/CPE/CCE/LFE/FIL/DSE/END byte-align) + r165 Pce::write + FrameAssembler::push_pce (294+ tests) |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~96% — baseline + progressive 4:2:0/4:2:2/4:4:4/grey + 12-bit YUV + SOF9 arithmetic + lossless SOF3 + RFC 2435 RTP/JPEG depacketization + packetization + docs-corpus CI tier-gated + r161 sixth cargo-fuzz target rtp_depacketize covering RFC 2435 surface (6 harnesses total) | ✅ ~95% — baseline + progressive + lossless SOF3 grey/RGB (all 7 Annex H predictors) + DRI/RSTn restart markers + non-zero point transform Pt 0..15 |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 41 SEI types + fuzz-hardened (r164: new `sei_payload` cargo-fuzz target + 2772-cell deterministic SEI panic-free sweep); lacks MBAFF, SVC/3D/MVC | 🚧 ~83% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite + IDR Intra_16x16 luma AC trellis + chroma AC trellis (1227 tests); ffmpeg PSNR_Y 44.20 dB |
...
| **H.266 (VVC)** | 🚧 ~66% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine sub-block MC + PROF + AMVP + SbTMVP + merge_subblock + §7.3.11.7 inter_affine_flag + cu_affine_type_flag + r164 §7.3.11.7 non-merge inter affine-syntax dispatcher composing both flags → MotionModelIdc via §8.5.5.2 eq. 160 (~1034 tests); lacks non-merge inter CU walker call-site + encoder-side dispatcher mirror | 🚧 ~85% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred — see crate README |
...
| **AV1** | 🚧 ~75% — full §5.11.5 decode_block syntax-walker end-to-end: OBU + uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.4 decode_partition + §5.11.15-18 tx-size + mode-info dispatcher + complete §5.11.22 intra mode-info + complete §5.11.23 inter mode-info cascade (find_mv_stack → assign_mv → motion_mode → interintra → compound_type → interpolation_filter) + §5.11.25 read_ref_frames + §7.10 find_mv_stack (spatial path) + §7.10.3/§7.10.4 has_overlappable_candidates + find_warp_samples + §5.11.30/§5.11.33 compute_prediction dispatcher + §7.11.2.5 DC_PRED leaf + §5.11.34 residual outer dispatch + §5.11.36 transform_tree recursion + §5.11.39 coefficients reader + §5.11.46 palette_colors_{y,u,v} + §5.11.49 get_palette_cache + §7.13 inverse transform (DCT/ADST/WHT/IDTX/FLIPADST × 4-64 × 16 PlaneTxType combos) + §7.12.2 dequant tables + §7.12.3 step-1 dequant + §5.11.47 transform_type + §7.5 get_scan dispatcher + §7.12.3 step-3 frame-buffer merge (per-plane CurrFrame[Y/Cb/Cr] with flipLR/flipUD + Clip1) (859 tests); **decoder now produces real sample output**. Lacks §9.5.3 Quantizer_Matrix + §7.10.2.5/.6 temporal-MV scan + 12 non-DC intra modes + inter prediction + loop filter + CDEF + LR + film grain | 🚧 scaffold |
...
| **JPEG XL** | 🚧 ~90% — ISO/IEC 18181-1:2024 lossless Modular path + 7 fixtures pixel-correct + VarDCT scaffold + Gaborish/EPF/AFV pure-math complete + §C.8.3 per-block HF coefficient loop + r164 typed TransformType-driven entry points (DCT16×16/DCT16×8/DCT32×32; 561 lib tests); lacks §C.7.2 histograms + per-frame Gaborish/EPF/CfL wiring (#799/#1077) | — retired |
