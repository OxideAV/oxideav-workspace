...
| **Opus** | ✅ CELT mono+stereo (incl. transient/short blocks); SILK NB/MB/WB mono 10+20+40+60 ms; SILK stereo | ✅ CELT full-band + transient short-blocks + SILK NB/MB/WB mono 20 ms + SILK NB/MB/WB stereo (NB 10/20/40/60 ms, MB+WB 20 ms; round-trip SNR 24-43 dB across the matrix) |
...
| **AAC** | ✅ AAC-LC + HE-AACv1 SBR + HE-AACv2 PS. **r20 fix-pack on real-content LC** brought solana-ad.mp4 from 84.41% → **99.41%** within ±1 LSB of ffmpeg + zero ±32K spike frames: TNS direction swap (was reversed) + TNS band-extent end clamp + M/S right-channel codebook check (was reading left) + IS phase-invert mask=1 only (was firing on mask=2) + i32 SpectralValues for codebook-11 escape range. r19 closed LC RMS interop (within ±5% in all four directions). | ✅ AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS. **r21 HE-AACv1 SBR ffmpeg-interop audit** (honest negative result): direct probe shows our encoder and fdkaac both transmit `bs_data_env[0] = 0` for tonal-low-band content (E_orig = 64 spec minimum) yet ffmpeg saturates ours and decodes fdkaac cleanly — bitstream envelope is NOT the source. End-to-end audit of §4.6.18.4.2 synthesis QMF gain, §4.6.18.7.1 E_orig formula, §4.6.18.7.5 limiter gain cap (Table 4.176), HF generator patches all spec-correct on the current freq-table choice; the divergence must come from how ffmpeg reads our specific SBR header configuration (start_freq=5, stop_freq=9, freq_scale=2, alter_scale=true, xover_band=0, noise_bands=2) vs fdkaac's. r22+ target. 170 tests + 1 ignored (the SBR amplitude test, with the round-21 audit notes pinned in). |
| **CELT** | ✅ full §4.3 pipeline (energy + PVQ + IMDCT + post-filter) | ✅ mono + stereo dual-stereo (intra-only long-block; energy + PVQ + fMDCT) |
| **Speex** | ✅ NB modes 1-8 + WB sub-modes 1-4 via QMF+SB-CELP (+ formant postfilter) + UWB folding layer; intensity stereo; typed in-band signalling per RFC 5574 / manual §5.5 | ✅ full NB ladder (sub-modes 1-8, 2.15-24.6 kbit/s) + full WB ladder (sub-modes 1-4, 16.8-32.6 kbit/s) + UWB null + folding layers (24.65 / 26.4 kbit/s); RFC 5574 §3.3 padding + Table 5.1 in-band message codec |
...
| **iLBC** (RFC 3951) | ✅ Narrowband 20 ms + 30 ms frames, RFC §4.6 six-PSSQ enhancer with §4.7 enhancer-delay synth shift (1 sub-block 20 ms / 2 sub-blocks 30 ms) | ✅ 20 ms + 30 ms frames (LPC analysis + LSF split-VQ + start-state coding + RFC §3.6 residual-domain 3-stage CB search + bit packer). **r20: §3.7 stage-0 gain-correction post-pass + §4.7 LPC shift in encoder analysis filter** — voiced 22.14 → 22.26 dB (20 ms), 24.53 → 24.73 dB (30 ms); sine modes stable at 24.81 / 26.53 dB. **r19**: residual-domain CB search + Table 3.2 bit-width caps + lMem=85 boundary CB + 30 ms LSF interpolation fix + §4.7 synth shift lift voiced SNR from 9–11 dB to 22–25 dB. Opt-in HP pre-processing biquad (§3.1) suppresses DC + mains hum (+7 dB on DC-biased input). |
...
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end + DRC + DE + outer metadata walker. r17 wired ASPX_ACPL_2; r18 ASPX_ACPL_1 joint-MDCT residual layer; r19 lands the `5_X_channel_element` walker family. r20 unblocks the ETSI Huffman-table audit (`tests/etsi_table_validation.rs` parses `ts_10319001v010401p0-tables.c` and checks every codebook byte-for-byte → 60 codebooks, 120 arrays, 0 divergences) + Cfg0/1/2 outer shells + `parse_asf_psy_info_lfe()`. **r21 ASPX_ACPL_3 transform synthesis (§5.7.7.6 Pseudocodes 118 + 119)** — `Transform()` / `ACplModule2()` / `ACplModule3()` building blocks; `run_pseudocode_118_5x()` driver wires three parallel `Transform()` outputs through D0/D1/D2 decorrelators + transient duckers, three `ACplModule2` channel-pair builds (L/Ls, R/Rs, C), three `ACplModule3` cross-residual corrections, final `sqrt(2)` scaling for `z1`/`z3`/`z4`. New `AcplMchState`/`AcplMchFrame`/`AcplMchOutput` types. 363 tests (+11 r21). Pending: ASPX_ACPL_1/2 multichannel wrappers (Pseudocode 117), 5_X-walker glue, `sf_data(ASF)` Huffman bodies for multichannel. | — |
...
| **H.264** | 🚧 **Spec-driven rewrite** (~45k LOC). All §7/§8/§9 core layers; I/P/B-slice reconstruction, POC 0/1/2, DPB + MMCO + RPLM, 6-tap luma / bilinear chroma MC, weighted + direct MV, deblock, CAVLC + CABAC, 4:2:0 + 4:2:2. **Decoder r6 — DPB output ordering on under-reported `max_num_reorder_frames`**: real-world H.264 encoders routinely emit a VUI `bitstream_restriction.max_num_reorder_frames` smaller than the actual reorder depth their stream uses (solana-ad.mp4 is the textbook case — High@L3.1 720p, claims `reorder=2`, decodes a 4-deep B-pyramid). Honouring the under-sized claim forced §C.4 to bump pictures before later POCs arrived; the engine inlet saw frames in something close to decode order with one trailing B-frame per 4 anchors. Fix in `output_dpb_sizing` (h264_decoder.rs §A.3.1 item h): **floor the reorder window at the level-derived MaxDpbMbs/PicSize cap** (Annex A Table A-1) so the §C.4 bumping process can hold all out-of-order pictures until POC ordering resolves. Companion fix in `oxideav-mp4` parses `ctts` (§8.6.1.3) + `edts/elst` (§8.6.6) so packets carry composition timestamps (CTS = DTS + ctts_offset − elst_media_time) instead of raw decode timestamps — without it the ordered-by-POC frames would still receive wrong-frame pts. End-to-end: every one of the 3960 frames on solana-ad now arrives at `oxideplay`'s engine with strictly monotonically-increasing pts. New tests `dpb_sizing_raises_undersized_vui_to_level_cap` + `b_pyramid_emits_in_poc_order_at_level_31_720p` pin the new behaviour. Hash digest changes from `da9c18e4008cfd37` → **`d4bf9d8d175a684b`** (the digest hashes pts and pts ordering changed — the pixel data is unaffected). The engine-side `insert_video_by_pts` band-aid in `oxideplay` is removed; the queue now always `push_back`s. **Decoder r5 — speed pass**: 2.01× end-to-end on `solana-ad.mp4` (1280×720 yuv420p High@3.1, 3960 frames, ~158 s of content) — wall-time **89.83 s → 44.76 s** via `oxideplay --vo hash --ao null`, hash digest unchanged (`da9c18e4008cfd37`, regression-checked). Three independent wins: (1) `src/simd/{scalar,chunked,portable}.rs` skeleton matching the `oxideav-mpeg4video` shape; chunked `interpolate_luma` rewrites the 6-tap quarter-pel kernel so the H-FIR row strip is computed once and fed to the V-FIR — diagonal-j position **22.6×** on the bench (3645 µs → 161 µs / 1000 16×16 blocks); (2) cached debug-env-var lookups (`OXIDEAV_H264_BIN_TRACE`, `OXIDEAV_H264_CABAC_DEBUG`, `OXIDEAV_H264_MB_TRACE`, `OXIDEAV_H264_RECON_DEBUG`, `OXIDEAV_H264_NO_DEBLOCK`, …) — they were each resolving via `getenv()` syscall on **every CABAC bin decode and every per-MB parse** (~140 M lookups in this clip); now `OnceLock<bool>`-cached on first access; (3) eliminated per-MB clone of the `CabacNeighbourGrid` in `parse_residual_cabac_only` (was cloning ~720 KiB per MB to dodge the borrow checker; replaced with a NLL re-borrow → ~10 TB of `_platform_memmove` removed across the file). Plus deblock-filter fast paths that bypass the `Picture::luma_at`/`set_luma` clipped accessors when the 8-sample window is fully inside the picture. **Decoder r4**: CABAC fix for High-profile B-slice content with `transform_size_8x8_flag=1`. Two related bugs — (1) Table 9-24 (ctxIdx 402..=459, Luma8x8 frame/field sig + last + level) was missing from the context init pipeline → cat-5 contexts collapsed to neutral `(0,0)` → arithmetic decoder drifted out of sync; (2) §9.3.3.1.1.9 cat=1/2 `transBlockN` derivation short-circuited to "unavailable" when neighbour had `t8x8=1`, ignoring the spec's "8x8 block is transBlockN, cbf inferred 1 from CBP" branch. On `solana-ad.mp4`: slice-skip errors **1900 → 0** end-to-end. **924 tests** (r6 +2: DPB sizing + B-pyramid POC-order regression; r5 +3 SIMD bit-exactness pins). Pending: 4:4:4, MBAFF, Annex F/G/H. | 🚧 Clean-room Baseline encoder: I-only modes + I_NxN + luma/chroma AC + deblock + Lagrangian RDO + **P-slice with integer + half-pel + quarter-pel ME (§8.4.2.2.1 eq. 8-243..8-261)** + **r19 4MV (P_8x8 PL08x8)** + **r20 B-slice (B_L0 / B_L1 / B_Bi 16x16)** + **r21 B_Skip / B_Direct_16x16 (§8.4.1.2.2 spatial direct)**. The encoder mirrors the decoder's spatial-direct derivation locally (MinPositive on refIdxLX per eq. 8-184/8-185, directZeroPredictionFlag per eq. 8-188..8-190, colZeroFlag per step 7, median MV via §8.4.1.3) and emits Direct/Skip when the derived predictor is competitive under a Lagrangian rate bias. New `EncodedFrameRef::partition_mvs` carries the L1 anchor's per-block colocated MVs for the step-7 colZeroFlag check. On a static-content B-frame this collapses the slice from 75 → 11 bytes (~85% reduction) at 52.36 dB Y PSNR, bit-equivalent against ffmpeg/libavcodec (max diff 0). ffmpeg interop bit-equivalent: 50.82 dB on 4-pixel-shift P, 49.40 dB ½-pel P, 53.61 dB ¼-pel P, **51.86 dB on bipred-midpoint B + 52.36 dB on static-content B-Skip (max diff 0)**. Bipred uses §8.4.2.3.1 default weighted average `(L0+L1+1)>>1`. All sub-pel via the decoder's `interpolate_luma` so encoder/decoder agree bit-for-bit. 929 tests. Next: intra fallback in P/B, B-slice 16x8 / 8x16 partitions, B_Direct_8x8, temporal direct, CABAC. |
| **H.265 (HEVC)** | ✅ I/P/B decode 8-bit + Main 10/12 in 4:2:0 + 4:2:2 (8/10/12-bit) intra+inter, bit-exact. SAO + deblock + scaling lists. HEIF/HEIC opt-in. r17 confirmed AMP already wired. **r18 `interSplitFlag` audit**: libx265 emits non-conformant bin at the spec-forbidden gate (`tr_depth=0 < MaxTrafoDepth=0` is false); empirical "read+use" path matched libx265's emission. **r19 `cu_skip_flag` ctxInc + merge `ref_poc` refresh** (§9.3.4.2.2 + §8.5.3.2.9): `PbMotion` grew an `is_skip` bit; `skip_ctx_inc` now reads neighbour `is_skip` (was approximating with `is_inter`). Companion `refresh_pb_ref_poc` rewrites a merge candidate's stale `ref_poc_{l0,l1}` against the current slice's RPL[ref_idx]. Lifts Main 10 inter PSNR from 25.54 → **33.57 dB**. **r20 TMVP scan-order audit (§8.5.3.2.8 / §8.5.3.2.9)**: BR → centre fallback now fires for every `availableFlagLXCol == 0` case, not just intra at BR — including the previously-missed case where the BR PB is inter but the §8.5.3.2.9 LT-flag gate fails. Added `NoBackwardPredFlag` derivation (`= 1` iff every L0/L1 ref has POC ≤ currPic) and the §8.5.3.2.9 listCol picker for both-flags-set collocated PBs (LX vs LN where N = `collocated_from_l0_flag`). Pre-r20 the merge path preserved the col PB's pred flags directly (skipping listCol selection entirely) and AMVP used the current-invocation `want_l0` as the selector — correct for P-slices but wrong for any B-slice with a backward L1 ref. Merge temporal candidate now invokes §8.5.3.2.8 once per side (L0 / L1) per §8.5.3.1.7 step 3/4 with `predFlagLX = availableFlagLXCol` (eqs. 8-115 / 8-118). New `hevc_b_slice_tmvp_scan_order_audit` test — 5-frame I-P-B-P-B fixture, 24.51 dB Y-PSNR. P-slice paths preserved byte-for-byte (Main 10 inter holds at 33.57 dB). 48 tests. Pending: 4:4:4 12-bit, broader B-slice merge / AMVP audit. | ✅ Baseline CAVLC I+P + Main-profile CABAC P-slice encoder. I 45 / P 31 dB via our decoder; ffmpeg accepts at 26.82 dB. |
| **H.266 (VVC)** | 🚧 4:2:0 IDR decodes end-to-end with color. Full intra + inverse transform + chroma reconstruction. Deblock + SAO + ALF + r17 CC-ALF apply (§8.8.5.7) + AlfFixFiltCoeff/AlfClassToFiltMap. r18 BDPCM intra mode (luma + chroma) — Tables 69/70/77/78 + 132 init/ctxInc, BdpcmDir mapping (0→ANGULAR18, 1→ANGULAR50), §8.7.3 eqs. 1145-1146 + 1153-1154 transform-skip dequant + accumulation. Gated on `sps_bdpcm_enabled_flag`. **r19 MIP (§8.4.5.2.2)** — boundary downsampling + 30 weight matrices (Tables 276..305) + transpose + upsampling end-to-end. **r19 CCLM (§8.4.5.2.14)** — 4:2:0 down-sampled-luma kernels (eqs. 366-369, both `chroma_vertical_collocated` branches), neighbour-sample kernels (eqs. 370-377 + bCTUboundary fallback eq. 373), eq. 400 divSigTable, 4-point min-max regression (eqs. 386-389), `(a, b, k)` derivation (eqs. 390-403), eq. 404 Clip1 predictor; wired for INTRA_LT_CCLM / INTRA_L_CCLM / INTRA_T_CCLM. **r20 ISP (§8.4.5.1)** — Intra Sub-Partitions: new `isp` module exposes Table 13 `IntraSubPartitionsSplitType` + `NumIntraSubPartitions` (NoSplit→1, (4,8)/(8,4)→2, else→4) and the eqs. 251-260 per-partition derivation (`nW`, `nH`, `nPbW = max(4, nW)`, `pbFactor`, `xPartIdx`, `yPartIdx`, `xPartPbIdx`). Leaf-CU reader gains `decode_transform_unit_isp` honouring §7.3.11.10 InferTuCbfLuma + last-partition chroma CBF gating; `LeafCuResidual.luma_subparts` carries per-partition residual records. `reconstruct_leaf_cu_isp_luma` walks subpartitions in spec order so partition i+1 reads samples freshly written by partition i; `pbFactor==2` (vertical splits with nW∈{1,2}) shares the prediction window across paired partitions per eq. 260. 427 lib + 14 integration tests (+11). Pending: P/B inter slices, dual-tree. | 🚧 Forward CABAC + forward DCT-II + flat quant landed (round-trip bit-identical against decoder). Residual emit + pixel output wiring still pending. |
| **VP6** | ✅ Full FLV playback (845/845 sample frames). | ✅ VP6F keyframe + skip + inter encoder (integer-pel SAD ME). **r21 fix — `DEF_MB_TYPES_STATS` pair-order spec alignment**: per `vp6_format.pdf` page 30 `VP6_BaselineXmittedProbs[3][20]` and page 29 Table 6 (second-dimension semantics), each row flattens as `(probSame_t, probDiff_t)` for `t` in `0..10`. Pre-fix the table stored pairs reversed; `Vp6Model::rebuild_mb_type_probs` (mirrors spec page 35's `probModeSame` formula) therefore produced `mb_type[ctx][prev][0]` values with stay-rate semantics where the spec yields switch-rate semantics, and the layout disagreed with the already-spec-compliant `PRE_DEF_MB_TYPE_STATS` (`VP6_ModeVq` page 32). After the swap both tables share the same pair-order convention end-to-end. Pinned by `tables::tests::def_mb_types_stats_matches_spec_baseline`. ffmpeg-side acceptance of the inter packet still pending — the swap is necessary but not sufficient on its own; r20 audit notes in `src/encoder.rs` (head doc) record the residual r21+ suspect list (per-MB coefficient state machine; `vector_predictors` ctx mapping per spec page 28 Table 5). New `r21_inter_frame_ffmpeg_decode_state` regression test records the contract and flips red the moment ffmpeg starts accepting inter packets so the assertion can be tightened to `n == 2`. **r20 fix — Buff2Offset spec compliance** retained: per `vp6_format.pdf` Tables 2 & 3 the field is the literal frame-buffer byte offset of partition 2; r19 had emitted/parsed it with a +/-2 fudge (internally consistent but wrong on the wire). Both encoder + decoder now match the spec definition (verified by manual hex parse of the dump produced by `VP6_DUMP_INTER=1`). 59 tests (+2 over r20). |
| **VP8** | ✅ I+P frames (6-tap sub-pel + MV decode + ref management) | ✅ I + P frames, all 5 intra modes + SPLIT_MV + loop filter (42-51 dB PSNR) |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV list + inter-mode ctx + compressed-header probs. **r20 §7.4.6 `read_skip` ctx fix**: both intra and inter `read_skip` were reading against a hard-coded `skip_probs[0]=192` since round 13; the spec-literal `ctx = (AvailU ? AboveSkip : 0) + (AvailL ? LeftSkip : 0)` derivation lived in `skip_ctx()` but was bypassed under `#[allow(dead_code)]`. Encoder now mirrors the fix in `tile.rs::emit_block`. Lift on the c64 diagnostic fixture: **Y 61.90 → 70.10 dB** (chroma stays bit-exact). Pattern fixture barely moves (9.90 → 9.67 dB) — the remaining lossless gap stacks additional spec divergences in the sub-8×8 mode-info path (round-15 empirical `+1` neighbour anchor in `read_intra_sub_mode` regresses compound when reverted to spec-literal `+idx/+idy`). r19 audit confirmed §8.7.1.10 WHT, §8.6.2 reconstruct, §8.5.1 DC_PRED, §9.3.2 KF_PARTITION_PROBS layout, §6.4.6 default_intra_mode tree, §6.4.25 get_scan, §6.4.24 token initial-context all spec-correct. 159 tests. | — |
| **AV1** | 🚧 OBU + range coder + coeff decode + partitions + transforms + all intra predictors + CDEF + LR + inter MC + palette + skipModeAllowed + multi-ref DPB compound MC. r17 fixed §5.9.2 `ref_order_hint` over-read + §5.9.20 `lr_unit_extra_shift` inversion. r18 `parse_global_motion_params` (§5.9.24/§5.9.25/§7.20) three correctness fixes (ROTZOOM/AFFINE read order, TRANSLATION absBits, PrevGmParams plumbing). r19 chain-walk diagnostic + AFFINE bit-count regression test pinned the SVT-AV1 chain baseline at 38/48 Frame OBUs. **r20 §7.8 `set_frame_refs()` + AV1_TRACE_BITS env-gated bit-position diagnostic.** New `frame_header::set_frame_refs()` runs the spec-verbatim §7.8 algorithm (find_latest_backward → ALTREF, find_earliest_backward → BWDREF + ALTREF2, Ref_Frame_List forward fill, global earliest fallback) when `frame_refs_short_signaling=1`; previously every entry of `ref_frame_idx[]` was left at 0 in this branch, so `dpb.saved_gm_params_for(ref_frame_idx[primary_ref_frame])` always looked up DPB slot 0 instead of the real `last_frame_idx` slot. The fix is bit-neutral by construction (consumes no bitstream bits) but is the spec-mandated input to `load_previous()` per §7.20. `AV1_TRACE_BITS=1` env var dumps `bit_position()` at every uncompressed-header checkpoint (and per-slot inside `parse_global_motion_params`) to bisect future bit-account regressions. The chain count stays at 38/48 because the r19-flagged first failure (pkt 3 Frame #7) is a bitstream-level issue independent of the §7.8 walk: AV1_TRACE_BITS confirms the bit position is exactly `reduced_tx_set=110`, slot 1 ROTZOOM consumes 18 bits to reach 128, slot 2 AFFINE type bits land at 128-130, leaving 5 bits for 6 params that need ≥24 bits. The 10 failing frames are all of this same shape, not a parser bug — reproducing the bit account from spec-only sources lands on the same numbers. 6 new `set_frame_refs_tests` unit tests lock the §7.8 algorithm (empty DPB, mixed forward+backward, all-forward, OrderHint disabled, determinism, primary_ref slot lookup). Sacred SkipMode + 41.97 dB compound invariants preserved. 356 tests (+6). Pending (carried into r21): the 10/48 AFFINE-overrun frames need separate spec study (likely a `load_previous_segment_ids`/`setup_past_independence` interaction or a film-grain-pre-padding gap), full primary_ref_frame=4 chain, palette finalization, TX shape coverage, loop-filter, intrabc. | — |
...
| **AVIF** | 🚧 End-to-end decode works (HEIF box walker → AV1 OBU handoff → `VideoFrame`). Flat-content fixtures decode cleanly; rich content gated by AV1 quality. r17 closed `bbb_alpha`/`kimono_rotate90` investigations (AV1 surfaces clean Unsupported TX-shape errors). **r18: MIAF brand validation** (typed `BrandClass` + 8 brand consts: `avif`/`avis`/`avio`/`mif1`/`msf1`/`miaf`/`MA1B`/`MA1A`; per ISO/IEC 23000-22 §7); end-to-end `imir` + `clap` round-trip tests; **`colr` surface** (nclx CICP triple OR ICC payload, grid → tile-0 fallback). **r20: CICP color path** — `CicpTriple` quadruple `(primaries, transfer, matrix, full_range)` with H.273 `Unspecified = 2/2/2/false` defaults applied via `AvifInfo::effective_cicp()`; predicates `is_identity_matrix` (RGB matrix=0), `is_libavif_srgb_default` ((1, 13, 6)), `has_reserved`; name lookups (`primaries_name`/`transfer_name`/`matrix_name`) covering BT.709/BT.2020/Display P3/sRGB/PQ/HLG/identity. Spec-correct: AVIF readers do NOT apply colour transforms (av1-avif §4.2.3.1). 106 tests (up from 86, +20). Pending: gated on oxideav-av1 TX-shape coverage + intrabc + grid hardening. | — |
