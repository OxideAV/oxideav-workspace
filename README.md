...
| **MP1** | ✅ clean-room Layer I decode to PCM (ISO/IEC 11172-3 only) — frame-header + sync + length + CRC-presence + bit-allocation/scalefactor/§2.4.3.2 requant + Table 3-B.1 rescale + 512-tap Table 3-B.3 polyphase synthesis filterbank (matrixing + cross-frame overlap-add); `oxideav_core::Decoder` registered (WAVE 0x0050 / A_MPEG/L1); mono/stereo/joint-stereo; 55 tests; lacks CRC-16 verify | 🚧 encoder not started (rebuild) |
...
| **MP3** | 🚧 clean-room rebuild underway (2026-05-24) — MPEG-1/2 frame-header parser + per-frame length + resyncing frame-walker + Layer III side-info parser (MPEG-1 2-granule + MPEG-2/2.5 LSF 1-granule, §2.4.1.7) + scalefactor decode stage (bit reservoir, slen tables, MPEG-1 scfsi reuse, LSF 4-partition derivation; §2.4.2.7/§2.4.3.4; 62 tests, ISO/IEC 11172-3/13818-3 only); lacks main-data Huffman big-values/count1 + requantize + IMDCT + synthesis (Annex B now staged), encoder | 🚧 not started (rebuild) |
| **AAC** | 🚧 ~87% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened bounds + gapless `iTunSMPB` + §4.6.18.5 SBR boundary upsample + AAC-LD (AOT 23) `er_raw_data_block` bootstrap for channelConfiguration 1 (SCE) and 2 (CPE common-window + independent-ICS) at 512/480-sample frame lengths + multi-RDB ADTS decode (1..4 raw_data_blocks/frame, CRC + no-CRC) + AAC-LD long-term prediction decode (§4.6.7 ltp_data + single-tap predictor) + AAC-LTP full frame decode (AOT 4, long-window SCE/CPE/LFE — §4.6.7 long-term prediction wired end-to-end through PNS/IS/M-S/TNS per Figure 4.2); lacks multichannel LD, low-overlap window, ELD, USAC | 🚧 ~84% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy + TNS (CPE + SCE) + perceptual M/S decision §6.6.1.3 with Johnston binaural masking + PE-based VETO/PROMOTE gates |
