...
| MSADDS audio | `msadds32.ax` | winxp | ✅ **PE-load complete (r56)** — `image_base=0x1c400000`, `entry=0x1c40233d`, `DllGetClassObject=0x1c403325` (DirectShow audio splitter). CRT drain rounds 48-56: `_endthreadex` → `_strnicmp` → `_beginthreadex` → `_ftol` → `rand`/`srand` (seedable per Sandbox) → `_CIpow`. Next: drive `DllGetClassObject` → `IClassFactory::CreateInstance` → audio decode pipeline | x87 FPU stack pop/push for `_CI*` math helpers; LCG-seeded `rand` for reproducible encodes |
