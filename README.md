...
| **AV1** | 🚧 ~58% — OBU + uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.4 decode_partition walker + §5.11.5 decode_block syntax-walker skeleton (intra arm composed, 4 next-stub variants) + 10 §5.11.x leaves (556 tests); lacks §5.11.16 read_block_tx_size + §5.11.18 inter_frame_mode_info + §5.11.30 compute_prediction + §5.11.34 residual + intra/inter prediction + IDCT + loop filter + CDEF + LR + film grain | 🚧 scaffold |
