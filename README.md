...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal-method compositor (§23) + structured Application Extension parsing (NETSCAPE2.0 loop+buffer, Adobe XMP, ICC profile §26.c.iv/v, Exif 2.3 §4.7.2) + lazy `Playback` / `LoopingFrameIter` (NETSCAPE2.0 semantics) + `decode_first_frame()` fast-path — clean-room rebuilt from CompuServe spec + Welch 1984 only | ✅ 100% — per-frame palettes + `optimize_color_tables()` GCT/LCT hoisting |
