...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate, 48 dB on HE-AACv1 reference. | ✅ AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS. ffmpeg decodes 25/25 frames, 0 errors (r16 confirmed the prior "sbr_dequant" warning is benign EOF cleanup). PCM-scale convention mismatch with ffmpeg/fdk-aac float [-1,1] still gates ffmpeg-interop PSNR — needs coordinated decoder QMF rescale. 151 tests. |
...
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end (HF adjust + noise + tone + limiter + complex-covariance TNS, 78-81 dB round-trip). DRC + A-CPL + DE parser stacks. Outer §4.2.14.1 metadata() walker. **§5.7.7 A-CPL QMF synthesis math landed** (1615-LOC: diff decode P121, dequant Tables 203-208, interpolate P109, decorrelator P111, transient ducker P112-114, channel-pair P115/116). 317 tests. Pending: wire A-CPL into Ac4Decoder, ASPX_ACPL_3 multichannel, P92-94 sinusoid. | — |
...
| **H.265 (HEVC)** | ✅ I/P/B decode for **8-bit + Main 10 + Main 12** in 4:2:0 + **4:2:2 (8/10/12-bit) intra + inter** all bit-exact (∞ dB) including testsrc (r16 fixed §7.4.9.10 4:2:2 cbf inference — was inheriting parent into stacked-vertical chroma slot). SAO + deblock + scaling lists. HEIF/HEIC opt-in. 45 tests. Pending: 4:4:4 12-bit, tiles+WPP, AMP, `interSplitFlag` desync. | ✅ Baseline CAVLC I+P + Main-profile CABAC P-slice encoder. I 45 / P 31 dB via our decoder; ffmpeg accepts at 26.82 dB. |
| **H.266 (VVC)** | 🚧 4:2:0 IDR decodes end-to-end with color. Full intra + inverse transform + chroma reconstruction. In-loop filters: deblock (short + long luma + chroma strong/weak) + SAO (EO/BO + CABAC parser) + **ALF (luma + chroma + classification — 25 classes × 4 transpose)**. 362 lib + 14 integration tests. Pending: CC-ALF apply, AlfFixFiltCoeff/AlfClassToFiltMap, P/B inter slices, dual-tree, CCLM, MIP/ISP/BDPCM. | 🚧 Forward CABAC engine + forward DCT-II + flat quant landed (round-trip bit-identical against decoder). Residual emit + pixel output wiring is r17. |
...
| **AV1** | 🚧 OBU + range coder + coeff decode + partition + transforms + all intra predictors + CDEF + Loop Restoration + inter MC + palette + skipModeAllowed + multi-ref DPB compound MC. r15 fixed §8.2.6 `read_symbol` inversion (cancelled in degenerate cases, hid for 14 rounds). r16 added §5.11.38 `Subsampled_Size` chroma TX dispatch + §7.13.3 dequant clips. SVT-AV1 SkipMode compound now decodes end-to-end (luma_mean 112.68/108.45). gray P-frame Y 41.97 dB invariant. 343 tests. Pending: bitreader "out of bits" gap in pkt 1, palette finalization, loop-filter gaps. | — |
