...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~78% — forward CABAC + DCT-II + per-CTU SAO RDO + APS-signalled luma ALF Wiener RDO + chroma residual + spec-shaped coding_tree shells + cu_qp_delta + 128×128 forced-QT-split + MTT BT+TT picker RDO + multi-row `CuNeighbourMap` + P-slice + B-slice (PRED_L0/L1/BI RDO, `(predL0+predL1+1)>>1` average bi-pred) + sub-pel MC on both (§8.5.6.3.2 Table 27 8-tap luma; P/B half-pel 51.57/quarter 52.39 dB); lacks multi-ref DPB, weighted bi-pred, chroma sub-pel |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle (per-plane uniform-fill envelope + libavcodec-version probe) | 🚧 ~58% — keyframe + luma+chroma intra-mode RDO + real P-frame inter (single LAST_FRAME ref + sub-pel ME ½/¼-pel via EightTap §6.3 — 55.23/55.88 dB at half/quarter-pel) + quadtree partition emit (64→32→16 PARTITION_SPLIT via Lagrangian SAD+λR per §6.4.16) |
...
| **Cinepak** (CVID) | ✅ ~95% — frame header + multi-strip + V1/V4 codebooks + intra + inter with skip + full selective-update family + grayscale + Sega FILM demuxer | ✅ ~95% — stateful `CinepakEncoder` with rolling codebooks + multi-strip + median-cut + skip-MB + two-pass rate ctrl + per-MB V1/V4 Lagrangian RDO + per-frame strip-count picker + LBG codebook split refinement (Linde-Buzo-Gray 1980); 40.77 dB on 64×64 at 4-strip — **+3.87 dB over ffmpeg's reference encoder** |
...
| MSADDS audio | `msadds32.ax` | winxp | 🚧 **Pipeline driven through Receive, E_FAIL inside inner-decode (r68)** — CRT drain r48-56; r57 audio CLSID `{22E24591-…}`; r58 pin topology + Run state S_OK; r59 ASF parser + WMA fixtures; r60 cracked QueryAccept (37-byte CLSID stowaway); r61 dual-pin allocator handshake; r62-65 inner-decode forensics; **r68: ffmpeg-derived extradata flips Receive HRESULT 0x8000FFFF → 0x80004005**, proves codec reads codec-private-data at init; r63 helper_addref patch likely retirable. Next: arm watchpoint inside RVA `0xc887..0xc973` to catch the 4 inner-decode arg-NULL guards | Same scaffolding as MP43 video; `AmtBlueprint::wma_{criteria_passing,with_ffmpeg_extradata_prefix}()`; clean-room QueryAccept disasm at `docs/codec/msadds32-query-accept-validation.md` |
...
| **`oxideav-audio-filter`** | Audio effects & conversions (streaming) | ✅ ~30 filters: Volume / NoiseGate / Echo / Resample / Spectrogram / Biquad family / Compressor / Limiter / DcBlocker / StereoWidener / Reverb (Schroeder) / Tremolo / Vibrato / AutoPan / LoudnessITU (BS.1770) / PitchShift (SOLA) / Chorus / Flanger / Phaser / Equalizer / Bitcrusher / TapeSaturation / HumFilter / Crossover / White/Pink/Brown noise / SilenceDetector — see crate README for the catalogue |
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~97 filter types / 117 factory names — see crate README for the catalogue |
