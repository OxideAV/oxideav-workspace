...
| **Vorbis** | ✅ all residue types (matches lewton/ffmpeg) | 🚧 floor1 envelope + ATH floor + **trained VQ codebooks (in-tree LBG, 4×256×16 books, 11.4% byte savings)** + **per-band point-stereo thresholds (4 bands above crossover with falling correlation thresholds 0.60/0.50/0.40/0.35 — HF coupled more readily; preserves phase on decorrelated HF that previously got monoised)** + **lookup-codebook optimiser (`codebook_optimizer::optimise_setup`: detects lookup_type-2 books whose entries lie on a Cartesian grid ≤ 32 distinct values per dimension, rewrites to lookup_type-1 with bit-identical `vq_lookup` outputs; 90-94% multiplicand-bit savings on synthetic grid books; in-tree LBG centroids correctly rejected via `NotPossible`)** + **floor0 LSP encoder (`floor0_encoder::should_use_floor0` spectral-flatness gate via Levinson-Durbin order-4 prediction-gain ≥ 4× picks floor0 for tonal content; LSP analysis chain autocorr → Levinson-Durbin → LPC → LSP via symmetric/antisymmetric polynomial root-finding 512-point grid + 40-iter bisection; 256-entry dim-2 [-1,1]² Cartesian-grid VQ lookup_type=1; tonal 440+880 Hz fixture round-trips via this crate's decoder with fundamental energy beating off-band probes ≥ 2×; default `make_encoder` still floor1, per-frame floor0/floor1 selection inside one setup pending mapping rewire)**; lacks bitstream-resident trained books in setup header |
...
| **MP3** | ✅ MPEG-1 Layer III M/S | 🚧 CBR + VBR mono+stereo + **MS-stereo joint mode (44.8% smaller on mono fold-down)** + **short/start/stop block selection on transients (per-channel energy-ratio detector + 1-granule lookahead block-type machine; isolated-transient pre-onset PSNR 54.37 → ~300 dB, +245 dB pre-echo elimination)** + **MPEG-1 intensity-stereo encode (per-band qualifier on silence/hard-pan/coherence ≥ 0.85 walking sfb 20 → 7, IS rewrite L := sign·sqrt(L²+R²) / R := 0; 18.8% smaller on HF-correlated stereo at q=4)** + **MPEG-2/2.5 LSF intensity-stereo encode (`IsVariant` dispatch from MPEG-1 vs MPEG-2 path; 5-bit `is_pos` 0..30 + sentinel 31; R-channel `scalefac_compress_9 = 358` packs IS half of `SCF_PARTITIONS_MPEG2[0][16..28]` at 98 bits; MPEG-2 side-info per-granule `scalefac_compress_9` was hard-coded 0; **27.9% saved on 22.05 kHz HF-correlated** vs 5-8% target, ffmpeg cross-decode clean L=15857/R=13420)**; lacks Bark-partition spreading psy-1 |
| **AAC** | 🚧 AAC-LC + HE-AACv1 SBR + HE-AACv2 PS; lacks LD/ELD, USAC | 🚧 AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS + **ffmpeg SBR warning closed via r26 3-bug fix (QMF matrix + envelope clamp + flush SBR FIL)** + **PNS (63.93% smaller on noise-rich)** + **5.1 multichannel (chcfg=6) + 7.1 multichannel (chcfg=7, [SCE C, CPE L,R, CPE Ls,Rs, CPE Lb,Rb, LFE], all 8 channels ffmpeg cross-decode clean at PSNR 22-36 dB)** + **gapless padding tuning (`gapless::GaplessInfo` + `AacEncoder::iTunSMPB_string()` exposing 2112-sample AAC-LC encoder delay / 2624-sample HE-AAC delay + packet-rounded trailing padding; flush-time silence-padding loop + `set_skip_gapless_padding` gate; iTunSMPB carrier hooks ride oxideav-id3 TXXX or oxideav-mp4 edts/elst; concatenated 0.5 s+0.5 s 440 Hz fixture decodes through ffmpeg with no audible click and no SBR warnings, per-sample boundary delta 562 vs ~2962 average sine slope)**; psychoacoustic model basic |
| **CELT** | ✅ full §4.3 pipeline | 🚧 mono + stereo intra-only long-block + short-block on transient detection + **per-band TF decisions (Viterbi search over TF_SELECT_TABLE, L1-norm cost per RFC 6716 §5.3.6)** + **dual-stereo Hybrid body for Opus stereo Hybrid** + **LM=2 short-block path (480-sample MDCT, configurable per-instance frame_samples/coded_n/lm; unlocks Opus 10 ms Hybrid)** + **encoder comb pitch pre-filter (`pitch_analysis` NCC autocorrelation over τ ∈ [15, 1022] with sub-multiple guard, 3-bit gain index from peak NCC, best-of-three tapset by minimum residual; encoder applies `comb_filter` with negated decoder-IIR gains, maintains per-channel pf_in_history matching decoder; mono + stereo wired with shared higher-NCC parameter set; tonal A/B test flips post_filter flag 6/6 packets at SNR ≥ pre-filter-off; libopus parses both A/B variants cleanly)**; lacks libopus-bit-exact IMDCT (separate gap, blocks absolute SNR convergence) |
...
| **AC-3** (Dolby Digital) | ✅ Full decode + downmix (90+ dB vs ffmpeg) | 🚧 acmod 1/2/3/6/7 + LFE + rematrix + short-block + spec-§8.2.2 transient detector + per-channel `fsnroffst[ch]` tuning + DBA + **5-fbw multichannel coupling (per §7.4.1, all fbw channels of 5.1 acmod=7 join cpl group; +3.12 dB avg-fbw PSNR at 320 kbps; required 3 bitstream fixes: phsflginu gated on acmod==2, cplabsexp clamped ≤12, overhead_bits_for skips phsflginu for non-2/0)** + **E-AC-3 (Annex E) independent-substream encode (ffmpeg cross-decode 20.21 dB)** + **per-block bit-pool tuning (`tune_per_block_snroffst`: per-block PSD demand grouped 0-2 vs 3-5 to align with D15 exponent strategy, search `(down, up) ∈ [0,8]²` step pairs with budget gate, optional fine-grained per-channel walk; `PerBlockSnr` carries [csnroffst;6]+[fsnroffst_ch;6]+[cplfsnroffst;6]+[lfefsnroffst;6], snroffste(blk) flag emitted at 27 bits/transition stereo; +1.07 dB block-3 PSNR on transient-block fixture at matched byte count, ffmpeg cross-decode clean)** + **E-AC-3 dependent-substream encode (per ATSC A/52 Annex E + Table E2.5: indep+dep pair from 7.1 input — indep `strmtyp=0` carries L/C/R/Ls/Rs/LFE 5.1 downmix at 384 kbps, dep `strmtyp=1` carries Lb/Rb at 192 kbps with `chanmape=1` + `chanmap=0x0200` (bit 6 = Lrs/Rrs-pair location); per-substream lfeexpstr/audblk D15 LFE expo + chanmape/chanmap insert after compre, convexpstre/convsnroffste suppressed on dep; ffmpeg decodes via §E.3.8.1 5.1 fallback to 6 channels with non-trivial L energy)** |
| **AC-4** (Dolby) | 🚧 A-SPX + DRC + DE walker + **ETSI Huffman tables (60 codebooks byte-for-byte)** + ASPX_ACPL_1/2/3 transform synth + multichannel `sf_data(ASF)` Huffman walk + **ASPX_ACPL_1/2/3 inner body walkers all wired** + **5_X + 7_X channel-element walkers (channels=5/6 → 5.0/5.1, channels=7/8 → 7.0/7.1; 7_X has 2-bit codec_mode vs 5_X 3-bit, no ASPX_ACPL_3, no companding for SIMPLE/ASPX, additional-channel block for SIMPLE/ASPX)** + **mono / stereo / joint-stereo short-frame `sf_data(ASF)` Huffman walk (per Tables 39-42: outer `for g = 0..num_window_groups` loop with per-group 8-bit reference_scale_factor + DPCM `first_scf_found` shared across groups + 1-bit `b_snf_data_exists` gate; `derive_per_group` resolves (transf_length_idx, transform_length, max_sfb) from (ti, psy) per Pseudocodes 2/5 incl. `b_different_framing` Pseudocode 3 grouping-bit shift + `num_windows_0 - 1` boundary; joint-stereo grouped variant carries band-wise max_quant_idx scalefactors + per-group `ms_used[g][sfb]` flags; wired into all four SIMPLE/ASPX/ASPX_ACPL1/ASPX_ACPL2 mono+stereo dispatch sites)** | — |
...
| **MPEG-4 Part 2** | ✅ I+P+B-VOP, 4MV direct, half/quarter-pel, field-MV/DCT/alt-vertical-scan, GMC (S(GMC) routing wired), data partitioning (ARTS@L1, DC/MOTION marker decode), **RVLC reverse decoder per Annex E.1.4.4 (forward + reverse walkers via per-buffer bit-reverse; reverse table verified prefix-free over 169-symbol Tcoef set; 30-bit escape detected via `s0000` reverse signature)** + **RVLC §E.1.4.4.2.1 strategy 1-4 production picker (chained reverse decoder with `pending` carry across LAST flags + per-block boundary tracking; clean DP+RVLC paths use Strategy-4 output-agreement shortcut for 16/16 bit-exact identity; mid-byte XOR conservatively cuts to `N-N1-1`/`N-N2-1` per Figure E.5/E.7 to avoid silently-valid-but-wrong overlap decodes since RVLC tables are dense; new `RvlcBlockDesc` / `RvlcBlockOutcome` / `RvlcPickerStats` API; routed from `dp::decode_ivop_dp` + `decode_pvop_dp_with_grid` when `vol.reversible_vlc=1`)** | 🚧 I+P+B + 1MV/4MV + intra-MB-in-P + quarter-pel + single-warp GMC + data partitioning + per-VOP-type quantiser knobs + GOP knob + **intra-in-P MBs under DP (PMbDp::{Inter, Intra} pre-pass, intra MCBPC in part-1, ac_pred + cbpy + 6 DC differentials in part-2, intra AC in part-3; scene-change fixture roundtrips at 38.69 dB Y through ffmpeg)** + **RVLC encoder (Table B.23 codewords + 30-bit escape, 16-block fixture with 3-byte mid-stream corruption: standard Tcoef recovers 7/16 forward-only vs RVLC 14/16 with reverse walker = 2× pixel-area beat)**; ffmpeg cross-decodes DP at 39-44 dB; lacks mid-VOP packet splits, Inter4MV under DP, DP+GMC, DP+B-VOPs, multi-warp GMC, ACE Sprite VOP, MPEG-4 Studio profile, scalability layers |
| **Theora** | ✅ I+P (Theora has no B-frames) | ✅ I+P incl. INTER_MV_FOUR |
| **H.263** | 🚧 I+P + half-pel + Annex J + D + F + E + N + G (PB-frames) + **Annex I (advanced INTRA + AC prediction)** + **Annex M (Improved PB-frames, per-MB Table M.1 MODB VLC selecting bidir / fwd-only / bwd-only B-block prediction)** + **Annex K slice-structured (§K.2 SSTUF/SSC/SEPB1/MBA/SEPB3/GFID header walk keyed off OPPTYPE bit 10; `try_consume_slice_header` snapshot+restore guard rejects false-positive 00-00-8? patterns inside zero runs; per-slice MV-grid reset matches §K.1 rule 1)**; lacks L-W | 🚧 I+P + diamond ME + Annex F/J/D/N + Annex G PB-frames + **r15 B-block residual (28.8→55-57 dB B-half)** + **Annex I (~21% smaller intra-rich)** + **Annex M (Lagrangian RDO across bidir / fwd / bwd shapes, -52% on mixed-motion fixture; ffmpeg lacks in-band Annex M signal so cross-decode is informational)** + **Annex K slice-structured emit (PLUSPTYPE picture-header SSS body + per-slice SSC/MBA emission with MBA/SWI bit widths from Tables K.2/K.3, MV-grid rebuilt every slice boundary so corrupted bytes mid-slice don't cascade)**; lacks intra-in-P MVD, PB+Annex F, plus L, P, Q, R, S, T, U, V |
...
| **H.264** | 🚧 ~45k LOC spec-driven rewrite; I/P/B + DPB + MMCO/RPLM + CAVLC + CABAC + 4:2:0/4:2:2 + B-pyramid POC ordering; lacks 4:4:4, MBAFF, Annex F (SVC), G (3D), H (MVC) | 🚧 Baseline-ish: I + P (1MV/4MV, ¼-pel) + B-slice 16x16/16x8/8x16 + B_Direct + **B_8x8 mixed sub_mb_type (44.61 dB)** + **explicit weighted prediction (98.2% smaller on fade-in)** + **4:2:2 IDR emit (47.6 dB)** + **4:4:4 IDR Intra_16x16 emit (42.25 dB, profile_idc=244)** + **intra fallback in P/B slices (Lagrangian RDO with snapshot/rollback; 81.6% smaller on occlusion fixture; ffmpeg + this crate's decoder both bit-exact)** + **CABAC encode for I + P (clean-room mirror of decoder's §9.3.4 state machine: `EncodeDecision`/`EncodeBypass`/`EncodeTerminate` + `RenormE` + §9.3.4.4 `put_bit` carry-propagation with outstanding-bits buffering; reuses decoder's `RANGE_TAB_LPS` / `TRANS_IDX_LPS/MPS` Tables 9-44/9-45 verbatim; init from existing `CtxState::init` Tables 9-12..9-26; binarisations for `mb_skip_flag`, `mb_type` I+P incl. intra-suffix Tables 9-36/9-37, `mvd_lX` UEG3, `ref_idx_lX` unary, `coded_block_pattern` 4-bit luma + 2-bit TU chroma, `coded_block_flag`, significance map, `coeff_abs_level_minus1` UEG0 uCoff=14, `coeff_sign_flag`, intra_chroma_pred_mode, mb_qp_delta, end_of_slice_flag, plus complete `encode_residual_block_cabac`; per-MB `CabacEncGrid` ctxIdxInc tracking with §9.3.3.1.1.9 transBlockN-unavailable-when-cbp-clear rule; PPS `entropy_coding_mode_flag=1` + slice header `cabac_init_idc`; 64×64 IDR self-roundtrip 47.58 dB Y bit-exact, ffmpeg interop 47.58 dB Y bit-exact, IDR+P bit-exact via P_Skip dominance — round-30 P MVs pinned to (0,0) for static fixtures, full mvp deferred)**; ffmpeg interop bit-exact at 50-52 dB on B-frames; lacks B-slice CABAC encode, P/B at 4:2:2/4:4:4, multi-ref temporal direct, VUI/SEI tuning, I_NxN under CABAC |
| **H.265 (HEVC)** | 🚧 I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2/4:4:4; SAO + deblock + scaling lists; HEIF/HEIC opt-in; broader B-slice merge/AMVP audit pending | 🚧 Baseline CAVLC I+P + Main CABAC P-slice + B-slice (TrailR mini-GOP=2) + per-CU SAD-RDO + **AMP partitions (−10.9% on high-contrast)** + **Main 10 IDR (45.05 dB ffmpeg, Qp'Y=38)** + **Main 12 IDR (45.04 dB, Qp'Y=50, profile_idc=4)** + **Main 4:4:4 8-bit IDR (44.99 dB ffmpeg, chroma_format_idc=3 + max_8bit/10bit/12bit constraint flags per §A.3.4)** + **Main 4:4:4 10 IDR (45.05 dB ffmpeg on Yuv444P10Le gradient, profile_idc=4 RExt with `max_10bit=1, max_8bit=0` distinguishing from 8-bit row, Qp'Y = 26 + QpBdOffsetY = 38, full luma-resolution chroma TBs at 16×16 with `log2TrafoSizeC = log2TrafoSize` per ChromaArrayType==3, NEUTRAL = 1<<9 = 512)** + **Main 4:4:4 12 IDR (45.04 dB ffmpeg on Yuv444P12Le gradient, §A.3.4 row `max_12bit=1, max_10bit=0, max_8bit=0`, Qp'Y = 26 + QpBdOffsetY = 50, NEUTRAL = 1<<11 = 2048; new `slice_writer_main444_12.rs` mirrors Main 4:4:4 10 at 12-bit precision; encoder dispatch arm placed BEFORE the broader (12,_) Main 12 row so the more-specific 4:4:4 match wins)**; lacks Nx2N / 2NxN, mvd_l1_zero_flag, mini-GOP > 2 at 10/12-bit, P/B at 4:4:4, RDO refinements |
| **H.266 (VVC)** | 🚧 4:2:0 IDR intra (BDPCM, MIP, CCLM, ISP) + ALF/SAO/CC-ALF + P/B merge+skip + §8.5.6.3 fractional-pel MC (8-tap luma + 4-tap chroma) + **HMVP (5-entry circular buffer §8.5.2.6)** + **temporal merge (collocated MV at BR/centre, §8.5.2.15 buffer compression, POC scaling per eqs 600-605)** + **pairwise-average merge candidate (§8.5.2.4 four-way per-list switch on `mergeCandList[0]+[1]`, §8.5.2.14 round_mv signed-magnitude divide-by-2-toward-zero, inserted between HMVP step 7 and zero-MV pad step 9 per §8.5.2.2 step 8)** + **MMVD merge (§8.5.2.7 Tables 17 + 18, `MMVD_DISTANCE_TABLE` regular [1,2,4,8,16,32,64,128] + fullpel ×4 variant gated on `ph_mmvd_fullpel_only_flag`, 4 cardinal MMVD_SIGN_TABLE directions, eqs 188/189 `MmvdOffset = (MmvdDistance<<2) * MmvdSign` lands in 1/16-pel; uni-pred + equal-POC bi-pred branches wired, CABAC Tables 103/104/105 ship with TR cMax=7 + FL cMax=3 binarisations)** + **CIIP merge (§8.5.6.7 weighted-average of planar intra + regular merge MC; weights from neighbour-CU intra-mode count `wIntra = isIntraTop + isIntraLeft` → weight pair (1,3)/(2,2)/(3,1); per-picture 4×4 intra-coded grid sampled at A `(xCb-1, yCb-1+cbHeight)` and B `(xCb-1+cbWidth, yCb-1)` neighbours; eq. 998 `(w * intra + (4-w) * inter + 2) >> 2` with bit-depth clamping; chroma reuses same `w` per eqs 995/996 SubWidthC/SubHeightC scaling + PLANAR chroma intra; CABAC Tables 106 (`ciip_flag`) + 92 (`cu_coded_flag`) shipped; §7.3.11.7 `regular_merge_flag` parse wired with `ctxInc = !cu_skip_flag` per Table 132)**; lacks GPM, AMVR/BCW, BDOF/DMVR/PROF, dual-tree, full inter residual decode, affine, explicit weighted prediction, asymmetric-POC bi-pred MMVD, CIIP-with-MMVD | 🚧 forward CABAC + DCT-II + flat quant only; residual emit + pixel output pending |
| **VP6** | ✅ Full FLV playback (845/845 sample frames); **Huffman coefficient path on `use_huffman==1` (per-frame `HuffmanTreeSet` built from spec-page-14 `VP6_CreateHuffmanTree` after bool-coded model updates, 24 AC trees indexed by `[prec][plane][band]`, 9-leaf ZRL trees + EOB/DC-zero raw-bit `1 + R(2)` per spec page 81)** | 🚧 keyframe + skip + inter + **real DCT residual** (43 dB internal-decoder) + **quarter-pel sub-pel ME (29→35 / 35→37 dB on translating fixtures, ffmpeg interop 31.65 dB)** + **golden-frame refresh (25% reduction on periodic-structure)** + **INTER_FOURMV (per-8×8 ME with ±2 pel + ±3 qpel refine, FOURMV-vs-single-MV Lagrangian RDO; 32% reduction on diverging-block fixture; ffmpeg cross-decode clean)** + **Huffman coefficient encode (`encode_keyframe_huffman` sets MultiStream=1, writes Buff2Offset R(16), bool-codes partition 1 with `use_huffman=1` then emits Huffman partition 2 via spec-faithful tables; ffmpeg rejects spec-compliant Huffman keyframes — its decoder uses non-spec `vp6_huff_coeff_map[22]` reorder per `libavcodec/vp6.c::vp6_build_huff_tree`, so the cross-decode test is logged as a probe rather than asserted; this crate's own decoder roundtrip is the binding correctness guard)** |
| **VP8** | ✅ I+P (6-tap sub-pel + MV decode + ref management) | 🚧 I+P + all 5 intra modes + SPLIT_MV + loop filter + alt-ref/golden-ref planning + Lagrangian RDO + **per-MB-context MV-ref probabilities (-13.4% / -22.9% on smpte/gray)** + **segment maps (variance-quartile classifier; -14.2% bytes on bit-saving config)** + **scene-cut adaptation (per-frame MAD vs running mean+stddev with absolute-12 floor; tapered qindex boost over 4 post-cut frames; ffmpeg cross-decode clean)**; lacks true look-ahead alt-ref synthesis |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV + compressed-header probs; r22 fixed §6.4.3 `decode_partition` HORZ/VERT double-call (lossless-pattern Y: 10.41 → 47.70 dB, chroma bit-exact); **r24 §6.4.16 sub-8×8 inter mode-info per-4×4-sub-block iteration** + **r25 within-MB sub-block MV mixing per §6.5.14 (`append_sub8x8_mvs` per-sub-block refined `MvRefs` written back into `BlockMvs[refList]`)** + **r26 between-MB §6.5.11 `get_sub_block_mv` (per-4×4 neighbour `SubMvs[r][c][refList][idx]` storage in `InterMiCell::sub_mvs: [[Mv; 4]; 2]`; populated per §6.4.4 line 2422 from sub-8×8 `block_mvs_a/b` or per §6.4.16 line 2700 cell-MV repeated for ≥8×8; lookup via `IDX_N_COLUMN_TO_SUBBLOCK = [[1,2],[1,3],[3,2],[3,3]]`; `block = -1` returns `sub_mvs[refList][3]` keeping cell-level path bit-identical; new `find_mv_refs_geom_block(..., block)` reads `get_sub_block_mv` in first-two-neighbour loop; both within-MB and between-MB §6.5 sub-block MV asymmetry axes now closed per spec; r22 lossless-pattern Y 47.70 dB + r24 + #180 numbers all preserved)** | — |
| **AV1** | 🚧 OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound; SVT-AV1 chain 48/48; `inverse_2d_spec` covers all 159 (TX_TYPE × TX_SIZE) pairs + **inter Y `tx_type` CDF reads (§5.11.45, +0.82 dB vs libdav1d)** + **inter chroma `tx_type` derivation per §5.11.40** + **palette intra-within-inter path (palette_screen.ivf decodes at 8.56 dB Y)** + **full per-MI per-edge loop_filter per §7.14 (`derive_lvl(mi, plane, pass, lf, seg)` picks `loop_filter_level[i]` + adds `feature_data[seg][SEG_LVL_ALT_LF_*]` + adds `(ref_deltas[ref] << n_shift) + (mode_deltas[modeType] << n_shift)` when mode_ref_delta_enabled, n_shift = lvl >> 5, INTRA skips mode_deltas, clamp [0, MAX_LOOP_FILTER=63]; `walk_pass8/16` walks MI grid in 4×4 luma units chroma stepped by `1 << sub_*`, `applyFilter = is_tx_edge && (is_block_edge || !skip || is_intra)`, vertical-then-horizontal per §7.14.1; 4-tap (size==4) / 6-tap (chroma >4) / 8-tap (luma 8) / 16-tap-as-8 (luma 16, acceptable per spec) dispatch; **side fix:** `narrow_mask` had `limit/blimit` swapped per §7.14.6.2 — symmetric so SVT chain still passed but per-pair was effectively no-op while cross-edge was over-strict; new SVT-AV1 fixture lf_active.ivf with deblock-only/CDEF-LR-off; SVT chain still 48/48)**; lacks intrabc, full primary_ref_frame=4 chain | — |
| **Dirac / VC-2** | ✅ VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + arithmetic + 10/12-bit; ffmpeg bit-exact 8-bit 4:2:2/4:4:4 + 10-bit 4:2:0 | 🚧 HQ + LD intra encoders + **Dirac core-syntax intra encoder (0x0C)** + Dirac core-syntax inter encoder (OBMC) + **quarter-pel sub-pel ME (per-level 8-neighbour gradient refinement, §15.8.11 8-tap half-pel filter + §15.8.10 bilinear sub-half, camera-pan fixture 26.92 → 52.04 dB Y, +25 dB)** + **OBMC-aware ME (per §15.8.6: after sub-pel SAD search, `obmc_refine_me` iterates `obmc_refine_passes` (default 2) and for each block scores its 8 sub-pel neighbours by rebuilding the same §15.8.5 weighted-sum reconstruction the decoder performs, keeping whichever MV minimises per-block SSE; reuses `obmc::spatial_wt`/`subpel_predict`/`interp2by2` for symbol-for-symbol decoder agreement; neighbour-sum incrementally maintained across overlapping blocks; **translate_2_-1 self-roundtrip 32.56 → bit-exact ∞ dB**, translate_0_-4 30.24 → 33.37 dB Y, camera-pan 26.92 → 28.02 dB Y; ffmpeg cross-decode capped near 19-22 dB on synthetic fixtures regardless of MV quality due to upstream ffmpeg OBMC convention difference, no-regression-vs-baseline guard test in lieu of A/B beat)**; homogeneous core-intra + core-inter chain ffmpeg cross-decode hard-asserted (intra 52.06 dB, inter 20.17 dB); lacks 2-ref bipred, wavelet residue |
...
| **EVC** (MPEG-5) | 🚧 NAL + SPS/PPS/APS + slice_header + **full §9.3 CABAC engine (now with 51 Main-profile init tables landed: §9.3.5 Tables 40-90 covering BTT/SUCO/MMVD/affine/IBC/AMVR/ATS/ALF/EIPD/sig_coeff/coeff_abs_level_greaterA-B/last_sig_coeff_x-y_prefix etc, all per-context (initValue, shiftIdx) installed via `init_main_profile_contexts()` per eq. 1425/1426; §9.3.4.2.x ctxInc helpers all 11 subclauses incl. Table 97 ctxSetIdx + Table 98 Rice-parameter; `MAX_CTX_TABLES` bumped 64→91 so spec table numbers index directly; 14 spot checks against printed Tables) + §7.3.8 slice_data walker + §8 intra prediction (5-mode Baseline) + §8.7.4 inverse DCT-II for nTbS ∈ {2,4,8,16,32,64} + §8.7.2/§8.7.3 dequant + per-CU intra/inter reconstruct + 8-bit yuv420p + Baseline P/B inter (8-tap luma + 4-tap chroma sub-pel, AMVP candidate list, default-weighted bipred) + cbf!=0 residual coding + EVC luma + chroma deblock per §8.8.2 eqs 1167-1182 (2-tap stencil with per-slice `slice_cb_qp_offset` / `slice_cr_qp_offset` wired into both IDR and inter pipelines)**; `make_decoder` works end-to-end on Baseline IDR + P + B (PSNR=∞ on flat fixtures); Main-profile syntax decode (BTT/SUCO/MMVD/affine/etc.) still surfaces `Unsupported` — round 27+ wires the actual tools against the now-shipped CABAC infrastructure; lacks RPL for non-IDR, HMVP, ALF, DRA, IBC, ATS, ADCC | — |
...
| **JPEG XL** | 🚧 Signature + SizeHeader + ImageMetadata + ISOBMFF `jxlp` container + **2019 committee-draft Modular sub-bitstream pixel decode (ABRAC + BEGABRAC + MA-tree + 5 named predictors per Annex H) + 2021 FDIS migration through round 7: ANS module + FrameHeader + TOC + ImageMetadata + LfGlobal + GlobalModular + modular_fdis live `JxlDecoder` factory, 7 documented spec typos in `project_jpegxl_fdis_typos.md`** + **round 7 typo #6 + #7 RESOLVED via the 2026-05-02 clean-room behavioural-trace doc `docs/image/jpegxl/libjxl-trace-reverse-engineering.md` (libjxl v0.12.0 commit 950c327, source-quote-free): `log_alpha_size_minus_5` was on the wrong branch of `use_prefix_code` (ANS-only `5 + u(2)`; Prefix branch fixes 15) AND `ans/prefix.rs::CLCL_VL_TABLE` had four of six entries with wrong bit codes — the original wasn't even prefix-free; restored canonical-Huffman per RFC 7932 §3.5; cjxl 8×8 grey lossless now decodes through the FULL MA-tree (7 nodes / 4 leaves all predictor=Gradient) into the symbol-stream prelude; stops at 2nd per-cluster prefix code on NEW typo #8 (cl_code Kraft sum ~4× over budget — cjxl emits a clcl array summing to Kraft 37 against spec's 32; djxl decodes it without trouble — round 8 target)** | — |
