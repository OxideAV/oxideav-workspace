...
| **Shorten** (.shn) | ✅ ~95% — all 10 FN cmds + filetypes 1-11 + format-v1 + running-mean estimator + 64-bit bit-reservoir reader using hardware `lzcnt`/`clz` + fused SoA stereo decode (predictor recurrence writes scratch, single strided commit applies bshift; 2.13× mono / 1.16× stereo throughput) | ✅ ~88% — production encoder + Levinson–Durbin LPC + BITSHIFT lossy + lossy `-n N`/`-r N` bit-budget modes + 64-bit reservoir BitWriter + DIFF0+mean correctness fix (172→194 tests) |
...
| **MPEG-4 Part 2** | ✅ ~85% — I+P+B-VOP + 4MV + ¼-pel + field-MV/DCT + GMC + DP + RVLC | 🚧 ~90% — I+P+B + 4MV + ¼-pel + multi-warp GMC + static-sprite + DP + RVLC + MPEG-quant + §6.3.5.2 mid-VOP resync marker emission (ffmpeg cross-decodes 96×96 period-9 at 44.5 dB I-VOP / 26.6 dB P-VOP) |
...
| **H.265 (HEVC)** | 🚧 ~74% — I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2/4:4:4 + SAO + deblock + 4:2:2 inter §7.3.8.9 stacked-pair chroma TB CABAC bin pair (Profile-4); HEIF/HEIC corpus 14/14; textured 4:2:2 P-slice CABAC drift pending docs trace (#444) | 🚧 ~75% — I+P + B (mini-GOP > 1 at 8/10/12-bit + 8-bit 4:4:4) + AMP + HBD + 4:4:4 P/B writers; lacks SAO/deblock RDO, HBD 4:4:4 AMP/merge/B_Skip |
...
| **VP6** | ✅ ~95% — full FLV playback (845/845 sample frames) | 🚧 ~90% — keyframe + inter + iterative diamond qpel ME with SATD sub-pel scoring (4×4 Hadamard, λ-scaled) + INTER_FOURMV + Huffman + bool/Huffman RDO + PID rate ctrl + Trellis quant |
