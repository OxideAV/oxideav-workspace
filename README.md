...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF atoms (gama/clap/pasp/colr/edts/elst/ctts/cslg/tref) + chan layout + faststart + udta + dref + tkhd matrix → rotation + chapter resolution + gmhd/text/tmcd headers + tmcd-in-stsd timecode parser + multi-hop alias-chain following (MAX_ALIAS_DEPTH=4 + cycle detection) + ISO BMFF `meta` mode (pitm + iinf + iloc + idat + xml/bxml + iref typed refs) + HEIF/HEIC `iprp/ipco/ipma` item-properties (colr/pasp/clap/pixi/ispe/irot/imir/auxC + Other passthrough) + meta-only files open without moov + styl/ftab/hlit/hclr/drpo text-sample style trailers + typed-iref helpers (`derived_from`/`auxiliary_for`/`thumbnail_of`/`describes`); rejects fragmented MP4 with hint to `oxideav-mp4` |
| AVI       | ✅ | ✅ | ✅ | OpenDML 2.0 super-index + AVIX continuation + dmlh (`dmlh_total_frames()` typed accessor) + vprp (per-field rect array surfaced) + LIST rec clusters + 2-field interlaced (`AVI_INDEX_2FIELD` 12-byte entries) + 02ix mid-`movi` index + truncated-head recovery + VBR audio + LIST INFO emit + read accessors (`info_for` / `info_all_for` / multi-value `avi:info.<fourcc>`) + xxpc palette-change tracking; idx1 keyframe seek with O(1) per-stream flags cache + `seek_to_keyframe_strict` returning `KeyframeSeekResult { target, landed, gop_distance }` |
...
| **AC-4** (Dolby) | 🚧 ~96% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/cfg1/cfg2/cfg3 dispatch all trailer-aware + LFE body + SSF/SNF + SAP Tables 181 + 183 a/b/c/d (sap_mode 0/1/3) + Ls/Rs surround mono walker + ASPX bandwidth-extension trailer capture wired end-to-end across cfg0/1/2/3 with per-channel `aspx_extend_pcm` + Pseudocode 121 companding (sync_flag=0 b_compand_avg=false branch); lacks sync_flag=1 + b_compand_avg=true companding sub-branches | — |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~55% — forward CABAC + DCT-II + per-CTU SAO RDO + ALF luma per-CTB filter-set RDO + primary chroma + CC-ALF chained into IDR pipeline + inline per-CTU ALF CABAC walk + PH-level ALF chain + quantised APS NAL emission + APS-signalled luma filter-set learning (Wiener filter design + Gauss-Jordan solve + lattice quant scale 2^7 saturate ±64 + AlfCoeffL APS id=2; per-CTU 17-trial RDO `{off, fixed 0..15, learned APS}`); end-to-end ALF round-trip verified |
...
| **VP8** | ✅ 100% — entire 15-fixture corpus bit-exact | 🚧 ~97% — I+P + B_PRED + SPLIT_MV + alt-ref/golden + Lagrangian RDO + libvpx-shape Trellis + activity AQ + RFC 6386 §15.2 mode/ref deltas (static + opt-in adaptive ladder via per-bucket SSE-deviation deltas, ±6 cap) + opt-in 4×4 B_PRED RDO + opt-in UV-mode RDO + opt-in joint LF-RDO + opt-in SPLIT_MV partition RDO + opt-in Trellis context-rate (real above/left non-zero predictor `nctx ∈ {0,1,2}`) + opt-in psy-RDO/ARNR |
...
| **SVG** | ✅ ~97% — full shape set + path + g + defs + gradients + text/tspan + mask + clipPath + use/symbol + svgz + SMIL animate/set/animateTransform at arbitrary `t` (paced + spline calcMode) + CSS3 Selectors L3 cascade + SVG2 §9.3.2 `d`-as-CSS + filter primitive rasterisation graph (15 primitives — feGaussianBlur / feOffset / feFlood / feComposite / feBlend / feMorphology / feColorMatrix / feMerge / feComponentTransfer / feDropShadow / feConvolveMatrix / feTurbulence / feDisplacementMap / feDiffuseLighting / feSpecularLighting with Distant/Point/Spot light sources) | ✅ ~85% — round-trips shape/stroke/fill/gradient/transform/mask/clipPath + PreservedExtras side-channel for `<style>`/`<filter>`/`<animate>`/`<foreignObject>` fragments |
| **PDF** | ✅ ~90% — bytes → Scene via legacy xref + cross-reference streams + object-stream resolver + recursive object parser (FlateDecode + PNG predictors) + content-stream operator parser + `/Prev` chain incremental updates + `/Encrypt` decode all revisions (R=2/3/4 RC4 + AES-128, R=5/R=6 AES-256) + per-stream `/Crypt /Identity` opt-out + public-key security handlers `adbe.pkcs7.s3/s4/s5` decode (CMS EnvelopedData with RSAES-PKCS1-v1_5 + SHA-1/256 Algorithm 12; both `IssuerAndSerial` + `SubjectKeyIdentifier` recipient forms; hand-rolled DER+CMS+X.509) | ✅ ~92% — PDF 1.4/1.5 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + xref-stream encoder + ObjStm encoder + incremental updates + Linearization (Annex F + F.4.2/F.4.3/F.4.4 hint-table headers) + ObjStm+encryption combined path + `/Encrypt` ENCODE all revisions + public-key ENCODE (`pkcs7_s4` / `pkcs7_s5_v4_aes128` / `pkcs7_s5_v5_aes256` constructors with multi-recipient + SKI-form support); lacks text, JPEG passthrough |
