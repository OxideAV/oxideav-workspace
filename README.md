...
| **APE** (Monkey's Audio) | 🚧 ~35% — every pinned layer implemented: ksum pivot + predictor inverse + five-level cascade walk + General-Decoding-Process pipeline (entropy behind injected boundary) + StreamConfig version/level dispatch, self-consistent PCM round-trips all levels; real-file PCM blocked on range-coder state machine + header tail + delta rule + shift position + KSum recurrence (extraction ask filed) | 🚧 ~5% — scaffold |
...
| **AV1** | 🟢 ~92% — full spec-driver decoder, 26-stream conformance corpus BYTE-EXACT (intra + inter GOPs, show-existing, 10/12-bit, QM, segmentation-inter, jnt-comp/OBMC/dual-filter, film grain, superres, tiles, edge-overhang) + registry serves whole-GOP inter decode (cross-packet session); lacks scaled-reference MC + one pre-existing CDEF+SGR entropy divergence (repro pinned) | 🚧 ~34% — intra encode YUV→IVF (10/13 own-stream conformance) + single-ref P-frame inter encoder (integer + sub-pel ME, decoder-reconstructible); lacks RD picker + compound/multi-ref inter encode |
