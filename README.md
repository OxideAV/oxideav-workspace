...
| DVD-Video | ✅ | — | — | `oxideav-dvd` Phase 3 — ISO 9660 + UDF 1.02 + VIDEO_TS + IFO (VMGI/VTSI/TT_SRPT/PGCI/chapter materialiser + VOBU_ADMAP + TMAPTI + VTSI_MAT/VMGI_MAT video/audio/subpicture + karaoke MC ext attribute blocks) + VOB demux (MPEG-PS + Nav-Pack PCI/DSI + AC-3/DTS/LPCM/subpicture router) + VOB → MKV + `dvd://` URI; Phase 3c VM (RegisterFile + RSM stack + SET-arith + CmpOps/SetOps + Type 4..6 compound) + SPU decoder + RGBA compositor + LPCM audio-pack + typed UOP mask + time-based seek. Lacks CSS auth |
...
| **Shorten** (.shn) | 🚧 ~33% (post-2026-05-18 orphan) — `ajkg` magic + v2/v3 ulong + svar(n) + per-block function dispatch + VERBATIM/QUIT + DIFF0..3 + Rice residual + per-channel carry + running mean + QLPC predictor + `decode_stream` + `Decoder` trait + streaming decode + write_diff0/1/2/3_block + write_qlpc_block + write_zero_block predictor encoders (DIFF0..3 + QLPC + ZERO) + min_energy selectors; lacks predictor-selection sequencer + BITSHIFT/BLOCKSIZE writers + #1267 | 🚧 scaffold |
...
| **MPEG-2 video** | 🚧 ~72% — §6.2.x walk + §7.6.3.x PMV + §7.6.4-8 forming-predictions/combine/add-saturate + §7.4 inverse-quant + §A IDCT + §7.2.2 residual VLC + §7.3 inverse scan + §7.2.1 intra-DC + §6.2.5/§6.2.6 macroblock-block + §6.2.4 slice walker (opt-in `block_decoding_enabled` runs §6.2.6 `block(i)` per pattern_code) + per-slice §7.2.1 DC predictor carried + §7.6.6 skipped-macroblock + §6.2.5.1 macroblock_modes() + §6.2.5 motion_vectors + marker_bit + CBP wire-parse + §6.3.17.4 pattern_code[12] (4:2:0/4:2:2/4:4:4); lacks §7.6.3.1 PMV reconstruction wiring + quant_matrix_extension downloads | 🚧 scaffold |
...
| **Theora** | 🚧 ~54% — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3 tables + §6.4.x quant + DCT-token Huffman + §7.1–§7.5 frame walk + §7.5 motion vectors + §7.7.1 EOB Token + §6.4.1 LFLIMS + §7.7.2 Coefficient Token + §7.7.3 DCT Coefficient + §7.8.1 DC predictor + §7.8.2 Inverting DC Prediction + §7.9.2 Dequantization + §7.9.1 Predictors (intra/whole-pixel/half-pixel) + §7.9.3 Inverse DCT + §7.9.4 per-block reconstruction + §7.9.4 frame-level driver (per-plane tiler over coded-order quadruple); lacks §7.10 loop filter + §2.3 coded-order resolver | 🚧 scaffold |
...
| **H.266 (VVC)** | 🚧 ~73% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine + PROF + AMVP + SbTMVP + VPS + §7.3.10.10 amvr CABAC + decomposed §7.3.10.10 mvd_coding body parser (per-bin inspection surface) | 🚧 ~93% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred + §7.3.11.7 non-merge inter pre-residual + amvr_enc + §7.3.10.5 bcw_idx_enc + multi-CP-MV affine MVD + composite affine+AMVR+BCW dispatchers + reader-side composite walkers (affine + AMVR / + BCW) + decomposed §7.3.10.10 mvd_coding writer |
| **VP6** | 🚧 r22 — §13 static tables + §3 RawBitReader + §7.3 BoolCoder + §13.2.1 DC arithmetic + §13.3.1 AC coefficient arithmetic decoder + edge-clamped MC fetch + §13.3.3.1 decode_ac_zero_run BoolCoder walk + §11.1 MV-component arithmetic decoder + §11.2 per-frame MV-probability update (Table 13/14/15 walk) | 🚧 scaffold |
| **VP8** | ✅ 100% | ✅ 100% |
...
| **AV1** | 🚧 ~95% — decoder feature-complete + standalone `decode_av1` entry + §6.7.2 Y-only monochrome + multi-SB dyn-Y dispatch up to 128×128 | 🚧 ~37% encoder — pixel-space YUV→IVF + 14-mode intra picker + §7.13.3 forward 2D + WHT lossless + forward quantize + §7.11.5.3 UV_CFL_PRED + base_q_idx>0 lossy + rectangular extents + monochrome encoder dyn driver + multi-super-block tiling (128 cap) + 4:2:0 YUV multi-super-block + §8.2.6 post-renormalisation probes + §7.13.3 rectangular TX_4X8/TX_8X4 forward dispatcher (× 2896 per-row post-scale); lacks remaining 12 rectangular TX sizes + §5.11.18 inter mode_info + RD picker |
...
| **JPEG XS** | 🚧 ~82% — ISO/IEC 21122 Part-1 + 5/3 DWT + Annex C/D/F/G + multi-component + CAP-bit + high bit depth + 4:2:0 chroma at NL,y≥3 | 🚧 ~94% — Nc 1/3/4 + Sd>0 + RCT + Star-Tetrix + NL up to 8 + odd dims + vertical prediction + per-band Q + NLT + high-bit-depth Star-Tetrix lossless+lossy + per-slice Q[p] override + rate-budget Q[p] picker + rate-budget R[p] picker + joint per-slice Q[p] + R[p] rate-budget picker (bd∈9..=16) + per-precinct Q[p] override (Annex C.2 Table C.1) |
| **AVIF** | 🚧 ~92% — HEIF→AV1 + grid + imir/clap/colr/pixi/pasp + HDR + AV1 wrap + DoS caps + HEIF item-properties + auxC URN + rloc/lsel/iovl/grpl + iscl/rref + `mif1` + §4.2.2 tmap + ISO 21496-1 Annex C.2 GainMapMetadata + §5.2.5.3/§5.2.7 value-comparison + §8.2/§8.3 still-image profile audit + av1-avif v1.2.0 §3 AVIS audit + AVIS sequence-track profile audit + §8.6.6 Edit List + §6.5.18 crtt creation-time item-property (u64 µs, 1904↔Unix epoch helpers) + `inspect_avis` aggregator | — |
...
| Format              | Decode | Encode | Notes |
|---------------------|:------:|:------:|-------|
| **ASS / SSA**       | ✅ | ✅ | Script Info + V4+/V4 Styles (BGR+inv-alpha) + override tags + `\fn`/`\fe`/`\b<weight>`/`\r[<style>]` + `\pbo` + face-flag toggles + typed `\p<scale>` + `\fax`/`\fay` shear + `\an<n>` numpad alignment + `\1a` primary-fill alpha + `\blur<strength>` Gaussian post-step + `\iclip(rect)` + `\iclip(drawing)` + `\fsp<spacing>` letter-spacing baked into renderer |
