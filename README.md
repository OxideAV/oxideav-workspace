...
| **MIDI** (SMF) | ✅ ~99% — SMF Type 0/1/2 → PCM via 32-voice mixer + SF2/SFZ/DLS + FF 01..07 text-meta iterator + smpte_offsets/FrameRate + channel-state seek primitive + sequencer_specifics + sequence_numbers + midi_ports + channel_prefixes + sysex_events (F0/F7) iterators + `SysExEvent::universal_classification()` (Table 4 Universal SysEx classifier); cargo-fuzz panic-free | ✅ SmfFile::to_bytes() / Track::to_bytes_chunk() mux-side writer + synthesis |
