...
- **3D scenes & assets** — typed `oxideav-mesh3d` (Scene3D / Mesh /
  ...
  `Mesh3DRegistry`). Per-format codecs: `oxideav-stl` (ASCII + binary
  + per-face attribute round-trip + 16-bit color extension VisCAM /
  Materialise + JSONL trace + fuzz-resistant header detection),
  `oxideav-obj` (+ MTL Phong + Wavefront-PBR + smoothing groups +
  multi-name groups + MTL Tf/Ke/sharpness extras + negative-index
  encoder), `oxideav-gltf` (glTF 2.0 + .glb + KHR_lights_punctual +
  BufferViewAsset + skeletal animation + skin + sparse accessors +
  multi-scene), `oxideav-usdz` (ZIP STORED walker + writer + USDA
  reader + writer + UsdPreviewSurface; `ZipStoredAsset` +
  `EncodeReport.pass_through_textures` confirm zero-re-encode
  USDZ→USDZ texture passthrough).
