...
| **AV1** | 🚧 ~94% — decoder feature-complete + **standalone `decode_av1` public entry** (1489 tests + integration roundtrips) | 🚧 ~25% encoder — pixel-space YUV→IVF driver + 14-mode intra picker + full §7.13.3 forward 2D dispatcher + WHT lossless + forward quantize + §5.11.x partition/transform_tree/coefficient + r194 §7.11.5.3 UV_CFL_PRED on dyn-extent driver (αU/αV joint linear predictor across `{±1,±2,±4}` 20-candidate grid excluding §6.10.36 forbidden `(0,0)`; `cfl_subsampled_luma_4x4_420_dyn` + `read_cfl_alphas` joint-sign+magnitude CDFs + `cfl_predict_4x4_for_plane_dyn`; encode→decode→pixel bit-exact at 32×32/64×64 lossless). Lacks rectangular TX sizes + §5.11.18 inter mode_info + RD + multi-SB tiling |
