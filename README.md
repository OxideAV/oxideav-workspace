...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz + `idct8x8_dc_only` fast path + SHA-256 lockstep pin on 9 fixtures + 128×128 interlaced apcn + FIPS 180-4 §B.1/§B.2 self-check + §6.1.1 Tables 5/6/7 color-metadata reverse helpers + typed `FrameHeader::color_primaries_kind()` + `matrix_coefficients_kind()` Table 5/6 + `transfer_characteristic_kind()` §6.1.1 OETF accessors (Bt1886/St2084/Hlg) + typed `FrameHeader::frame_rate()` Table 4 (eleven named rates as `Rational`, codes 0 / 12..=15 → None) | ✅ ~97% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + ffmpeg cross-decode + SHA-256 lockstep pin across every encoder entry + SHA-256 pin on 10/12-bit encoder paths |
