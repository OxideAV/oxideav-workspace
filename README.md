...
- **3D scenes & assets** — typed `oxideav-mesh3d` (Scene3D / Mesh /
  ...
  + multi-scene + morph-targets (POSITION/NORMAL/TANGENT delta accessors
  + `mesh.weights`) + accessor min/max bounds validation + encoder-side
  normalised-int animation quantisation), `oxideav-usdz` (ZIP STORED
  walker + writer + USDA reader + writer + UsdPreviewSurface + per-mesh
  `xformOp:translate/orient/scale/transform` + multi-primitive
  sibling-Mesh emit-and-fold + `UsdMediaSpatialAudio` reader + writer
