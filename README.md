...
| **JPEG XL** | 🚧 ~80% — rounds 1-9 against ISO/IEC 18181-1:2024 final core (master @ bb08c4c). **All 5 small lossless fixtures decode PIXEL-CORRECT** vs expected.png: pixel-1x1, gray-64x64, gradient-64x64-lossless, palette-32x32, grey_8x8_lossless. Coverage: C.2.1 + H.2 + H.3 + H.4.1+H.4.2 + H.6.4 inverse Palette + H.6.3 inverse RCT (6 modes × 6 perms) + H.6.2 inverse Squeeze + H.5 Self-correcting predictor + Annex E.4 ICC profile decoder. **Round 7** four-piece multi-group refactor; synthetic 320×320 multi-group fixture committed. **Round 8** resolved C.2.5 SPECGAP empirically (i<min(alphabet_size,table_size)); VarDCT Annex I scaffold + 8×8 IDCT primitive. **Round 9** resolved synth_320's "0-byte PassGroup" via 3 concurrent fixes: §F.3.1 HfGlobal unconditional (NOTE 1: 0-byte for kModular) — round-8 had it kVarDCT-gated; §F.3 zero-padding sub-reader (per-section EOF reads pad zero); per-PassGroup transforms (Annex H.6 inside G.4.2 — edge groups carry Palette nb_colours=191). synth_320 reaches end-of-frame; ~21k of 102400 pixels match in PG[0][0,1]; edge groups (PG[0][2,5,6,7,8] 64-pixel sections) still drift — round-10. 255 tests. FDIS-2021 typo #5 fixed by 2024; typos #1-#4 retained. Decoder rounds 7-11 + encoder rounds 1-6 retired 2026-05-08 under fruits-of-poisonous-tree (`old` branch); v0.0.5/v0.0.6/v0.0.7 yanked. Lacks LF/HF subband decode + IDCT dispatch / XYB | — RETIRED 2026-05-08, re-author after decoder forward progress |
...
| **PDF** | ✅ ~75% — bytes → Scene via xref + recursive object parser (FlateDecode) + content-stream operator parser + `/Encrypt` decode at every standard revision (R=2 RC4-40, R=3 RC4-128, R=4 RC4-128 + AES-128, R=5 Adobe ext AES-256, R=6 ISO 2.0 AES-256 with Algorithm 2.B iterated SHA-256/384/512) per ISO 32000-1 §7.6 + ISO 32000-2 §7.6.4.4 + Algorithms 1-13; `/Info` → Metadata; lacks cross-reference streams (PDF 1.5+), public-key handlers (adbe.pkcs7.*) | ✅ ~70% — PDF 1.4 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + `/Info` dict; lacks text, JPEG passthrough, encryption-on-encode |
