...
| MOV (QuickTime) | ✅ | — | ✅ | Apple QTFF + ISO BMFF meta + HEIF/HEIC item-properties + grid/iovl/tmap + symmetric muxer + fragmented-MP4 seek + DASH sidx/styp + r147 saiz/saio sample-aux (CENC envelope); ffprobe-accepted |
...
| **FLAC** | ✅ 100% — bit-exact vs RFC 9639 + CUESHEET → Chapter API | ✅ 100% — bit-exact roundtrip + LPC order/window/precision search + r143 closed-form Rice estimate (4.6–6.0× faster); r147 committed flamegraphs (best_partition at ~61% encode CPU) |
...
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM; r147 §2.4.3.2 polyphase synthesis filterbank primitive (Table 3-B.3, 512 D[i] + 64×32 N_ik); lacks decode-loop wiring + Layer II encoder + LSF (#1076) | ✅ ~100% Layer I encoder — Annex C polyphase + scalefactor + energy allocator + Table C.3 quantizer; 1 kHz tone @ 192 kbit/s mono self-roundtrip RMS<0.01; lacks Layer II encoder + Annex D psy |
...
| **MP3** | ✅ ~100% — MPEG-1/2/2.5 frame walker + Layer III main-data Huffman + requantize + stereo (MS+intensity) + IMDCT + polyphase synthesis; end-to-end bitstream→PCM bit-exact vs mpg123 on test corpora; ID3v2/Xing demuxer w/ seek | 🚧 ~75% — Phase-2 encoder: PCM→MP3 driver + outer loop + bit-reservoir + true-VBR + Xing TOC + CRC-16 + stereo + joint-stereo MS (84/85 dB @ 192 kbit/s) + r147 §C.1.5.4.3 scalefac_scale escalation + r148 §C.1.5.4.3.4 preemphasis decision; ffmpeg + mpg123 accept; lacks Annex D psy (#1048) + intensity-stereo + LSF |
...
| **MPEG-4 Part 2** | 🚧 ~45% (post-2026-05-18 orphan) — I-VOP intra decode through §7.4.5 IDCT + Figure 7-5 predictor gathering + r147 §6.2.5 video_packet_header (rectangular); lacks Figure 7-34 MV-predictor (#1125) + inter+B-VOP + encoder | 🚧 scaffold |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 40 SEI types + fuzz-hardened; lacks MBAFF, SVC/3D/MVC | 🚧 ~82% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite + r148 opt-in CABAC IDR Intra_16x16 AC trellis (1221 tests); ffmpeg PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 ~35% (post-2026-05-18 orphan) — VPS+SPS+PPS bodies + scaling-list + scan + §9.3 CABAC engine + slice header through r147 §7.3.6.1 num_ref_idx override prelude; lacks §9.3.4.2 binarization+ctxIdx (#444), remaining inter-slice tail, residual/IDCT | 🚧 scaffold |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures (incl. ARNR, golden refresh, SPLITMV recon) | 🚧 ~65% — Phase-2 I+P stream encoder + r147 §16.4 SPLITMV picker + r148 §16.2 GOLDEN/ALTREF ref-frame selector (49.76 dB on stripe→flat→stripe at qi=16); lacks multi-partition inter + §9.4 mode/ref delta + caller-driven §9.7 refresh |
...
| **AV1** | 🚧 ~32% — orphan-rebuild; uncompressed header + msac symbol decoder + §9.4 default-CDF families + §8.3.2 ctx selectors + §5.11.49 palette_tokens walker + r148 §9.3 block-size conversion tables (344 tests); lacks tile-content reconstruction + ref-frame update | 🚧 scaffold |
...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW (aprn/aprh) refused; ffmpeg interop 60-68 dB | ✅ ~95% — RDD 36 across all 6 profiles + interlaced + alpha + perceptual quant matrices + multi-frame rate-control + 12-bit cross-decode + r148 ffmpeg cross-decode acceptance for the r144 perceptual qmat preset (8 cases 58.0-63.8 dB luma PSNR) |
| **EVC** (MPEG-5) | 🚧 ~80% — NAL + SPS/PPS/APS + §9.3 CABAC + §8 intra (Baseline) + DCT-II + P/B inter + RPL + HMVP + DPB + ALF (per-CTB type filtering r145) + DRA + IBC §8.6 + r148 §8.9.5 per-sample co-located-luma chroma DRA offset (339 tests); lacks Main-profile toolset (BTT/ADMVP/EIPD/ATS/AMVR/affine) | — |
...
| **Cinepak** (CVID) | ✅ ~98% — frame header + multi-strip + V1/V4 codebooks + intra/inter + grayscale + Sega FILM demuxer + Saturn/3DO deviant + r148 cargo-fuzz harness (4 panic-free targets, 2 strip-header subtract-with-overflow bugs found + fixed) | ✅ ~98% — stateful encoder w/ rolling codebooks + RDO + LBG + 3-axis grid picker + bitrate-target rate control + keyframe-interval enforcement |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate (BT.601 YCbCr→RGBA); `oxideav_core::Decoder` registered | 🚧 ~55% (post-r144 Hat-3 rebuild) — VP8L lossless encoder w/ LZ77 + r146 §4.1 spatial-predictor + r147 §3.5.2/§4.2 color-transform + r148 §5.2.3 color_cache_code_bits sweep (-3.2% on 16-color palette); container + metadata + animation builders; lacks VP8-lossy encode + 9 r131-r144 pieces (#1120) |
...
| **JPEG XL** | 🚧 ~86% — ISO/IEC 18181-1:2024 lossless Modular path + 7 fixtures decode pixel-correct + VarDCT scaffold (IDCT + dequant + HfPass + CfL) + r141 Gaborish + r144 EPF + r147 Annex I.2.2 AFV basis primitives; lacks §C.7.2 histograms + per-block coeff loop + per-frame Gaborish/EPF/CfL wiring (#799/#1077) | — retired |
