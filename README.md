...
| Ogg       | ✅ | ✅ | ✅ | Vorbis/Opus/Theora/Speex pages + comments; page-granule bisection + page-level seek index + chained-link-aware duration + page-loss/hole detection + page-sync recapture + public CRC-32 validation API + r172 Criterion bench harness + r183 streaming CRC check in Page::parse (eliminates per-page Vec clone + memcpy, ~15-20% throughput gain on M1) |
| Matroska  | ✅ | ✅ | ✅ | MKV/MKA/MKS; Cues seek; SeekHead/Chapters/Attachments/subtitles; opt-in block lacing on write; EBML CRC-32 validation; typed Tag/TrackOperation/ContentEncodings/chapters() decode; typed Video FlagInterlaced/FieldOrder + geometry quartet + Colour master + SMPTE 2086 MasteringMetadata + StereoMode + r177 Projection (RFC 9559 §5.1.4.1.28.41-46) + r183 AlphaMode / AspectRatioType / UncompressedFourCC typed decode (final Video sub-element gap closed) + 16-test injection-robustness pin |
...
| TIFF      | ✅ | ✅ | — | TIFF 6.0 single-image + r177 BigTIFF write (magic 43 / 8-byte offsets / LONG8 strip+tile arrays) + r183 PhotometricInterpretation=8 1976 CIE L*a*b* decode (§23, Rgb24 via Lab→XYZ@D65→NTSC→sRGB; Gray8 L*-only) |
...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing + r183 Decoder-trait MPEG-2 LSF widening (mono/stereo/joint-MS+intensity, 611 tests; MPEG-2.5 still observer-gated) | 🚧 ~93% — Phase-2 + cross-channel-MS agreement + r165 §2.4.3.4.10 attack-detector DEFAULT_AMBIENT_LEAK empirical-corpus calibration; lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element + r177 §4.4.1 GASpecificConfig extensionFlag + Table 1.15 epConfig (333 tests); decoder body still pending | 🚧 scaffold — Phase-2 writers: section_data + ics_info + pulse_data + tns_data + scale_factor_data + DPCM + r160 §4.4.2.1 raw_data_block frame assembler + r165 Pce::write + r183 §4.4.6.5/Table 4.12 gain_control_data SSR parser+writer (353 tests) |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 41 SEI types + fuzz-hardened (r164 `sei_payload` + r177 §D.2.20 OOM fix) + r183 SEI type 46 multiview_view_position §G.13.2.10 (first MVC parser, 1024-cap); lacks MBAFF, SVC/3D/MVC | 🚧 ~83% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite (1227 tests); ffmpeg PSNR_Y 44.20 dB |
...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz harness (3 panic-free targets incl. r183 direct §5.1/§6.1.1/§6.3/§5.3 header parsers) + r161 profiling driver + baseline | ✅ ~95% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + multi-frame rate-control + 12-bit cross-decode + ffmpeg cross-decode acceptance |
...
| Format              | Decode | Encode | Notes |
|---------------------|:------:|:------:|-------|
| **ASS / SSA**       | ✅ | ✅ | Script Info + V4+/V4 Styles (BGR+inv-alpha) + override tags (b/i/u/s/c/fn/fs/pos/an/k/kf/ko/K/N/n/h). Typed positional/rotation/blur/border/karaoke tags + r172 `\fn`/`\fe`/`\b<weight>`/`\r[<style>]` + r177 `\pbo` + r183 `\i`/`\u`/`\s` face-flag toggles (with dual-path passthrough + Segment-wrapper walking) extraction via `extract_cue_animation` → `RenderState`; `[Aegisub Project Garbage]` + `[Fonts]`/`[Graphics]` round-trip via extradata |
