...
| **MS-MPEG-4** (v1 / v2 / v3) | 🚧 Clean-room Implementer handoff per `docs/video/msmpeg4/` (spec/99 + 40 tables). **Round 8**: DC spatial predictor (§7.4.3 L/T/TL gradient, `|A-D|<|A-B|` → left else top) + AC-scan dispatcher + MCBPCY canonical Huffman wired into joint-VLC decode. **Round 9: v3 P-frame decoder landed** — motion-vector VLC + residual macroblock + P-MB CBPY/CBPC plumbing on top of the v3 I-frame pipeline. AC VLC gap (candidate VMAs `0x1c25fad0`/`0x1c25f6c8`) remains the one blocker for full I-frame conformance; P-frame decoding runs on top of the same table so it lights up automatically once those tables are filled. v1/v2 I-frame pending. | — |
