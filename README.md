...
|-----------|:-----:|:---:|:----:|-------|
| WAV       | ✅ | ✅ | ✅ | Full metadata-chunk family (BWF bext, LIST/INFO, iXML, smpl/inst) + RF64/BW64 64-bit form |
...
| AVI       | ✅ | ✅ | ✅ | AVI 1.0 + OpenDML 2.0; interlaced + VBR audio + LIST INFO + WAVEFORMATEXTENSIBLE + ODML keyframe seek + typed header accessors |
...
| IFF (EA IFF 85) | ✅ | ✅ | — | `FORM/LIST/CAT` family — Amiga 8SVX + ILBM (EHB/HAM, palette-change chunks) + ANIM op-0/5/7 + Apple AIFF/AIFF-C + fuzz harness |
...
| **MP3** | ✅ 100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~99% — full Layer III + Annex D psychoacoustics COMPLETE (Models 1 + 2 + §C.1.5.3.2.1 Layer III adaptation); lacks intensity-stereo |
| **AAC** | 🚧 ~40% — ADTS + AudioSpecificConfig + full Table 4.50 channel-body parse (all spectral codebooks + TNS coefficient tools + Table 4.56 spectral_data); lacks TNS region slicer + dequant/filterbank | 🚧 ~20% — Phase-2 writers for every syntax element + TNS decode tool; SBR pending QMF |
...
| **G.728** | 🚧 ~90% — LD-CELP decode: Annex A-D + Levinson + postfilter chain | 🚧 ~85% — analysis-by-synthesis loop complete, bit-exact lockstep with decoder; lacks ICOUNT stagger + Annex G fixed-point |
| **G.729** | 🚧 ~25% — tables + serial parser + full §4.1 per-frame parameter chain (LSP / pitch / FCB / gains incl. parity concealment) over 18 222 conformance frames; lacks §4.1.6 excitation/synthesis (first PCM) + postfilter + Annex B DTX | 🚧 ~5% — scaffold |
...
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + ETSI codebooks + ACPL cfg0..3 + SAP + IMS bitstream walker; lacks object/A-JOC | 🚧 ~78% — IMS v0/v2 mono → 7.1 with real per-band coupling parameters + envelope/chparam builders; lacks γ₁..γ₄ + 7_X ACPL_3 β + back-pair Lb/Rb |
| **MIDI** (SMF) | ✅ ~99% — SMF 0/1/2 → PCM via 32-voice mixer + SF2/SFZ/DLS soundfonts + complete typed meta/sysex iterator surface; fuzz panic-free | ✅ ~95% — SMF writer + synthesis |
...
| **TTA** (True Audio) | ✅ ~98% — TTA1 fmt 1/2 + password + trailers + streaming + random-access decode + typed header surface + fuzz | ✅ ~96% — bit-exact self-roundtrip |
| **Musepack** | 🚧 ~40% — SV8 packet walker + all 21 canonical-Huffman tables wired + per-case sample decoders + SV7 primitives + requantiser constants; lacks §3.4 sparse/escape cases + SV7 header field map + 32-band synthesis | 🚧 ~5% — scaffold |
...
| **Theora** | 🚧 ~63% — setup headers + full token/coefficient decode + DC prediction + dequant/IDCT + per-block reconstruction + complete loop filter + §7.11 step-1 chain; lacks step 5/6 frame-assembly dispatch | 🚧 ~5% — scaffold |
...
| **MS-MPEG-4** (v1/v2/v3) | 🚧 ~50% — clean-room MV-predictor framework + G0..G3 packed-Huffman primary VLC wired; lacks alt-MV VLC source + 4-MV MCBPC (docs-gapped) | — |
...
| **H.265 (HEVC)** | 🚧 ~65% — parameter sets + §9.3 CABAC engine + full slice header + intra-mode signalling + full §7.3.8.11 residual_coding() driver + fuzz-hardened; lacks ctx-init Tables 9-26..31 + §8.6 scaling/IDCT + reconstruction | 🚧 ~5% — scaffold |
...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~53% — header walk + Bool decoder + coefficient/dequant/inverse-transform + intra prediction + §8.8 loop filter COMPLETE + inter header parse; lacks inter-frame decode + decode_vp9 wiring | 🚧 ~5% — scaffold |
| **AV1** | 🚧 ~35% — standalone intra decode, 4:2:0/monochrome multi-superblock to 128×128; lacks inter prediction + in-loop filters (CDEF/restoration) + film grain + superres | 🚧 ~30% — intra encode YUV→IVF + §5.11 write side through residual coefficients, decode-walker lockstep; lacks inter transform-tree write + RD picker + inter reconstruction/encode chain |
...
| **EVC** (MPEG-5) | 🚧 ~93% — Baseline profile complete (CABAC + intra/inter + ALF + DRA + IBC + AMVR/MMVD + tiles); lacks Main-profile toolset (BTT/ADMVP/EIPD/ATS/affine) | — |
...
| **Indeo 3** (IV31/IV32) | 🚧 ~60% — headers + VQ codebooks + MV decode + cell decomposition + plane plumbing; lacks §7.2 boundary fix-up + MC inner loop | — |
...
| **TIFF** (6.0) | ✅ ~98% — II/MM + BigTIFF + 7 photometrics + all baseline compressions + CCITT fax + tiles + multi-page + JPEG-in-TIFF + fuzz | ✅ ~95% — chunky + planar + tiled across Gray/RGB/Palette/CIELab/CMYK/YCbCr with predictor + fax modes |
...
| **JPEG 2000** | 🚧 ~65% (post-2026-05-20 orphan) — T.800 headers + tier-2/tier-1 MQ decode + IDWT cascade + MCT (RCT + ICT, i64 reversible) + Annex E reassembly + 4 fuzz targets; lacks HTJ2K + top-level decode wiring | 🚧 ~5% — scaffold |
...
| **Farbfeld** | ✅ 100% | ✅ 100% |
| **HDR** (Radiance RGBE) | ✅ ~99% — new/old RLE + all axis flags + header metadata + derived colorimetry + fuzz | ✅ ~98% — RLE modes + XYZE↔RGB + 8 tonemap ops |
| **QOI** | ✅ 100% | ✅ 100% |
| **TGA** | ✅ 100% | ✅ 100% |
...
| **SVG** | ✅ ~99% — full SVG 1.1 + SVG 2 feature grid (shapes / text / gradients / masks / markers / SMIL / CSS3 selectors + media queries) + all 16 §15 filter primitives rendered + feDropShadow pixel evaluation | ✅ ~88% — round-trips full shape graph + PreservedExtras |
| **PDF** | ✅ ~99% — bytes → Scene via xref/ObjStm + encryption R=2..6 + signatures + text extraction + Tagged-PDF + 5 stream filters + optional content + annotations; Movie/Screen/3D/RichMedia remain `Other` | ✅ ~99% — multi-page writer + encryption + signatures + AcroForm + annotation/embedded-file/timestamp writers |
...
| **glTF 2.0** (+ .glb) | ✅ ~98% — JSON + .glb + full PBR + 12+ KHR extensions + skins/animations/morph targets + 14 spec-MUST validators; Draco/meshopt descriptors parsed (bitstream decode pending) | ✅ ~93% — symmetric round-trip incl. XMP |
| **USDZ** (+ USDA) | ✅ ~95% — ZIP walker + USDA composition (LIVRPS / variants / references) + `.usdc` Crate parser through SPECS framing; lacks §4.5 PATHS tail (docs gap) + FIELDS value-rep type codes + UsdSkel | ✅ ~88% — symmetric writer + pass-through + composition arcs |
| **FBX** | 🚧 ~92% — binary + ASCII + object graph + mesh/animation/deformers + Properties70 grammar + class-default resolution; lacks multi-LayerElementNormal | ✅ ~58% — symmetric binary + ASCII writer + opt-in deflate |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MOD / STM / XM** | ✅ ~97% MOD · ~92% STM · ~90% XM — shared Paula/FT2 mixer + full effect sets + typed sample-header accessors + fuzz | — |
...
| **S3M** | ✅ ~96% — stereo + full ST3 v3.20 effect set + per-channel effect memory + header-driven playback corrections; lacks AdLib FM synth | — |
