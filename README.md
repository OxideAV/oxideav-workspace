...
| **AV1** | 🟢 ~97% — full spec-driver decoder, 41-stream conformance corpus BYTE-EXACT (intra + inter GOPs, show-existing, 10/12-bit, QM, segmentation-inter, jnt-comp/OBMC/dual-filter, film grain incl. forwarding, superres+LR at upscaled extent, multi-tile rows, edge-overhang, intra-block-copy, scaled-reference MC, delta-q, global-warp, inter-intra warp blends, CfL TU-overhang) + 60-config black-box encoder sweep byte-identical to the reference decoder + public `decode_av1` at FULL spec-driver parity (Frame::Spec surface, per-fixture parity asserted corpus-wide) + registry serves whole-GOP inter decode; lacks conformance-grade inter encoding | 🚧 ~45% — CONFORMANCE-GRADE intra keyframe encoder (public `encode_av1`, [8,512] per axis 4:2:0 lossless+lossy: spec-faithful §5.11.7 syntax, 7-mode SSD picker + chroma CFL, 8×8/4×4 partition RD; §5.3.4 trailing-bits + §8.2.4 arithmetic-termination wire bugs fixed) — self-encoded streams decode byte-identical in TWO independent reference decoders + the in-tree spec driver (7 configs), 2 pinned into the corpus + single-ref P-frame inter encoder (legacy mirror path, decoder-reconstructible); lacks D-mode/palette/filter-intra leaves + ≥16×16 TX/partitions + conformant inter P-frame emission + rate control |
