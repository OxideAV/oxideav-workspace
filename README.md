...
| **AAC** | 🚧 AAC-LC + HE-AACv1 SBR + HE-AACv2 PS; lacks LD/ELD, USAC, 4.0/5.1/7.1 channel layouts on PCE | 🚧 AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS; LC core decodes byte-tight via ffmpeg at 24 kHz/1 kHz; HE-AAC saturates ffmpeg via the SBR FIL extension (r22 ruled out SBR header fields; r23 ruled out the LC core MDCT scale; r24 target: bs_invf_mode / bs_add_harmonic / HF gain in SBR FIL bytes); psychoacoustic model basic; lacks PNS, gapless padding tuning, multichannel encode |
...
| **AC-4** (Dolby) | 🚧 A-SPX + DRC + DE walker + ETSI Huffman tables (60 codebooks bit-validated) + ASPX_ACPL_1/2/3 transform synth (Pseudocodes 117/118/119) + multichannel `sf_data(ASF)` Huffman walk (long-frame 1-window-group); pending short/grouped multichannel walker, inner ACPL_3 body walker | — |
...
| **H.264** | 🚧 ~45k LOC spec-driven rewrite; I/P/B + DPB + MMCO/RPLM + CAVLC + CABAC + 4:2:0/4:2:2 + B-pyramid POC ordering; lacks 4:4:4, MBAFF, Annex F (SVC), G (3D), H (MVC) | 🚧 Baseline-ish: I + P (1MV/4MV, ¼-pel) + B-slice 16x16/16x8/8x16 + B_Skip/B_Direct_16x16/8x8 (spatial direct); lacks temporal direct, CABAC encode, intra fallback in P/B, weighted-pred, 4:2:2/4:4:4 emit, B_8x8 mixed sub_mb_type, VUI/SEI tuning |
| **H.265 (HEVC)** | 🚧 I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2; SAO + deblock + scaling lists; HEIF/HEIC opt-in; pending 4:4:4 12-bit, broader B-slice merge/AMVP audit | 🚧 Baseline CAVLC I+P + Main CABAC P-slice + B-slice (TrailR mini-GOP=2, one past + one future ref, default-weighted bipred); ffmpeg cross-decodes I-P-B-P-B at 33-45 dB; lacks merge encode, B_Skip CUs, AMP / Nx2N / 2NxN partitions, mvd_l1_zero_flag, mini-GOP > 2 / B-pyramid, 10/12-bit, RDO refinements |
| **H.266 (VVC)** | 🚧 4:2:0 IDR intra (BDPCM, MIP, CCLM, ISP) + ALF/SAO/CC-ALF + P-slice merge+skip with §8.5.6.3 fractional-pel MC (8-tap luma Table 27 + 4-tap chroma Table 33 at 1/32-pel); lacks B-slices, HMVP, temporal merge, MMVD/CIIP/GPM, dual-tree, full inter residual decode, affine/scaled-ref filter tables, BCW/BDOF/PROF | 🚧 forward CABAC + DCT-II + flat quant only; residual emit + pixel output pending |
...
| **AV1** | 🚧 OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound; r21 SVT-AV1 chain 48/48 (full pass); r22 `inverse_2d_spec` covers all 159 spec-allowed (TX_TYPE × TX_SIZE) pairs (not yet wired into superblock caller); pending palette finalization, intrabc, full loop-filter, full primary_ref_frame=4 chain | — |
