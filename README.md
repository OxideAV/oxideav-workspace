...
| **FLAC** | ✅ 100% — bit-exact vs spec + RFC 9639 §8.7 CUESHEET tracks → Chapter API | ✅ 100% — bit-exact roundtrip + per-subframe LPC coefficient-precision search (≤15-bit) + LPC order search 1..=12 (RFC 9639 §9.2.6) |
| **Vorbis** | 🚧 r8 (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook + Huffman tree + full §4.2.4 setup-header walker + §3.2.1/§3.3 VQ vector unpack (entry → vector via multiplicands + minimum_value + delta_value + sequence_p) + §8.6 residue decode (formats 0/1/2) + §7.2.3/§7.2.4 floor type 1 packet decode + curve computation + §6.2.2/§6.2.3 floor type 0 LSP per-packet decode + curve computation + §1.3.2/§4.3.1 Vorbis window + §4.3.5 inverse channel coupling; lacks §4.3.2 audio-packet driver / MDCT+TDAC | 🚧 scaffold |
| **Opus** | 🚧 r11 (post-2026-05-20 orphan) — RFC 6716 §3.1 TOC + §3.2 frame packing + §4.1 range decoder + §4.2.7.1–§4.2.7.5.1 SILK header + §4.2.7.4 subframe gains + §4.2.7.5.2 LSF Stage-2 residual + §4.2.7.5.3 NLSF reconstruction (Tables 23/24 cb1_Q8 + IHMW w_Q9 in [1819, 5227] + final NLSF_Q15 clamp) + §4.2.7.5.4 NLSF stabilization (RFC 8251 §7 erratum) + §4.2.7.5.5 NLSF interpolation + §4.2.7.5.6 NLSF→LPC core conversion (`silk_NLSF2A`: Table 27 ordering + Table 28 Q12 cosine + i64 P/Q recurrence) + §4.2.7.5.7 LPC range-limiting bandwidth expansion (silk_bwexpander_32 chirp loop + Q12 saturation) + §4.2.7.5.8 LPC prediction-gain stability limiting (Levinson recurrence + chirp); 210 tests; lacks LTP + excitation + CELT | 🚧 scaffold |
| **MP1** | ✅ clean-room Layer I decode to PCM (ISO/IEC 11172-3 + MPEG-2 LSF 16/22.05/24 kHz per 13818-3 §2.4.2.3) — frame-header + sync + length + CRC-presence + bit-allocation/scalefactor/§2.4.3.2 requant + Table 3-B.1 rescale + 512-tap Table 3-B.3 polyphase synthesis filterbank; registered both via `oxideav_core::register!` and direct `decoder::make_decoder` / `encoder::make_encoder` factory endpoints; mono/stereo/joint-stereo + §2.4.3.1 CRC-16 error_check verification + mute concealment; lacks frame-repeat concealment | ✅ clean-room Layer I encoder (ISO/IEC 11172-3 Annex C) — 32-band polyphase analysis filterbank (Table C.1) + scalefactor pick + energy-driven non-psy allocator + Table C.3 A/B quantizer (MSB-inverted, bit-exact inverse of decoder); `oxideav_core::Encoder` registered; self-roundtrip 1 kHz tone at 192 kbit/s mono RMS < 0.01; 89 tests; lacks Annex D psychoacoustic model |
...
| **WavPack** | 🚧 r6 (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + run-length n-decoder + Golomb (base,add) interval + per-sample value reconstruction; 84 tests; lacks median-adaptation amount (#992) / prediction loop / float+multichannel / CRC / encoder | 🚧 scaffold |
