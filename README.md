...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek | 🚧 ~83% — Phase-2 PCM→MP3 + outer loop + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + r151/r152 short+mixed forward MDCT toggles + r154 §C.1.5.4.4.8 linbits-reach filter (#1106); lacks Annex D psy (#1048) + intensity-stereo + LSF + block-type auto-decision |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~80% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + P-frame pixel lockstep + r154 §9.4 filter_type knob (normal vs simple loop filter, encoder-decoder lockstep at both branches) |
...
| **AV1** | 🚧 ~44% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + Partition_Subsize tables + §5.11.4 decode_partition + §5.11.11 read_skip + r154 §5.11.10 read_skip_mode (SkipModes[] grid, 3 short-circuit cases, 417 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
