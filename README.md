...
| **Vorbis** | ✅ ~95% — all residue types per RFC 5215 | 🚧 ~75% — BitrateTarget {Low/Medium/High/HighTail} bank with mu-law-companded non-uniform multiplicand axis on lookup_type=1 (+1.5 dB Laplacian SNR); per-target point-stereo crossover (3/4/6 kHz); ffmpeg cross-decodes |
| **Opus** | 🚧 ~85% — full TOC + RFC 7845 pre-skip + output_gain + §4.3.7.2 CELT de-emphasis + §4.3.5 CELT auto-scale (libopus interop 10-26 dB on 5 fixtures) + §4.2.7.4 integer silk_log2lin + §4.2.7.8 Q23 shell-pulse + §4.2.7.8.6 LCG dither (SILK NB/MB/WB libopus interop 11-12 dB); lacks NLSF stage-2 residual decoding (Tables 17-22/25-30, last gap to 20+ dB) | ✅ ~85% — CELT full-band + SILK NB/MB/WB + Hybrid mono/stereo at 10/20 ms; ffmpeg + libopus cross-decode clean |
...
| **MP3** | ✅ ~95% — MPEG-1 Layer III M/S | 🚧 ~82% — CBR + VBR + MS-stereo + MPEG-1/2/2.5 intensity-stereo + Annex D Psy-1 (24 Bark partitions, peak-detection tonality, TMN/NMT offsets) on long + short blocks |
| **AAC** | 🚧 ~80% — AAC-LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE; lacks LD/ELD, USAC | 🚧 ~78% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + gapless + AscBuilder + Bark-band PE/SMR psy default-on across mono/v2/stereo (M/S CPE IMDCT side-lobe leakage closed via use_tns gate; pns-noise gate); occasional bitreader edge case on conformant streams (recoverable, executor tolerates) |
...
| **AC-3** (Dolby Digital) | ✅ ~95% — full decode + downmix (90+ dB vs ffmpeg) | 🚧 ~85% — acmod 1/2/3/6/7 + LFE + rematrix + transient detector + DBA + 5-fbw coupling + per-channel D15/D25/D45 chexpstr (default-on, ~430 bits/ch saved at D45) + E-AC-3 indep + dep substream encode |
| **AC-4** (Dolby) | 🚧 ~67% — A-SPX + DRC + DE walker + 60 ETSI Huffman codebooks + ASPX_ACPL_1/2/3 + 5_X/7_X channel walkers + mono/stereo/joint short-frame sf_data(ASF) + SSF bitstream walker + SSF PCM synthesis (Pseudocodes 4a-d/4e/26-39, C-matrix subband predictor) + SHORT_STRIDE env_prev tracking + walker state hoisting (RNG continuity across frames) | — |
...
| **H.264** | 🚧 ~75% — I/P/B + 4:2:0/4:2:2 + CAVLC + CABAC + DPB + B-pyramid POC + 8 SEI payload types + Level 1b sizing; lacks 4:4:4, MBAFF, SVC/3D/MVC | 🚧 ~70% — I + P (1MV/4MV, ¼-pel) + B 16x16/16x8/8x16/B_8x8 + B_Skip/B_Direct_16x16 (§8.4.1.2.2 spatial-direct) + weighted pred + 4:2:2/4:4:4 IDR + CABAC I/P/B with real ME (mvp §6.4.11.7); 99 dB Y on translation, 47 dB on per-half-motion partitions; lacks per-cell mixed B_8x8 sub_mb_type ∈ {1,2,3}, P/B CABAC at 4:4:4 |
...
| **H.266 (VVC)** | 🚧 ~35% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + PH pred_weight_table + BDOF (wired into leaf-CU bipred dispatch + BitDepth+6 high-precision intermediate) + PicturePlane u8/u16 storage for HBD; lacks GPM, AMVR, DMVR/PROF, affine | 🚧 ~5% — forward CABAC + DCT-II + flat quant only; residual emit pending |
...
| **VP8** | ✅ ~99% — I+P + 6-tap luma/chroma + per-MB loopfilter slab + persistent ref/mode_deltas + correct §16.3 split_mv_tree; lacks small loopfilter ±1 / sub-pel rounding gap on 4 ReportOnly | 🚧 ~83% — I+P + B_PRED intra-in-P + SPLIT_MV + alt-ref/golden + Lagrangian RDO + segment QP/LF + per-frequency AC/DC deltas |
...
| **Dirac / VC-2** | ✅ ~90% — VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit; ffmpeg bit-exact 8-bit 4:2:2/4:4:4 + 10-bit 4:2:0 | 🚧 ~78% — HQ + LD intra + Dirac core-syntax intra + core-syntax inter (OBMC + LeGall 5/3 wavelet residue, ffmpeg cross-decode 34 dB on translating square / 53 dB on camera-pan qpel; bit-exact lossless self-roundtrip); lacks 2-ref bipred |
...
| **EVC** (MPEG-5) | 🚧 ~50% — NAL + SPS/PPS/APS + §9.3 CABAC (Baseline + 51 Main init tables) + §8 intra (5-mode Baseline) + DCT-II + Baseline P/B inter + cbf!=0 residual + deblock + RPL non-IDR (full §7.3.7/§7.4.8 ref_pic_list_struct) + HMVP infrastructure (§8.5.2.7 LRU + §8.5.2.4.4 derive_default_mv); lacks multi-reference DPB, HMVP-as-AMVP-fallback, POC reordering, ALF, DRA, IBC | — |
| **HuffYUV** / FFVHuff | ✅ ~95% — v2 yuv420p/422p/rgb24/bgra + v3 yuv 444/422/420 + gray at 8/10/12/16-bit (incl. §5.5/§9.7 high-bits + 2-raw splice); 11 ffmpeg cross-decode fixtures bit-exact; canonical PLANE GRADIENT + mod-2^bps MEDIAN (both fixed this round); lacks gbrp/gbrap, yuv411p, interlaced | ✅ ~90% — bit-exact `huffyuv` / `ffvhuff` encoder with LEFT/GRADIENT/MEDIAN; same surface as decode minus gbrp/gbrap/yuv411p/interlaced; 4 ours→ffmpeg cross-decode fixtures clean |
...
| **WebP VP8L** | ✅ 100% — full lossless; 7/7 BitExact vs `dwebp` | ✅ ~92% — subtract-green + predictor + colour transform + RDO + K=16 meta-Huffman + cache-bits {0,4,6,7,8,9,10,11} + LZ77 16384-px window/256-deep chains + near-lossless + palette RDO bias; beats cwebp on natural fixtures (-46% on 128×128 natural); lacks multi-pass cost-modelled LZ77 + EntropyImage transform |
| **WebP VP8** | ✅ ~98% — via VP8 + bit-exact YUV→RGB + fancy chroma upsample; +1 LSB loopfilter rounding gap on a few P-frame edges | 🚧 ~90% — VP8 I-frame + ALPH + per-segment QP/LF + per-frequency AC/DC deltas + quality-driven quant matrix curve; animated mixed lossy/lossless; dwebp cross-decode clean; 350K+ fuzz runs/min clean; lacks psy-RDO + per-MB rate control (upstream vp8) |
...
| **JPEG 2000** | ✅ ~88% — Part-1 baseline + multi-tile + MQ + EBCOT + 5/3 + 9/7 + JP2 + 5 progression orders + POC + HTJ2K (Part 15) FBCOT cleanup/SigProp/MagRef; HTJ2K 5/3 + 9/7 fixtures bit-exact (pblk plumbing closed) | ✅ ~82% — 5/3 lossless + 9/7 irreversible RGB + 5 progression orders + POC + PPM/PPT + HTJ2K Part-15 cleanup-pass encoder round 1 (single-tile Gray8 NL=0; ojph_expand bit-exact) |
| **JPEG XL** | 🚧 ~45% — `jxlp` container + 2019 committee-draft Modular + 2021 FDIS through round 11 (ANS + LfGlobal + GlobalModular + cl_code + kRCT/kPalette refactored to Appendix B + kSqueeze); blocked on Appendix B §B.3.1/§B.4 Path 1 docs gap (libjxl returns in-palette value for trivial Path 1, doc says kDeltaPalette) | 🚧 ~25% — round 4 lossless modular + Gradient predictor + ANS + multi-group |
| **JPEG XS** | 🚧 ~70% — ISO/IEC 21122 Part-1 codestream + inverse 5/3 DWT + Annex C/D/F/G entropy + quant + colour transforms + multi-component (4:2:2/4:2:0) + multi-level DWT cascade + CAP-bit decoder | 🚧 ~45% — multi-component (Nc 1/3 + RCT cpih ∈ {0,1}) + multi-decomp NL ∈ {1,2} + odd dimensions + Dr=0 VLC + Fq=8 lossy (PSNR ≥40 dB at q=1) + 4:2:2/4:2:0 sub-sampling; bytes -26% vs raw at q=8; lacks vertical-prediction VLC, significance coding, Star-Tetrix, NLT |
...
| **DDS** (DirectDraw Surface) | ✅ ~50% — 124-byte DDS_HEADER + 20-byte DDS_HEADER_DXT10 + uncompressed (A8R8G8B8 / X8R8G8B8 / A8B8G8R8 / R5G6B5 / A1R5G5B5 / A4R4G4B4 / R8G8B8 / A8L8 / L8 / A8); BC1-7 recognised as raw block bytes; lacks BC1-7 decompression, mipmaps, cubemaps, texture arrays | ✅ ~40% — uncompressed only; rejects block-compressed |
| **OpenEXR** | 🚧 ~40% — magic + version + 8 required attributes + chlist HALF/FLOAT + ZIP/NO_COMPRESSION (16 lines/block via flate2 + spec interleave + predictor); pure-Rust IEEE 754-2008 binary16 (bit-exact across all 65536 finite halves); exrheader cross-validates; lacks PIZ/RLE/B44/DWAA-B, tiled, multipart, deep-data, UINT, sub-sampled chroma | ✅ ~40% — RGBA HALF/FLOAT scanline + ZIP/uncompressed |
| **Farbfeld** | ✅ 100% — full spec (8B `farbfeld` magic + W/H u32 BE + RGBA u16 BE rows) | ✅ 100% — full spec |
| **HDR** (Radiance RGBE) | ✅ ~95% — `#?RADIANCE`/`#?RGBE` magic + KEY=VALUE attrs + new-RLE encode/decode + old-RLE decode + 8 axis-flag combos + shared-exponent codec | ✅ ~90% — new-RLE encode (canonical `-Y H +X W`); lacks old-RLE encode + XYZE→RGB helpers |
...
| **TGA** | ✅ ~90% — types 1/2/3/9/10/11 read at 8/15/16/24/32 bpp; auto-flip top/bottom origin; TGA 2.0 footer recognised; magick cross-validated | ✅ ~70% — type 2 (uncompressed) + type 10 (RLE) at 24/32 bpp; lacks types 1/3/9/11 write, extension area emit |
| **ICER** (JPL) | 🚧 ~30% — Mars-rover heritage; full SegmentHeader/PacketHeader bidirectional; integer 5/3 wavelet (filter Q) forward + inverse + 2-D + dyadic D-level passes; binary arith coder + ContextModel scaffolded; uncompressed-segment path round-trips; lacks bit-plane scan orchestration, compressed segments, multi-segment images, float filters A-G | ✅ ~30% — uncompressed §III.D path round-trips Gray8 bit-exact |
| **WBMP** (WAP Bitmap) | ✅ 100% — Type 0 monochrome + MBI variable-length integers (Types 1+ never normatively defined) | ✅ 100% — Type 0 |
| **PCX** (ZSoft) | ✅ ~75% — 1 bpp × 1/4 planes (mono + 16-colour EGA) + 8 bpp × 1 plane (256-colour VGA tail) + 8 bpp × 3 planes (24-bit RGB planar); RLE codec; magick cross-validated | ✅ ~70% — 8 bpp indexed + 24 bpp planar (PCX 5.0); lacks 2/4 bpp packed-bits, indexed-from-RGB |
| **ILBM** (Amiga IFF) | ✅ ~85% — BMHD/CMAP/CAMG/BODY chunks + uncompressed planar + ByteRun1 RLE + EHB (32→64 palette mirror) + HAM6/HAM8 decode (per-pixel R/G/B state, control-op + channel-widening); under existing `oxideav-iff` crate; lacks PBM chunky variant, ANIM, SHAM/PCHG | ✅ ~50% — indexed only (1..=8 bitplanes); lacks HAM/EHB write |
| **PICT** (Apple QuickDraw) | ✅ ~50% — v2 16-bit opcode walker covering all pen/colour/text/shape state ops + raster (PackBitsRect 1-bit + DirectBitsRect 16-bit A1R5G5B5 + 32-bit XRGB/ARGB packType=1); v1 sentinel detected (raster opcodes deferred); lacks DirectBitsRect packType 2-4, RegionRect paths, CompressedQuickTime opcode, drawing-command rasteriser | — |
