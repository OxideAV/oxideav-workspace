...
| **Opus** | 🚧 ~30% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + §3.1/§4.2 framing dispatch + r162 §3.4 R1..R7 malformed-input audit + r182 celt_band_layout 21-band MDCT + r183 §4.3.4.3 spread + r190 §4.3.4.5 TF-resolution adjustment lookup + r191 (CELT side) §4.3.3 `LOG2_FRAC_TABLE` + `intensity_rsv` + `reserve_stereo` budget-gate helpers (158 CELT tests, closes intensity-stereo half of #943); remainder of #943 (`cache_caps50` init + full band-boost) + #936 Laplace algorithm gated | 🚧 scaffold |
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM + §2.4.3.1 CRC-16 verify + mp2 frame-level decode loop + r191 Annex D Phase-2 psy building blocks (Tables D.2a-f critical-band boundaries via `psy::critical_band_table(Layer, fs)` + Step 6 `av_tm`/`av_nm` indices + 4-piece `vf(dz, X)` masking function + composite `LT_tm`/`LT_nm` + Step 7 `LTg` power-domain global threshold + Table D.5 coder partition; 222 tests; allocator unchanged pending D.1/D.3/D.4 PNG→text #1262) | 🚧 ~70% — Layer I encoder + Layer II §C.1.5.2.7 bit-allocation + Table C.5 SNR + r160 §2.4.2.3 mp2 frame-header writer + r185 §2.4.1.6/§2.4.3.3.4 Layer II SAMPLES region writer; lacks Layer II ALLOC/SCFSI writers + Annex D psy |
