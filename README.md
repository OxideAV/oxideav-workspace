...
| **H.261** | ✅ ~98% — I+P QCIF/CIF + integer-pel + loop filter + §5.4 BCH FEC + §5.2 + Annex B HRD + RFC 4587 RTP payload format (4-byte §4.1 header + GOB-aligned packetizer + `-16` MVD guard) | ✅ ~98% — spiral+diamond ME + GQUANT-from-bitrate + BCH framing + RTP wrap + RFC 3550 §5.1 `RtpPacketizer` (M-bit + seq + ts + SSRC over GOB-aligned payloads); 45 dB at 64 kbit/s QCIF |
...
| **Indeo 3** (IV31/IV32) | 🚧 r2 — clean-room codec-frame header + bitstream header + spec/02 picture-layer plane-prelude parser (per-plane `num_vectors` + `mc_vectors[]` with §3.3 half-pel scaling + packed-MV formula + NULL-frame skip); lacks spec/03 macroblock layer / VQ / MC | — |
...
|-----------|------|--------|
| **`oxideav-source`** | URI resolution + file reader + prefetching BufferedSource | ✅ `file://` + `mem://` + `data:` (RFC 2397 inline base64/percent) drivers + `FileScope` allow-list policy; generic `SourceRegistry` for pluggable schemes |
...
| **`oxideav-rtmp`** | RTMP ingest + push | ✅ Server accepts incoming publishers + client pushes to remote servers; AMF0 handshake / chunk stream demux + **AMF3 wire-format parser + builder per Adobe AMF3 §3.1 / §1.3.1 / §2.2**; Enhanced-RTMP v1 video (HEVC/AV1/VP9/AVC FourCC) + v2 audio (Opus/FLAC/AC-3/E-AC-3/MP3/AAC FourCC); pluggable key-verification hook; `rtmp://` registered as a `PacketSource` on `SourceRegistry` |
...
| **`oxideav-pipeline`** | Pipeline composition (source → transforms → sink) | ✅ JSON transcode-graph executor; pipelined multithreaded runtime + `Executor::with_channel_caps(ChannelCaps { packets, frames })` configurable per-track depth (embedded `{1,1}` → offline `{64,32}`) |
