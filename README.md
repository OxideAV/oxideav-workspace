...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~87% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed MDCT + auto block-type SM + r158 §C.1.5.4.3 auto block-type × outer-loop integration (Long→long-loop, Short→short-loop, Start/End fixed-gain); lacks Annex D psy (#1048) + intensity-stereo + transition-skeleton outer loop + mixed-block outer loop + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~85% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + P-frame pixel lockstep + §9.4 filter_type + §13.4 keyframe+inter caller-driven + keyframe observed-counts fitter + r158 §13.4 observed-counts fitter for inter path (−8.2 to −30.0% inter wire, 523 tests) |
...
| **AV1** | 🚧 ~50% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.10/11/12/13/55/56 leaves + r158 §5.11.20 read_is_inter (4-arm dispatch + IsInters[] grid, 483 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~76% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + §4.1 size_bits sweep + r158 §5.2.2 three-position lazy LZ77 matcher (depth=3 default); lacks VP8-lossy encode (pending vp8 republish #1068) |
