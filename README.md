...
| **JPEG 2000** | 🚧 ~72% — END-TO-END decode (headers → tier-2/tier-1 MQ → IDWT → MCT → Annex E reassembly across all 5 §B.12 progression orders incl. RPCL/PCRL/CPRL, §D.3.4 π-membership fix + per-coefficient §D.2.1 Nb → rate-truncated 9-7 now ±1 of reference) + 4 fuzz targets; lacks HTJ2K + multi-layer reassembly | 🚧 ~5% — scaffold |
