...
| **MP3** | ✅ ~100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~96% — Layer III + Annex D psychoacoustic Phase-2 bit-allocation loop now closed (MNR-select → quantize-promote → recompute-MNR → budget-update + iterate/terminate); lacks Steps 1-3 FFT/SPL/tonality classifier + Model 2 + intensity-stereo |
...
| **AV1** | 🚧 ~35% — intra decode: standalone `decode_av1` + 4:2:0/monochrome multi-superblock to 128×128 + full base_q_idx; lacks inter prediction, loop filter, CDEF, loop restoration, film grain, superres, scalability | 🚧 ~30% — intra encode: `encode_av1` 4:2:0 YUV→IVF + 14-mode picker + all §5.11 TX_SIZES_ALL shapes + lossy/lossless + composed §5.11.23 `write_inter_block_mode_info` (ref_frames→mode→drl→assign_mv); lacks §5.11.27-29 motion-mode/inter-intra/compound-type writers + RD picker + inter reconstruction/encode chain |
