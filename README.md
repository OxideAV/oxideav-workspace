...
| **Shorten** (.shn) | 🚧 r14 (post-2026-05-18 orphan) — `ajkg` magic + v2/v3 ulong + svar(n) + per-block function dispatch + VERBATIM/QUIT + DIFF0..3 + Rice residual + per-channel carry + spec/05 §2.5 running mean + QLPC predictor + r7 `decode_stream` + r145 `Decoder` trait + r181 block-by-block + r187 streaming `Decoder` + r191 envelope encoder surface + r197 `write_diff0_block` + r209 **`write_diff1_block` order-1 predictor encoder** (full `<fn=1> <energy> <residual>×bs` command per spec/03 §3.2 + spec/05 §1.1 + §3.1; seeds `s(t-1)` from `carry.at(0)` and slides `s_m1` to each just-emitted sample; mean-invariant per spec/05 §2 so no `mu_chan` param; `min_energy_for_diff1` selector shares the natural-width scan with DIFF0; encode→decode round-trips byte-exact through `decode_stream` across mono/stereo/3-channel round-robin, cross-block carry continuity, VERBATIM-DIFF1 splice, silent block, constant signal, ±127 max-natural; +20 tests = 224); lacks DIFF2..3/QLPC predictor encoders + #1267 spec/04 §2 BLOCK_FN_QUIT contradiction | 🚧 scaffold |
