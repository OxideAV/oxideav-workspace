...
| **MP3** | ✅ ~100% — MPEG-1/2/2.5 frame walker + Layer III main-data Huffman + requantize + stereo (MS+intensity) + IMDCT + polyphase synthesis; end-to-end bitstream→PCM bit-exact vs mpg123 on test corpora; ID3v2/Xing demuxer w/ seek | 🚧 ~78% — Phase-2 encoder: PCM→MP3 driver + outer loop + bit-reservoir + true-VBR + Xing TOC + CRC-16 + stereo + joint-stereo MS + r147 scalefac_scale escalation + r148 preemphasis decision + r149 §2.4.2.3 auto MS/LR per-frame picker (84/85 dB on correlated content @ 192 kbit/s); ffmpeg + mpg123 accept; lacks Annex D psy (#1048) + intensity-stereo + LSF + short/mixed block switching |
| **AAC** | 🚧 Phase 1 (post-r111 orphan-rebuild) — ADTS + raw_data_block walker + r126 AudioSpecificConfig + program_config_element + Phase-2 encode writers: section_data + ics_info + pulse_data + tns_data + r149 §4.4.6 scale_factor_data parser+writer (Table 4.53 + Table 4.A.1 codebook 12 over 19 bits; 5th syntax-element writer, 227 tests); decoder body still pending | 🚧 scaffold |
...
| **FFV1** | 🚧 ~70% (post-2026-05-18 orphan) — RFC 9043 §3.x decoder + §4.x demux + end-to-end decode_frame driver (YCbCr + RGB Y/Cb bit-exact; RGB Cr divergence open) + r142 §4.9 Slice Footer encoder + r146 §4.6 Slice Header encoder + r149 §3.8.2 Golomb-Rice content encoder primitives (BitWriter + put_ur/put_sr/put_vlc_symbol; 281 tests); lacks Decoder registration (#904) + RGB Cr fix + §4.8 run-mode encoder dispatch + frame-level encoder | 🚧 scaffold |
| **MPEG-1 video** | 🚧 clean-room rebuild (post-audit 2026-05-18) — sequence/GOP/picture/slice headers + macroblock walk + intra-DC + §2.4.3.7 dct_coeff walker + §2.4.4.1/§2.4.4.2 dequantiser bodies + r149 §7.6.3.6 MPEG-2 dual-prime additional arithmetic; IDCT pending (Annex A normative ref to IEEE P1180/D2 #1110) | 🚧 scaffold |
...
| **H.266 (VVC)** | 🚧 ~65% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + affine sub-block MC + PROF + AMVP candidate + bcw_idx + SbTMVP + merge_subblock + eq.85 MaxNumSubblockMergeCand + r146 merge_data() wire-up + r149 per-CB MergeSubblockFlag/InterAffineFlag neighbour grid fuse (replaces r146 stub; 999 tests); lacks non-merge inter CU walk + encoder-side subblock-merge emission | 🚧 ~85% — see crate README |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures (incl. ARNR, golden refresh, SPLITMV recon) | 🚧 ~68% — Phase-2 I+P stream encoder + SPLITMV picker + GOLDEN/ALTREF selector + r149 §9.5/§20.4 multi-partition inter (bit-identical PSNR across N∈{1,2,4,8}, 47.0 dB at qi=16); lacks §9.4 mode/ref delta + caller-driven §9.7 refresh |
| **VP9** | 🚧 ~35% (post-2026-05-20 orphan) — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header sweeps + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §8.6.2 reconstruct + §6.4.3 decode_partition recursive driver + §6.4.13 read_is_inter + r149 §6.3.11 read_is_inter_probs (349 tests); lacks §6.4.4 decode_block + §6.4.11 inter driver + §6.4.16 inter_block_mode_info + §8.4 loop filter | 🚧 scaffold |
| **AV1** | 🚧 ~35% — orphan-rebuild; uncompressed header + msac symbol decoder + §9.4 default-CDF families + §8.3.2 ctx selectors + §5.11.49 palette_tokens walker + §9.3 block-size tables + r149 §5.11.49 caller-side arg derivation (Y/UV branches, <4-bump, 359 tests); lacks tile-content reconstruction + ref-frame update | 🚧 scaffold |
| **Dirac / VC-2** | ✅ ~94% — VC-2 LD+HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit + bit-exact intra fixtures + r128 inter-mean rounding fix | 🚧 ~95% — HQ+LD intra + Dirac core-syntax + per-block adaptive sub-pel + 2-ref bipred + post-OBMC mode refinement + r138/r141 picture/sequence rate-control (PerPicture/CBR) + r146 HQ VBV + r149 LD `Vbv { buffer_bytes }` leaky-bucket (peak-caps at target+buffer; degenerates to PerPicture at 0 and CBR at ∞) |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate; `oxideav_core::Decoder` registered | 🚧 ~60% (post-r144 Hat-3 rebuild) — VP8L lossless encoder w/ LZ77 + §4.1 spatial-predictor + §3.5.2/§4.2 color-transform + §5.2.3 color-cache sweep + r149 §3.7.2.1.1 simple code-length chooser (-82% on 1×1, -60-76% on solid/gradient fixtures); container + metadata + animation; lacks VP8-lossy encode + 8 r131-r144 pieces (#1120) |
