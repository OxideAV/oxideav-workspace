...
| **NSF** (NES) | 🚧 ~96% — full 6502 + IRQ/NMI + 5/5 2A03 APU + DMC DMA + six expansion chips + NSF v1/v2/NSFe + Dendy region + r154 Namco 163 + r185 VRC7 OPLL pipeline + r199 VRC7 register semantics + r204 VRC7 KSR pipeline + r209 **§4 KSL formula scaffold** (`ksl_attenuation_env_levels(block, fnum_hi, ksl)` + `KSL_BASE_BYTE_TABLE` + `OpllChannel::{mod_ksl,car_ksl}` captured from patch; §4 formula `(base) >> (3 - KSL)` wired into both modulator + carrier paths in `sample_with_test`; block 0 bit-exact per §4 schema's explicit zero row, blocks 1..=7 zero scaffold pending §4 byte base table — single-cell edit when staged; 202 tests). Lacks §4 byte base table rows 1..=7 + §7 per-rate env tables (provenance-pending) + rhythm mode | — synthesis only |
...
| **Cook** (RealMedia) | 🚧 r6 — flavor table + cookie parser + 8 DSP parameter tables + open-time `DecodeConfig` + wire-level real-stream integration test + cookie→flavor multi-match API + r209 **per-buffer XOR descramble + `CommonMode` toggle** (`src/descramble.rs` word-wise 32-bit LE pass with `xor_key(in_ptr,in_len)`; `descramble_packet` returns zero-copy `Cow::Borrowed` on the validated common-mode-off path, `Cow::Owned` on-path; self-inverse + length-preserving on bundled `FUN_RM_32.rm` packets 0/100 for two keys; 39 lib + 4 integration tests); lacks transform / entropy decode | — |
