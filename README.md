...
| Blu-ray (BD-ROM) | ✅ | — | — | UDF 2.50 + BDMV + `.m2ts` + `bluray://`; playlists / chapters / multi-angle + EP_map keyframe seek + AACS hook + PGS subtitle-segment parser (PCS/WDS/PDS/ODS + RLE); lacks HDMV opcode exec + BD-J |
...
| **G.728** | 🚧 ~91% — LD-CELP decode: Annex A-D + Levinson + postfilter chain + ICOUNT=3 update stagger + Annex I §I.4.1 frame-erasure excitation extrapolation + §I.4.2 LPC softening + §I.4.5 gain-growth limit + Annex G §G.1.2/§G.1.3 fixed-point arith primitives | 🚧 ~87% — analysis-by-synthesis loop complete, bit-exact lockstep with decoder incl. ICOUNT stagger; lacks §G.2/§G.3 fixed-point coder |
| **G.729** | 🚧 ~30% — tables + serial parser + full §4.1 per-frame parameter chain (LSP / pitch / FCB / gains incl. parity concealment) + §4.1.6 LP synthesis (adaptive/fixed excitation → first reconstructed-speech PCM) over 18 222 conformance frames + §4.2.2 short-term postfilter H_f(z) + §4.2.3 tilt compensation + §4.2.4 adaptive gain control + §4.2.1 long-term postfilter (integer-delay eqs 78–83); lacks §4.2.1 1/8 fractional pass + Annex B DTX | 🚧 ~5% — scaffold |
...
| **MS-MPEG-4** (v1/v2/v3) | 🚧 ~57% — v3 I/P decode + both MV VLC variants (default + alt) + v1/v2 P-frame pixels end-to-end (skip + inter MBs, half-pel MC); lacks 4-MV MCBPC + v1/v2 intra DC rule (docs now unblocked) | — |
...
| **SVQ1/SVQ3** (Sorenson) | 🚧 ~42% — SVQ1 codebook payloads + leaf stage-accumulation reconstruction (spec/04 §4.5) + motion-vector median prediction + SVQ1 inter MV cache + INTER/INTER_4MV neighbour geometry + SVQ3 transform/dequant/intra/interp primitives + chroma DC full-dequant pipeline; lacks full block-reconstruction composition (5 precise docs gaps filed) | — |
...
| **DDS** | ✅ ~99% — header + DXT10 + BC1-7 + BC6H all modes + cubemaps/arrays/volumes + 16-bit/float + packed R11G11B10_FLOAT + R9G9B9E5_SHAREDEXP + R10G10B10A2_UINT + sub-sampled packed R8G8_B8G8/G8R8_G8B8 + 8/16/32-bit plain-integer UINT/SINT HDR uncompressed surfaces + daily fuzz | ✅ ~96% — uncompressed + BC1-7 + BC6H + mip chains + cubemap/array |
| **OpenEXR** | 🚧 ~94% — scanline (+PXR24) + tiled + deep + multi-part across all 4 part types + mip/ripmap + typed attribute inspectors; PIZ/B44 blocked on docs trace | ✅ ~96% — scanline (+PXR24) + tiled + deep + multi-part mixed write |
| **Farbfeld** | ✅ 100% | ✅ 100% |
