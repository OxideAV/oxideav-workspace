...
| **FLAC** | ✅ 100% — bit-exact vs RFC 9639 + CUESHEET → Chapter API | ✅ 100% — bit-exact roundtrip + LPC order/window/precision search + closed-form Rice estimate (4.6-6.0× faster); r147 flamegraphs (`best_partition` ~61% of encode CPU) + r150 +3 bench scenarios (mono-S24, wasted-bits, CONSTANT); scratch-reuse refactor evaluated and rejected (~10% regression on macOS arm64) |
...
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM + §2.4.3.1 CRC-16 verification + r150 mp2 §2.4.1.6/§2.4.3.1/§2.4.3.2 frame-level decode loop (`decode_frame()` returns 1152 PCM samples/channel; staged 31-frame stereo fixture decodes cleanly; 98 tests); lacks bit-exact PSNR validation + Layer II encoder + LSF (#1076) + Annex D psy | ✅ ~100% Layer I encoder — Annex C polyphase + scalefactor + energy allocator + Table C.3 quantizer; 1 kHz tone @ 192 kbit/s mono self-roundtrip RMS<0.01; lacks Annex D psy |
...
| **MP3** | ✅ ~100% — MPEG-1/2/2.5 frame walker + Layer III main-data Huffman + requantize + stereo (MS+intensity) + IMDCT + polyphase synthesis; end-to-end bitstream→PCM bit-exact vs mpg123 on test corpora; ID3v2/Xing demuxer w/ seek | 🚧 ~80% — Phase-2 encoder: PCM→MP3 + outer loop + bit-reservoir + true-VBR + Xing TOC + CRC-16 + stereo + joint-stereo MS + auto MS/LR per-frame picker (r149) + r150 trait-factory wrapper for auto MS/LR; lacks Annex D psy (#1048) + intensity-stereo + LSF + short/mixed block switching |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures (incl. ARNR, golden refresh, SPLITMV recon) | 🚧 ~72% — Phase-2 I+P stream encoder + SPLITMV picker + GOLDEN/ALTREF selector + multi-partition inter + r150 §9.7/§9.8 caller-driven RefreshControls (refresh_*/copy_buffer_to_* ladder + RefFrameSlot::Eq for tests); lacks per-MB §9.4 mode/ref delta layer |
...
| **AV1** | 🚧 ~38% — orphan-rebuild; uncompressed header + msac symbol decoder + §9.4 default-CDF families + §8.3.2 ctx selectors + §5.11.49 palette_tokens walker + caller-side args + §9.3 block-size tables + r150 §9.3 Partition_Subsize[10][BLOCK_SIZES] + 19 named BLOCK_* ordinals (375 tests); lacks §5.11.4 decode_partition body + tile-content reconstruction | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate; `oxideav_core::Decoder` registered | 🚧 ~65% (post-r144 Hat-3 rebuild) — VP8L lossless encoder w/ LZ77 + §4.1 spatial-predictor + §3.5.2/§4.2 color-transform + §5.2.3 color-cache sweep + §3.7.2.1.1 simple code-length + r150 §4.4 color-indexing forward pass (-15% on binary-coherent fixture); lacks VP8-lossy encode + §6.2.2 multi-meta-prefix + 7 r131-r144 pieces (#1120) |
