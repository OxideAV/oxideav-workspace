...
| **HEIF / HEIC** (ISO/IEC 23008-12 + MIAF) | ✅ ~90% — new `oxideav-heif` (2026-09-18): items / properties / grid-iovl-iden composition / alpha + depth aux / sequences, `hvc1` + `av01` items decoded through the registry, 14/14 staged bundles (5 sample-exact, rest ≤2 LSB), MIAF checks, fuzzed; lacks `iscl`, gain-map (`tmap`) application, L-HEVC/AVC items | ✅ ~85% — MIAF-conformant writer + sequence muxer (HEVC/AV1 items via the registry encoders, thumbnails, alpha, grid, Exif/XMP/ICC, transforms); AVIF profile migration onto this crate pending |
