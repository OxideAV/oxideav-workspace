...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + B-pyramid POC + 8 SEI types + fuzz-hardened slice/MC/SPS bounds; lacks MBAFF, SVC/3D/MVC | 🚧 ~80% — I+P (1MV/4MV, ¼-pel) + B 16x16/16x8/8x16/B_8x8 + per-cell mixed B_8x8 + B_Skip/B_Direct + weighted pred + CABAC at all chroma layouts; ffmpeg PSNR_Y 44.20 dB |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~68% — forward CABAC + DCT-II + per-CTU SAO RDO (luma+chroma) + APS-signalled per-class luma ALF Wiener-design RDO + chroma residual emit + explicit CBF CABAC + spec-shaped coding_tree shells + cu_qp_delta + 128×128 forced-QT-split + MTT BT+TT picker RDO (`SSE + λ·bits` over {leaf, BT_VERT/HORZ, TT_VERT/HORZ}) + multi-row `CuNeighbourMap`; lacks inter-frame P-slice pipeline |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle (per-plane uniform-fill envelope + libavcodec-version probe) | 🚧 ~35% — keyframe + simple P-frame + per-block luma intra-mode RDO; smooth gradient 53.06 dB at base_q_idx=64 |
| **AV1** | 🚧 ~72% — OBU + range coder + all intra preds + CDEF + LR + inter MC + palette + multi-ref compound + super-res + tx_type read-gating; SVT-AV1 48/48; lacks intrabc | 🚧 ~55% — forward range coder + forward DCT-II 8/16/32 + full coefficient emitter + partition/mode/TX emit; self-roundtrip bit-exact via own decoder, dav1d still rejects 64×64 |
...
| **JPEG XL** | 🚧 ~85% — ISO/IEC 18181-1:2024 final core. 6 small lossless fixtures decode PIXEL-CORRECT (incl. alpha-64x64 RGBA + Palette transforms + extra-channel). Modular path complete + ISOBMFF `FF 0A` codestream-signature strip; VarDCT scaffold with Annex I.2 IDCT primitive + Annex I.9.3..I.9.7 non-DCT helpers wired. d1-Squeeze divergence localised to upstream prelude D[] / TOC boundaries (per crate diagnostics); lacks HfPass + PassGroup HF + GetDCTQuantWeights + CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **`oxideav-audio-filter`** | Audio effects & conversions (streaming) | ✅ Volume, NoiseGate, Echo, Resample (polyphase windowed-sinc), Spectrogram, Biquad (LP/HP/BP/notch/peaking/shelving), Compressor, Limiter (look-ahead) |
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~70 filter types / 82 factory names — see crate README for the catalogue |
