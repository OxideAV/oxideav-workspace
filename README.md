...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing (Fraunhofer extension; bit-exact decode still MPEG-1-only) | 🚧 ~86% — Phase-2 PCM→MP3 + outer loop + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed forward MDCT + linbits-reach filter + auto block-type SM + r157 §C.1.5.4.3 short-block outer-loop primitive (per-(sfb,window) scalefac_s amp + bounded subblock_gain search; not yet wired into Mp3Encoder); lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~84% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + P-frame pixel lockstep + §9.4 filter_type + §13.4 keyframe+inter caller-driven + r157 §13.4 observed-counts fitter for keyframe path (−9.6 to −33.6% keyframe wire, 517 tests) |
...
| **AV1** | 🚧 ~48% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.10/11/12/13 leaves + r157 §5.11.55/56 clear_cdef/read_cdef (cdef_idx grid + 64×64 anchor mask + 128×128 four-anchor stamp, 468 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~75% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + §4.1 size_bits sweep + r157 §5.2.2 two-position lazy LZ77 matcher (depth=2 default); lacks VP8-lossy encode (pending vp8 republish #1068) |
