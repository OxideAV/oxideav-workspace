...
| **PICT** (Apple QuickDraw) | ✅ ~99% — v1 + v2 opcode walkers + drawing rasteriser + DirectBitsRect packType 0..4 + Region + clip + pen-size + Compressed/UncompressedQuickTime skip + r186 indexed PixMap variants + r199 §A-3 reserved-Apple-use v2 opcode skip + r205 **v1 (8-bit-opcode) §A-3 Table A-3 completion** (state/text-setup + text-glyph + 20 Same-shape opcodes via `opcode-8` verb-nibble routing; v1 state-machine-parity with v2 except glyph rendering); lacks text rasterisation + embedded `CompressedQuickTime 0x8200` JPEG decode | ✅ ~95% — `PictBuilder` + every v2 drawing-command family + magick cross-decode bit-exact + r211 **indexed-PixMap encoder** (BitsRect/PackBitsRect/BitsRgn/PackBitsRgn at 1/2/4/8 bpp w/ embedded ColorTable; §A-3 footnote `§` rowBytes high-bit dispatch; pairs the round-186 decoder) |
