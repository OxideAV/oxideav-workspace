...
| **JPEG XL** | 🚧 ~78% — rounds 1-8 against ISO/IEC 18181-1:2024 final core (master @ 58b55d2). **All 5 small lossless fixtures decode PIXEL-CORRECT** vs expected.png: pixel-1x1, gray-64x64, gradient-64x64-lossless, palette-32x32, grey_8x8_lossless. Coverage: C.2.1 + H.2 + H.3 + H.4.1+H.4.2 + H.6.4 inverse Palette + H.6.3 inverse RCT (6 modes × 6 perms) + H.6.2 inverse Squeeze + H.5 Self-correcting predictor + Annex E.4 ICC profile decoder (992 LOC, 41-context entropy stream + "acsp" magic verified). **Round 7** landed the four-piece multi-group refactor (GlobalModular partial decode + stream_index threading + TOC addressing + post-PassGroup transforms); synthetic 320×320 multi-group fixture committed (num_groups=9 at group_dim=128). **Round 8** empirically resolved the C.2.5 alphabet_size SPECGAP via three-way bake-off (interpretation C: `i < min(alphabet_size, table_size)` — only one matching cjxl); VarDCT Annex I scaffold + 8×8 IDCT primitive added. synth_320 LfGlobal now parses cleanly to bit 238; remaining blocker is a separate G.4.2 question (cjxl 0.11.1 emits 0-byte PassGroup[0][0]). 252 tests. FDIS-2021 typo #5 fixed by 2024; typos #1-#4 retained. Decoder rounds 7-11 + encoder rounds 1-6 retired 2026-05-08 under fruits-of-poisonous-tree (`old` branch); v0.0.5/v0.0.6/v0.0.7 yanked. Lacks LF/HF subband decode + IDCT dispatch / XYB | — RETIRED 2026-05-08, re-author after decoder forward progress |
