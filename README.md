...
- **Aggregator** — `oxideav-meta` exposes
  ...
  `source-drivers`. Slim builds via
...
| **Vorbis** | ✅ ~95% — all residue types per RFC 5215 | 🚧 ~85% — BitrateTarget {Low/Medium/High/HighTail} bank with mu-law-companded lookup_type=1 axis + per-target stereo crossover; 3-class residue for High/HighTail (extra 22×22 codebook for silent / active / high-energy partition); per-target silence percentile + residue begin offset; frame-level global M/S correlation override on Low; ffmpeg cross-decodes |
...
| **AAC** | 🚧 ~82% — AAC-LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + LD/ELD AudioSpecificConfig parse scaffold (objectType 23/39); lacks LD/ELD frame decode, USAC | 🚧 ~78% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + gapless + AscBuilder + Bark-band PE/SMR psy default-on across mono/v2/stereo (M/S CPE IMDCT side-lobe leakage closed via use_tns gate; pns-noise gate) |
| **CELT** | ✅ ~95% — full §4.3 pipeline | 🚧 ~78% — mono + stereo intra-only long-block + short-block on transients + per-band TF + LM=0/1/2 (120/240/480-sample MDCT) + comb pitch pre-filter + anti-collapse flag + LM heuristic |
...
| **AC-4** (Dolby) | 🚧 ~78% — A-SPX + DRC + DE walker + 60+ ETSI Huffman codebooks + ASPX_ACPL_1/2/3 + 5_X/7_X channel walkers + 5_X ACPL_3 mch synthesis (Pseudocode 118) + mono/stereo/joint short-frame sf_data(ASF) + SSF bitstream walker + SSF PCM synthesis (Pseudocodes 4a-d/4e/26-39 + 77 atsg-border derivation for FIXVAR/VARFIX/VARVAR) + SNF spectral-noise inject (§5.1.4 LCG) + §5.2.5.2.2 Heuristic Scaling | — |
...
| **Shorten** (.shn) | ✅ ~98% — all 6 internal_ftype variants + DIFF0-3 + QLPC (Levinson-Durbin) + BITSHIFT + adaptive Rice; bit-exact ffmpeg silence + ffmpeg cross-decode all 4 modes | ✅ ~98% — DIFFn predictor pick + QLPC + BITSHIFT detection + Rice-k closed-form + FN_ZERO + real WAV VERBATIM header; bit-exact roundtrip; ffmpeg cross-decodes |
...
| **VP6** | ✅ ~95% — full FLV playback (845/845 sample frames); Huffman coefficient path on `use_huffman==1` | 🚧 ~78% — keyframe + skip + inter + real DCT residual (43 dB internal) + ¼-pel ME + scene-change-driven golden refresh (EMA SAD spike) + INTER_FOURMV + Huffman inter encode + bool/Huffman RDO (smaller path wins per-frame) |
| **VP8** | ✅ 100% — I+P + 6-tap luma/chroma + per-MB loopfilter slab + persistent ref/mode_deltas + correct §16.3 split_mv_tree + RFC 6386 §17.1 default_mv_context high-bit probs + RFC §18.1 luma-MV doubling + RFC §20.13 sixtap H/V intermediate clamp; entire 15-fixture corpus uniformly bit-exact | 🚧 ~88% — I+P + B_PRED intra-in-P + SPLIT_MV + alt-ref/golden + Lagrangian RDO + segment QP/LF + per-frequency AC/DC deltas + Trellis quantisation (libvpx vp8_optimize_b shape) + rate-aware sub-pel ME + combined-feature gain +0.52 dB / -8.16 % bytes vs plain-RDO |
...
| **Dirac / VC-2** | ✅ ~90% — VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit; ffmpeg bit-exact 8-bit 4:2:2/4:4:4 + 10-bit 4:2:0 | 🚧 ~88% — HQ + LD intra + Dirac core-syntax intra + core-syntax inter (OBMC + LeGall 5/3 wavelet residue, ffmpeg cross-decode 34 dB translating-square / 53 dB camera-pan qpel; bit-exact lossless self-roundtrip) + 2-ref bipred B-picture at integer-pel ME (~50 dB ffmpeg, root-caused 8-tap half-pel filter convention drift at 2-ref OBMC blend) |
...
| **HuffYUV** / FFVHuff | ✅ ~100% — v2 yuv420p/422p/rgb24/bgra + v3 yuv 444/422/420 + gbrp/gbrap (8/10/12-bit) + yuv411p + gray at 8/10/12/16-bit (incl. §5.5/§9.7 high-bits + 2-raw splice) + interlaced field mode; 11+ ffmpeg cross-decode fixtures bit-exact | ✅ ~98% — bit-exact `huffyuv` / `ffvhuff` encoder with LEFT/GRADIENT/MEDIAN over every decode surface; ffmpeg cross-decode bit-exact |
...
| **Ut Video** | ✅ ~92% — ULRG + ULY2 with NONE/LEFT/MEDIAN/GRADIENT (incl. interlaced field-parity-aware predictor) + UQ 10-bit (Pro variant: frame_info-at-start + per-plane Huffman lengths + mod-1024 LEFT seed 0x200) + UM SymPack (LeBitReader + 3-bit control sign-flip mapping); UL/UQ/UM 8/10-bit families bit-exact across 11 ffmpeg fixtures | — |
...
| **WebP VP8L** | ✅ 100% — full lossless; 7/7 BitExact vs `dwebp` | ✅ ~98% — subtract-green + predictor + colour transform + RDO + K=16 meta-Huffman + cache-bits + LZ77 16384-px window/256-deep chains + near-lossless + palette RDO bias + three-pass cost-modelled LZ77 (Viterbi-style optimal DP at ≥256×256, with bin-boundary length probing) + multi-iteration Huffman codebook refit + predictor-tile-bits + EntropyImage tile-bits RDO sweeps; byte-parity vs cwebp on 256×256 synthetic; landscape-256 1.0124× cwebp; residual 1–4 % gap to cwebp on natural photos sits in entropy-based predictor-mode scoring (cwebp uses Shannon entropy, ours uses SAD) |
| **WebP VP8** | ✅ 100% — via VP8 (whole 15-fixture corpus uniformly bit-exact) + bit-exact YUV→RGB + fancy chroma upsample | 🚧 ~92% — VP8 I-frame + ALPH + per-segment QP/LF + per-frequency AC/DC deltas + quality-driven quant matrix curve + Trellis quant + rate-aware sub-pel ME (upstream vp8); animated mixed lossy/lossless; dwebp cross-decode clean; 350K+ fuzz runs/min clean |
...
| **DDS** (DirectDraw Surface) | ✅ ~75% — DDS_HEADER + DDS_HEADER_DXT10 + uncompressed (10 layouts) + BC1/BC2/BC3/BC4_unorm/snorm/BC5_unorm/snorm decompression + mipmap chain + 6-face cubemaps + DX10 texture arrays + full 132-entry DXGI table; lacks BC6H/BC7 decompression | ✅ ~40% — uncompressed only; rejects block-compressed |
| **OpenEXR** | 🚧 ~65% — magic + 8 required attributes + chlist HALF/FLOAT/UINT + NO_COMPRESSION/ZIP/ZIPS/RLE + tiled (single_tile/ONE_LEVEL) + sub-sampled chroma + predictor +128 centring fix; exrmetrics cross-validates; lacks PIZ/B44/B44A/DWAA-B, multi-part, deep-data, multi-level mip/rip-maps | ✅ ~55% — RGBA HALF/FLOAT/UINT scanline + ZIP/ZIPS/RLE/uncompressed |
...
| **ICER** (JPL) | 🚧 ~55% — Mars-rover heritage; bit-plane scan orchestration (significance/refinement passes) + compressed-segment encode/decode + multi-segment muxer/demuxer + float filters A-F (CDF/Daubechies/Haar lifting); lacks stripe-ordered scan, real JPL context tables, multi-packet ordering, filter G | ✅ ~55% — compressed + uncompressed segments + multi-segment + filter Q + filters A-F |
