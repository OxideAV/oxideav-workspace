...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures + 0.1.13 public-surface lock + standalone-API & ffmpeg-oracle e2e (PSNR 42-61 dB bidirectional) + criterion benches; motion_comp in-bounds fast path -30% sub-pel; nightly std::simd inverse-WHT -24% | ✅ 100% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §11 intra picker + §13.4 token-prob fitter + real complexity-aware two-pass rate-control (Vp8Encoder/Vp8EncoderConfig/Vp8EncoderStats/LoopFilterMode + 28 DEFAULT_* knobs); profile-driven log2 LUT (-22% inter encode, -8% keyframe) |
...
| **AV1** | 🚧 ~57% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.7-13/19-21/55/56 leaves + r164 use_intrabc + r165 §5.11.7/§5.11.22 intra_frame_y_mode syntax element (539+ tests); lacks §5.11.5 decode_block body + §5.11.18 inter_frame_mode_info dispatcher | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ 100% — RFC 9649 container + VP8L lossless + ALPH + animated + VP8-lossy via sibling crate + API-COMPAT-0.1.2 surface lock + standalone-API e2e (8 tests under `--no-default-features`) + external-oracle e2e (3 directions, byte-exact lossless) + criterion benches; SWAR add_pred/average2/inverse_subtract_green -23% lossless decode; scalar argb→rgba repack -93% (14.5×); nightly std::simd u8x16 shuffle -95% vs baseline | ✅ 100% — VP8L lossless encoder (LZ77 + spatial/color transforms + color cache + multi-meta-prefix + Shannon chooser) + VP8-lossy via oxideav-vp8 0.2.1 (RIFF/WEBP wrap + From<Vp8Error> 1-to-1 adapter + lossy roundtrip) |
