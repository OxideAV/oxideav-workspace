...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S, IMDCT) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate (QMF-domain + allpass decorrelator + Ra/Rb mixing). **Output-scale fix (round 5)**: S16 path was multiplying already-int-ranged IMDCT by 32767 on top; two-line fix lifted **HE-AACv1 stereo 0.88 dB → 48.28 dB** vs ffmpeg. AAC-LC mono 22.56 dB, AAC-LC stereo 27.51 dB. Gaps: hybrid sub-QMF filterbank, IPD/OPD parsed but not applied, 34-band native resolution, mid-stream phase glitch on long decodes. | ✅ AAC-LC (mono+stereo + PNS + intensity stereo + pulse data) + HE-AACv1 mono encoder |
...
| **AC-3** (Dolby Digital) | ✅ Full decode pipeline + FFT-backed IMDCT (§7.9.4) + §7.8 downmix. Sine fixture: **92.02 dB** vs ffmpeg after fixing 3 table transcription bugs. §7.9.4.2 short2 IMDCT TDAC sign-pattern fixed (round 5). Transient fixture still at 15 dB — bisected to **bit-allocation divergence at MDCT bins 4-5 during bursts** (zeroing those bins recovers ~30 dB); suspect is the `bndstrt==0` lowcomp path in `run_bit_allocation`. | — |
| **AC-4** (Dolby) | 🚧 Sync / TOC / presentation / substream parse + full A-SPX front-end: `aspx_config` / `companding_control` / `aspx_framing` / `aspx_delta_dir` / `aspx_hfgen_iwc_{1,2}ch`. **Round 5 unblocked**: all 18 Annex A.2 Huffman codebook arrays transcribed from the ETSI normative `ts_10319001v010401p0-tables.c` + `aspx_ec_data()` parser (Table 57) + `AspxHcb` lookup by `(data_type, quant_mode, stereo_mode, hcb_type)` (Pseudocode 79). Stub decoder still emits silence (downstream QMF synthesis / HF regeneration / A-CPL pending). | — |
...
| **FFV1** | ✅ v3: 4:2:0 / 4:4:4 YUV + RGB via JP2K RCT + `extra_plane` alpha + 9..16-bit RGB (§3.7.2.1 BGR exception) + 10-bit YUV. Range-coded `coder_type=1` + custom state-transition delta `coder_type=2` + Golomb-Rice `coder_type=0`. Per-plane `qt_idx` from slice header. **Round 5**: `initial_state_delta` (§4.2.15, FFmpeg `-context 1` extradata) parsed and wired. Remaining: RGB encode, Golomb-Rice for >8-bit or with alpha, cross-frame state retention for `intra=0`. | ✅ v3 range-coded YUV + **multi-slice** (slices=N, squarest divisor grid, chroma-aligned boundaries) + **10-bit YUV** 4:2:0/4:2:2/4:4:4 — ffmpeg bit-exactly decodes our multi-slice 10-bit output. |
| **MPEG-1 video** | ✅ I+P+B frames | ✅ I+P+B frames (half-pel ME, FWD/BWD/BI B-modes, 43 dB PSNR) |
| **MPEG-4 Part 2** | ✅ I + P + B-VOP with 4MV direct mode, half-pel MC, quarter-pel MC in B-VOPs, decode→display reorder, **field-MV decode + field-DCT residual reorder + alt-vertical-scan for interlaced B-VOPs** (round 5). Full 12-frame fixture decodes at ~30 dB. Frame-7 outlier at 22 dB localized to MB(3,3) — MBTYPE=Backward MV(0,-16) parses correctly but ffmpeg produces forward-zero-MV output; f_code=1 MVD semantics suspect. Interlaced field-sample MC still uses frame-average (parsed MVs captured, not yet applied to MC fetch). | ✅ I+P-VOP (41-43 dB PSNR, 21% vs all-I) |
| **Theora** | ✅ I+P frames | ✅ I+P frames incl. INTER_MV_FOUR (45 dB PSNR, 3.7× vs all-I) |
| **H.263** | ✅ I+P pictures, half-pel MC, Annex J deblock, Annex D URMV, Annex F Advanced Prediction (4MV + OBMC). 5-frame testsrc QCIF: 60–69 dB vs ffmpeg. **Annex E SAC core** (`src/sac.rs`, 819 LOC: SacEncoder/Decoder + PscFifo + all 23 §E.8 models, 11 round-trip tests) landed round 5; MB-layer VLC→SAC swap deferred (no ffmpeg SAC encoder exists as a test oracle). Long-clip ~1 dB/P-frame drift diagnosed: **IDCT LSB accumulation through reference feedback** (first I-frame has 300 pels differing by `|Δ| ≤ 1`, signed sum +288); not OBMC-specific — plain 1MV streams show the same pattern. | ✅ I+P pictures, diamond-pattern motion search (±15 pel range), 46 dB PSNR on sliding-gradient |
...
| **H.265 (HEVC)** | ✅ I / P / B slice decode, 8-bit 4:2:0 + SAO + deblock. Main 10 intra bit-exact vs ffmpeg. **Main 10 inter 20.73 dB → 24.19 dB (round 5)** after finding the real drift source: `cbf_cb` / `cbf_cr` were decoded at TU leaves only, but §7.3.8.9 requires them at every tree depth with `log2TrafoSize > 2`, BEFORE the split, gated by parent cbf. The round-4 bisection candidates (MC clip order / 4×4 `trType` / TMVP scaling) were all wrong. Also landed: spec-exact MC pipeline (shift1/shift2/shift3 per BitDepth) + AMP part_mode ctxInc fix. HEIF/HEIC still-image decode via opt-in `heif` feature. Gaps: 12-bit, 4:2:2 / 4:4:4, residual-coding drift on non-trivial 8×8 inter TUs (round-6 target), AMP / long-term refs / scaling lists / tiles+WPP; HEIF gaps: grid items, multi-extent iloc, alpha aux, transforms. | — |
| **H.266 (VVC)** | 🚧 **Full VVC front-end now parses real streams**: NAL + VPS + SPS full tail + HRD timing + PPS full tail + **picture-header tail** (round 5) + **header-level `ref_pic_lists()`** (§7.3.9) + stateful slice header with **slice-tail past deblocking** + RefPicList construction + **PPS partition block + `sh_slice_address` derivation** (§6.5.1) + DCI + OPI + APS. 161 tests. Remaining front-end edge cases: `pred_weight_table()`, `sps_range_extension()`, `ref_pic_lists()` consumption when `pps_rpl_info_in_ph_flag==0`, multi-subpic `CurrSubpicIdx`, entry-point offsets with entropy-coding-sync. CTU walker / reconstruction not yet started. | — |
...
| **VP9** | 🚧 Keyframe + inter (single + compound ref, scaled refs) + per-block segmentation + bit-accurate §6.5 MV list + inter-mode ctx + full §6.3 compressed-header probability-update + §8.10 saved slots. **Round 5**: `AboveNonzeroContext` / `LeftNonzeroContext` (§6.4.22) tracking landed — but PSNR still 10.94 dB because the real bottleneck surfaced: **bool-decoder bit desync in `decode_inter_block`'s MV / ref / mode reads**. On inter frames, only the first 64×64 SB decodes as inter (`is_inter_prob[0]=9` expects 97% → inter); the rest flip to intra-over-grey-predictors (Y≈128, σ≈1). Round-6 target. §8.4 `adapt_probs` + `UsePrevFrameMvs` + 10/12-bit + 4:2:2/4:4:4 still pending. | — |
| **AV1** | 🚧 Full OBU parse, range coder + CDFs, coefficient decode + partition + transforms + all intra predictors with edge filter/upsample + **§7.15 CDEF** + Loop Restoration (Wiener + SGR) + inter MC (NEWMV + GLOBALMV) + ref-scaling. **Round 5 chroma fix (§7.11.2.1)**: `gather_neighbors` was reading past TX-unit edges into un-reconstructed territory; spec-correct `aboveLimit` / `leftLimit` clamps landed. Chroma on flat-gray U/V jumped **12 dB → 48/45 dB**. Luma on testsrc still ~11 dB — traced to bitstream syntax-layer desync: `read_block_tx_size()` (§5.11.16) never called, plus `read_skip` / `filter_intra_mode_info` / `palette_mode_info` / `use_intrabc` / `read_delta_qindex` missing or mis-ordered in `decode_leaf_block`. Multi-day rewrite. | — |
...
| **JPEG 2000** | 🚧 Part-1 baseline + multi-tile (§B.3) + MQ + EBCOT + 5/3 + 9/7 IDWT + tier-2 + LRCP / RLCP + JP2 wrapper. Two real bug fixes (MQ state-table `nlps`/`nmps` swap; T1 `pi`-flag §D.3.4). Interop PSNR: spike4 + const32 bit-exact; opj16_l1 35.09 dB, opj32 29.79 dB. **Round 5 diagnostic** narrows the residual to "one magref bit flipped at the last bit-plane (bpno=1)": our forward-DWT of the reference matches ffmpeg's values, and the first 21 bytes of our MQ byte stream match OpenJPEG exactly — divergence at byte 22. Multi-layer + user precinct grids + CPRL / PCRL / RPCL + Part-2 still pending. | ✅ 5/3 lossless + 9/7 irreversible RGB (forward RCT/ICT; JP2 box wrapper) |
...
| **XM** (FastTracker 2) | 🚧 Structural parse + playback with shared mixer (`XmPitch` Amiga + Linear tables; delta-PCM sample decode). **Round 5**: volume + panning envelopes (tick-based linear interpolation, sustain stall, loop wrap), fadeout (per-instrument `volume_fadeout` register decremented each tick after key release), key-off (note 97 per FT2). Test asserts tail-RMS < half of head-RMS after key-off. Gaps: vibrato, tone portamento, Bxy/Dxy jumps, Exy subcommands, volume-column slides, Gxy/Hxy globals. | — |
