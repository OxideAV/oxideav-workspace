...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures; r167 0.1.13 public-surface lock (Vp8Decoder/Vp8Frame/frame_tag/ivf/registry shells + 8 compat tests; both registry & standalone builds green) | 🚧 ~93% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + r165 §11 intra-pick parallel-composed with §13.4 fitter on stream refresh path + r167 0.1.13 public-surface lock (Vp8Encoder/Vp8EncoderConfig/Vp8EncoderStats/LoopFilterMode + 28 DEFAULT_* tuning knobs + TwoPass family shapes stubbed; 549+ tests) |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated + r165 §5.2/§6.2.2 decode_argb bit-prefix property tests + r167 API-COMPAT-0.1.2 surface lock (8 pub-mod shims + 27-test compile-only contract suite); VP8-lossy via sibling crate | 🚧 ~80% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + Shannon chooser + depth-4 lazy LZ77 (398 tests); lacks VP8-lossy encode (pending vp8 republish #1068) |
