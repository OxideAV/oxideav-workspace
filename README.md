...
| **WebP VP8L** | ✅ 100% — full lossless; 7/7 BitExact vs `dwebp` | ✅ ~98% — subtract-green + predictor + colour transform + RDO + K=16 meta-Huffman + cache-bits + LZ77 16384-px window/256-deep chains + near-lossless + palette RDO bias + three-pass cost-modelled LZ77 (pass 3 = Viterbi-style optimal DP, ≥256×256) + predictor-tile-bits RDO sweep + EntropyImage tile-bits sweep (3/4/5 = 8/16/32-px); byte-parity vs cwebp on 256×256 synthetic, beats cwebp -46% on 128×128 natural; residual 1.5–4% on natural ≥256×256 photos (Huffman-fit + per-tile transform selection) |
...
- `--no-hwaccel` skips the `videotoolbox` / `audiotoolbox` register
  calls so the pure-Rust codec implementations win at dispatch time.
