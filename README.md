...
| **AAC** | 🚧 AAC-LC + HE-AACv1 SBR + HE-AACv2 PS; lacks LD/ELD, USAC | 🚧 AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS + PNS (-63.9% on noise-rich) + 5.1 + 7.1 multichannel (ffmpeg cross-decode 22-36 dB) + gapless padding tuning (iTunSMPB hook, ffmpeg concat clean); psychoacoustic model basic |
...
| **VP8** | ✅ I+P (6-tap sub-pel + MV decode + ref management) | 🚧 I+P + all 5 intra modes + SPLIT_MV + loop filter + alt-ref/golden-ref planning + Lagrangian RDO + per-MB-context MV-ref probabilities (-13.4%/-22.9% smpte/gray) + segment maps (-14.2% on bit-saving config) + scene-cut adaptation + look-ahead alt-ref temporal-filter synthesis (default 7-frame window, motion-comp + exp(-diff²/σ²) weights, hidden frame at lower q for cleaner reference; +1.23% gain on motion-rich at fixed q, ffmpeg cross-decode clean) |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV + compressed-header probs; lossless-pattern Y 47.70 dB + chroma bit-exact (r22 §6.4.3 fix); sub-8×8 inter mode-info per-4×4 iteration + within-MB §6.5.14 + between-MB §6.5.11 sub-block MV mixing (both §6.5 asymmetry axes closed per spec) | — |
| **AV1** | 🚧 OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound; SVT-AV1 chain 48/48; full 159-pair `inverse_2d_spec` + inter Y `tx_type` CDF reads (+0.82 dB vs libdav1d) + inter chroma `tx_type` derivation + palette intra-within-inter (8.56 dB Y) + full per-MI per-edge loop_filter; lacks intrabc, full primary_ref_frame=4 chain | 🚧 round 1 OBU framing + sequence/frame headers (profile 0 8-bit 4:2:0, ≤64×64); lacks forward range coder + coeff emit |
...
| **WebP VP8L** | ✅ full lossless | ✅ subtract-green + predictor + colour transform + 32-trial RDO sweep over (subtract-green × colour-transform × predictor × cache_bits); 1.17× cwebp on photo-like, 0.98× on palette-like (narrowly beats cwebp) |
| **WebP VP8** | 🚧 via VP8 decoder (VP8 decoder is complete) | 🚧 via VP8 I-frame + ALPH sidecar; animated encode (ANIM + ANMF chunks, 4-frame round-trip + webpinfo + dwebp cross-validation clean) |
