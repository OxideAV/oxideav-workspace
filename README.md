...
| **MPEG-4 Part 2** | 🚧 ~76% — I/P/B texture + GMC global-motion warp end-to-end (§6.2.5 sprite_trajectory + §7.8.4–7.8.6 warp geometry/luma+chroma reconstruction) + quarter-sample + full padding family + interlaced info + B-VOP MV bodies + §7.7.2.1 field-MV reconstruction + §7.6.2.2 quarter-sample field MC + §E.1.4.4 two-way RVLC Tcoef (forward + backward) + §E.1.4.4.2.1 RVLC strategy selection + §7.4.2 SA-DCT modified inverse scan + §A.3.2 inverse SA-DCT + §A.4.2 inverse ∆DC-SA-DCT; lacks mcsel-gated GMC routing into §7.3 recon loop + static sprite + field-MV predictor | 🚧 ~5% — scaffold |
| **Theora** | 🚧 ~85% — intra AND inter frames decode END-TO-END sample-exact from real packets (§6.4 setup-header + §7.9.4 motion-compensated reconstruction incl. half-pixel MV split) + registry `Decoder` trait wired; lacks Ogg carriage | 🚧 ~15% — intra (keyframe) encode→decode self-roundtrip: §7.9.3.3 forward DCT + forward quant + forward DC pred + §7.7 token/EOB writer + frame driver; lacks inter encode + header-packet serialization + Encoder trait |
