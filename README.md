...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing + r183 Decoder-trait MPEG-2 LSF widening + r185 LAME-extension gapless playback; 634 tests | 🚧 ~93% — Phase-2 + cross-channel-MS agreement + r192 `DEFAULT_ATTACK_THRESHOLD = 10.0` empirical default + r194 §D.1 per-band threshold-in-quiet scaffold (`XminThresholds` long/short/mixed + `outer_loop_search_long_per_band` consuming `xmin[sfb]` at every §C.1.5.4.3.5 amplification + §C.1.5.4.3.6 termination + §C.1.5.4.3.4 preemphasis test; uniform shim bit-identical to scalar path; 649 tests; PSNR 65.77-69.94 dB per-band-LTq self-decode); lacks Model 1/2 psy + intensity-stereo + LSF audio-chain |
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element + r177 §4.4.1 GASpecificConfig extensionFlag + Table 1.15 epConfig + r192 §1.6.5 Table 1.15 trailing `syncExtensionType=0x2b7` implicit-SBR/PS/ER-BSAC probe (`AudioSpecificConfig.trailing_sbr_probe`; ext-AOT 5 reads sbrPresentFlag + optional 4-bit ext-sfi w/ 24-bit escape + secondary `0x548` sync gating psPresentFlag; ext-AOT 22 reads sbrPresentFlag + mandatory 4-bit ext-channel-config; `parse_bits_bounded` for LATM/esds carrier-bounded callers) + r194 §4.5.4.1 SWB offset tables (`SWB_OFFSET_LONG_WINDOW[13]` / `SWB_OFFSET_SHORT_WINDOW[13]` from Tables 4.129-4.141; `long_window_offsets`/`short_window_offsets` accessors) + §4.6.13 `apply_pulse_data` reconstruction (first spectrum-layer entry point folding per-pulse ±amp into `x_quant`; 454 tests); decoder body still pending Huffman codebooks 1-11 + channel-element body walker | 🚧 scaffold — Phase-2 writers: section_data + ics_info + pulse_data + tns_data + scale_factor_data + DPCM + r160 raw_data_block + r165 Pce::write + r183 gain_control_data SSR + r187 §4.4.2.7 extension_payload; SBR types pending QMF |
| **CELT** | 🚧 r11 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 prefix + §4.3.2.1 coarse-energy scaffold + §4.3.3 bit-allocation fields + §4.3.4 tf_change/tf_select + r181 §4.3.4.3 spread + r187 §4.3.7.1 post-filter + §4.3.7.2 de-emphasis + r195 §4.3.4.5 Walsh-Hadamard primitives (`walsh_hadamard_inplace` + sequency permutation + `apply_tf_resolution_change` butterfly + Q15 `HADAMARD_LEVEL_SCALE=1/√2`; textbook derivation from `H_2`, `H_{2n}=H_2⊗H_n`; 169 tests); blocked on docs #936 (Laplace) | 🚧 scaffold |
| **Speex** | 🚧 ~25% — Ogg stream-header + NB + WB high-band + §5.5 in-band signalling + r179 `BitWriter` + r187 encoder-side `write` + r191 22 CELP companion-table accessors + r194 NB LSP-VQ → Q10 LSP reconstruction wired through real fixture; 174 tests; lacks §9.1 LSP→LPC + sub-frame interpolation + synthesis + UWB framing | 🚧 scaffold |
...
| **G.728** | 🚧 r195 ~8% — clean-room decoder front-end: Annex A/B/C/D tables + block-50 Levinson-Durbin + blocks 29/31/32 (codebook + gain + 50th-order all-pole synthesis w/ §5.13 saturation) + r195 blocks 30/33 backward synthesis-filter + vector-gain adapters (hybrid_window + gain_adapter + synthesis_adapter, 1374 LOC); lacks postfilter | 🚧 scaffold |
| **G.729** | 🚧 ~6% — clean-room from staged trace #859: r173 numeric tables + r189 7 more tables + r191 ITU serial bitstream parser + conformance-corpus harness + r195 §3.2.4 LSP-quantiser L1 (128×10 Q13) + L2/L3 packed (32×10 Q13) codebooks with bounds-checked lookups + corpus harness validating every `LSP.BIT` frame's L1/L2/L3 indices lie in NC0/NC1; 55 tests; lacks MA predictor fg + §3.2.4 reconstruction pipeline + gain GA/GB + postfilter + Annex B DTX | 🚧 scaffold |
...
| **Musepack** | 🚧 r194 — SV7 §2.5/§2.6 requantiser constants + r194 SV7/SV8 stream-magic recognisers + SV8 packet outer-frame walker (full §3.2 packet-key enum `SH/RG/EI/SO/ST/AP/SE`; `parse_sv8_magic` `MPCK` + `PacketHeader` `[key][varint size][payload]` + `parse_varint` continuation-bit big-endian; both inclusive-and-exclusive varint conventions exposed pending observer trace; 34 tests); lacks per-field internals (#1263 observer trace) + Huffman wiring + CNS generator + 32-band synthesis | 🚧 scaffold |
| **Cook** (RealMedia) | 🚧 r3 — flavor table + cookie parser + 8 DSP parameter tables + r194 open-time `DecodeConfig` wired (cookie ↔ flavor cross-check + `sub_packets_per_call = frame_bytes / sub_packet_size` + `pcm_bytes_per_call` + first/steady-state warm-up budget + 4 typed errors for divide-by-zero / mismatch / non-integer ratios; validator-confirmed bit-exact `2,936,832` total PCM bytes / `20,480` per-call / `8,192` warm-up / `16.649s` decoded duration / `5` sub-packets per call against `FUN_RM_32.rm`; 30 unit + 2 integration tests); lacks MDCT inverse + spectral VLC + per-band dequant | — |
| **WavPack** | 🚧 r15 (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + Golomb (base,add) interval + r172 per-term `decorrelation_sample_count` + r179 `PackedSamples` view + r186 `parse_block` aggregate + r191 `AdaptiveMedians` §3.2 + r194 **first PCM-producing API** `decode_packed_samples_mono` via `decode_sample_stateful` (spec §4.2 full per-sample sequence: zero-run + unary + LIMIT_ONES escape + holding-bit fold + 31-bit interval + §3.2 median adapt BEFORE mantissa + truncated-binary + sign; 200+ bit-exact reconstructed samples + 64 adaptive-drift + 32 packed-mono-loop tests; 239 tests); lacks stereo per-term sample count / prediction loop / float+multichannel / hybrid 0x0B+0x0C / CRC / encoder | 🚧 scaffold |
...
| **FFV1** | 🚧 ~73% — RFC 9043 decoder + demux + decode_frame driver (YCbCr + RGB Y/Cb bit-exact; RGB Cr divergence open) + r179 `coder_type==2` alternative state-transition table wired through decode + encode | 🚧 ~96% — Slice Footer + Slice Header + Golomb-Rice primitives + frame-level Golomb-Rice + YCbCr encoder + r164 range-coded SliceContent encoder + r179 derived-table encode path + r190/r193 §4.7 RGB + RCT encode across all 3 `coder_type` branches + r196 **unified `encode_frame` dispatch helper** (single entry routes YCbCr / RGB / RCT × `coder_type ∈ {0,1,2}` via configurer, eliminating caller branches; closes the prior "lacks unified encode_frame" tail); lacks RGB Cr decode fix |
| **MPEG-1 video** | 🚧 ~42% — sequence/GOP/picture/slice + macroblock walk + intra-DC + §2.4.3.7 dct_coeff walker + §2.4.4 dequantiser + r185 §A 8×8 IDCT + IEEE P1180/D2 conformance + r194 §7.3 `mpeg2_inverse_scan` (Figure 7-3 zigzag + alternate-scan dispatch + scan-flag boundary; 932 LOC + structural tests) | 🚧 scaffold |
...
| **MPEG-4 Part 2** | 🚧 ~62% — I-VOP intra + inter texture + §6.2.5 video_packet_header + §7.8.7.3 GMC + r182 §7.6.2.1 half-sample bilinear + r190 §7.6.2.2 quarter-sample + Table 7-13 chroma MV reduction + r193 §7.6.9.5.2 B-VOP direct-mode MV derivation + r195 §7.6.9.5.3 B-VOP luminance prediction-block (`generate_b_vop_luma_prediction` per-sub-block §7.6.2.1/§7.6.2.2 + §7.6.9.4 averaging `(Pf + Pb + 1) >> 1`; `BVopPredictionMode::{ForwardOnly, BackwardOnly, Bidirectional, Direct}`; 508 lib tests); lacks B-VOP chroma + §7.6.1.6 padding + §6.2.6.2 MV-body parser wiring + encoder | 🚧 scaffold |
| **Theora** | 🚧 ~46% — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3-default tables + §6.4.x quant + DCT-token Huffman + §7.1–§7.5 frame walk + r160 §7.5 motion vectors + r179 §7.7.1 EOB Token decode + r185 §6.4.1 LFLIMS + r191 §7.7.2 Coefficient Token Decode + r195 §7.7.3 DCT Coefficient Decode driver (per-frame `ti` 0..=63 zig-zag walk, Table 7.42 HG lookup, luma/chroma split via NLBS, bit-by-bit `HuffmanTable::read_token`, dispatch to §7.7.1/§7.7.2, closing-paragraph `EOBS=0` + `TIS=64` enforcement; 286 tests); lacks §7.8 DC prediction | 🚧 scaffold |
| **H.263** | 🚧 ~89% (post-2026-05-18 orphan) — §5.1-§5.4 baseline + §6 IDCT/MV/half-pel/INTER + Annex J §J.3 deblock + Annex I AIC + Annex D UMV + Annex F 4-MV + OBMC + §5.1.4 PLUSPTYPE + Annex K §K.2 SS + r187/r192 §I.3 AIC reconstruction pipeline + r196 §I.2/§I.3 **AIC MB-grid driver wiring** (`DecodeOptions::aic` opt-in + parser reads §I.2 INTRA_MODE VLC between MCBPC and CBPY + per-block §I.3 "same video picture segment" availability via GOB-id + `decode_intra_macroblock_aic` end-to-end leaf reconstruction; 369 tests; 1002 LOC); lacks PLUSPTYPE→aic auto-wiring + Annex K driver + PB-frames | 🚧 scaffold |
...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~41% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header primitives chain complete (§6.3.1-11 / 13-15 / 17-18) + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §6.4.3 decode_partition + §6.4.13 read_is_inter + r183 §6.3.17 update_mv_prob + r194 §6.3.18 `setup_compound_reference_mode` (3-arm CompFixedRef/CompVarRef partition per LAST/GOLDEN/ALTREF sign-bias; `RefFrameSignBias` newtype + ref-frame sentinels; 411 tests); lacks §6.3.12 frame_reference_mode + §6.3.16 mv_probs outer driver + §6.4.4 decode_block + §8.4 loop filter | 🚧 scaffold |
| **AV1** | 🚧 ~94% — decoder feature-complete + **standalone `decode_av1` public entry** (1498 tests + integration roundtrips) | 🚧 ~27% encoder — pixel-space YUV→IVF driver + 14-mode intra picker + full §7.13.3 forward 2D dispatcher + WHT lossless + forward quantize + §5.11.x partition/transform_tree/coefficient + r194 §7.11.5.3 UV_CFL_PRED on dyn-extent driver + r196 **`base_q_idx > 0` (lossy quant) on dyn driver** (`encode_intra_frame_yuv_dyn_with_q` routes leaf through §7.13.3 forward DCT_DCT + §7.12.3 forward quantize; FH carries `TxModeLargest` per §5.9.21; decoder reads `base_q_idx` from FH and threads `lossless = (qp.base_q_idx == 0)` through every `inverse_transform_2d`; encode→decode→pixel byte-for-byte at q ∈ {1,16,64,255} on 32×32/64×64; 1498+35 tests). Lacks rectangular TX sizes + §5.11.18 inter mode_info + RD picker + multi-SB tiling + per-block delta_q |
| **Dirac / VC-2** | ✅ ~95% — VC-2 LD+HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit + bit-exact intra fixtures + r165 fuzz oracle + r190 Criterion bench harness + r195 `vh_synth`/`vh_analysis` row-major slice driving (decode q=32 −1.15% stat-sig; encode/roundtrip within noise; DD9/7 bench row added; harness now 12 timed scenarios across LeGall+DD9/7 wavelets; 338 tests) | 🚧 ~95% — HQ+LD intra + Dirac core-syntax + adaptive sub-pel + 2-ref bipred + post-OBMC refinement + picture/sequence rate-control + r179 intra-encoder fuzz oracle + r193 inter-encoder fuzz oracle (3-oracle coverage complete) |
...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz + r185 `idct8x8_dc_only` fast path + r195 §5.1+§7.5.3 SHA-256 lockstep pin on two 1920×1080 interlaced apcn 10-bit fixtures (frame-0 byte-level pin against `expected.yuv.sha256`; float-IDCT path vs fixed-point ~1-LSB drift logged not hidden; 251 tests) | ✅ ~97% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + r193 ffmpeg cross-decode acceptance §5.3 MBs/slice knob (58.85-63.93 dB luma PSNR) |
| **EVC** (MPEG-5) | 🚧 ~89% — NAL + SPS/PPS/APS + §9.3 CABAC + §8 intra + DCT-II + P/B inter + RPL + HMVP + DPB + ALF + DRA + IBC §8.6 + r187 §8.9.7 `DraChromaDerived` + r193 §8.9.8 `DraJoinedScaleFlag=1` joined-chroma-scale + r195 §7.4.3.1 SPS-signalled `ChromaQpTable` (eq. 74 pivot-point derivation w/ anchor at qpInVal[0] + down-fill + per-segment linear interpolation + up-fill; `SignalledChromaQpTableParams` + `build_signalled_chroma_qp_table` + `Sps::chroma_qp_table`; tightened `num_points_in_qp_table_minus1[i]` bound to `57 + QpBdOffsetC − (global_offset_flag ? 16 : 0)`; 417 tests); lacks Main-profile toolset (BTT/ADMVP/EIPD/ATS/AMVR/affine) | — |
| **HuffYUV** / FFVHuff | ✅ ~97% — HFYU + FFVH FourCCs + 6 predictors + 8-bit only + interlaced field-stride=2 + fast-LUT decoder + SWAR 8-byte gradient post-pass + r181 YUY2 LEFT macropixel-step branch-free decoder + r196 cargo-fuzz **encode_roundtrip** target + narrow-YUY2 Median forward-encode width fix (per-pixel-pair stride correctness across `width % 4 ≠ 0`) | ✅ ~96% — full encoder symmetry × YUY2/RGB24/RGB32 + v1.x + v2.x ClassicV2/CustomV2 + walking-stride interlaced + predictor auto-selection + r95 SWAR forward-gradient + r100/r103/r115 fused residual + r181 YUY2 LEFT forward branch-free + r186 `forward_rgb_left_subtract_linear` single-stride RGB24/RGB32 LEFT-residual walk (~20× M1; LLVM autovectorises to NEON `vsubq_u8` / SSE2 `psubb`) |
...
| **Cinepak** (CVID) | ✅ ~98% — frame header + multi-strip + V1/V4 codebooks + intra/inter + grayscale + Sega FILM demuxer + Saturn/3DO deviant + r181 codebook_chunk_apply + r192 `decode_vector_chunk` cargo-fuzz target + criterion benches + r196 `decode_multi_frame` cargo-fuzz target (cross-frame inter-strip state walk; 0 crashes) | ✅ ~98% — stateful encoder + rolling codebooks + RDO + LBG + 3-axis grid picker + bitrate-target rate-control + keyframe-interval (34.18 dB PSNR; decode 4.4 GiB/s, stateful GOP 13.5 ms/frame) |
| **SVQ1/SVQ3** (Sorenson) | 🚧 r9 (orphan rebuild) — SVQ1 frame-header + framework registry + SVQ3 SEQH + slice + MB-type tree + residual walker + r179 SVQ3 inter-MB MV-precision selector + r191 SVQ1 block-tree subdivision walker + r194 SVQ1 L=0..L=3 codebook payload (`svq1_codebook` + `build.rs` parses `tables/codebook-l0l3.csv` → `[i8; 23004]` + 36-B descriptor + 16-entry block-shape LUT bit-exact from `quicktimethirdparty.qtx` `0x5d200..0x62c00`; `Svq1Level::codebook_bytes_per_half` 768/1536/3072/6144 for L0..L3, `None` for L4/L5); 192 tests; lacks intra-vs-inter ordering + stage interleave (sibling docs task) + SVQ3 MV-VLC + #1256 svq3.c attribution scrub | — |
| **Indeo 3** (IV31/IV32) | 🚧 r14 — clean-room codec-frame header + bitstream header + spec/02 picture-layer + spec/03 macroblock-layer + spec/04 VQ codebook + spec/06 byte-level entropy + spec/07 output-reconstruction + four cell-shape kernels + spec/02 strip-context array + spec/03 per-cell sub-array wiring + r181 spec/05 §1 mc_table + r186 spec/05 §2.2/§2.3/§3.3/§3.4 packed-MV bit-layout + r196 spec/05 §5.4 / spec/07 §7.2 **cell-position decoding** (`CellPosition::from_packed` decodes per-strip cell anchor + cell-shape kernel routing through `cell_position_resolve` to coordinate-grid offsets; 832 LOC); lacks cell-stack pre-population (spec/03 §6 Q4) + pixel-buffer edge fix-up + MC inner loop (spec/05 §5.x outer) | — |
...
| **JPEG 2000** | 🚧 r15 (post-2026-05-20 orphan) — T.800 main-header + SOT/SOD + typed COC/QCC/POC/RGN/PLT/PPT + JP2 box + §B.10 tier-2 + §B.5 ResolutionLevel + §B.6 precinct + §B.7 code-block partition + Annex C §C.3 tier-1 MQ + Annex D 19 contexts + §B.12.1 5 packet-progression iterators + §B.12.2 POC + r181 Annex F.3 inverse DWT + r187 4 cargo-fuzz targets + r192 Annex E code-block→sub-band reassembly + r195 **Annex G MCT** (`inverse_rct` §G.2.2 eqs G-6/G-7/G-8 i32 `⌊·/4⌋` floor-toward-minus-inf; `inverse_ict` §G.3.2 eqs G-12/G-13/G-14 f32 with `1.402` / `0.34413` / `0.71414` / `1.772`; `inverse_dc_level_shift_unsigned` §G.1.2 `+2^(Ssiz−1)` clamped 1..=31; `forward_rct` + `forward_ict` symmetric encoder primitives; 615-LOC `mct.rs`; 364 tests); lacks per-resolution cascade + HTJ2K Part-15 | 🚧 scaffold |
| **JPEG XL** | 🚧 ~92% — ISO/IEC 18181-1:2024 lossless Modular path + 7 fixtures pixel-correct + VarDCT scaffold + Gaborish/EPF/AFV pure-math complete + §C.8.3 per-block HF coefficient loop + r190 `PerPassNonZerosGrids` per-pass container + r191 WP trace oracle isolating #799 divergence + r195 WP state-evolution backward bisect narrowing the +21/-21 symmetric `true_err` delta to sample 129 (te_n@193 == te_nw@194 == 716 vs spec 737; wp_pred8@193 = 917 → true_err@193 = 317; +4 serial-tagged integration tests) + Hat-2 scrub of 7 pre-existing decorative `libjxl` attribution lines (paraphrased to "reference-decoder behaviour observed via cjxl/djxl"); 641 tests; lacks upstream WP state-evolution fix at sample 129 + §C.7.2 histograms + per-frame Gaborish/EPF/CfL wiring | — retired |
| **JPEG XS** | 🚧 ~82% — ISO/IEC 21122 Part-1 + 5/3 DWT + Annex C/D/F/G + multi-component + CAP-bit + high bit depth + r190 4:2:0 chroma at NL,y≥3 | 🚧 ~88% — Nc 1/3/4 + Sd>0 + RCT + Star-Tetrix + NL up to 8 + odd dims + vertical prediction + per-band Q + NLT + r193 Annex G.5 NLT extended at bd∈9..=16 + r195 high-bit-depth Star-Tetrix (Cpih=3 + `B[i]∈9..=16` extended via existing encoder spine; closes the prior "lacks Star-Tetrix high-bit-depth" tail) |
| **AVIF** | 🚧 ~88% — HEIF→AV1 + grid + imir/clap/colr/pixi/pasp + HDR metadata + AV1 wrap + DoS caps + HEIF item-properties + auxC URN + rloc/lsel/iovl/grpl + `mif1` + r130 tmap §4.2.2 + r188 ISO 21496-1 Annex C.2 `GainMapMetadata` + r193 §5.2.5.3+§5.2.7 value-comparison shalls + r195 §8.2/§8.3 AVIF profile-compliance audit (`MA1B` Main+L5.1 / `MA1A` High+L6.0 per-item `av1C[1]` decode + `AvifInfo::avif_profile_compliance` + `avif_profile_strict_compliant()`) + libavif/avifenc/libheif decorative-attribution scrub (42→0 citations; one rename `is_libavif_srgb_default` → `is_sdr_srgb_bt601_default`); 252 + 57 tests; closes #1228 | — |
...
| **OpenEXR** | 🚧 ~87% — magic + 8 required attrs + HALF/FLOAT/UINT + NO_COMPRESSION/ZIP/ZIPS/RLE + tiled ONE_LEVEL + sub-sampled chroma + single-part deep scanline + multi-part deep scanline + r130 single-part deep tiled + r181 multi-part deep TILED + r192 multi-part flat TILED ONE_LEVEL read + r196 multi-part flat **MIPMAP_LEVELS** read (per-level tile-grid count + offset-table walk + edge-tile clamp across levels; 1386 LOC); PIZ blocked on docs trace | ✅ ~94% — RGBA scanline + ZIP/ZIPS/RLE + tiled-output ONE_LEVEL/MIPMAP/RIPMAP + multi-part scanline + deep scanline + r130 single-part deep tiled + r181 multi-part deep TILED + r192 multi-part flat TILED ONE_LEVEL write + r196 multi-part flat **MIPMAP_LEVELS** write (`MultipartTiledMipmapPart` per-level tile-emit + per-part `tiles[tiledesc]` MIPMAP_LEVELS round-multiplier 4 stride; validated via exrheader + exrmultipart -separate pixel-exact across all levels) |
...
| **HDR** (Radiance RGBE) | ✅ ~99% — new-RLE + old-RLE + 8 axis-flag combos + shared-exponent + multi-record EXPOSURE/COLORCORR + typed COLORCORR/PRIMARIES/VIEW + apply_exposure/apply_colorcorr + r189 luminance_lm_per_sr_per_m2 + r192 committed-fixture regression anchors + r196 **uncompressed scanline R+W** (4-byte-RLE absent path: per-pixel `[R,G,B,E]` straight-through reader + symmetric encoder switch; cross-decoded vs r192 fixtures + new uncompressed-fixture pin; closes #1056 provenance audit clean) | ✅ ~98% — new-RLE + old-RLE + auto-RLE + XYZE↔RGB + 8 tonemap ops + CRLF + r179 zero-copy `reorient_for_axis_flags` (~6% encode throughput at 1024²) |
...
| **FBX** | 🚧 ~74% — binary container (32/64-bit) + object-graph + mesh + animation + deformers + Material/Texture/Video surfacing + bind pose + r178 LayerElementMaterial + r184 LayerElementColor + r191 Properties70 P-record grammar + PBR mapper + r194 multi-UV-set surfacing on `Primitive::uvs` (every `LayerElementUV` record populates a separate `[f32; 2]` per-corner buffer in document order; cubes-ASCII fixture byte-value round-trip; 71 unit + 90 integration tests). Lacks: ASCII FBX parser (#785), Light/Camera NodeAttribute, multi-LayerElementNormal | ✅ ~58% — symmetric binary writer + opt-in zlib deflate |
