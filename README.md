...
| **Opus** | 🚧 ~80% — full TOC + RFC 7845 pre-skip + output_gain + §4.3.7.2 CELT de-emphasis + §4.3.5 CELT auto-scale (libopus interop 10-26 dB on 5 fixtures); lacks SILK §4.2.7.4 gain_index_to_q16 / §4.2.7.8 Q23 shell-pulse / §4.2.7.8.6 LCG dither | ✅ ~85% — CELT full-band + SILK NB/MB/WB + Hybrid mono/stereo at 10/20 ms; ffmpeg + libopus cross-decode clean |
...
| **AAC** | 🚧 ~80% — AAC-LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE; lacks LD/ELD, USAC | 🚧 ~75% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + gapless + AscBuilder + Bark-band PE/SMR psy default-on (mono + v2; pns-noise gate); HeAacStereo psy off pending M/S CPE IMDCT side-lobe fix |
...
| **8SVX** | ✅ 100% | ✅ 100% |
| **iLBC** (RFC 3951) | ✅ 100% — NB 20/30 ms + RFC §4.6 enhancer + §4.7 synth shift | ✅ ~98% — 20/30 ms LPC + LSF split-VQ + §3.6 residual CB + opt-in §3.1 HP biquad + §3.5.1 position-bit selection (energy-ratio gate); lacks §3.5.1 block_class variable start_idx |
...
| **AC-4** (Dolby) | 🚧 ~60% — A-SPX + DRC + DE walker + 60 ETSI Huffman codebooks + ASPX_ACPL_1/2/3 + 5_X/7_X channel walkers + mono/stereo/joint short-frame sf_data(ASF) + SSF bitstream walker + SSF PCM synthesis (Pseudocodes 4a-d/4e/26-39, C-matrix subband predictor) | — |
...
| **H.264** | 🚧 ~75% — I/P/B + 4:2:0/4:2:2 + CAVLC + CABAC + DPB + B-pyramid POC + 8 SEI payload types + Level 1b sizing; lacks 4:4:4, MBAFF, SVC/3D/MVC | 🚧 ~62% — I + P (1MV/4MV, ¼-pel) + B-slice + weighted pred + 4:2:2/4:4:4 IDR + CABAC I/P/B incl. real ME (mvp §6.4.11.7) + B_Skip/B_Direct_16x16 (§8.4.1.2.2 spatial-direct); 99 dB Y on translation; lacks B 16x8/8x16/B_8x8 CABAC, P/B at 4:4:4 |
...
| **JPEG XS** | 🚧 ~70% — ISO/IEC 21122 Part-1 codestream + inverse 5/3 DWT + Annex C/D/F/G entropy + quant + colour transforms + multi-component (4:2:2/4:2:0) + multi-level DWT cascade + CAP-bit decoder | 🚧 ~30% — multi-component (Nc 1/3 + RCT cpih ∈ {0,1}) + multi-decomp NL ∈ {1,2} + odd dimensions; lossless self-roundtrip ∞ dB; Dr=1 raw mode only — VLC bitplane modes + Fq=8 lossy + 4:2:2/4:2:0 pending |
