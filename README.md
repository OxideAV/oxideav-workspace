...
| **MP3** | ✅ ~100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~96% — Layer III + Annex D psychoacoustic Phase-2 bit-allocation loop (MNR-select → quantize-promote → recompute-MNR); lacks Steps 1-3 FFT/SPL/tonality classifier + Model 2 + intensity-stereo |
...
| **AV1** | 🚧 ~35% — intra decode: standalone `decode_av1` + 4:2:0/monochrome multi-superblock to 128×128 + full base_q_idx; lacks inter prediction, loop filter, CDEF, loop restoration, film grain, superres, scalability | 🚧 ~30% — intra encode: `encode_av1` 4:2:0 YUV→IVF + 14-mode picker + all §5.11 TX_SIZES_ALL shapes + lossy/lossless + §5.11.18-32 inter mode-info writers (ref_frames/mv/drl/assign_mv); lacks RD picker + inter reconstruction/encode chain |
