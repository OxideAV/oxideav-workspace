...
| **Vorbis** | ✅ all residue types (matches lewton/ffmpeg) | 🚧 floor1 step1/step2 envelope (binary-search dB quantise + smearing) + point-stereo coupling above crossover + ATH floor; lacks per-band point-stereo thresholds, Annex B reference codebooks, floor0 LSP, lookup-codebook optimisation |
| **Opus** | ✅ CELT mono+stereo + SILK NB/MB/WB mono+stereo all frame sizes + Hybrid | 🚧 CELT full-band + SILK NB/MB/WB + Hybrid mono 20 ms (SWB config 13 + FB config 15); SILK MB/WB stereo only at 20 ms (lacks 10/40/60 ms); no Hybrid stereo, no 10 ms Hybrid (LM=2 CELT path) |
| **MP1** | ✅ all modes | 🚧 CBR only (no VBR) |
| **MP2** | ✅ all modes | 🚧 CBR mono+stereo only (no joint-stereo, no VBR) |
| **MP3** | ✅ MPEG-1 Layer III M/S | 🚧 CBR + VBR mono+stereo (per-frame bitrate slot via greedy-quantiser psy model); lacks Bark-partition spreading psy-1, short/start/stop block selection, joint-stereo, intensity-stereo encode, LSF MPEG-2/2.5 |
| **AAC** | 🚧 AAC-LC + HE-AACv1 SBR + HE-AACv2 PS; lacks LD/ELD, USAC, 4.0/5.1/7.1 channel layouts on PCE | 🚧 AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS; LC core decodes byte-tight via ffmpeg at 24 kHz/1 kHz; HE-AAC saturates ffmpeg via SBR FIL extension (ffmpeg logs "No quantized data read for sbr_dequant" → ffmpeg rejects our SBR data structurally before envelope arithmetic; r22 ruled out SBR header fields, r23 ruled out LC core MDCT scale, r24 ruled out env_sf via OXIDEAV_AAC_SBR_ENV_FORCE_ZERO probe; r25 leads: bs_amp_res header-vs-grid mismatch, bs_df_env/bs_df_noise time-vs-freq direction, bs_extended_data framing); psychoacoustic model basic; lacks PNS, gapless padding tuning, multichannel encode |
| **CELT** | ✅ full §4.3 pipeline | 🚧 mono + stereo intra-only long-block + short-block on transient detection (peak/median ratio); lacks per-band tf_res TF decisions, libopus-bit-exact IMDCT, encoder-side comb pitch pre-filter |
...
| **AC-3** (Dolby Digital) | ✅ Full decode + downmix (90+ dB vs ffmpeg) | 🚧 acmod 1/2/3/6/7 + LFE + rematrix + short-block + transient + coupling + DBA; ffmpeg cross-decodes 5.1 cleanly (per-channel 14-45 dB PSNR); lacks E-AC-3 extension, multichannel coupling (>2 fbw), per-block bit-pool tuning, balanced per-channel snroffst |
| **AC-4** (Dolby) | 🚧 A-SPX + DRC + DE walker + ETSI Huffman tables (60 codebooks bit-validated) + ASPX_ACPL_1/2/3 transform synth (Pseudocodes 117/118/119) + multichannel `sf_data(ASF)` Huffman walk (long-frame + grouped/multi-window-group) + ASPX_ACPL_3 inner body walker (`stereo_data` + `aspx_data_2ch` + `acpl_data_2ch`); pending ASPX_ACPL_1/2 inner body walker, 7_X channel-element walker, mono/stereo short-frame `sf_data` walk | — |
...
| **H.263** | 🚧 I+P + half-pel + Annex J (deblock) + Annex D (UMV) + Annex F (4MV/OBMC) + Annex E (SAC) + Annex N (RPS) + Annex G (PB-frames decode); lacks Annex I (advanced INTRA), K-W, Annex M (Improved PB) | 🚧 I+P + diamond ME + Annex F/J/D/N + Annex G PB-frames (encoder + decoder symmetric, ffmpeg 8.1 accepts streams); lacks B-block residual emission (CBPB), MVDB selection, intra-in-P MVD, Annex M, PB+Annex F, plus I, K, L, P, Q, R, S, T, U, V |
...
| **H.264** | 🚧 ~45k LOC spec-driven rewrite; I/P/B + DPB + MMCO/RPLM + CAVLC + CABAC + 4:2:0/4:2:2 + B-pyramid POC ordering; lacks 4:4:4, MBAFF, Annex F (SVC), G (3D), H (MVC) | 🚧 Baseline-ish: I + P (1MV/4MV, ¼-pel) + B-slice 16x16/16x8/8x16 + B_Skip/B_Direct_16x16/8x8 (spatial OR temporal direct, configurable per-slice via `direct_temporal_mv_pred`); ffmpeg interop bit-equivalent (max diff 0) at 50-52 dB on B-frames; lacks CABAC encode, intra fallback in P/B, weighted-pred, 4:2:2/4:4:4 emit, B_8x8 mixed sub_mb_type, multi-ref temporal direct, VUI/SEI tuning |
| **H.265 (HEVC)** | 🚧 I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2; SAO + deblock + scaling lists; HEIF/HEIC opt-in; pending 4:4:4 12-bit, broader B-slice merge/AMVP audit | 🚧 Baseline CAVLC I+P + Main CABAC P-slice + B-slice (TrailR mini-GOP=2, one past + one future ref, default-weighted bipred) + per-CU pick across B_Skip / Merge / Explicit AMVP by SAD (encoder + decoder agree on 5-entry merge candidate list byte-for-byte); ffmpeg cross-decodes I-P-B-P-B at 33-45 dB, static skip-path B-slices collapse to 24/26 bytes; lacks AMP / Nx2N / 2NxN partitions, mvd_l1_zero_flag, mini-GOP > 2 / B-pyramid, 10/12-bit, RDO refinements |
| **H.266 (VVC)** | 🚧 4:2:0 IDR intra (BDPCM, MIP, CCLM, ISP) + ALF/SAO/CC-ALF + P-slice merge+skip + B-slice merge+skip with default-weighted bipred MC (`(L0+L1+1)>>1` byte-exact across luma+Cb+Cr) + §8.5.6.3 fractional-pel MC (8-tap luma Table 27 + 4-tap chroma Table 33 at 1/32-pel); lacks HMVP, temporal merge, pairwise-average, MMVD/CIIP/GPM, AMVR/BCW, BDOF/DMVR/PROF, dual-tree, full inter residual decode, affine/scaled-ref filter tables, explicit weighted prediction | 🚧 forward CABAC + DCT-II + flat quant only; residual emit + pixel output pending |
| **VP6** | ✅ Full FLV playback (845/845 sample frames) | 🚧 keyframe + skip + inter (integer-pel ME); **ffmpeg interop unblocked in r23** (Vp3VersionNo=0 was spec-forbidden — must be 6/7/8 per §9 Table 2; encoder default flipped to sub_version=6, ffmpeg now decodes both keyframe + inter packets); lacks real residual-coefficient encoding for inter (currently zero-block shortcut only), sub-pel ME, golden-frame refresh, INTER_FOURMV, Huffman coefficient path |
| **VP8** | ✅ I+P (6-tap sub-pel + MV decode + ref management) | 🚧 I+P + all 5 intra modes + SPLIT_MV + loop filter + alt-ref / golden-ref planning + Lagrangian RDO (30% smaller + 14 dB Y-PSNR vs SAD-only on cyclic content); lacks scene-cut adaptation, true look-ahead alt-ref synthesis, real per-MB-context MV-ref probs, segment maps |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV + compressed-header probs; r21 made c64-constant lossless bit-exact (∞ dB); r22 fixed §6.4.3 `decode_partition` HORZ/VERT double-call bug + spec-literal `+idx`/`+idy` sub-mode anchor (lossless-pattern Y: **10.41 → 47.70 dB**, both pattern chroma planes bit-exact); pending identical bug in `inter.rs::decode_partition`, residual 2% luma byte diffs on lossless-pattern | — |
| **AV1** | 🚧 OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound; r21 SVT-AV1 chain 48/48 (full pass); r22 `inverse_2d_spec` covers all 159 spec-allowed (TX_TYPE × TX_SIZE) pairs; **r23 wired into all 4 superblock call sites** (intra Y vs libdav1d 8.85 → 9.49 dB, +0.64); pending inter-path migration audit, palette finalization, intrabc, full loop-filter, full primary_ref_frame=4 chain, edge-tile select_square_tx (bbb_alpha/kimono_rotate90) | — |
...
| **JPEG XL** | 🚧 Signature + SizeHeader + partial ImageMetadata + ISOBMFF `jxlp` container parsing; Modular (MA-tree) and VarDCT pixel decode pipelines **blocked on ISO/IEC 18181-1** (paid spec, see `crates/oxideav-jpegxl/SPEC_BLOCKED.md`) | — |
