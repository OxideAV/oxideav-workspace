...
| **G.728** | 🚧 ~91% — LD-CELP decode: Annex A-D + Levinson + postfilter chain + ICOUNT=3 update stagger + Annex I §I.4.2 frame-erasure LPC softening + §I.4.5 gain-growth limit | 🚧 ~87% — analysis-by-synthesis loop complete, bit-exact lockstep with decoder incl. ICOUNT stagger; lacks Annex G fixed-point |
...
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + ETSI codebooks + ACPL cfg0..3 + SAP + IMS bitstream walker; lacks object/A-JOC | 🚧 ~80% — IMS v0/v2 mono → 7.1, all eleven ACPL coupling layers real (α/β/γ incl. β₃) + TIME-direction A-SPX envelope DPCM + 2-channel ASPX multi-envelope builder; lacks 7_X Table-202 back-pair Lb/Rb |
