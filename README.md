...
| Matroska  | ✅ | ✅ | ✅ | MKV/MKA/MKS; Cues seek + SeekHead/Chapters/Attachments + lacing + CRC-32 + typed RFC 9559 element surface (Tags, Colour/HDR mastering, Projection, BlockAdditions read+write, TrackOperation read+write, …) |
...
| MOV (QuickTime) | ✅ | — | ✅ | QTFF + ISO BMFF meta + HEIF/HEIC item properties + fragmented-MP4 seek + edit-list mapping + `cmov` compressed-movie decompression + §8.14 sub-track groups + §8.7.8/§8.7.9 saiz/saio + §8.6.1.3 ctts composition-offset muxer write; ffprobe-accepted |
...
| **Opus** | 🚧 ~42% — RFC 6716 range decoder + full SILK pipeline + CELT side through coarse energy (per-LM inter α,β), allocation search, PVQ codebook + spreading + framing incl. Appendix-B self-delimiting + §4.3.7 inverse-MDCT overlap window + §4.3.4.1 pulse-cost cache (unblocks PVQ reallocation) + §4.3.6 band denormalisation | 🚧 ~5% — scaffold |
...
| **Musepack** | 🚧 ~63% — SV8 packet walker + all 21 Huffman tables + §3.4 classifier-driven band dispatcher (decode_sv8_band routes CNS/empty/escape arms) incl. large-coefficient escape + SV7 §2.5 band-decode ladder wired end-to-end (decode_sv7_band); lacks sparse case-1 (docs ask filed) + SV7 header field map + 32-band synthesis | 🚧 ~5% — scaffold |
...
| **WMA** | 🚧 ~13% — patent-disclosed primitives (analysis/synthesis windows + codebook grid + quantization-band layout + §4 energy-derived quantization matrix + §5 open-loop stereo channel-coding decision + §8 decoder-tail stereo time-domain reconstruction + §6 entropy front-half assembled (run-level → coefficients spectral→dequant chain); lacks Huffman codeword tables + exponent partition + sign layout (docs-gapped) | — |
| **WavPack** | 🚧 ~94% (post-2026-05-18 orphan) — v4 block/metadata/entropy parse + full §4.2 entropy ladder + multi-block PCM composer + inverse entropy encoder (bit-exact round-trip) + typed-refuses joint-stereo/cross-decorrelation (flag bits 4/5); lacks decorrelation prediction loop + hybrid consumer (docs gaps) + float + multichannel | 🚧 ~10% — scaffold |
...
| **DTS** (Core) | 🚧 ~57% — frame header + 14↔16-bit pack/unpack + side-information subframe walker + Annex C reconstruction primitives + Annex D codebooks (incl. §D.6 block-code books + §C.2.1 table-look-up decoder) + §D.8 FIR tables + fused 32-band synthesis QMF driver (§C.2.5 QMFInterpolation) + §D.2 step-size tables + §5.5 inverse-quant scale composition + §D.5.7 13-level audio book; lacks audio-array decode | — |
...
| **H.265 (HEVC)** | 🚧 ~70% — parameter sets + §9.3 CABAC engine with COMPLETE §9.3.2.2 context-init (Tables 9-5..9-42) + full slice header + residual_coding() driver + §D.2 SEI parse (mastering-display + content-light + recovery-point + decoded-picture-hash) + §8.6.2/§8.6.3/§8.6.4 scaling + inverse transform + §7.3.8.9 mvd_coding + §7.3.8.6 merge_flag binarization + §8.4.2/§8.4.3 intra pred-mode derivation + §8.4.4.2 intra sample prediction (substitution/filtering/planar/DC/angular) + §6.4/§6.5 z-scan/tile-scan neighbour availability; lacks inter sample prediction + reconstruction loop | 🚧 ~5% — scaffold |
| **H.266 (VVC)** | 🚧 ~75% — 4:2:0 IDR intra + full inter toolset (ALF/SAO/HMVP/MMVD/CIIP/BCW/BDOF/GPM/DMVR/affine/PROF/SbTMVP) + typed RBSP/parameter-set surface + Annex-D SEI prefix indication (201) + LMCS arrays + sample-domain luma map/chroma residual scale (§8.7.5.2/§8.8.2) + §8.7.4.6 inverse adaptive colour transform + §8.7.2 scaling-and-transformation orchestrator (codedCIdx + joint Cb-Cr residual eqs. 1130-1132) + §8.5.6.6.3 explicit weighted sample prediction (uni/bi-pred + §7.4.7 chroma weight/offset) | 🚧 ~93% — forward CABAC + DCT-II + MTT RDO + P/B + sub-pel MC + weighted bi-pred + affine/AMVR/BCW dispatchers |
| **VP6** | 🚧 ~60% — BoolCoder + DC/AC coefficient decode + MV decode/reconstruction + custom scan + per-block reconstruction + §2/§13/§17 block-to-plane raster frame assembly + §9 output-scaling typed surface + §10 macroblock coding-mode traversal + §9 BoolCoder frame-header tail; lacks IDCT | 🚧 ~5% — scaffold |
| **VP8** | ✅ 100% | ✅ 100% |
