...
| **CELT** | 🚧 r11 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 prefix + §4.3.2.1 coarse-energy scaffold + §4.3.3 bit-allocation fields + §4.3.4 tf_change/tf_select + r181 §4.3.4.3 spread + r187 §4.3.7.1 post-filter + §4.3.7.2 de-emphasis + r194 §4.3.4.5 Walsh-Hadamard primitives (`walsh_hadamard_inplace` + `walsh_hadamard_sequency_inplace` + `apply_tf_resolution_change` butterfly + Q15 `HADAMARD_LEVEL_SCALE=1/√2`; textbook derivation from `H_2=[[1,1],[1,-1]]`, `H_{2n}=H_2⊗H_n`; 169 tests); blocked on docs #936 (Laplace) | 🚧 scaffold |
| **Speex** | 🚧 ~25% — Ogg stream-header + NB + WB high-band + §5.5 in-band signalling + r179 `BitWriter` + r187 encoder-side `write` + r191 22 CELP companion-table accessors + r194 NB LSP-VQ → Q10 LSP reconstruction wired through real fixture (`NarrowbandFrameBody::reconstructed_lsp_q10(submode)` combines 5 staged stages with per-stage scaling `1/256`/`1/512`/`1/1024`; ≥90% of fixture audio packets yield non-zero LSPs; 174 tests); lacks §9.1 LSP→LPC + sub-frame interpolation + synthesis + UWB framing | 🚧 scaffold |
