...
| AVI       | ✅ | ✅ | ✅ | AVI 1.0 + OpenDML 2.0 demux/mux complete; AVIX/dmlh/vprp + 2-field interlaced + VBR audio + LIST INFO + typed PaletteChange/TextChunk/AvihFlags/Idx1Flags + opt-in idx1↔ix## synth + ODML keyframe seek + WAVEFORMATEXTENSIBLE + `strn`/`strd`/padding + CBR-audio validator + `dmlh.dwTotalFrames` cross-check + IDIT/ISMP/rcFrame/wLanguage round-trip + per-stream `strh.dwInitialFrames` + r157 file-global `avih.dwInitialFrames` parse+emit |
...
| **FLAC** | ✅ 100% — bit-exact vs RFC 9639 + CUESHEET → Chapter API | ✅ 100% — bit-exact roundtrip + LPC order/window/precision search + closed-form Rice estimate + flamegraphs + §8.6 PADDING writer + composable block-header serialiser + r158 `FlacEncoderOptions::padding_bytes` opt-in PADDING reservation |
...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~91% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + auto MS/LR + auto SM + outer-loop on Long/Short/Mixed/Start/End + auto-block-type mixed promotion + r162 §2.4.3.4.9 indep-stereo widening of force-short/force-mixed/auto toggles (586 tests); lacks Annex D psy (#1048) + intensity-stereo + MS-stereo cross-channel agreement + LSF audio-chain |
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element; decoder body still pending | 🚧 scaffold — Phase-2 writers: section_data + ics_info + pulse_data + tns_data + scale_factor_data + DPCM accumulator pair + r160 §4.4.2.1 raw_data_block frame assembler (SCE/CPE/CCE/LFE headers + FIL/DSE escape + END byte-align; PCE deferred; 294 tests) |
...
| **Speex** | 🚧 r5 — Ogg stream-header + NB frame (Table 9.1 budgets + CELP body bit-reader) + WB high-band (r160 Table 10.1 budgets + body bit-reader, modes 0..=4) + §5.5 in-band signalling bodies (89 tests); lacks LSP-VQ + pitch/innovation codebooks (#969) + LSP→LPC + synthesis + UWB framing | 🚧 scaffold |
...
| **DTS** (Core) | 🚧 ~25% — frame-sync header (104/120-bit window) + 14↔16-bit pack/unpack + four-encoding parse/encode round-trip + r159 iter_frames_resync error-tolerant multi-frame walker (163 tests); lacks §5.3.1 alloc/scfac tables (#928) + §5.4 polyphase filterbank (#1055) + 14-bit container-byte advance (#1084) | — |
...
| **FFV1** | 🚧 ~72% — RFC 9043 decoder + demux + decode_frame driver (YCbCr + RGB Y/Cb bit-exact; RGB Cr divergence open) | 🚧 ~25% — Slice Footer + Slice Header + Golomb-Rice primitives + r152 §4.8 run-mode + scalar encode_line + r159 frame-level Golomb-Rice + YCbCr encoder (round-trips 2×2 grid + 1×3 stack + 8/10-bit; 307 tests); lacks Decoder registration (#904) + RGB Cr fix + range-coded SliceContent encoder + RGB encode path |
...
| **MPEG-2 video** | 🚧 ~42% — §6.2.x sequence/GOP/picture/slice + macroblock_type + cbp + macroblock_modes + motion_vectors + §7.6.3.x PMV + §7.6.4 forming-predictions pel reader + r159 §7.6.7 combine-predictions + §7.6.8 add-and-saturate (490 tests); lacks residual VLCs / IDCT (#1110) | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 ~48% — I-VOP intra decode through §7.4.5 IDCT + Figure 7-5 predictor gathering + §6.2.5 video_packet_header + §7.8.7.3 S(GMC)-VOP averaged-vector substitution + r157 §6.2.2 VisualObject() verid/priority + video_signal_type()/colour_description (Tables 6-7/8/9/10); lacks Figure 7-34 MV-predictor (#1125) + inter+B-VOP + encoder | 🚧 scaffold |
| **Theora** | 🚧 ~33% — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3-default tables + §6.4.2/3/4 quant + DCT-token Huffman + §7.1–§7.5 frame walk (frame-header + long-/short-run + coded-block flags + macro-block modes + r160 §7.5 motion vectors: Table 7.23 Huffman + MVMODE=1 fixed + LAST1/LAST2 + INTER_MV_FOUR PF=0/2/4 chroma averaging, 207 tests); §6.4.1 LFLIMS body blocked (#944) | 🚧 scaffold |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 41 SEI types (incl. r158 typed ATSC1 A/53 §6.2.3 closed-caption envelope sub-parser) + fuzz-hardened; lacks MBAFF, SVC/3D/MVC | 🚧 ~83% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite + IDR Intra_16x16 luma AC trellis + chroma AC trellis (1227 tests); ffmpeg PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 ~40% — VPS+SPS+PPS bodies + scaling-list + scan + §9.3 CABAC engine + slice header through inter mvd/cabac-init/collocated block + r158 §7.3.6.1 five_minus_max_num_merge_cand + full inter tail walk (P/B end-to-end through byte_alignment; 198 tests); lacks pred_weight_table()/ref_pic_lists_modification() in-place wiring + §9.3.4.2 binarization+ctxIdx (#444) + §7.4.8 inter-RPS-prediction + residual/IDCT | 🚧 scaffold |
| **H.266 (VVC)** | 🚧 ~65% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine sub-block MC + PROF + AMVP luma cand + temporal-Col + affine-AMVP + bcw_idx + SbTMVP + merge_subblock + §7.3.11.7 inter_affine_flag + r159 §7.3.11.7 cu_affine_type_flag CABAC + Table 85 ctx (~1019 tests); lacks non-merge inter CU walk + encoder-side subblock-merge | 🚧 ~85% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred — see crate README |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~90% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §13.4 fitter + r162 §11 4Y×4UV intra-within-inter picker threaded into Vp8InterStreamEncoder stream driver (3 opt-in entry-points; 539 tests) |
| **VP9** | 🚧 ~38% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header sweeps + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §8.6.2 reconstruct + §6.4.3 decode_partition + §6.4.13 read_is_inter + §6.3.9/10/11 prob sweeps + r158 §6.3.14 read_y_mode_probs (373 tests); lacks §6.3.12 frame_reference_mode + §6.4.4 decode_block + §8.4 loop filter | 🚧 scaffold |
| **AV1** | 🚧 ~54% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.7/8/9/10/11/12/13/20/55/56 leaves + r162 §5.11.19 inter_segment_id(preSkip) 2-call protocol + above/left seg-pred contexts (520 tests); lacks §5.11.5 decode_block body + §5.11.18 inter_frame_mode_info dispatcher | 🚧 scaffold |
| **Dirac / VC-2** | ✅ ~94% — VC-2 LD+HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit + bit-exact intra fixtures | 🚧 ~95% — HQ+LD intra + Dirac core-syntax + adaptive sub-pel + 2-ref bipred + post-OBMC refinement + picture/sequence rate-control PerPicture/CBR/Vbv + r159 VbvHysteresis drain-cap variant + per-picture running_surplus_bytes telemetry (302 tests) |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~80% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + depth-3 lazy LZ77 + r162 §4.1 sub-image-aware Shannon chooser (44% reduction on 256×128 gradient predictor stream; 389 tests); lacks VP8-lossy encode (pending vp8 republish #1068) |
...
| **JPEG XL** | 🚧 ~89% — ISO/IEC 18181-1:2024 lossless Modular path + 7 fixtures pixel-correct + VarDCT scaffold + Gaborish/EPF/AFV pure-math complete + r159 §C.8.3 per-block HF coefficient loop scaffolding (Listings C.13/C.14, DCT8×8, 553 lib tests); lacks §C.7.2 histograms + per-frame Gaborish/EPF/CfL wiring (#799/#1077) | — retired |
...
| **glTF 2.0** (+ .glb) | ✅ ~94% — JSON + .glb + full PBR + 11 KHR_materials extensions (unlit/emissive_strength/ior/specular/clearcoat/sheen/transmission/volume/iridescence/anisotropy + r158 dispersion) + skin + skeletal animation + sparse accessors + morph-targets + 12 spec-MUST validators + KHR_texture_transform + JSON fuzz hardening; lacks KHR_audio_emitter / KHR_materials_diffuse_transmission | ✅ ~91% — symmetric + sparse-encoding heuristic + signed+unsigned normalised-int quantisation + KHR_materials_unlit emit |
...
| **`oxideav-rtmp`** | RTMP ingest + push | ✅ Server + client; AMF0 handshake/chunk-stream + AMF3 parser/builder; Enhanced-RTMP v1 video + v2 audio + ModEx; pluggable key-verification; `rtmp://` PacketSource; symmetric teardown (server StreamEOF + Unpublish.Success + half-close + r158 client `poll_event` surfacing server-originated UserControl/onStatus/_result/_error) |
