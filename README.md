...
| DVD-Video | ✅ | — | — | `oxideav-dvd` Phase 2 — ISO 9660 + UDF 1.02 mount + VIDEO_TS walk + IFO body parser (VMGI/VTSI + TT_SRPT + VTS_PTT_SRPT + PGCI + VTS_C_ADT, chapter materialiser with cell ranges + PgcTime BCD) + `dvd://` URI handler. Lacks VOB demux, VM (HDMV opcodes + SPRMs/GPRMs), CSS auth (Phase 3 + `oxideav-css`) |
