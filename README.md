...
| **FLAC** | ✅ 100% — bit-exact vs spec + RFC 9639 §8.7 CUESHEET tracks → Chapter API | ✅ 100% — bit-exact roundtrip + per-subframe LPC coefficient-precision search (≤15-bit) + LPC order search 1..=12 + per-subframe Welch/Hann/Tukey apodization window search (RFC 9639 §9.2.6) + r143 closed-form Rice parameter estimate (RFC 9639 §9.2.5; cuts encode 4.6×–6.0× across mono/stereo/multichannel benchmark scenarios, byte-identical output vs brute-force scan) |
...
| **Opus** | 🚧 r11 (post-2026-05-20 orphan) — RFC 6716 §3.1 TOC + §3.2 frame packing + §4.1 range decoder + §4.2.7.1–§4.2.7.5.1 SILK header + §4.2.7.4 subframe gains + §4.2.7.5.2 LSF Stage-2 residual + §4.2.7.5.3 NLSF reconstruction (Tables 23/24 cb1_Q8 + IHMW w_Q9 in [1819, 5227] + final NLSF_Q15 clamp) + §4.2.7.5.4 NLSF stabilization (RFC 8251 §7 erratum) + §4.2.7.5.5 NLSF interpolation + §4.2.7.5.6 NLSF→LPC core conversion (`silk_NLSF2A`: Table 27 ordering + Table 28 Q12 cosine + i64 P/Q recurrence) + §4.2.7.5.7 LPC range-limiting bandwidth expansion (silk_bwexpander_32 chirp loop + Q12 saturation) + §4.2.7.5.8 LPC prediction-gain stability + §4.2.7.6 LTP parameters + §4.2.7.7 LCG seed + §4.2.7.8 excitation reconstruction (all 6 substeps, Tables 43–53) + §4.2.7.9.2 LPC synthesis filter (carries d_LPC unclamped lpc[] across subframes, clamp to ±1) + §4.2.7.9.1 LTP synthesis filter (5-tap Q7 + Region-A/B rewhitening of out[]/lpc[] + LSF-interp split branch) + §4.2.8 SILK stereo unmixing (MS→LR) + §4.2.3 SILK header bits (VAD + LBRR) + §4.2.4 per-frame LBRR flags (Table 4 40/60 ms PDFs) + §4.2.9 SILK resampler delay budget (Table 54 NB/MB/WB = 0.538/0.692/0.706 ms; SILK internal 8/12/16 kHz; output 8/12/16/24/48 kHz); 339 tests; lacks CELT | 🚧 scaffold |
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM + r126 §2.4.3.1 CRC-16 verification on Layer II (header bits 16-31 + alloc + scfsi); lacks Layer II encoder, Annex D psychoacoustic model | ✅ clean-room Layer I encoder (ISO/IEC 11172-3 Annex C) — 32-band polyphase analysis filterbank (Table C.1) + scalefactor pick + energy-driven non-psy allocator + Table C.3 A/B quantizer (MSB-inverted, bit-exact inverse of decoder); `oxideav_core::Encoder` registered; self-roundtrip 1 kHz tone at 192 kbit/s mono RMS < 0.01; 89 tests; lacks Annex D psychoacoustic model |
...
| **MP3** | 🚧 clean-room rebuild underway (2026-05-24) — MPEG-1/2 frame-header parser + per-frame length + resyncing frame-walker + Layer III side-info parser (MPEG-1 + MPEG-2/2.5 LSF, §2.4.1.7) + scalefactor decode stage (bit reservoir, slen tables, scfsi reuse, LSF 4-partition) + main-data Huffman decode (big_values 3-region split, count1 quad A/B, linbits ESC, bit-budget termination; Table 3-B.7 codebooks 0..=13 transcribed from staged Annex B render) + §2.4.3.4.7 requantize (long/short/mixed/LSF, pretab, subblock_gain → float xr[576]) + §2.4.3.4.8 short-block reorder + §2.4.3.4.9 stereo processing (MS + intensity, MPEG-1 + LSF) + complete Table 3-B.7 Huffman codebooks (0..=31, incl. tables 15/16/24 + linbits aliases) + §2.4.3.4.10.1 alias reduction + §2.4.3.4.10.2 IMDCT + windowing (all 4 block types incl. mixed-block) + §2.4.3.4.10.4 overlap-add + §2.4.3.4.10.5 frequency inversion + §2.4.3.2 polyphase synthesis filterbank (granule → 1152 PCM samples — **end-to-end bitstream→PCM decode REACHED**) + **`Demuxer` impl** (ID3v2/Xing/seek; +4.5% duration delta vs ffprobe pending LAME-tag #989) + **Phase 1 encoder** (frame-header + side-info writers + valid silent frame; ffmpeg + mpg123 accept as decodable MP3) + Phase 2 encoder step 1: §2.4.3.4.10.2 forward MDCT primitive (n=12/36 closed-form + TDAC bin-exact `mdct(imdct(X))=(n/2)X`) + Phase 2 step 2/3: analysis windowing + forward overlap + polyphase analysis filterbank + step 4/5/6: §2.4.3.4.7 quantize primitive + §C.1.5.4.4 inner-loop global-gain search + exact §C.1.5.4.4.5/.8 Huffman bit count + §2.4.1.7 Huffman bit emission (round-trips to decoder; bit_len == exact count) + §2.4.1.7 main_data() assembly (main_data_begin=0 self-contained frames; scalefactors+Huffman round-trip at exact length) + §2.4.2.7 cross-frame bit-reservoir scheduler (main_data_begin > 0; 3-frame busy-middle round-trip bit-exact through decoder reservoir) + Phase 2 step 10: top-level `Mp3Encoder` PCM→MP3-frame stream driver (mono MPEG-1 CBR; 1-sec 440 Hz sine self-decode 86 dB through demuxer + decode pipeline) + Phase 2 step 11 §C.1.5.4.3 outer (distortion-control) loop (per-band amplify-on-noise-over-threshold; multi-tone PSNR +0.28 dB vs fixed-gain at 128 kbit/s) + Phase 2 step 12 `oxideav_core::Encoder` trait wiring (`Mp3CoreEncoder` AudioFrame→Packet adaptor + `register_codecs` for runtime-context API; 86 dB sine round-trip via trait matches direct API) + r141 Phase 2 step 13 `oxideav_core::Decoder` trait wiring (`Mp3CoreDecoder` packet→`AudioFrame`; mono MPEG-1 byte-exact vs direct chain across registered `make_decoder` factory) + r142 opt-in Xing/Info VBR-info-frame emission (`Mp3Encoder::enable_xing_info`; carrier-first silent Layer III frame with magic at expected offset; auto-fills frames/bytes from audio region post-reservoir; `Mp3Demuxer::open` round-trip) + r143 Phase 2 step 14 true-VBR per-frame bitrate-index emit (`Mp3Encoder::enable_vbr(min,max)` over the §2.4.2.3 ladder; mixed-content stream picks 32/40/64/96/320 kbps frames) + Xing TOC auto-fill (`toc[i] = floor(256·cumulative_offset_i/total_bytes)`) + r144 Phase 2 step 15 opt-in §2.4.3.1 CRC-16 frame protection (`Mp3Encoder::with_protection_bit(true)`; CRC slot between header and side-info, per-granule budget shrinks by 16 bits) + r145 Phase 2 step 16 independent-stereo encode through the trait (`ChannelMode::Stereo` + `DualChannel`; 1-sec 440/880 Hz LR sine @ 192 kbit/s self-decodes at L=85.13 dB / R=80.42 dB per-channel, zero cross-channel leakage) + r146 Phase 2 step 17 §2.4.3.4.9.2 joint-stereo MS encode (`Mp3Encoder::new_joint_stereo_ms`; opt-in forward M/S rotation per granule-pair, header `mode='01'`/`mode_extension='10'`; 70/30-pan 440 Hz @ 192 kbit/s self-decodes at L=84.16 dB / R=85.16 dB); 443 tests, ISO/IEC 11172-3/13818-3 only; lacks Phase 2 psy + intensity-stereo encode + LSF + MPEG-2.5 frame-parser | 🚧 not started (rebuild) |
| **AAC** | 🚧 Phase 1 (post-r111 orphan-rebuild) — ADTS header + raw_data_block walker + r126 AudioSpecificConfig parser (ISO/IEC 14496-3 §1.6.2.1 + §4.4.1 — GA AOTs 1-7/17-23, SBR/PS outer wrap, inline PCE when chanCfg=0) + §4.4.1.1 program_config_element parser + Phase 2 begin: §4.4.6 `ics_info()` (window_sequence/shape, max_sfb, scale_factor_grouping, Main predictor_data, ltp_data non-LD + ER-LD, §4.5.2.3.4 window-group derivations) + §4.4.6.1 section_data() run-length codebook sections + §4.4.6 section_data() encoder writer (first encode-side syntax-element writer; Table 17 inverse, self-roundtrip bit-exact) + §4.4.6 ics_info() encoder writer (Table 4.6 + Table 4.55 LTP body; second encode-side syntax-element writer; self-roundtrip bit-exact across all AOTs/window-sequences) + r142 §4.4.6.3 `pulse_data()` parser + encoder writer (Table 4.7; third encode-side syntax-element writer; self-roundtrip bit-exact) + r146 §4.4.6 `tns_data()` parser + encoder writer (Table 4.54 + §4.6.9.2 Table 4.155 + §4.6.9.3 `coef_bits = (3+coef_res)−coef_compress ∈ {2,3,4}`; fourth encode-side syntax-element writer; self-roundtrip bit-exact across all window_sequences/coef_res/coef_compress); raw_data_block walker emits Pce instead of Unsupported (no decode body yet) | 🚧 scaffold |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~96% — baseline + progressive 4:2:0/4:2:2/4:4:4/grey + 12-bit 4:2:0/4:2:2/4:4:4 YUV + SOF9 arithmetic + lossless SOF3 grey P=2..16 + 3-comp RGB P=8 + RFC 2435 RTP/JPEG depacketization (+ §4.2 cross-frame static-Q table caching) + packetization | ✅ ~95% — baseline + progressive + lossless SOF3 grey/RGB (all 7 Annex H predictors) + DRI/RSTn restart markers + non-zero point transform Pt 0..15 |
| **FFV1** | 🚧 r5 (post-2026-05-18 orphan) — RFC 9043 §4.2/§4.3 cfg-record + §4.6 SliceHeader + §4.7/§4.8 Slice Content scaffold + §3.8.2 Golomb-Rice (ur/sr + ESC) + §3.3 median predictor + §3.5 context model + §3.8.2.4 VLC symbol decoder + per-row `decode_line` → `Vec<i32>` sample_difference + §4.1 Quantization Table Set cascade (8/16-bit quant tables, context_count bit-exact) + §4.3.2/§4.9.3 Configuration Record CRC validation (IEEE poly, zero-residue fixity; bit-exact on 4 v3 fixtures) + §4.9 Slice Footer parser (slice_size + error_status + slice_crc_parity, whole-Slice CRC fixity) + §4.8 per-plane Golomb-Rice pixel reconstruction (median + §3.1 border + §3.8 modular add-back) + §3.7/§3.8.1.2/§4.8 per-plane range-coder pixel reconstruction (state-table cascade + carryless renormalise + zero-vs-significant split + median + add-back) + §4.9.1 trailer-pointer chain walk (backward slice_size walk → forward SliceExtent[]) + **end-to-end `decode_frame` driver** (trailer-walk → per-slice footer/header/content → range/Golomb plane reconstruct → multi-slice blit into DecodedFrame; YCbCr plane-major, chroma-shifted, bit-exact vs reference trace after §4.4 keyframe-bit fix) + §3.7.2 RGB/RCT line-major decode driver (Y+Cb bit-exact; Cr-plane divergence open) + end-to-end Golomb-Rice multi-slice full-frame assembly coverage (synthetic 2×2 / 1×3 grids, bit-exact) + §3.8.1 binary RangeEncoder + §3.8.1.2 scalar put_ur/put_sr/put_br (symmetric inverse of decoder; 21 round-trip tests with context-window equality); + r142 §4.9 Slice Footer encoder (first frame-level encoder primitive; CRC residue driven to zero by construction via `CRC(M || CRC(M)) == 0`) + r146 §4.6 Slice Header encoder (symmetric inverse of `parse_slice_header`; per-field `put_ur` against the shared 32-slot context window with `PARAMETERS_INITIAL_STATE=128` seed; chained `_to_encoder` variant for `coder_type≥1` slices; encoder corpus round-trip on `v3-default`/`v3-grayscale`/`v3-rgb-bgr0` fixtures); 259 tests; lacks `Decoder` registration (gated on #904) + RGB Cr fix + Slice Content encoder | 🚧 scaffold |
| **MPEG-1 video** | 🚧 clean-room rebuild (post-audit 2026-05-18) — sequence/GOP/picture/slice headers + §6.2.5 macroblock_address_increment + §6.2.5.1 macroblock_type (Annex B Tables B-2/B-3/B-4) + §6.2.5.3 coded_block_pattern (Table B-9 + 4:2:2/4:4:4 ext) + macroblock-layer quantizer_scale + §6.2.5.1 macroblock_modes() motion-type/dct_type tail (Tables 6-17/6-18/6-19) + §6.2.5.2 motion_vector(s) (Annex B Table B.4) + §2.4.4.2/§2.4.4.3 MV reconstruction (r_size/f derivation, complement + little/big, PMV wrap [-16f, 16f-1], full_pel shift, luma/chroma half-pel split on negative recon, P-zero / B-carryover absence paths) + §2.4.2.8/§2.4.3.7 intra-block DC prelude (Tables B.5a/B.5b VLCs + differential sign-extension) + §2.4.4.1 8×8 zig-zag scan + §2.4.3.7 `dct_coeff_first` / `dct_coeff_next` walker (Tables B.5c/d/e VLCs + Table B.5f short 14-bit / long 22-bit escape + FIRST-vs-NEXT (0,1) disambiguation + end_of_block) + r146 §2.4.4.1/§2.4.4.2 dequantiser bodies (intra 4-loop with `dct_dc_y/cb/cr_past` chain + `past_intra_address > 1` reset + `Sign()` even-mismatch + [-2048, 2047] saturate; non-intra `(2·dct_zz[i] + Sign(dct_zz[i]))` dead-zone + zeroing pass; §2.4.3.2 default intra/non-intra matrices); IDCT pending (Annex A normative ref to IEEE P1180/D2 not staged) | 🚧 scaffold |
| **MPEG-2 video** | 🚧 r6 (post-2026-05-18 orphan) — §6.2.2.1/§6.2.2.3/§6.2.2.6 sequence/GOP + §6.2.3 picture_header + §6.2.3.1 picture_coding_extension + §6.2.4 slice_header + §6.2.5 macroblock_address_increment (Annex B Table B-1 33-code VLC + escape chain + MPEG-1 stuffing) + §6.2.5.1 macroblock_type + §6.2.5.3 coded_block_pattern (Table B-9 + 4:2:2/4:4:4 ext) + §6.2.5.1 macroblock_modes() motion-type/dct_type tail (Tables 6-17/6-18/6-19) + §6.2.5.2 motion_vectors() / motion_vector() (Tables B-10 motion_code + B-11 motion_residual, dual-prime + concealment) + §7.6.3.1 PMV reconstruction (wrap-around + half-pred for field-in-frame vertical + PMV reset + §7.6.3.7 chroma scaling) + §7.6.3.3 inter-vector PMV-update (Tables 7-10 frame / 7-11 field, intra ‡/◊ + non-intra §-zero-reset rows + spec-unreachable rejection); r13, 231 unit tests; lacks §7.6.3.6 dual-prime additional + residual VLCs / IDCT | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 r9 (post-2026-05-18 orphan) — VS/VO/VOL + §6.2.3 + GOV + VOP header + §6.2.3.3 quant-matrix + §6.2.6 I/P-VOP MB header + §6.2.6 B-VOP MB header (modb Table B.3 + mb_type B.4 non-scalable + B.5 scalable + cbpb 4:2:0 + dbquant Table 6-33; Direct/Forward/Backward/Interpolated typed enum) + §6.2.6.2 motion_vector + §7.6.3 differential-MV (Table B.12 VLC + Table 7-9 wrap) + §7.6.5 median MV predictor + §7.4.1.1 intra-DC texture decode (dct_dc_size VLC + differential DC) + §7.4.1.2 AC-coefficient (EVENT) decode + §7.4.2 inverse scan (QFS[n] → PQF[v][u]; alternate-horizontal/vertical/zigzag) + §7.4.4 inverse quantisation (Figure 7-7 QF→F''→F'→F pipeline: intra-DC, both methods, §7.4.4.4 saturation + §7.4.4.5 mismatch toggle) + §7.4.5 IDCT (orthonormal 8×8, IEEE 1180 ≤1 LSB) + §6.2.7 intra I-VOP block(i) MB assembly (4 luma + 2 chroma → 16×16+8×8 reconstruction over the full §7.4.x chain) + §7.4.3 / Figure 7-5 intra-block predictor candidate gathering (IntraBlockGrid + per-block A/B/C neighbour resolution + §7.4.3.1 default fallback) + r17 §7.4.1.3 Type 4 escape for short_video_header == 1; 340 tests; lacks Figure 7-34 MV-predictor gathering / inter+B-VOP reconstruction / interlaced_information / encoder | 🚧 scaffold |
| **Theora** | 🚧 r8 (post-2026-05-20 orphan) — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3-default tables + §6.4.2 quantization-parameters + §6.4.3 quant-matrix + §6.4.4 DCT-token Huffman tables (80-table binary-tree) + §7.1 frame-header decode (FrameType + qis) + §7.2 long-/short-run bit strings (Tables 7.7/7.11, RLEN=4129 fresh-BIT exception) + §7.3 coded block flags decode (intra short-circuit + inter SBPCODED/SBFCODED/per-block chain with edge-super-block step 2(g)); 159 tests; §6.4.1 LFLIMS body blocked (#944) | 🚧 scaffold |
| **H.263** | 🚧 r8 (post-2026-05-18 orphan) — §5.1 picture + §5.2 GOB + §5.3 MB header (full Tables 7/8/12/14) + §5.4 block data (Table 15/16 VLC + zigzag) + §6.1/§6.2.1 H.261-style inverse-quant + §6.2.3 zigzag scatter + §6.2.4 orthonormal IDCT (f64, OnceLock cosine table) + §6.3.2 [0,255] clip → `reconstruct_intra_block` + §6.1.1 MV reconstruct + §6.1.2 half-pel interp + §6.3.1 INTER summation + Annex J §J.3 in-loop deblocking filter (four-tap edge filter + Table J.2 STRENGTH + horizontal-before-vertical ordering + picture-edge skip) + Annex I Advanced INTRA Coding §I.2 INTRA_MODE VLC (Table I.1) + §I.3 alternate DCT scans (Fig I.2-a/b) + scan-selection + full-picture decode driver (baseline INTRA/INTER/skip + Figure-12 MV prediction + optional Annex J deblock → YuvFrame) + Annex D §D.2 Unrestricted Motion Vector mode (extended ±63 half-pel range, predictor-dependent difference-pair selection) + Annex F §F.2 four-MV candidate-predictor (Figure F.1 per-block neighbour mapping) + Table F.1 chroma 1/16-pixel→half-pel snap + Annex F §F.3 overlapped block MC (H0/H1/H2 weight matrices + 3-way weighted average for 4MV) + §5.1.4 extended-PTYPE (PLUSPTYPE) picture-header parse + Annex I §I.3 / Table I.2 separate INTRA-coefficient VLC (Advanced INTRA Coding event-level primitive, 102 regular entries + §5.4.2 ESCAPE) + r143 Annex K §K.2 Slice Structured mode slice-layer header parse (SSC + SEPB1 + optional SSBI Table K.1 + MBA Table K.2 + SQUANT + optional SWI Table K.3 + SEPB3 + GFID; default + Annex Q RRU columns for all 6 standard picture sizes); 278 tests; lacks Annex K driver wiring (decode_picture walks GOB headers only) / PB-frames / PLUSPTYPE-gated driver / multi-picture demux / AIC §I.3 absorbed-INTRADC reframing | 🚧 scaffold |
...
| **H.265 (HEVC)** | 🚧 r6 (post-2026-05-18 orphan) — Annex B + §7.3.1.2 NAL + §7.3.2.1 VPS + §7.3.3 PTL + §7.3.2.2 full SPS body (prefix + PCM + §7.3.7 short-term RPS both explicit + inter-RPS-prediction forms + long-term RPS + MVP/smoothing + opaque VUI/ext tail) + §7.3.2.3.1 PPS (tiles + deblocking-control + lists-mod/parallel-merge tail, se(v)) + §7.3.6.1 slice header (both IDR and non-IDR independent I-slice end-to-end through byte_alignment; POC + short-term RPS sps-flag/inline/idx + long-term RPS array; P/B ref-list/pred-weight bodies opaque) + §7.3.4 scaling_list_data() parse + §7.4.5 ScalingList derivation (default Tables 7-5/7-6 + pred/explicit forms, wired into SPS+PPS) + §6.5.3 up-right diagonal scan + §7.4.5 ScalingFactor 2-D quant-matrix derivation (eq 7-44..7-51, DC override + 16×16/32×32 replication + 4:4:4 chroma) + §6.5.4/6.5.5/6.5.6 scans + §7.4.2 ScanOrder accessor + §9.3 CABAC arithmetic engine (init + decode_decision/bypass/terminate + Tables 9-52/9-53) + §7.3.2.1 VPS tail (layer_id_included_flag matrix + optional vps_timing_info_present_flag block) + §E.2.2/§E.2.3 hrd_parameters() + sub_layer_hrd_parameters() bodies decoded inline in the VPS HRD loop (replaces opaque-tail; §E.3.2/3.3 inferences + monotonicity checks) + §E.2.1 vui_parameters() body decode (aspect-ratio/EXTENDED_SAR + video-signal-type+colour + chroma-loc + default-display-window + timing+nested HRD + bitstream-restriction); + §7.3.6.2 ref_pic_lists_modification() standalone parser (L0 + B-gated L1, per-entry `Ceil(Log2(NumPicTotalCurr))`-bit width, range-checked) + §7.4.7.2 NumPicTotalCurr derivation (`NumPicTotalCurrInputs::compute()` covers F.7.4.7.2 multilayer + SCC PPS `curr_pic_ref` paths) + §7.4.7.1 long-term `UsedByCurrPicLt[i]` SPS-table-lookup resolver + §7.3.6.3 `pred_weight_table()` standalone parser (luma + chroma log2 weight denoms + 7-33/7-34 WpOffsetHalfRange{Y,C} bounds + L1 mirror + §7.4.7.3 sumWeightLXFlags ≤ 24 cap + accessor inference of absent LumaWeightLX/ChromaWeightLX/ChromaOffsetLX); r17, 184 tests; lacks VPS/SPS/PPS extension-data / §9.3.4.2 binarization+ctxIdx (#444) + §7.4.7.2 NumPicTotalCurr derivation (unblocks in-place §7.3.6.1 wiring) | 🚧 scaffold |
| **H.266 (VVC)** | 🚧 ~64% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + chroma 4-tap sub-pel + DMVR §8.5.3.2.4 + affine sub-block MC §8.5.5.9 + PROF §8.5.6.4 + §8.5.5.5/§8.5.5.6 affine merge candidates + §8.5.5.2 subblockMergeCandList insertion order + merge_subblock_idx pick + §8.5.5.2 steps 3-6 neighbour/corner-selection cascade (inherited-A/B scans + constructed-corner availability under parallel-merge gate) + §7.3.10.10 mvd_coding() + §9.3.3.14 limited-EGk + §7.3.10.8 non-merge inter MVP-side syntax (inter_pred_idc / sym_mvd_flag / ref_idx_lX / mvp_lX_flag) + §8.5.2.8/§8.5.2.9/§8.5.2.10 AMVP luma candidate derivation (spatial A/B scan with DiffPicOrderCnt==0 gate + §8.5.2.14 AMVR round + Col gate + HMVP fill + zero-pad + mvp_lX_flag select + mvd fold) + §8.5.2.11 live temporal-Col AMVP candidate (POC-scaled, AMVR-rounded) + §8.5.5.7 affine AMVP candidate list (luma CPMV predictors: §8.5.5.5 inherited-A/B scans + §8.5.5.6 corner-constructed cascade + AMVR round + zero pad to max-num=2) + §7.3.10.5 bcw_idx CABAC reader (Table 91 TR cMax = NoBackwardPredFlag ? 4 : 2) + §7.3.10.5 bcw_idx gate evaluator + MvField fuse (BcwIdxGate seven-input conditional + inference on closed gate) + §8.5.5.3/4 SbTMVP record + availability gate + §8.5.5.3 SbTMVP CTU-walker fuse (per-8×8-sub-block mvLXSbCol/predFlag fill from collocated motion field + centre-default fallback) + §7.3.11.7 merge_subblock_flag + merge_subblock_idx CABAC readers (Table 107/108 ctx; merge-side cond gating; live-stream entry into the SbTMVP / sub-block-merge-cand-list driver) + §7.4.3.4 eq.85 MaxNumSubblockMergeCand SPS derivation (affine + sbtmvp branches, [0,5] clamp; drives the merge_subblock_idx cMax) + r146 §7.3.11.7 `merge_data()` wire-up of the merge_subblock_flag/idx readers behind the eq.85 gate (`MaxNumSubblockMergeCand > 0 && cbW ≥ 8 && cbH ≥ 8`; subblock-merge branch short-circuits regular/MMVD/CIIP/GPM per §7.4.12.7 inferences); 992 tests; lacks non-merge inter CU walk + live per-CB merge_subblock/inter_affine neighbour grid + encoder-side subblock-merge emission | 🚧 ~85% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B slice + sub-pel MC ½/¼-pel (luma + chroma) + multi-ref DPB + weighted bi-pred — see crate README |
...
| **VP8** | 🚧 r8 (post-2026-05-20 orphan) — RFC 6386 §7 bool decoder + §9.1 + §19.2 + §11 KF MB mode layer + §12 intra-prediction kernels + §13 DCT-coefficient token decoder (coeff_tree walker + §13.2 EOB-skip + §13.5 default coef_probs[4][8][3][11] table + extra-bits decode through CAT6 11-bit DCTextra) + §14 dequant + inverse WHT/DCT + summation + §15 loop-filter per-segment kernels (simple/normal + §15.4 control params) + §16.1 interframe intra-MB mode decode (IF_YMODE/IF_UV/BMODE trees + per-frame F-gated overlay) + §14.2 per-MB reconstruction orchestrator (non-B_PRED: §13 tokens → §14.1 dequant → §14.3 WHT → §14.4 DCT → §12 intra-pred → §14.5 sum) + §11.3/§12.3 B_PRED per-sub-block intra walker (10 sub-modes, in-place neighbour evolution, §12.3 right-edge above-right copy_down, no-Y2) + per-frame keyframe raster walker (per-MB neighbour-strip assembly, §12.3 above-right clamp, I420 plane writeback) + §13.3 per-MB token walk (`decode_mb_coeffs`: §20.16 nonzero-context tables, skip/Y2-preserve, zigzag→raster) + §14.1 Y2/chroma dequant scaling (`MbDequantFactors`: Y2 DC×2, Y2 AC×155/100 min 8, UV DC cap 132, frame + §10 per-segment) + `decode_and_dequantize_mb` — **keyframe decode chain complete bitstream→dequant→reconstruct→pixels** + §15.1 loop-filter frame geometry (per-MB filter-level + edge-limit/interior-limit derivation, MB-edge + 3 internal subblock-edge iteration applying simple/normal kernels) + top-level `decode_vp8` driver + `oxideav_core::Decoder` registered (id `vp8`; tags VP80/vp08/V_VP8) — **key-frame decode complete, bit-exact vs libvpx on 10 fixtures** (intra-pred + dequant + IDCT/WHT + loop-filter + multi-partition) + §17 motion-vector component decode (read_mv/read_mvcomponent + prob-update resolution) + §16.2 reference-frame selection + §18 whole-pixel motion compensation + §18.3 sub-pixel MC (sixtap + bilinear) + inter-MB reconstruction + §16.2/§16.3 near/nearest MV census + inter-mode tree + §18.1 clamp + §16.4 SPLITMV per-sub-block MV walk + §18 SPLITMV reconstruction (`decode_split_mv_mb`) + top-level multi-frame `Vp8DecoderState` driver (golden/altref slot refresh + per-frame ref-buffer rotation + inter-mode dispatch — keyframe→interframe pipeline **bit-exact vs libvpx/ffmpeg reference output** on 4 multi-frame fixtures incl. `i-frame-then-p-frame-64x64`, 5-frame mid-GOP golden refresh, 10-frame auto-alt-ref + ARNR); 346 tests; lacks encoder | 🚧 r132 Encoder Phase 2 — §13 DCT-token block encoder over §7.3 BoolEncoder + §14 forward 4×4 DCT/WHT + per-MB block-set walker + §12.2 whole-block intra mode pick (DC/V/H/TM SAD picker, luma+chroma) + §11.3/§12.3 B_PRED 4×4 sub-block pick (10 sub-modes, in-place neighbour evolution, ~54 dB diagonal-tile MB) + r135 per-frame keyframe raster driver (full §9 header + §11 mode layer + §13 tokens, neighbour-strip threading MB→MB, self-decode ~41 dB @ qi32) + r136 RD-aware intra mode pick (J = SSD + λ·bits via exact self-decode + §13 token cost; smaller files AND higher PSNR at every qi) + r137 §9.5/§20.4 multi-partition DCT output (1/2/4/8 partitions, row-r%N distribution, bit-exact at every count) + r138 §15 non-zero loop filter wired into the keyframe driver (`KeyframeParams::loop_filter_level`/`sharpness_level` honoured end-to-end; self-decode 43.3–44.7 dB across levels 0/1/8/24) + r139 `Vp8KeyframeStreamEncoder` multi-frame driver (per-frame counter + dimension lock + §9.7/§9.8 three-slot refresh; 5-frame I420 sequence self-decodes through `Vp8DecoderState` at 45.4-48.5 dB per frame) + r140 minimum-viable P-frame encoder (`encode_p_frame_zero_mv`; every MB inter/ZEROMV/LAST + identity motion comp + intra-token residual; I+P synthetic 64×64 self-decodes at 43.78 dB whole-frame) + r141 `Vp8InterStreamEncoder` multi-frame I+P stream driver (keyframe_interval scheduling + force_keyframe override; 10-frame K-P-P-P-K-P-P-P-K-P self-decodes mean 41.35 dB at qi=32) + r142 whole-pixel motion-search primitive (`block_sad_16x16` + 4-neighbour small-diamond descent; §17.1 ±1023 clamp + §20.14 edge-replicate-safe fetch; infra only — no NEWMV emit yet) + r143 per-MB ZEROMV/whole-pixel NEWMV RD picker (`J = SAD + λ·(mode_bits + §17 mv_bits)`; NEWMV emit walks `mv_ref_tree` "1110" + §17.2 `write_mv` differential; translated 2-frame I+P fixture picks NEWMV on 4 of 16 MBs, Y-PSNR 50.3 dB at qi=32) + r144 §18.3 half-pixel refinement post-pass (3 of 16 half-pel NEWMVs on a +0.5 px shifted scene, 57.0 dB Y-PSNR at qi=4; tie-breaks prefer whole-pixel to protect §17.2 bits) + r145 §18.3 quarter-pel refinement post-pass (9 of 16 quarter-pel-only NEWMVs on a +0.25 px shifted scene, 48.85 dB Y-PSNR at qi=4) + r146 picker now scores all 4 whole-MB `mv_ref_tree` leaves (ZEROMV / NEARESTMV / NEARMV / NEWMV via the §16.3 census + per-MB clamp + tie-break on bit-cost; emits 23 of 24 NEARESTMV on a `(+4,+8)` px uniform-translation scene, Y-PSNR 48.14 dB at qi=4); 474 tests; lacks SPLITMV + GOLDEN/ALTREF + multi-partition inter + per-MB §9.4 mode/ref delta layer |
| **VP9** | 🚧 r11 (post-2026-05-20 orphan) — full §6.2 walk + §9.2 Boolean decoder + §6.3.1/§6.3.2/§6.3.3/§6.3.7/§6.3.8 compressed-header sweeps + §6.4.24 / §6.4.26 coefficient-token decoder + §8.6.1 dequant + §8.7 inverse transforms (DCT/ADST/WHT 1D + 2D driver, cos64_lookup + SINPI_*_9 spec verbatim) + §8.5.1 intra prediction (all 10 modes, neighbour-array construction with availability rules + edge clamps) + §8.6.2 reconstruct driver (token→dequant→idct→intra→add-residual→Clip1; mode2txfm_map TX_32X32 + lossless overrides) + §6.4.25 get_scan (10 §10.1 scan tables) + §6.4.24 tokens() per-block coefficient driver (§10 band tables + §9.3.2 token-cache neighbour ctx) + §6.4.21 residual() intra driver (per-plane block walk into §6.4.24 tokens → §8.6 reconstruct) + §6.4.8 read_skip + §6.4.10 read_tx_size (TX_MODE_SELECT) + §9.3.3 tree_decode generic walker + §6.4.7 intra_segment_id + §6.4.6 intra_frame_mode_info keyframe driver + §6.4.15 intra_block_mode_info inter-frame intra-block reader + §6.4.3 decode_partition_type() primitive (§3 enums + §9.3.1 trees + §10.2/10.4/10.5 lookups) + §6.4.3 recursive `decode_partition()` driver (TL→TR→BL→BR SPLIT recursion + §6.4.3 tail write-back to partition-context strips + Keyframe/Inter prob-source switch); + §6.4.12 inter_segment_id + §6.4.14 get_segment_id + §7.4 segmentation-prediction context strips (inter-frame companion to the intra_segment_id primitive; all four §6.4.12 paths incl. seg_id_predicted write-back into Above/Left SegPredContext) + r146 §6.4.13 read_is_inter + §9.3.2 is_inter ctx (4-branch above/left intra-derivation handling `Avail*` + None sentinel + seg-feature `SEG_LVL_REF_FRAME` short-circuit per §6.4.13 + §10.5 `default_is_inter_prob = {9, 102, 187, 225}`); 342 tests; lacks §6.4.4 decode_block + §6.4.11 inter driver + §6.4.16 inter_block_mode_info + §8.4 loop filter | 🚧 scaffold |
| **AV1** | 🚧 r11 (post-2026-05-20 orphan) — §5.3 OBU + §5.5 sequence + §5.9.2 prefix + §5.9.5–§5.9.9 frame_size + §5.9.3 allow_intrabc + §5.9.15 tile_info + §5.9.12 quantization_params + §5.9.14 segmentation_params + §5.9.17 delta_q + §5.9.18 delta_lf + §5.9.11 loop_filter_params + §5.9.19 cdef_params + §5.9.20 lr_params + §5.9.21 read_tx_mode + §5.9.22 skip_mode_params + §5.9.23 frame_reference_mode + §5.9.24 global_motion_params + §5.9.30 film_grain_params (intra + inter uncompressed-header complete end-to-end — set_frame_refs §7.8 / frame_size_with_refs / ref_frame_idx; film-grain-on FGS bit-exact) + §8.2 symbol (msac) arithmetic decoder (init/read_symbol/read_bool/read_literal/exit + §8.3 CDF update) + §9.4 default-CDF subset (intra-mode + partition + skip + segment + motion-vector + inter-mode + ref-frame + palette + filter-intra + CFL groups) + §8.3.1 init + §8.3.2 selection feeding the symbol decoder end-to-end + §9.4 transform-size + inter-frame transform-type + interp_filter + motion_mode + compound-prediction + intra-mode (y_mode/uv_mode) + angle-delta + coeff-token entry (txb_skip / eob_pt_N / eob_extra / dc_sign) + intra-frame transform-type (Set1/Set2) + `Default_Coeff_Base_Eob_Cdf` + `Default_Coeff_Base_Cdf` + `Default_Coeff_Br_Cdf` (`[4][5][2][21][5]`, `LEVEL_CONTEXTS=21` + `BR_CDF_SIZE=4`) CDF groups (all three coeff-CDF braid members landed) + §8.3.1 init_coeff_cdfs base_q_idx→idx ladder + §8.3.2 `get_coeff_base_ctx()` / `get_coeff_base_eob_ctx()` / `get_br_ctx()` neighbour-context derivation (Sig_Ref_Diff_Offset / Mag_Ref_Offset_With_Tx_Class scans feeding the coeff_base / coeff_base_eob / coeff_br selectors) + §5.11.40 `compute_tx_type()` per-plane/per-block derivation (Tx_Size_Sqr_Up / Mode_To_Txfm / Tx_Type_In_Set_{Intra,Inter} + is_tx_type_in_set admission filter; luma cache / inter-chroma Max-lift / intra-chroma uv_mode branches) + §9.4 inter-intra default-CDF group (`Default_Inter_Intra_Cdf` + `Default_Inter_Intra_Mode_Cdf` + `Default_Wedge_Inter_Intra_Cdf` + INTERINTRA_MODES + `interintra_ctx` selector via Size_Group) + §9.4 `Default_Wedge_Index_Cdf[BLOCK_SIZES][16+1]` + §8.3.2 `wedge_index_cdf(mi_size)` selector (§5.11.28 + §5.11.29 wedge branch) + §8.3.2 `split_or_horz` / `split_or_vert` cdf derivation (psum reduction over §9.4 partition rows; W128 PARTITION_*_4 drop) + PARTITION_NONE..PARTITION_VERT_4 + EXT_PARTITION_TYPES + BLOCK_128X128 ordinals + r146 §5.11.50 `get_palette_color_context` derivation (`palette_color_context_from_neighbors` partial-3-iteration sort + `Palette_Color_Hash_Multipliers`-weighted score → `PaletteColorContext { color_order, color_context_hash }`; 2-D entry-point + §5.11.50 `r>0`/`c>0` boundary guards); 323 tests; lacks tile-content decode / ref-frame update process | 🚧 scaffold |
| **Dirac / VC-2** | ✅ ~93% — VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit + §5.4 unbiased DC-prediction rounding (all 5 intra fixtures bit-exact vs ffmpeg) + r128 §12.3.6.6 inter DC-prediction unbiased-mean floor rounding (closes +1 LSB bias on negative-sum intra blocks; 2 inter ReportOnly fixtures promoted to Tier::BitExact) | 🚧 ~94% — HQ + LD intra + Dirac core-syntax + per-block adaptive sub-pel-vs-int-pel selection on 1-ref P-path (pre- AND post-OBMC) + 2-ref bipred path widened to strict-superset {int-pel, half-pel, sub-pel} + post-OBMC bipred mode-only refinement pass (+0.80 dB Y PSNR on camera-pan ME-only, ffmpeg cross-decode ceiling preserved) + §11.3.3 core-intra codeblock spatial-partition encoder (cumulative-quant decoder fix) + §13.4.3.3 all-zero codeblock skip (zero_flag) + VLC (non-arithmetic) core-syntax intra encoder (parse_code 0x4C) + VC-2 LD/HQ §12.4.5.3 custom quantisation matrix on the encoder (`with_custom_quant_matrix`, HQ q=8 53.5 dB Y self-roundtrip, LD q=0 bit-exact) + VC-2 HQ per-slice adaptive qindex + VC-2 LD picture-level auto-qindex picker + VC-2 LD multi-picture rate-controlled sequence driver (PerPicture / CBR carry-over) + r138 VC-2 HQ picture-level constant-qindex rate-control picker (`pick_hq_picture_qindex` + `encode_single_hq_intra_stream_with_size_target`; never overshoots, HQ twin of LD picker) + r141 VC-2 HQ multi-picture rate-controlled sequence driver (HQ twin of r134 LD driver; PerPicture + CBR within 0.04% of N×target on 5-picture run) + r146 VC-2 HQ leaky-bucket VBV variant `HqRateControl::Vbv { buffer_bytes }` (peak-cap every per-picture request at `target + buffer_bytes`; strict generalisation — `0` ≡ PerPicture byte-identical, `u32::MAX` ≡ CBR byte-identical on no-overshoot streams) |
...
| **BMP** | ✅ ~96% — 1/4/8/16/24/32-bit + V4/V5 + OS/2 BITMAPCOREHEADER + RLE4/RLE8 + top-down rows + daily fuzz CI (6.5M runs, 0 crashes) | ✅ ~96% — top-down + minimal `biClrUsed`-trimmed palette encoder |
...
| **JPEG XL** | 🚧 ~86% — ISO/IEC 18181-1:2024 final core. 7 small lossless fixtures decode PIXEL-CORRECT. Modular path + ISOBMFF `FF 0A` strip + 1..16 bpp pack + §F.3 zero-pad single-TOC fast path; VarDCT scaffold with Annex I.2 IDCT + GetDCTQuantWeights + 17-slot dequant set + §C.7.1 HfPass + §C.8.3 PassGroup HF + §F.3 HF dequantisation + §I.2.5 LLF-from-LF math step + §C.7.1 DecodePermutation (non-natural coefficient orders)+ Annex G chroma-from-luma pure-math primitive (`apply_hf_plane_inplace` per-64×64-tile cache + LF + raw forms); lacks §C.7.2 histograms + per-block coefficient loop + Gaborish / EPF + CfL pipeline wiring; r141 Annex J.2 Gaborish (Gabor-like-transform) pure-math primitive + r144 Annex J.3 edge-preserving-filter pure-math primitive (distance/weight/sigma/apply for passes 0/1/2, Mirror1D-bounded; per-frame loop deferred) | — retired |
| **JPEG XS** | 🚧 ~80% — ISO/IEC 21122 Part-1 + 5/3 DWT + Annex C/D/F/G + multi-component + CAP-bit + `Cw > 0` + `Sd > 0` (CWD) cascade + Sd>0 composes with Cpih∈{1,3} per Annex F.2 Table F.1 + high bit depth B[i]∈8..16 (u16-LE plane packing) + r143 ISO/IEC 21122-2:2019 Annex A profile/level/sublevel conformance (`Profile::from_ppih` round-trips all 9 Table A.5 rows; `check_profile` enforces Nc, B[i], chroma format, NL,x/NL,y, Qpih DZQ-vs-Uniform split, slice-height = 16, Light-Subline Cs ≤ 2048; `check_level` enforces Wmax/Hmax/Lmax per Table A.6) | 🚧 ~78% — Nc 1/3/4 + Sd>0 raw-tail (Nc up to 8) + RCT + Star-Tetrix + NL up to 8 + odd dims + vertical prediction + significance coding + per-band Q + NLT + `Cw > 0` cascade + r95 `Sd > 0` ∘ `Cpih ∈ {1, 3}` (RCT on c<3; Star-Tetrix on c<4; lossless at Nc=4 + RCT and Nc=5 + Star-Tetrix) + Fs=1 separate sign sub-packet (Annex C.5.5) + multi-slice `Hsl` emission (Annex B.10, one SLH per slice) + `Qpih=1` uniform/Neumann-series inverse quantizer (Annex A.4.4 Table A.10 / Annex D.3) via `encode_planar_qpih` (data sub-packet byte-identical to Qpih=0; lossless self-roundtrip) + Qpih-aware forward quantizer (Qpih=1 uniform round-to-nearest indices per Annex D.5 Table D.4 instead of deadzone-floored) + r115 `R[p] > 0` precinct refinement (Annex C.2/C.6.2 — per-band priorities + precinct refinement threshold) + r118 high bit depth B[i]∈9..16 lossless (`encode_planar_highbd`) + r133 high-bit-depth lossy q>0 (`encode_planar_highbd_lossy`, deadzone truncation, ≥40 dB @ 10-bit q=1) |
...
| **glTF 2.0** (+ .glb) | ✅ ~92% — JSON + .glb + full PBR + KHR_lights_punctual + KHR_materials_unlit + KHR_materials_emissive_strength + KHR_materials_ior + KHR_materials_specular + KHR_materials_clearcoat + KHR_materials_sheen + KHR_materials_transmission (decode + encode + §3.12 validator) + KHR_materials_volume (decode + encode + thickness/attenuation §3.8 validator) + KHR_materials_iridescence (decode + encode + §3.12 validator) + skin + skeletal animation (LINEAR/STEP/CUBICSPLINE) + sparse accessors + morph-targets + 12 spec-MUST validators + KHR_texture_transform (per-textureInfo UV affine, decode + encode + §3.12 validator) + JSON fuzz hardening; lacks KHR_audio_emitter / KHR_materials_anisotropy/dispersion | ✅ ~91% — symmetric + sparse-encoding heuristic + signed+unsigned normalised-int quantisation + KHR_materials_unlit emit |
