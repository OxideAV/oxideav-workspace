...
| **MP3** | ✅ MPEG-1 Layer III M/S | 🚧 CBR + VBR mono+stereo + **MS-stereo joint mode (44.8% smaller on mono fold-down)** + **short/start/stop block selection on transients (per-channel energy-ratio detector + 1-granule lookahead block-type machine; isolated-transient pre-onset PSNR 54.37 → ~300 dB, +245 dB pre-echo elimination)** + **intensity-stereo encode (per-band qualifier on silence/hard-pan/coherence ≥ 0.85 walking sfb 20 → 7, IS rewrite L := sign·sqrt(L²+R²) / R := 0, R-channel scalefac_compress=13 packs 21 sfbs at 3 bits each, mode_extension bit 0x1 may combine with #115 MS bit 0x2; 18.8% smaller on HF-correlated stereo at q=4 with IS active 37/39 frames; ffmpeg cross-decode clean, MS test still picks MS 39/39 frames)**; lacks Bark-partition spreading psy-1, LSF MPEG-2/2.5 IS encode |
...
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV + compressed-header probs; r22 fixed §6.4.3 `decode_partition` HORZ/VERT double-call (lossless-pattern Y: 10.41 → 47.70 dB, chroma bit-exact); **r24 §6.4.16 sub-8×8 inter mode-info per-4×4-sub-block iteration (decode_inter_block now does (idy, idx) walk for B4x4/B4x8/B8x4 — was reading exactly one inter_mode + assign_mv per ref slot per cell)** + **r25 within-MB sub-block MV mixing per §6.5.14 (`append_sub8x8_mvs`: anchors `nearest = BlockMvs[refList][0]` for blocks 1/2 and `BlockMvs[refList][2]` for block 3, walks idx=1,0 with dedup, falls back to cell `RefListMv[]` then ZeroMv; `MvRefs::best_mv_override` pins NEWMV's BestMv to cell `RefListMv[0]` while NEAREST/NEAR pull from rebuilt `list[0..2]`; per-sub-block refined `MvRefs` written back into `BlockMvs[refList][(idy+y2)*2+idx+x2]` for every 4×4 cell of the sub-block footprint; r22/r24 PSNR baselines unchanged where the fixture's (idy, idx) walk lands on block=0 / ZEROMV / NEWMV no-op cases)**; pending per-4×4 neighbour `SubMvs[r][c][refList][idx]` via §6.5.11 `get_sub_block_mv` (between-MB axis) | — |
| **AV1** | 🚧 OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound; SVT-AV1 chain 48/48; `inverse_2d_spec` covers all 159 (TX_TYPE × TX_SIZE) pairs + **inter Y `tx_type` CDF reads (§5.11.45, +0.82 dB vs libdav1d)** + **inter chroma `tx_type` derivation per §5.11.40 (TxTypes[][] grid tracked per superblock, chroma derives from luma via `is_inter_tx_type_in_set` membership check + ext-tx-set selection, defensive Unsupported→DctDct fallback)** + **palette intra-within-inter path (palette_screen.ivf decodes at 8.56 dB Y)**; lacks intrabc, full loop-filter, full primary_ref_frame=4 chain | — |
| **Dirac / VC-2** | ✅ VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + arithmetic + 10/12-bit; ffmpeg bit-exact 8-bit 4:2:2/4:4:4 + 10-bit 4:2:0 | 🚧 HQ + LD intra encoders + **Dirac core-syntax intra encoder (0x0C)** + Dirac core-syntax inter encoder (OBMC) + **quarter-pel sub-pel ME (per-level 8-neighbour gradient refinement, §15.8.11 8-tap half-pel filter + §15.8.10 bilinear sub-half, camera-pan fixture 26.92 → 52.04 dB Y, +25 dB)**; homogeneous core-intra + core-inter chain ffmpeg cross-decode hard-asserted (intra 52.06 dB, inter 20.17 dB); lacks OBMC overlap compensation, 2-ref bipred, wavelet residue |
...
| **EVC** (MPEG-5) | 🚧 NAL + SPS/PPS/APS + slice_header + **full §9.3 CABAC engine + §7.3.8 slice_data walker + §8 intra prediction (5-mode Baseline) + §8.7.4 inverse DCT-II for nTbS ∈ {2,4,8,16,32,64} (the §8.7.4.3 64-point ambiguity resolved via closed-form `M[m][n] = round(64·√2·cos(π·m·(2n+1)/128))` cross-checked against printed tables) + §8.7.2/§8.7.3 dequant + per-CU intra/inter reconstruct + 8-bit yuv420p + Baseline P/B inter (8-tap luma + 4-tap chroma sub-pel, AMVP candidate list, default-weighted bipred) + cbf!=0 residual coding + EVC luma deblock + EVC chroma deblock per §8.8.2 eqs 1167-1182 (2-tap stencil mutating only sB/sC vs luma 4-tap, BS shares the §8.8.2.3 step-2 cbf_luma trigger over the luma 4×4 grid, edge spacing scales by Table-2 factors via `chroma_subsampling`, per-slice `slice_cb_qp_offset` / `slice_cr_qp_offset` (i32 −12..=12) wired into both IDR and inter pipelines, `qp_c = Clip3(0, 51, slice_qp + chroma_qp_offset)` per eq. 1194; QP=27 ramp [100,110,120,130] yields hand-computed sB'=111 / sC'=119 with sA/sD untouched on every Cb row)**; `make_decoder` works end-to-end on Baseline IDR + P + B (PSNR=∞ on flat fixtures); lacks Main profile init tables, RPL for non-IDR, HMVP/MMVD/AMVR/affine/ALF/DRA/IBC/ATS/ADCC | — |
