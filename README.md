...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~97% — baseline + progressive 4:2:0/4:2:2/4:4:4/grey + 12-bit YUV + SOF9 arithmetic + lossless SOF3 + RFC 2435 RTP/JPEG + §G.1.1 SOF2 4-component CMYK/YCCK + arith_decode fuzz + §3.1.7 restart-aligned packetization + 4-component lossless SOF3 P=8 (Adobe APP14 CMYK) decode + RGB24 baseline JPEG packed-RGB output + decode-free SOF/chroma/colour-hint inspector + typed JFIF APP0 view (T.871 §10.1: `JfifUnits` enum + density/thumbnail + DPI/DPCM conversion + `parse_jfif_app0` validator) | ✅ ~97% — baseline + progressive + lossless SOF3 grey/RGB + DRI/RSTn + Pt 0..15 + 4-component CMYK encoder + 4-component lossless SOF3 P=8 + SOF0 Gray8 single-component lossy + progressive SOF2 single-component grayscale (3 spectral-selection scans DC + AC-low + AC-high) + SOF0 RGB24 baseline (Adobe APP14 + 'R'/'G'/'B' IDs) |
...
| **JPEG XS** | 🚧 ~82% — ISO/IEC 21122 Part-1 + 5/3 DWT + Annex C/D/F/G + multi-component + CAP-bit + high bit depth + 4:2:0 chroma at NL,y≥3 | 🚧 ~96% — Nc 1/3/4 + Sd>0 + RCT + Star-Tetrix + NL up to 8 + odd dims + vertical prediction + per-band Q + NLT + high-bit-depth Star-Tetrix lossless+lossy + per-slice / per-precinct Q[p] + R[p] override + joint per-slice + per-precinct rate-budget pickers + `pick_qpr_rpr_for_target_bytes` rate-budget driven per-precinct picker (Annex C.2 + C.6.2) + typed `Codestream::cts()` / `crg()` / `nlt()` accessors (Annex A.4.6/A.4.8/A.4.9) |
