...
| **FLAC** | ✅ 100% | ✅ 100% |
| **Vorbis** | ✅ ~98% — sample-exact decode incl. floor-0 + hostile-codebook fence | 🚧 ~90% — psy VBR+ABR (spread masking + phase tonality, masking-weighted VQ, masking-driven coupling, adaptive transients); beats the black-box reference at equal rate on mono/tones/transients/pre-echo; trails low-knob noise + dense stereo |
| **Opus** | ✅ ~96% — SILK bit-exact vs the RFC listing, CELT/hybrid, all §4.2.9 output rates, FEC/PLC/DTX | ✅ ~97% — unified encoder, every §2.1 knob + §4.5 normative transitions, registry options; lacks signal-adaptive speech/music election |
...
| **AC-4** (Dolby) | 🚧 ~99% — A-JOC static-dmx A-SPX/A-CPL carrier synthesis into the object path + dialogue enhancement applied on both routes; fuzz-hardened (5 fix rounds) | 🚧 ~93% — ICE encode + multi-envelope P-frames with per-trailer sticky xover |
...
| **MPEG-2 video** | 🚧 ~97% — 12/12 conformance corpus + §7.10 data partitioning (byte-exact split/merge); lacks scalable multi-layer loops | 🚧 ~93% — interlaced encode incl. skips, concealment MVs, 3:2 pulldown, full entropy flags; lacks 4:2:2 interlaced + scalable layers |
...
| **AV1** | ✅ ~99% — 155-stream corpus byte-exact; decode peak RSS −35% (byte-identical) | 🟢 ~98% — pyramid short-ref signaling + delta-q × lossless-segment tables; conformance-grade all §6.4.1 pairings; lacks large-scale-tile/scalability tail |
