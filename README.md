...
| **AAC** | 🚧 ~48% — ADTS + full channel-body parse + §4.6.1.3/§4.6.2.3.3 dequantisation + §4.6.11 IMDCT/sine-KBD filterbank + overlap-add (streaming TDAC perfect-reconstruction); lacks M/S/intensity/PNS synthesis for byte-exact PCM | 🚧 ~20% — Phase-2 writers for every syntax element + TNS decode tool; SBR pending QMF |
...
| **H.266 (VVC)** | 🚧 ~75% — 4:2:0 IDR intra + full inter toolset (ALF/SAO/HMVP/MMVD/CIIP/BCW/BDOF/GPM/DMVR/affine/PROF/SbTMVP) + typed RBSP/parameter-set surface + LMCS arrays + sample-domain luma map/chroma residual scale (§8.7.5.2/§8.8.2) | 🚧 ~93% — forward CABAC + DCT-II + MTT RDO + P/B + sub-pel MC + weighted bi-pred + affine/AMVR/BCW dispatchers |
...
| **AV1** | 🚧 ~36% — standalone intra decode, 4:2:0/monochrome multi-superblock to 128×128 + §7.15 CDEF driven from the decoded cdef_idx grid → real post-CDEF frame + §7.17 loop-restoration runs end-to-end (Wiener + self-guided filters applied from decoded §5.11.58 units) + §7.11.3 single-ref translational inter block reconstructs end-to-end from decoded mode-info; lacks compound/warp/OBMC inter + film grain + superres | 🚧 ~34% — intra encode YUV→IVF + §5.11 write side incl. §5.11.47 transform_type + §5.11.57/§5.11.58 read_lr/read_lr_unit loop-restoration unit syntax (use_wiener/use_sgrproj/restoration_type S() + Wiener-tap/sgr-xqd signed-subexp-bool) in decode-walker lockstep; lacks RD picker + inter reconstruction/encode chain |
...
| **DDS** | ✅ ~99% — header + DXT10 + BC1-7 + BC6H all modes + cubemaps/arrays/volumes + 16-bit/float extended uncompressed surfaces + daily fuzz | ✅ ~96% — uncompressed + BC1-7 + BC6H + mip chains + cubemap/array |
