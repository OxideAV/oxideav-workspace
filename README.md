...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate, 48 dB on HE-AACv1 reference. r17 wired `bs_limiter_gains` (Table 4.176). | ✅ AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS. **r19 closes the LC interop investigation**: §4.6.2.3.3 `gain = 2^(0.25*(sf-100))` SF_OFFSET, §4.6.1.3 `sign(q)*|q|^(4/3)`, §4.6.11.3.1 IMDCT `2/N` (effective via `2/input_n × 0.5` S16 stage) — all three audited and confirmed spec-correct. The previously-claimed "~3.33× LC peak gap" was a **peak-metric artefact**: ffmpeg's encoder fills HF bands with PNS noise (codebook 13) that our spec-compliant decoder reconstructs as additive noise. **RMS interop within ±5% of unity across all four directions** on a 440 Hz / 0.3 amp / 44.1 kHz mono test (ours→ours 0.97×, ours→ffmpeg 0.96×, ffmpeg→ours 0.99×, ffmpeg→ffmpeg ref 1.00×). New `tests/lc_rms_interop_r19.rs` pins the 0.9..1.1 band. Encoder `MDCT_FORWARD_SCALE = 65_536` re-derived from §4.5.2.3.6. 166 tests + 1 ignored (the unrelated HE-AACv1 SBR amplitude saturation, r20+ target). |
