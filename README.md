...
| **AAC** | 🚧 ~85% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR/ICS/ADTS bounds; lacks LD/ELD, USAC, SBR upsampling at output boundary (#771) | 🚧 ~80% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy default-on + TNS for CPE stereo (sparse-spectrum-gated; +0.65/+0.95 dB on transients) |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~80% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B slice (BI average) + sub-pel MC ½/¼-pel + multi-ref DPB (MAX_REF_PICS=4, 58.41 dB on 3-frame P-slice); lacks weighted bi-pred, chroma sub-pel — see crate README |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle (per-plane uniform-fill envelope + libavcodec-version probe) | 🚧 ~62% — keyframe + intra-mode RDO + P-frame inter (LAST_FRAME ref + ½/¼-pel ME, 55.23/55.88 dB) + quadtree partitions to 16×16 with PARTITION_{NONE,HORZ,VERT,SPLIT} via SAD+λR (+3.55 dB on per-8×8 motion fixture); 8×8 is NONE-only |
...
| **Cinepak** (CVID) | ✅ ~95% — frame header + multi-strip + V1/V4 codebooks + intra + inter with skip + full selective-update family + grayscale + Sega FILM demuxer | ✅ ~96% — stateful `CinepakEncoder` with rolling codebooks + multi-strip + skip-MB + two-pass rate ctrl + per-MB V1/V4 Lagrangian RDO + LBG codebook refinement + luma-weighted distance + median-cut seeding; 42.39 dB on 64×64 — **+5.49 dB over ffmpeg's reference encoder** |
...
| **PDF** | ✅ ~94% — bytes → Scene via xref/xref-streams/ObjStm + `/Prev` incremental + `/Encrypt` R=2..6 + per-stream `/Crypt` + public-key `adbe.pkcs7.s3/s4/s5` (5 curves) + PKCS#7 verify + `/Sig` AcroForm verify + text extraction (ToUnicode CMap + Identity-H + WinAnsi/MacRoman) + Linearization §F.2 + Tagged-PDF reading-order; see crate README | ✅ ~98% — PDF 1.4/1.5 multi-page + paths/gradients/opacity/clip + RGBA + xref-stream + ObjStm + Linearization writer + `/Encrypt` ENCODE all revisions + public-key ENCODE + `/Sig` writer §12.8.1.1 (RSA+ECDSA) + AcroForm widget writer §12.7.4 + annotation writer §12.5.6 (Text/Link/FreeText/Highlight family/Stamp/Square/Circle/Ink, 8 kinds; qpdf --check accepts) |
...
| MSADDS audio | `msadds32.ax` | winxp | 🚧 **Pipeline driven through Receive, E_FAIL inside inner-decode (r69)** — full PE-load + COM + dual-pin allocator handshake green; ffmpeg-derived extradata flips Receive HRESULT 0x8000FFFF → 0x80004005 (proves codec reads codec-private-data). **r69 falsified the 4 NULL-arg guards at `0xc898..0xc8b7`** — all pass; real blocker now localised to `0xe0f4` chain (call at `0xea3a` returns non-zero, steering to E_FAIL emission at `0xe2bb`). r63 helper_addref patch confirmed retirable. See crate README for round ladder. | Same scaffolding as MP43 video; `AmtBlueprint::wma_{criteria_passing,with_ffmpeg_extradata_prefix}()`; QueryAccept disasm at `docs/codec/msadds32-query-accept-validation.md` |
