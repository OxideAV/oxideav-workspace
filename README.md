...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF + ISO BMFF meta (pitm/iinf/iloc/iref/ipro) + HEIF/HEIC item-properties + derived images (grid/iovl/iden/tmap) + 29-variant BrandClass + Movie Fragment decode (§8.8) + symmetric muxer (non-fragmented + DASH/CMAF fragmented) + fragmented-MP4 seek via tfra/mfro/tfdt; ffprobe-accepted |
...
| **Opus** | 🚧 ~88% — TOC + CELT + SILK NB/MB; libopus interop 10-26 dB | ✅ ~86% — CELT full-band + SILK NB/MB/WB + Hybrid + NLSF stage-1 codebook search §4.2.7.5.1 (5.1% residual reduction on vowel /a/, hysteresis-anchored to avoid LPC thrashing); ffmpeg + libopus cross-decode clean |
...
| **AAC** | 🚧 ~85% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR/ICS/ADTS bounds; lacks LD/ELD, USAC, SBR upsampling at output boundary (#771) | 🚧 ~82% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy default-on + TNS for CPE stereo + SCE mono (sparse-spectrum-gated; +0.92 dB on transient masked-tail) |
...
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/1/2/3 + LFE + SSF/SNF + SAP + Pseudocode 121 companding + IMS bitstream_version≥2 walker; lacks ETSI fixture RMS audit, object/a-joc substreams | 🚧 IMS ~45% — v0/v2 TOC + mono SIMPLE/ASF (MDCT/KBD + HCB1..11 + DP-optimal sections + SNF; 27.5 dB white-noise SNR) + stereo SIMPLE 2× SCE split-MDCT (24.8 dB) + joint M/S CPE (34.5 dB on matched L=R; correlation-heuristic dispatcher at 0.7 threshold); lacks multichannel forward analysis |
...
| **FFV1** | ✅ 100% — bit-exact ffmpeg | ✅ 100% — bit-exact ffmpeg + range-coded YUVA encode (-6.7% on 720p YUVA fixture vs Golomb-Rice baseline) |
...
| **H.266 (VVC)** | 🚧 ~52% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + chroma 4-tap sub-pel decode; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~85% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B slice + sub-pel MC ½/¼-pel (luma 8-tap + chroma 4-tap §8.5.6.3.4, PSNR_Cb 49.96/Cr 50.39 dB) + multi-ref DPB (MAX_REF_PICS=4) + weighted bi-pred §8.5.6.5 (per-CU SSE RDO) — see crate README |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle (per-plane uniform-fill envelope + libavcodec-version probe) | 🚧 ~64% — keyframe + intra-mode RDO + P-frame inter (LAST_FRAME ref + ½/¼-pel ME) + quadtree partitions to 8×8 with full four-way RDO at every level: 16×16 {NONE,HORZ,VERT,SPLIT} + 8×8 {NONE,HORZ B8x4,VERT B4x8,SPLIT B4x4} via §6.4.16 (idy,idx) sub-block walk; ≥1 dB gain on per-4×4 stripes |
| **AV1** | 🚧 ~72% — OBU + range coder + all intra preds + CDEF + LR + inter MC + palette + multi-ref compound + super-res + tx_type read-gating + §5.11.4 partition force-split + §5.11.39 sign-loop interleave; SVT-AV1 48/48; lacks intrabc; sign-bit divergence at call_idx=27 vs dav1d unresolved (black-box CLI ceiling reached, see `docs/video/av1/specs/dav1d-range-coder-divergence-call-idx-27.md`) | 🚧 ~60% — forward range coder + DCT-II 8/16/32 + coefficient emitter + partition/mode/TX emit; **dav1d 1.5.x cross-decodes every single-SB square 8×8..64×64** via spec-derived `bsl_ctx` per §5.11.4 + chroma `txb_skip_ctx=7` per §5.11.39 |
...
| **Cinepak** (CVID) | ✅ ~95% — frame header + multi-strip + V1/V4 codebooks + intra + inter with skip + full selective-update family + grayscale + Sega FILM demuxer | ✅ ~97% — stateful `CinepakEncoder` with rolling codebooks + multi-strip + skip-MB + two-pass rate ctrl + per-MB V1/V4 Lagrangian RDO + LBG refinement + luma-weighted distance + median-cut seeding + post-classification Lloyd polish + 3×3 RD grid picker over (strips, λ); 43.44 dB on 64×64 — **+6.55 dB over ffmpeg's reference encoder** |
...
| **PDF** | ✅ ~95% — bytes → Scene via xref/xref-streams/ObjStm + `/Prev` incremental + `/Encrypt` R=2..6 + per-stream `/Crypt` + public-key `adbe.pkcs7.s3/s4/s5` (5 curves) + PKCS#7 verify + `/Sig` AcroForm verify + text extraction (ToUnicode CMap + Identity-H + WinAnsi/MacRoman) + Linearization §F.2 + Tagged-PDF reading-order + EmbeddedFiles name-tree reader §7.11; see crate README | ✅ ~98% — PDF 1.4/1.5 multi-page + paths/gradients/opacity/clip + RGBA + xref-stream + ObjStm + Linearization writer + `/Encrypt` ENCODE + public-key ENCODE + `/Sig` writer §12.8.1.1 (RSA+ECDSA) + AcroForm widget writer §12.7.4 + annotation writer §12.5.6 (8 kinds) + embedded file attachment writer §7.11 (FlateDecode-compressed `/EmbeddedFile` + `/Filespec` + name tree + optional `/FileAttachment` annotation; qpdf accepts) |
...
| **`oxideav-audio-filter`** | Audio effects & conversions (streaming) | ✅ ~36 filters: classic + transient/spatial/restoration family + MidSide / EnvelopeFollower / DeEsser / Wah / OctaveDoubler / AdaptiveNoiseGate — see crate README for the catalogue |
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~103 filter types / 125 factory names — see crate README for the catalogue |
