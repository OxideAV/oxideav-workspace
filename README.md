...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~92% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + auto MS/LR + auto SM + outer-loop on Long/Short/Mixed/Start/End + auto-block-type mixed promotion + r163 §2.4.3.4.9 cross-channel-MS block-type agreement on all four toggles (589 tests); lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~91% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §13.4 fitter + r163 §9.4 mb_lf_adjustments × §11 intra-pick composed on stream refresh path (544 tests) |
...
| **AV1** | 🚧 ~55% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.7/8/9/10/11/12/13/19/20/55/56 leaves + r163 §5.11.21 get_segment_id() predicted-segment-id helper (532 tests); lacks §5.11.5 decode_block body + §5.11.18 inter_frame_mode_info dispatcher | 🚧 scaffold |
