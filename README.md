...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF atoms (gama/clap/pasp/colr/edts/elst/ctts/cslg/tref) + chan layout + faststart + udta + dref + tkhd matrix → rotation + chapter resolution (`chapters_for(track)` with UTF-8 + Mac-Roman fallback + `encd` text-encoding override) + gmhd/text/tmcd headers + tmcd-in-stsd timecode parser + alias-chain following (`open_with_aliases` follows one `rmra/url` hop); rejects fragmented MP4 with hint to `oxideav-mp4` |
| AVI       | ✅ | ✅ | ✅ | OpenDML 2.0 super-index + AVIX continuation + dmlh + vprp + LIST rec clusters + 2-field interlaced (`AVI_INDEX_2FIELD` 12-byte entries) + 02ix mid-`movi` index emit/parse + truncated-head recovery + VBR audio + LIST INFO emit (incl. multi-value `avi:info.<fourcc>`); idx1 + ODML std-index seek |
...
| **AC-4** (Dolby) | 🚧 ~92% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/cfg1/cfg2/cfg3 dispatch + LFE body + SSF/SNF + SAP Table 183 a/b/c/d coefficient extraction (sap_mode 0/1/3 per Pseudocode 59) + Ls/Rs surround mono walker (ACPL_1 joint-MDCT residual pair persisted + IMDCT'd into surround PCM); lacks ASPX bandwidth-extension + companding for cfg2 four-channel | — |
...
| **Shorten** (.shn) | ✅ ~95% — all 10 FN cmds + filetypes 1-11 + format-v1 + running-mean estimator + 64-bit bit-reservoir reader using hardware `lzcnt`/`clz` (2.13× full-decode throughput) | ✅ ~85% — production encoder + Levinson–Durbin LPC + BITSHIFT lossy + lossy `-n N`/`-r N` bit-budget modes |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~50% — forward CABAC + DCT-II + per-CTU SAO RDO + ALF luma per-CTB filter-set RDO + primary chroma + CC-ALF chained into IDR pipeline + ALF bitstream emit (slice/CTU CABAC for `alf_ctb_flag[]` / `alf_use_aps_flag` / `alf_luma_*_idx` / `alf_ctb_filter_alt_idx[]` / `alf_ctb_cc_*_idc`) + quantised APS NAL emission (chroma-ALF + CC-ALF prefix-APS); decoder ALF parse round-trip verified |
...
| **VP8** | ✅ 100% — entire 15-fixture corpus bit-exact | 🚧 ~97% — I+P + B_PRED + SPLIT_MV + alt-ref/golden + Lagrangian RDO + libvpx-shape Trellis + activity AQ + RFC 6386 §15.2 mode/ref deltas (per-MB filter-level varies by ref/mode) + opt-in 4×4 B_PRED RDO + opt-in UV-mode RDO + opt-in joint LF-RDO with real rate term + opt-in psy-RDO/ARNR |
...
| **Lagarith** | ✅ ~95% — all 11 wire types (1-11 + NULL replay) + modern + legacy adaptive-CDF range coder + Fibonacci-Zeckendorf prefix + JPEG-LS Median + G-pivot decorr + zero-run RLE; lacks pair-packed 513-entry CDF | 🚧 ~70% — encoder for SOLID/RGB/RGBA/YV12/YUY2/legacy-RGB; byte-exact vs proprietary encoder Auditor-blocked |
...
| **Cinepak** (CVID) | ✅ ~95% — frame header + multi-strip + V1/V4 codebooks + intra + inter with skip + full selective-update family + grayscale + Sega FILM demuxer | ✅ ~85% — stateful `CinepakEncoder` with rolling codebooks (selective-update / full-replace / chunk-omit; 91.6% wire saved on static-fixture) + multi-strip + median-cut + skip-MB + two-pass rate ctrl + windowed bisection over N-frame budget + tighter Lloyd refinement (`lloyd_max_iter` + `lloyd_eps`); ffmpeg AVI roundtrip 36.9 dB |
...
| **WebP VP8** | ✅ 100% — via VP8 + bit-exact YUV→RGB + fancy chroma upsample + streaming `WebpAnimDecoder` (`new`/`next_frame`/`reset`/`info`/`done` pull-driven; cumulative `pts_ms` + on-disk frame bbox + dispose/blend) | 🚧 ~95% — VP8 I-frame + ALPH + per-segment QP/LF + Trellis + animated ANIM/ANMF with file-level ICCP/EXIF/XMP + standalone `encode_vp8l_argb_with_metadata` + `encode_vp8_lossy_*` + registry-side `make_encoder_with_metadata` parity for both VP8 + VP8L `default-features = false` entry points; dwebp cross-decode clean |
...
| **JPEG XL** | 🚧 ~83% — ISO/IEC 18181-1:2024 final core. 5 small lossless fixtures decode PIXEL-CORRECT. Modular path complete; VarDCT scaffold (Quantizer + LfCoefficients + F.1 LF dequant + F.2 adaptive smoothing + HfMetadata + DctSelect derivation + HfBlockContext custom branch + HfGlobal C.6.2 dequant-matrix encoding modes per Listing C.10) wired into pipeline; lacks HfPass C.7 + PassGroup HF C.8.3 + GetDCTQuantWeights materialisation + IDCT dispatch + XYB / CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **DDS** | ✅ ~98% — DDS_HEADER + DXT10 + uncompressed (10 layouts) + BC1-5/7 + BC6H all 14 modes + mipmap + 6-face cubemaps + DX10 arrays + full 132-entry DXGI table | ✅ ~88% — uncompressed + BC1-5 + BC7 modes 0/1/2/3/6/7 (1-/2-/3-subset; rank-3 multi-axis 30.4 dB) + BC6H all 14 modes (1-subset 10/11/12/16-bit base + 2-subset 32-partition sweep with iterative LSQ refinement; multi-axis 8×8 25.5 dB, tight-gradient >35 dB, 2-cluster >40 dB) + box-downsample-then-encode mip chains + cubemap/array; lacks BC7 mode 4/5 channel-rotation, BC6H_SF16 signed-format encoder |
...
| **SVG** | ✅ ~95% — full shape set + path + g + defs + gradients + text/tspan + mask + clipPath + use/symbol + svgz + SMIL animate/set/animateTransform at arbitrary `t` (paced + spline calcMode) + CSS3 Selectors L3 cascade + SVG2 §9.3.2 `d`-as-CSS + filter primitive rasterisation graph (10 primitives — feGaussianBlur / feOffset / feFlood / feComposite / feBlend / feMorphology / feColorMatrix / feMerge / feComponentTransfer / feDropShadow) | ✅ ~85% — round-trips shape/stroke/fill/gradient/transform/mask/clipPath + PreservedExtras side-channel for `<style>`/`<filter>`/`<animate>`/`<foreignObject>` fragments |
| **PDF** | ✅ ~82% — bytes → Scene via legacy xref + cross-reference streams (`/Type /XRef`) + object-stream resolver (`/Type /ObjStm`) + recursive object parser (FlateDecode + PNG predictors) + content-stream operator parser + `/Prev` chain incremental updates + `/Encrypt` decode all revisions (R=2/3/4 RC4 + AES-128, R=5 Adobe AES-256, R=6 ISO 2.0 AES-256) + per-stream `/Crypt /Identity` opt-out; lacks public-key handlers (adbe.pkcs7.*) | ✅ ~88% — PDF 1.4/1.5 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + xref-stream encoder + ObjStm encoder + incremental updates + Linearization (Annex F: lin-dict in first 1024 bytes + first-page xref + page-offset hint stream + main xref) + ObjStm+encryption combined path + `/Encrypt` ENCODE all revisions with bounce round-trip; lacks text, JPEG passthrough |
...
| MSMPEG4 v3 (DIV3) | `mpg4c32.dll` | wmpcdcs8-2001 reference binary | ✅ ICERR_OK on keyframe 0 (176×144 → BI_RGB 24bpp non-zero pixels written) | Required: 13 stubs + `Registry::register_data` channel (`_adjust_fdiv` data symbol) + x87 ISA (8-deep f64 stack, FLD/FST/FILD/FIST/FADD…/FXCH/FCHS/FNSTSW/FLDCW + FSIN/FCOS/FPREM/FSCALE) + lowercase FOURCC pre-DRV_OPEN + DirectShow GUID handshake (`b4c66e30-0180-11d3-bbc6-006008320064` planted at `[esi+0xb4..0xc8]` before DriverProc DRV_OPEN, gated on MP43/MP42/MPG4 fccHandler). Bit-perfect cross-check vs ffmpeg + P-frame walk are r23 work |
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | PE-load + DllMain ✓ | DirectShow filter ABI is round-22+ work |
| WMV1/2 DShow | `wmvds32.ax` | winxp | PE-load + DllMain ✓ | Same DirectShow ABI gap |
