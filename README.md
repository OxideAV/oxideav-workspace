...
| **Indeo 3** (IV31/IV32) | 🚧 r4 — clean-room codec-frame header + bitstream header + spec/02 picture-layer plane-prelude parser (per-plane `num_vectors` + `mc_vectors[]` with §3.3 half-pel scaling + packed-MV formula + NULL-frame skip) + spec/03 macroblock-layer binary-tree walk (MC_TREE H/V split + VQ_TREE → typed INTRA/INTER cell tree) + spec/04 VQ codebook materialisation (static dyad/seed tables + packed-entry decode + per-frame alt_quant arena overlay); lacks per-cell unpack/RLE (spec/06) / MC (spec/05) / pixel reconstruction (spec/07) | — |
