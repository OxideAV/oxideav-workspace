...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal-method compositor (§23) + structured Application Extension parsing (NETSCAPE2.0 loop+buffer, Adobe XMP, ICC profile §26.c.iv/v) + lazy `Playback` / `LoopingFrameIter` (NETSCAPE2.0 semantics: 1 pass / infinite / N+1) — clean-room rebuilt from CompuServe spec + Welch 1984 only | ✅ 100% — per-frame palettes |
...
| **USDZ** (+ USDA) | ✅ ~88% — own ZIP STORED walker (rejects DEFLATE entries; validates 64-byte alignment) + USDA tokenizer + prim-tree parser + UsdGeomMesh → Mesh+Primitive + UsdPreviewSurface → PBR + UsdUVTexture (`ZipStoredAsset` exposes `raw_storage("zip-stored")` for pass-through) + upAxis + metersPerUnit + per-mesh `xformOp:translate/orient/scale/transform` + multi-primitive sibling-Mesh emit-and-fold + `usd:no_fold` flag + UsdMediaSpatialAudio reader + `variantSet` structured parsing + **variant-selection composition with LIVRPS strength** (a `def Mesh` inside a selected variant materialises as Scene3D Mesh+Node; `Prim::resolved_variants()` exposes the composed view) + line:column error reporting; lacks `.usdc` binary backend (#754), UsdSkel*, UsdGeomSubset, variant *writer* (read-side composition only) | ✅ ~85% — symmetric ZIP STORED writer with 64-byte alignment + USDA writer + `EncodeReport.pass_through_textures`/`pass_through_audio` confirm zero-re-encode USDZ→USDZ for textures + audio when source `raw_storage("zip-stored")` matches archive scheme + strips/fans tessellation + `UsdGeomBasisCurves`/`Points` for line/point topologies |
...
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~57 filter types / 69 factory names (blur / edge / resize / sharpen / unsharp / gamma / brightness / contrast / level / normalize / posterize / solarize / threshold / negate / sepia / modulate / grayscale / motion-blur / emboss / flip / flop / rotate / crop / vignette / colorize / equalize / auto-gamma / tint / sigmoidal-contrast / implode / swirl / despeckle / wave / spread / charcoal / convolve / polar / morphology (dilate/erode/open/close) / perspective / distort / tilt-shift / composite (16 Porter-Duff + PDF §11.3.5 ops) / roll / shave / extent / trim / channel-extract / morphology-edge{in,out,magnitude} / evaluate / cycle / statistic / affine / srt / clamp / auto-level / contrast-stretch / linear-stretch / color-matrix (3×3+offset) / function (Polynomial/Sinusoid/ArcSin/ArcTan parametric map) / laplacian (3×3 second-derivative → Gray8) / canny (4-step Gaussian→Sobel→NMS→hysteresis edge detection)) |
