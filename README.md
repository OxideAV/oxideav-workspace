...
- **Infrastructure** — `oxideav-core` (primitives: Packet / Frame / Rational /
  Timestamp / PixelFormat (incl. 16-bit planar YUV + deep Yuva 4:2:2/4:4:4 at 10/12/16-bit; VideoFrame palette + per-plane significant-bits side-channels for mixed-depth streams; r429: deep Yuva 4:2:0 + 8/16-bit planar GBR(A) formats + Ogg BOS-magic codec resolution) / ExecutionContext + **DoS framework: `DecoderLimits`
...
| **Musepack** | ✅ ~92% — SV7 AND SV8 stream→PCM ±1 LSB vs two oracles (§6.5 wide-M/S enumerative fix + gapless window re-pinned black-box); lacks CNS noise waveform (oracle-generator docs gap) + SO/ST seek payloads | ✅ ~80% — from-PCM SV8 encoder (32-band analysis, absolute-law quantisation, M/S election, whole-stream writer w/ pinned SH CRC-32; 81–85 dB SNR, reference-decoder-aligned playback, ffmpeg parity ≤1 LSB) + SV7 frame-body corpus-conformant + registry dual-API; lacks SV7-from-PCM + psymodel |
