...
| **JPEG XL** | 🚧 ~45% — `jxlp` container + 2019 committee-draft Modular + 2021 FDIS through round 11 (ANS + LfGlobal + GlobalModular + cl_code + kRCT/kPalette refactored to Appendix B + kSqueeze); decoder forward progress blocked on a strict-isolation cleanroom workspace (libjxl-trace-reverse-engineering.md retired 2026-05-06 under fruits-of-poisonous-tree; needs Specifier wall that never read libjxl source) | 🚧 ~42% — round 6 widens per-image predictor candidate set across FDIS Listing C.16 ids {1..=5, 7..=12}: Left / Top / Average / West-Predictor / Gradient + TopRight / TopLeft / LeftLeft / Avg(L,NW) / Avg(NW,T) / Avg(T,TR); 256×256 grey natural-image fixture 4.12 bpp (51.5% of raw), bit-exact lossless via own decoder + libjxl `djxl`; predictor 13 (Six-Tap) excluded — diverges from djxl on natural data; lacks predictor 6 (Annex E Weighted), multi-leaf MA tree, lossy VarDCT |
