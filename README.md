...
| Format              | Decode | Encode | Notes |
|---------------------|:------:|:------:|-------|
| **ASS / SSA**       | ✅ | ✅ | Script Info + V4+/V4 Styles (BGR+inv-alpha) + override tags + r172 `\fn`/`\fe`/`\b<weight>`/`\r[<style>]` + r177 `\pbo` + r183 face-flag toggles + r186 typed `\p<scale>` + r198 `\fax`/`\fay` shear baked into per-cue affine + r204 `\an<n>` numpad alignment baked into renderer + r210 **`\1a` primary-fill alpha baked into rasterised fill** (`AnimatedRenderedDecoder` maps the override-spec wire byte via `final_a = 255 - ass_a` per the `0 = opaque, 255 = transparent` convention; cue-level `\fad` / `\fade` envelope stays on the animation `Group`'s `opacity` so the two compose multiplicatively per `final_primary_alpha = primary_alpha.unwrap_or(style) * alpha_mul`; opt-out via `default-features = false`; +4 integration tests pin `\1a&H00&`/`\1a&H80&`/`\1a&HFF&` ink-mass + `\1a&H80&\fad(500,500)` at `t=0`; 231 tests) + bundled rule-E scrub of pre-existing brand-named comment in `tests/ass_parse.rs:231` (#1333) |
