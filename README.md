...
| **Dirac / VC-2** | ✅ ~97% — VC-2 LD+HQ + Dirac intra/inter + OBMC + 7 wavelets + 10/12-bit + fragmented pictures + asymmetric transforms; bit-exact intra | 🚧 ~97% — HQ+LD + sub-pel 2-ref bipred + rate control + inter sequence-level rate driver (PerPicture/Cbr/Vbv/VbvHysteresis residue-byte) + §11.3.3 spatial-partition codeblock grid (per-codeblock differential quantiser; bit-exact ≥4×4-sample; 1-ref + bipred; codeblock-aware rate control) + asymmetric transforms |
...
| **Ut Video** | ✅ ~97% — 5 FourCCs × 4 predictors + slice-parallel decode (5.6× at 720p) + opt-in strict-padding conformance decode + panic-free inspector Kraft accessors | ✅ ~96% — slice-parallel encode (3.3×) + fuzz oracle |
| **MagicYUV** | ✅ 100% | ✅ 100% |
...
| **Indeo 2/4/5** | 🚧 ~5% — Indeo5 (IV50) decode bootstrap (LSB-first bitreader + file/GOP/frame/band/picture headers) + entropy/transform primitives (canonical-Huffman codebook + zig-zag level table + CDF 5/3 LeGall wavelet recompose + saturation-clip table + per-band tile geometry); Indeo 4/5 also run sandboxed via `oxideav-vfw` | — |
