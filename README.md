...
| **H.264** | 🚧 ~75% — I/P/B + 4:2:0/4:2:2 + CAVLC + CABAC + DPB + MMCO/RPLM + B-pyramid POC + SEI T.35/pan-scan + Level 1b DPB sizing; lacks 4:4:4, MBAFF, SVC/3D/MVC | 🚧 ~55% — I + P (1MV/4MV, ¼-pel) + B-slice (16×16/8×8 mixed) + weighted pred + 4:2:2/4:4:4 IDR + CABAC for I/P + level_idc auto-derive; ffmpeg cross-decode 50-52 dB on B-frames; lacks B-slice CABAC encode, P/B at 4:2:2/4:4:4, multi-ref temporal direct, full mvp under CABAC |
| **H.265 (HEVC)** | 🚧 ~65% — I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2/4:4:4 + SAO + deblock + scaling lists; HEIF/HEIC opt-in (corpus 5/14 BitExact via tolerance tier); broader B-slice merge/AMVP audit pending | 🚧 ~60% — Baseline CAVLC I+P + Main CABAC P + B-slice (mini-GOP=2) + per-CU SAD-RDO + AMP partitions + Main 10/12 IDR + Main 4:4:4 8/10/12 IDR (45 dB ffmpeg); lacks Nx2N/2NxN, mvd_l1_zero_flag, mini-GOP > 2 at 10/12-bit, P/B at 4:4:4 |
...
| **AV1** | 🚧 ~70% — OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound + spec-correct update_cdf + HasChroma/cfl in keyframe + inter intra paths; SVT-AV1 chain 48/48; lacks intrabc, full primary_ref_frame=4 chain | 🚧 ~25% — forward range coder + intra DC + 4×4 DCT + coeff emit + dav1d cross-validation (profile 0 8-bit 4:2:0, ≤64×64); lacks B-frames, full inter |
...
| **WebP VP8L** | ✅ ~95% — full lossless + spec-correct mode-5 predictor (libwebp interop) | ✅ ~85% — subtract-green + predictor + colour transform + 32-trial RDO + K=4 meta-Huffman + near-lossless smoothing + simple-Huffman + palette RDO bias; 1.17× cwebp on photo, 0.98× on palette |
| **WebP VP8** | 🚧 via VP8 decoder (VP8 decoder is complete) | 🚧 ~75% — VP8 I-frame + ALPH + per-segment QP tuning by quality; animated encode + per-frame mixed lossy/lossless (Auto picks smaller); webpinfo + dwebp cross-decode clean |
