...
| **AVIF** | 🚧 ~85% — HEIF→AV1 + grid + imir/clap/colr/pixi/pasp + HDR metadata + AV1 wrap pass-through + DoS caps + HEIF item-properties + auxC URN routing + rloc / lsel / iovl / grpl parsers + `mif1` compliance audit + a1op/a1lx AV1 layered-image properties + essential-property enforcement + r130 tmap §4.2.2 file-shape audit + r172 §7 grid-derivation audit + r182 av1-avif §2.1 Sequence Header OBU count audit + r188 ISO 21496-1:2025 Annex C.2 `GainMapMetadata` parser (tmap descriptor body: version/multichannel/HDR-headroom + per-channel min/max/gamma/offset rationals, all C.2.3 shalls enforced); AV1 pixel decode + gain-map composition gated on sibling rebuild | — |
