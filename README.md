...
| **MPEG-2 video** | 🚧 r1 (post-2026-05-18 orphan) — §6.2.2.1 sequence_header parse (dim/aspect/frame-rate/bit-rate/VBV + optional quantiser matrices); lacks sequence_extension / GOP / picture / slice / IDCT | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 r1 (post-2026-05-18 orphan) — §6.2 VS/VO/VOL header parse (profile_level + dim + time_increment_resolution + aspect_ratio + vbv_parameters); lacks GOV/VOP/MB decode | 🚧 scaffold |
| **Theora** | 🚧 r1 (post-2026-05-20 orphan) — Theora I Spec §6.1 common header + §6.2 identification header (sync / VMAJ-VMIN-VREV / FMBW-FMBH / PICW-PICH + bounds / FRN-FRD / PARN-PARD / CS / NOMBR / QUAL-KFGSHIFT-PF / Res) + NSBS/NBS/NMBS Tables 6.5/6.6; lacks comment/setup header + frame decode | 🚧 scaffold |
...
| **SVQ1** (Sorenson) | 🚧 r1 (orphan rebuild) — clean-room frame-header parse (22-bit FC + tref + picture type + I-frame trailer + checksum/unknown-flag chains, 21 tests); blocked on docs (§14.10/§14.11 codebook bytes #429) | — |
