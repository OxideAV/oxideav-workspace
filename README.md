...
| MPEG-TS   | ✅ | — | — | ISO/IEC 13818-1 transport stream — packet/PSI/descriptor walk; Table 2-17 PES header fully decoded incl. PES_extension body (private data, pack_header, packet-sequence counter, P-STD buffer) |
...
| **Vorbis** | 🚧 ~86% (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook + §4.2.4 setup walker + §3.2.1/§3.3 VQ unpack + §8.6 residue (0/1/2) + §7.2 floor 1 + §6.2 floor 0 LSP + Vorbis window + §4.3.5 channel coupling + §4.3.6 `window_premultiply` primitive + §4.3.7 IMDCT + streaming overlap-add | 🚧 ~42% — identification/comment/codebook/floor 1/floor 0/residue/mapping/mode header WRITE + §4.3.1 audio-packet header WRITE primitive + §4.3.7 forward-MDCT cosine-summation kernel + §7.2.3 floor 1 audio-packet body WRITE primitive + HuffmanTree::encode_entry + §4.3.8 `FrameSplitter` encoder-side framing-inverse primitive (windowed-block extraction with alignment recurrence) + §4.3.5 forward channel coupling primitives (algebraic inversion of decoder rule) + §4.3.6/§4.3.7 `apply_window_and_mdct[_vec]` composition primitive (window pre-multiply + forward MDCT, decoder-side `apply_imdct_and_window` mirror) + §8.6.2 residue classification packing primitive (per-partition classification → codeword packing for the audio-packet residue body); + §8.6.3-5 per-partition VQ codeword WRITE + §8.6.2 wrapping residue-body writer (classbook + partition interleave in decoder read order); lacks VQ-encode stage + §4.3 audio-packet splice + setup-header splice |
| **Opus** | 🚧 ~42% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + §3.1/§4.2 framing dispatch + §4.5 state-reset + §4.5.1.4 redundant-frame params + §4.3.2.1 CELT coarse-energy Laplace E_PROB_MODEL + §4.3.3 CACHE_CAPS50 + alloc-trim PDF + per-band allocation-trim offsets + band-boost + reservation block + per-band `thresh[]` + Appendix B self-delimiting framing (Figures 25..29) + §4.5.3 normative + recommended-non-normative transition tables (Figures 18+19 + `BoundaryOp` markers) + §4.3.3 STATIC_ALLOC Table 57 (21×11 Q5 grid + Q5→Q3 unit conversion) + §4.3.3 1/64-step interpolated allocation search (`search_q_fp` returning highest non-budget-exceeding `q_fp`) + §4.3.4.2 PVQ codebook-size `V(N,K)` recurrence (`pvq_codebook_size` O(N·K) time / O(K) space, §4.1.5 `ec_dec_uint ≤ 2³²−1` overflow guard) + §4.3.4.3 spreading rotation (Tables 56/59) + per-LM inter-mode `(α,β)` coarse-energy prediction (Appendix-A quant_bands tables); §4.3.4.1 pulse-cache now groundable via Appendix A | 🚧 scaffold |
| **MP1 / MP2** | ✅ Layer I + Layer II decode + §2.4.3.1 CRC-16 + frame loop + Annex D Phase-3 LTq + Model 2 spreading + free-format probe + ISO 13818-3 Annex B LSF Layer II allocation + Annex D Table D.3a partial calculation-partition anchor (first 20/63 rows) + Table D.1a LTq partial anchor (6 rows) + Step 3 composition; allocator still pending D.1/D.3/D.4 | 🚧 ~88% — Layer I encoder + §C.1.5.2.7 bit-allocation + §C.1.3 polyphase analysis + §C.1.5.1.4 per-part scalefactor + §2.4.1.6/§C.1.5.2 top-level Layer II encoder + Mp1Layer2FrameEncoder stateful PCM-in + §2.4.1.8 ancillary_data tail (CRC-safe) + Mp1Encoder Layer I/II dispatch switch; pending Table C.4 SCFSI |
| **MP2** | 🚧 ~50% (post-2026-05-24 orphan) — Layer II header parser + frame sizing + Annex B tables + joint-stereo + scfsi + sample requantizer + LSF Layer II + Table C.4 SCFSI encoder + write_audio_data + §C.1.5.2.7 bit-allocator + encoder sample quantizer + frame-level `encode_frame` orchestrator + §D.1 Step 1 Hann window + power-density spectrum + 96 dB SPL normalisation + §D.1 Step 4 tonality classifier (is_local_maximum / is_tonal_layer2 / tonal_spl_db) + §D.1 Step 6 vf masking-function + Step 7 LTg global masker + §D.1 Step 4(b) tonal-neighbourhood zero-out + Step 4(c) non-tonal listing (Tables D.2d/D.2e/D.2f Layer-II boundaries) + Step 5(b) sliding 0.5-Bark tonal decimation + Step 8 `MIN[LTg(i)]` per-subband reduction + Step 9 elementwise SMR; lacks D.1 Step 5(a) (#1262) + §D.2 Model 2 | 🚧 scaffold |
| **MP3** | ✅ ~100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~99% — Layer III + Annex D psychoacoustic: Phase-2 bit-allocation loop closed + Model 1 Steps 1-7 end-to-end + Model 2 COMPLETE + §C.1.5.3.2.1 Layer III adaptation COMPLETE (Tables C.7/C.8 transcribed, step m pre-echo control, PE>1800 window switching, dual-FFT granule walk); lacks intensity-stereo |
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element + §4.4.1 GASpecificConfig + Table 1.15 epConfig + §1.6.5 SBR/PS probe + §4.5.4.1 SWB tables + §4.6.13 pulse_data + §4.6.9.4 TNS clamp + §4.4.6 ics_body + §4.6.3 spectral codebook + HCB1..HCB11 (Table 4.A.10 / 4.A.11 / 4.A.12 — 169-entry expanded-LAV unsigned pair + 289-entry HCB11 ESC codebook 17×17 lattice, full Kraft equality) + §4.6.9.3 TNS `tns_decode_coef` (signed-magnitude sign-extension + sin-based inverse quantisation with iqfac/iqfac_m half-bit-offset branch) + LPC step-up loop + §C.6 encoder companion `tns_encode_coef` (NINT(arcsin·iqfac_branch) → clamp → pack) + §4.6.9.3 `tns_ar_filter` all-pole IIR pass (`y(n)=x(n)−Σ lpc[k]·y(n−k)` over a strided spectrum region, both filter directions; both TNS coef building blocks now landed) + §4.6.3.3 Table 4.56 spectral_data walker (full Table 4.50 channel body now parseable); pending tns_decode_frame region slicer + dequant/filterbank | 🚧 scaffold — Phase-2 writers: section_data/ics_info/pulse_data/tns_data/scale_factor_data/DPCM/raw_data_block/Pce/gain_control/extension_payload + §4.6.9 TNS decode tool complete (tns_decode_frame); SBR pending QMF |
| **CELT** | 🚧 ~31% (post-2026-05-20 orphan) — RFC 6716 range decoder + §4.3 prefix + coarse-energy scaffold + bit-allocation fields + tf_change/select/spread + post-filter + de-emphasis + Walsh-Hadamard + cache_caps50 + dynamic-band-boost + initial-reservations walk + per-band minimums + trim_offsets + Table 55 + Table 57 static-allocation matrix + static-allocation search + §4.3.4.2 PVQ codebook + per-band shape decoder + §4.3.4.1 bits-to-pulses search + balance accumulator + §4.3.4.3 spreading rotation chain (forward/reverse + pre-rotation + multi-block) + §4.3.4.4 PVQ band-split gating + recursion geometry + §4.3.2.1 `E_PROB_MODEL` 4×2×21=168-pair Laplace probability/decay table + typed `ProbDecay` accessor + §4.3.6 band denormalization + §4.3.7 IMDCT/window/WOLA synthesis chain + §4.3.2.1 `ec_laplace_decode` recurrence + coarse-energy decode COMPLETE (per-LM inter (α,β), budget-keyed fallbacks, encoder-inverted bit-for-bit over all 168 model cells; RFC 6716 Appendix A carve-out); NOTE pending 0.2.0 release — pin-propagation sweep required (#1648) | 🚧 scaffold |
| **Speex** | 🚧 ~38% — Ogg stream-header + NB + WB high-band + §5.5 in-band signalling + `BitWriter` + encoder-side write + 22 CELP companion-table accessors + NB LSP-VQ → Q10 LSP + §9.1 per-sub-frame LSP interp + NB 3-tap pitch-gain VQ + §10.4 Table 10.1 high-band excitation-gain index (gain magnitudes blocked on one scalar-quantiser docs gap) + WB-HB 2-stage LSP MSVQ Q10 + NB+HB fixed-codebook (innovation) sub-vector lookup + per-mode dispatcher (NB innovation decodes modes 2-6+8, only 1/7 unbound) + NB adaptive-codebook 3-tap pitch-prediction index resolver + §9.2 Eq. 9.1 adaptive-codebook contribution dot-product sum + §8.4 raw-excitation `e[n] = p[n] + c[n]` composition primitive; lacks §9.1 LSP→LPC + synthesis + UWB framing + mode-4 HB codebook binding | 🚧 scaffold |
| **GSM 06.10** | 🚧 ~90% — clean-room §5.3 RPE-LTP decode + §4.4 in-band homing + §1.7 unpack | 🚧 ~95% — full §5.2 RPE-LTP encode pipeline + §1.7 packer; `make_encoder` live, bit-exact lockstep with decoder; lacks §6 conformance-vector pinning (docs-blocked) + ICOUNT stagger |
| **G.711** (μ/A-law) | ✅ 100% | ✅ 100% |
| **G.722** | 🚧 clean-room SB-ADPCM decoder bring-up + BLOCK1/QMF predictor split + Table 19 RIL=11111 sign-anomaly fix + Figure 1 auxiliary-data channel + clause 2 transmission characteristics (typed normative-limits + dBm0 ↔ uniform-PCM bridge + idle-noise check) + clause 2.5.2 Figure 12 reconstructing-filter mask + clause 2.5.1 Figure 11 anti-aliasing filter mask (transmit-side companion to Figure 12) + clause 2.4.2 / Figure 10 codec end-to-end attenuation/frequency-distortion mask + clause 2.5.5 / Figure 14 S/D-vs-level masks + clause 2.5.4 receive idle-noise bound | 🚧 SB-ADPCM encoder bring-up + Mode-2/Mode-3 silence envelope round-trip + Appendix-II test-sequence harness (+ §II.3.2 Config-2 input #3) + QMF-bypass entry points |
| **G.723.1** | ✅ 100% | ✅ 100% — both 5.3k + 6.3k |
| **G.728** | 🚧 ~90% — clean-room LD-CELP decode: Annex A-D + Levinson + blocks 29-33/71/72 + §4.6/4.7 postfilter chain | 🚧 ~85% — analysis-by-synthesis loop complete (§3.9 codebook search + §3.10 memory update); `encode_vector` live, bit-exact lockstep with decoder; lacks ICOUNT stagger + §3.11 sync-bit + Annex G fixed-point |
| **G.729** | 🚧 ~25% — clean-room from staged trace #859: tables + serial parser + LSP-quantiser codebooks + corpus harness + §3.2.4 MA-predictor `fg` + LSP-frame reconstruction + per-subframe LSP interpolation + §3.2.6 LSP→LP conversion + §4.1 / Table-8 parameter unpacker (15 typed codewords + pitch parity) + §3.9.2 conjugate-structure gain-VQ decode reconstruction + §3.9.1 4th-order MA gain prediction + §3.9.3 gain-quantiser codeword inverse-permutation (imap1/imap2 demap before §3.9.2 codebook lookup) + §4.1.3 pitch-delay decode (P1, P2) → (T1, T2, t_min) with encode-inverse round-trip + §4.1 stateful per-frame parameter chain (serial frame → typed LSP/pitch/FCB/gain structs incl. §4.1.2 parity concealment; 18 222 conformance-corpus frames); lacks §4.1.6 excitation/synthesis (first PCM) + postfilter + Annex B DTX | 🚧 scaffold |
| **IMA-ADPCM (AMV)** | ✅ 100% | ✅ 100% |
| **MS-ADPCM / IMA-ADPCM (WAV)** | ✅ 100% | ✅ 100% — block-aligned WAV encoder for both nibble layouts + 3-bit DVI mode (both directions) |
...
| **NSF** (NES) | 🚧 ~97% — full 6502 + IRQ/NMI + 5/5 2A03 APU + DMC DMA + six expansion chips + NSF v1/v2/NSFe + Dendy + Namco 163 + VRC7 OPLL pipeline + register semantics + KSR (Key Scale of RATE) + §4 KSL byte base table (YM2413 Table III-5) + Table III-7 decay-rate per-sample Q16 step + Decay/percussive-Sustain/Release lookup + Table III-7 attack-time per-sample Q16 step (closes the Attack column of the per-RATE envelope ladder) + MMC5 PCM Mode/IRQ + VRC6 sawtooth 14-step + E-clear accumulator zero + NSFDRV driver-ID tag + VRC7/OPLL rhythm-mode register semantics ($0E decode, Table III-9 slots, drum-ROM carve-out, VRC7-inert proof); lacks §7 LFO AM/VIB step arrays (docs gap) | — synthesis only |
| **Shorten** (.shn) | 🚧 ~35% (post-2026-05-18 orphan) — `ajkg` magic + v2/v3 ulong + svar(n) + per-block function dispatch + VERBATIM/QUIT + DIFF0..3 + Rice residual + per-channel carry + running mean + QLPC predictor + `decode_stream` + `Decoder` trait + streaming decode + write_diff0/1/2/3_block + write_qlpc_block + write_zero_block predictor encoders + min_energy selectors + write_bitshift_command + write_blocksize_command (BLOCK_FN_BLOCKSIZE housekeeping) + encoder QLPC auto-selection (order search + least-squares coefficient derivation, full transmission-overhead cost model; 74% saving on periodic material) + Rice-n over-cap representability fix (latent undecodable-stream bug) + spec/05 §6 typed H_filetype accessor + per-block predictor-selection sequencer (DIFF0..3 + ZERO, energy-based cheapest pick) + spec/02 §2.1 Rice-`n` statistical-optimum energy selection (`residual_bits_at_energy` / `optimal_energy_for_residuals`); lacks QLPC auto-selection + #1267 | 🚧 scaffold |
...
| **Musepack** | 🚧 SV7/SV8 — SV8 canonical-Huffman decoder walk (all 21 tables decode) + §2.5/§2.6 requantiser constants + stream-magic recognisers + SV8 packet outer-frame walker + SV7 mpc_huffman + CNS PRNG + §2.5 per-band sample-decode dispatcher + §2.6 reconstruction primitives + §2.4 SCF coding-method decoder + §2.3 band-type header loop walker + SV8 packet-stream walker + typed §3.2 packet surface + `Sv8BandDecodeCase` structural case classifier (§3.4 8-variant ladder) + 21 SV8 canonical-Huffman length+symbol tables wired (Bands/Res/Scfi/Dscf/Q1..Q9up) + `Sv8TableRole`-keyed dispatcher + §3.4 per-case sample decoders (grouped + context cases); lacks §3.4 sparse/escape cases + SV7 fixed-header field map + §2.3-VLC→§2.5-case remap + 32-band synthesis | 🚧 scaffold |
| **Cook** (RealMedia) | 🚧 r13 — flavor table + cookie parser + 8 DSP parameter tables + open-time `DecodeConfig` + wire-level real-stream integration test + cookie→flavor multi-match API + selector-family classification + typed per-family GAP errors + stateful `CallSession` RADecode call-counter + PCM-cursor + structural `Driver`/`PreparedCall`/`decode_call` orchestrator + typed `CategoryIndex`/`CategoryParameters` parallel-table bundle + structural cookie-geometry guard + typed `Descriptor::recover_samples_per_frame` + typed `FlavorRecord::is_sentinel()` playable/sentinel discriminator + `iter_playable_flavor_records()` walker + RAGetNumberOfFlavors* audit-anchored constants + typed `BitAllocAxisPosition`/`BitAllocCategory` newtypes + `bit_alloc_category_for_position` 51→20 LUT accessor (audit point #14 `cook.dll!0x8c40`) + typed MDCT-window accessors + typed reciprocal-divisor accessor (every extracted DSP table now behind a range-guarded API); lacks backend frame-decode (bitstream syntax docs-gapped, 4-part ask filed) | — |
| **WMA** | 🚧 r13 — patent-disclosed primitives incl. §3 sine analysis/synthesis window pair + weighted overlap-add + §6 codebook grid + escape disposition + §4 patent-disclosed quantization-band layout + §6 end-of-block terminator selector + §4 per-block overall step-size carrier + typed `EscapeLiteral` carrier for §6 escape-symbol literal payload; lacks codeword Huffman tables / exponent partition / LSP codebook / sign-bit layout | — |
| **WavPack** | 🚧 ~94% (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + §4.2 entropy ladder fully typed (steps 1-7 + adaptation) + multi-block `decode_stream` PCM composer + typed correction/overflow (0x0B/0x0C) views + stream-introspection accessor sweep + spec-§4.2 inverse entropy encoder (`encode_packed_samples_{mono,stereo}` — bit-exact round-trip through the real decoder; building it exposed + fixed 3 decoder conformance bugs); lacks decorrelation prediction-loop (docs gap) + hybrid 0x0B/0x0C consumer (docs gap) + float + multichannel + CRC algorithm + block writer | 🚧 scaffold |
...
| **H.264** | 🚧 ~83% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 50 SEI types (Annex H/I block 50-54 complete) + fuzz-hardened + POC i64-staged + strict avcC parser + High-family extension trailer + CAVLC call-contract guards + Annex G MVC SEI 39/40/41/43 + NAL extension header (MVC/SVC/3D-AVC) + Annex H/I 3D-AVC SEI 50/51/53/54 + typed §H.13.2.4 `NumSampleShift` accessor on SEI 51 + typed §H.13.2.3 `depth_nonlinear_representation_num_segments` + `_model_len` accessors on SEI 50 + typed §D.2.13 `average_bit_rate_bps()` / `average_frame_rate_fps()` on SEI 11 + typed §D.2.13 `sub_seq_duration_seconds()` accessor on SEI 12 (90-kHz tick → seconds, `None` when duration_flag==0) + typed §D.2.18 `num_refinement_steps() -> u64` on ProgressiveRefinementSegmentStart SEI (payload type 16: 1+ue(v) → u64; matches spec semantics for the count of refinement frames in the upcoming sub-sequence) + §7.3.2.1.3 subset SPS (NAL 15) with full Annex G MVC SPS extension + MVC VUI (profiles 118/128/134; SVC/MVCD bodies typed-deferred); lacks MBAFF, NAL 20 slice path, SVC/MVCD bodies | 🚧 ~83% — I+P (1MV/4MV, ¼-pel) + B + CABAC all chroma layouts + Trellis-quant RDOQ-lite; PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 ~65% — VPS+SPS+PPS + scaling-list + scan + §9.3 CABAC + slice header through §7.3.6.3 pred_weight_table + §7.3.6.2 ref_pic_lists_modification + §7.4.8 inter-RPS-prediction + §7.3.2.3.1 PpsExtensionFlags + §9.3.4.2 binarization + Table 9-49 split_cu/cu_skip + six Table 9-48 ctxInc + §7.3.2.2.1 SPS extension + §7.3.4 sao() per-CTU + §9.3.4.2.5 sig_coeff_flag + §9.3.4.2.6/.7 coeff_abs_level_greater{1,2} + §9.3.3.11 coeff_abs_level_remaining + §9.3.4.2.8 palette_run_prefix ctxInc + Table 9-48 coeff_sign_flag bypass-coded sign bit + §7.4.9.11 signed-level composition + §9.3.4.2 Table 9-48 `cu_chroma_qp_offset_{flag,idx}` ctxInc + typed `CuChromaQpOffset` decode primitive + §9.3.4.2 `cu_transquant_bypass_flag` ctxInc + decode primitive + §9.3.4.2 `rqt_root_cbf` decode primitive (§7.3.8.5 inter-CU gate; Table 9-14 initValue=79, Table 9-43 FL cMax=1, Table 9-48 bin-0 ctxInc=0; inferred-value=1 distinguishes from cu_transquant_bypass_flag_inferred=0) + §9.3.4.2 `pred_mode_flag` entry (Table 9-10 initValues {149,134} initType 1/2; Table 9-48 cMax=1 ctxInc=0; §7.4.9.5 not-present derivation: I→MODE_INTRA, P/B+skip→MODE_SKIP) + typed `CuPredMode` enum (Inter / Intra / Skip) + intra-mode signalling binarizations complete (mpm/rem/chroma + §8.4.3 IntraPredModeC) + Table 9-50 i=15 erratum consumed (#1414 closed) + full §7.3.8.11 residual_coding() driver + fuzz-hardened (§7.4.3.2.1/§A.4.1 SPS validation, EGk u64 saturation, scaling-list delta bounds — 3 real overflow fixes); lacks Tables 9-26..9-31 ctx-init transcription + transform_skip/RDPCM + §8.6 scaling/IDCT | 🚧 scaffold |
| **H.266 (VVC)** | 🚧 ~75% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine + PROF + AMVP + SbTMVP + VPS + §7.3.10.10 amvr CABAC + decomposed §7.3.10.10 mvd_coding body parser + §7.3.2.22 sps_range_extension() typed decoder + §7.3.2.5 pps_subpic_id_mapping() typed decoder + §7.3.3.2 / §7.4.4.2 typed `general_constraints_info()` decoder (66 baseline flags + V4 additional-bit block) + §7.3.2.10 / §7.4.3.10 `access_unit_delimiter_rbsp()` typed decoder (Reserved range preserved) + §7.3.2.13 `filler_data_rbsp()` typed decoder + §7.3.2.11 / §7.3.2.12 `end_of_seq_rbsp()` + `end_of_bitstream_rbsp()` distinct-marker decoders + §7.3.2.16 `rbsp_trailing_bits()` + §7.3.2.17 `byte_alignment()` + §7.3.2.15 `rbsp_slice_trailing_bits()` reader-side typed validators (stop-bit + pad-zero rejection per §7.4.3.16/§7.4.3.17 + §7.3.2.15 16-bit `rbsp_cabac_zero_word` walk per §7.4.3.15 0x0000 byte-aligned-pair constraint) + §7.4.3.19 LMCS derived arrays | 🚧 ~93% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred + §7.3.11.7 non-merge inter pre-residual + amvr_enc + §7.3.10.5 bcw_idx_enc + multi-CP-MV affine MVD + composite affine+AMVR+BCW dispatchers + reader-side composite walkers (affine + AMVR / + BCW) + decomposed §7.3.10.10 mvd_coding writer + §7.3.2.19 LMCS APS data parser |
| **VP6** | 🚧 r26 — §13 static tables + §3 RawBitReader + §7.3 BoolCoder + §13.2.1 DC arithmetic + §13.3.1 AC coefficient arithmetic decoder + edge-clamped MC fetch + §13.3.3.1 decode_ac_zero_run BoolCoder walk + §11.1 MV-component arithmetic decoder + §11.2 per-frame MV-probability update (Table 13/14/15 walk) + §10 Table 10 per-block FourMV codeword decoder + §10 Nearest/Near alternative-MV neighbour walker + §11 differential-MV reconstruction (BoolCoder-independent: combines round-21 delta with same-reference above/left neighbour MV via DIFF_REFERENCE_OFFSETS `[(-1,0),(0,-1)]`; defaults to `(0,0)` when neither qualifies; per-component sum at §11.1 ±127 magnitude cap; `reconstruct_diff_mv` / `reconstruct_new_mv` typed entry points) + §12.2 custom scan order (Table 16 band ranges + coeff-band update flags; 7th BoolCoder layer) + §13 per-block coefficient reconstruction (DC seed + AC token walk + zero-run → scan → dequant, IDCT-ready) | 🚧 scaffold |
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~53% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header primitives + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §6.3.12 frame_reference_mode + §6.3.16 mv_probs outer sweep + §6.4.1 get_tile_offset + §6.4.2 decode_tile + §6.3 inter-arm `parse_compressed_header_inter` + §6.4 lines 2306-2311 tile-payload sizer + §8.8.1 loop_filter_frame_init builder + §8.8.3 filter_size( ) per-edge primitive + §8.8.4 adaptive_filter_strength( ) per-edge `(lvl, limit, blimit, thresh)` derivation + §8.8.5.1 `filter_mask` per-edge leaf primitive (hev_mask/filter_mask/flat_mask/flat_mask2, full BitDepth 8/10/12) + §8.8.5.2 `narrow_filter` per-edge sample-mutation primitive (hev=1 two-sample + hev=0 four-sample branches, BitDepth 8/10/12) + §8.8.5.3 `wide_filter` per-edge low-pass leaf (log2=3 8-tap + log2=4 16-tap, edge-replication Clip3 clamp) + §8.8 loop filter COMPLETE (frame-level raster driver over the §8.8.2 per-superblock walk); lacks §6.2.5 inter-frame + §6.4.4 decode_block_apply + decode_vp9 wiring | 🚧 scaffold |
| **AV1** | 🚧 ~35% — intra decode: standalone `decode_av1` + 4:2:0/monochrome multi-superblock to 128×128 + full base_q_idx; lacks inter prediction, loop filter, CDEF, loop restoration, film grain, superres, scalability | 🚧 ~30% — intra encode: `encode_av1` 4:2:0 YUV→IVF + 14-mode picker + all §5.11 TX_SIZES_ALL shapes + lossy/lossless + §5.11.23 `write_inter_block_mode_info` write side complete (ref_frames→mode→drl→assign_mv + interp_filter + §5.11.27-29 tail fold + decode tail-order fix + §5.11.26 intrabc PredMv + §5.11.7 fully threaded into decode_block_syntax — both arms live: composite intra else-arm (palette/CFL/filter-intra feeding §5.11.33/34 chroma+residual ctx) + intrabc find_mv_stack/assign_mv MV chain + §5.11.49 palette_tokens) + encoder partition-tree write threading of the same §5.11.7 body (sentinel-lockstep + 11-grid mirror vs the decode walker) + §5.11.34 residual write side — non-skip intra leaves round-trip per-TU coefficients (§5.11.35-§5.11.40 dispatch + §7.5 scan + tx-grid mirrors); lacks §8.3.2 level-context mirrors + §5.11.36 inter transform_tree write + RD picker + inter reconstruction/encode chain |
| **Dirac / VC-2** | ✅ ~97% — VC-2 LD+HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit + bit-exact intra + fuzz oracle + Criterion bench + row-major slice + §12.4.4 extended_transform_parameters + §14.3/§14.4 fragmented-picture reassembler + v3 §10.5.2 Table 5 predicates + §14.5 fragmented_wavelet_transform DC-prediction kick + §14 FragmentedPictureDecoder (picture-level driver tying assembler to §13.5 LD/HQ slice decoders) + §12.4.4 asymmetric (horizontal-only) transform decode end-to-end (slice unpack → §13.5.5 asymmetric quantisers → idwt_with_ho; HQ bit-exact q=0 roundtrips incl. distinct wavelet_index_ho) | 🚧 ~97% — HQ+LD intra + Dirac core-syntax + adaptive sub-pel + 2-ref bipred + post-OBMC + rate-control + inter-encoder fuzz + VC-2 v3 symmetric/asymmetric extended_transform_parameters + §14.2 fragment-header parser |
| **AMV video** | 🚧 typed §4a `AmvVideoFrame` geometry binding (intra-only, entropy window between SOI/EOI; 1116-frame fixture pinned) — frame decode blocked on trace §4a hardcoded-table gap | 🚧 scaffold |
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz + `idct8x8_dc_only` fast path + SHA-256 lockstep pin on 9 fixtures + 128×128 interlaced apcn + FIPS 180-4 §B.1/§B.2 self-check + §6.1.1 Tables 5/6/7 color-metadata reverse helpers + typed `FrameHeader::color_primaries_kind()` + `matrix_coefficients_kind()` Table 5/6 + `transfer_characteristic_kind()` §6.1.1 OETF accessors (Bt1886/St2084/Hlg) + typed `FrameHeader::frame_rate()` Table 4 (eleven named rates as `Rational`, codes 0 / 12..=15 → None) + typed `FrameHeader::aspect_ratio()` Table 3 (square/4:3/16:9 as `Rational`) + Annex A IDCT qualification harness (5 criteria, ~200× margin) | ✅ ~97% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + ffmpeg cross-decode + SHA-256 lockstep pin across every encoder entry + SHA-256 pin on 10/12-bit encoder paths |
| **EVC** (MPEG-5) | 🚧 ~93% — NAL + SPS/PPS/APS + §9.3 CABAC + §8 intra + DCT-II + P/B inter + RPL + HMVP + DPB + ALF + DRA + IBC §8.6 + §8.9.7/§8.9.8 DRA + §7.4.3.1 SPS-signalled ChromaQpTable + §8.5 AMVR + §7.4.7 MMVD + §8.5.2.3.9 bipred MMVD + entry-process signed POC scaling + §8.5.2.3.10 merge redundancy + §6.5.1 tile-grid iterator + §6.4.2 availLR derivation + §6.5.2 zig_zag_scan / §6.5.3 inverse_scan public surface (eq. 33 + eq. 34) + §6.5.1 ColWidth/RowHeight tile-extent derivations (eqs. 24/25) + §6.4.3 MV-candidate single-block availability + §6.4.4 ALF neighbouring-block single-block availability + §6.4.1 base neighbouring-block availability (closes the §6.4 single-block quartet alongside §6.4.2; structural pinning §6.4.3 = §6.4.1 + intra/IBC bullet, §6.4.4 = §6.4.3 − tile bullet) + §6.5.1 tile/CTB scanning complete (eq. 24-32) + §7.4.5 slice-tile resolution (eq. 78-82) with arbitrary-slice parser fixes; lacks Main-profile toolset (BTT/ADMVP/EIPD/ATS/affine) + #1278 + #1470 | — |
| **HuffYUV** / FFVHuff | ✅ ~97% — HFYU + FFVH FourCCs + 6 predictors + 8-bit + interlaced field-stride=2 + fast-LUT decoder + SWAR gradient post-pass + YUY2 LEFT macropixel-step + encode_roundtrip fuzz + Median tail-loop strip + LEFT-helper dedup + macropixel-step YUY2 Huffman-decode + half-macropixel inverse MEDIAN decoder body (2-byte step avoids RAW aliasing per §2.3) | ✅ ~97% — encoder symmetry × YUY2/RGB24/RGB32 + v1.x + v2.x ClassicV2/CustomV2 + YUY2 LEFT forward + forward_rgb_left_subtract_linear + dead-branch parity + pixel-step Huffman-ENCODE bodies (YUY2 + RGB24 histogram + RGB32 4-byte hoist + RGB32 histogram pixel-step hoist) + YUY2 forward MEDIAN macropixel-step body (ILP via 4-byte unroll) + RGB24/RGB32 decode slot/store binding hoist (decode-side completion) |
...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal compositor + structured Application Extensions + Plain Text Extension + lenient mode + lazy Playback + animation-timing accessors + fluent AnimationBuilder + spec-derived fuzz corpus + §18.c.v Sort Flag accessors + §20.c.ix Local Color Table Size typed accessors | ✅ 100% — per-frame palettes + `optimize_color_tables()` GCT/LCT hoisting + `optimize_frame_rects()` §20 changed-region cropping (compose-identical) + §7 Required Version enforcement |
...
| **Netpbm** (PBM/PGM/PPM/PNM/PAM) | ✅ ~95% — all 8 magics at 1/8/16-bit + 6 PAM TUPLTYPEs + cargo-fuzz harness + decoder pre-allocation OOM hardening + `read_be16_row` P5/P6/P7 16-bit + `swap_bytes_u16_row` LE→BE encode-side row helper (~48-50 GiB/s) + native 16-bit GRAYSCALE_ALPHA (`Ya16Le`, lossless — prior path truncated to 8-bit) | ✅ ~95% — incl. P7 GRAYSCALE_ALPHA MAXVAL 65535 |
...
| **OpenEXR** | 🚧 ~93% — magic + 8 required attrs + HALF/FLOAT/UINT + NO_COMPRESSION/ZIP/ZIPS/RLE + tiled ONE_LEVEL + sub-sampled chroma + single-part deep scanline/tiled + multi-part deep scanline/TILED + multi-part flat TILED ONE_LEVEL/MIPMAP/RIPMAP + single-part deep tiled MIPMAP/RIPMAP + multi-part deep tiled RIPMAP + multi-part mixed spanning all 4 part types incl. deep (split-validated bit-exact; shared-displayWindow union fix) + typed inspectors for int/double/string/v2i/v3i/v3f/m33f/m44f/chromaticities/box2f attribute payloads; PIZ blocked on docs trace | ✅ ~96% — RGBA scanline + single-part deep tiled + multi-part deep TILED + multi-part flat MIPMAP/RIPMAP + single-part deep tiled MIPMAP/RIPMAP + multi-part deep tiled RIPMAP + multi-part mixed scanline+tiled |
...
| **TGA** | ✅ 100% — typed §C.6.4 KeyColor + §C.6.5 PixelAspectRatio + §C.6.6 GammaValue + §C.6.7 SoftwareVersion accessors + footer-walker helpers + typed Field 11/12/14/16 ASCII accessors (Author Name / Comments / Job Name / Software ID) + §C.6.9 scan-line tables computed from pixel data + random-access single-row decode | ✅ 100% |
...
|--------|----------|--------|--------|-------|
| **`oxideav-videotoolbox`** | macOS / iOS | 🚧 H.264 + HEVC + ProRes + MJPEG + MPEG-2 + VP9 + MPEG-4 Pt 2 + AV1 (M3+) + VVC | 🚧 H.264 + HEVC + ProRes + MJPEG | Encoder knobs: bit_rate→AverageBitRate, quality→Quality, profile aliases (H.264 / HEVC main/main10/main4_2_2_10)→ProfileLevel; ProRes 6 fourCCs; data_rate_limits→DataRateLimits CFArray + constant_bit_rate→ConstantBitRate (macOS 13+). PSNR_Y: MPEG-2 ~61 / H.264 ~51 / HEVC ~54 / ProRes ~52 / MJPEG ~36 / AV1 ≥30 dB. iOS uses build.rs framework-link + `dlsym(RTLD_DEFAULT)`; macOS keeps the existing `dlopen` absolute-path code path. iOS-specific encoder gaps (AV1 needs A17+, ProRes encode needs iPhone 13 Pro+) surface as `kVTPropertyNotSupportedErr` and degrade gracefully. |
...
| Format              | Decode | Encode | Notes |
|---------------------|:------:|:------:|-------|
| **ASS / SSA**       | ✅ | ✅ | Script Info + V4+/V4 Styles (BGR+inv-alpha) + override tags + `\fn`/`\fe`/`\b<weight>`/`\r[<style>]` + `\pbo` + face-flag toggles + typed `\p<scale>` + `\fax`/`\fay` shear + `\an<n>` numpad alignment + `\1a` primary-fill alpha + `\blur<strength>` Gaussian post-step + `\iclip(rect)` + `\iclip(drawing)` + `\fsp<spacing>` letter-spacing baked into renderer + `\shad`/`\xshad`/`\yshad` drop-shadow bake + `\bord`/`\xbord`/`\ybord` border ring baked (+ glyph cache-alias fix) + typed `Dialogue: Effect:` column (Karaoke / Scroll up / Scroll down / Banner per SSA v4.x) + typed [Fonts]/[Graphics] attachment decoder + typed per-event `MarginL/R/V` overrides (SSA v4.x "all-zero = style default" carve-out) + typed `Dialogue: Layer:` column accessor + tokenizer types `\a` legacy alignment / `\an1`-`\an9` / `\k`-family karaoke / `\pos`/`\org`/`\move` (re-emit byte-identical) |
...
| **DVB subtitles**   | ✅ | ✅ | ETSI EN 300 743 segments + 2/4/8-bit pixel-coded objects; encoder: full segment writers + 2/4/8-bit RLE + RGBA display-set encoder (PES-level), roundtrip-pinned |
