...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF atoms + chan layout + faststart + udta + dref + tkhd matrix → rotation + chapter resolution + gmhd/text/tmcd headers + tmcd-in-stsd + multi-hop alias-chain following (MAX_ALIAS_DEPTH=4 + cycle detection + `file://` opener via `open_file_url`) + ISO BMFF `meta` (pitm + iinf + iloc + idat + xml/bxml + iref typed refs) + HEIF/HEIC `iprp/ipco/ipma` item-properties + meta-only files open without moov + HEIF derived-image `grid` / `iovl` payload parsers (16/32-bit dim shapes) + `primary_item_data()` walks pitm → iloc → idat / file extents in one call + styl/ftab/hlit/hclr/drpo text-sample style trailers + typed-iref helpers (`derived_from`/`auxiliary_for`/`thumbnail_of`/`describes`); rejects fragmented MP4 with hint to `oxideav-mp4` |
| AVI       | ✅ | ✅ | ✅ | OpenDML 2.0 super-index + AVIX continuation + dmlh (`dmlh_total_frames()` typed) + vprp (per-field rect array; `VprpConfig::with_field_descs` muxer override) + LIST rec clusters + 2-field interlaced (`AVI_INDEX_2FIELD` 12-byte entries) + 02ix mid-`movi` index + truncated-head recovery + VBR audio + LIST INFO emit + read accessors (`info_for` / `info_all_for` / multi-value `avi:info.<fourcc>`) + xxpc / xxtx skip-and-count + typed AVIF_* flag accessors (`AvihFlags` per `vfw.h` bits); idx1 keyframe seek with O(1) per-stream flags cache + `seek_to_keyframe_strict` returning `KeyframeSeekResult { target, landed, gop_distance }` |
...
| **AC-4** (Dolby) | 🚧 ~97% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/cfg1/cfg2/cfg3 dispatch all trailer-aware + LFE body + SSF/SNF + SAP Tables 181 + 183 a/b/c/d (sap_mode 0/1/3) + Ls/Rs surround mono walker + ASPX bandwidth-extension trailer capture wired end-to-end across cfg0/1/2/3 + Pseudocode 121 companding all branches (PerSlot / Averaged / SyncPerSlot / SyncAveraged; M=1 exact, M>1 single-channel approximation documented) + ACPL_3 path companding via stereo CPE + ASPX_ACPL_1 sb0=acpl_qmf_band hookup; lacks multi-channel `g_synch(ts) = (∏g_ch)^(1/M)` cross-channel exact synchronisation | — |
...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~57% — forward CABAC + DCT-II + per-CTU SAO RDO + inline per-CTU ALF CABAC walk + PH-level ALF chain + quantised APS NAL emission + APS-signalled per-class luma filter-set learning (25 separate filter rows via §8.8.5.3 `(filtIdx, transposeIdx)` classification + Wiener filter design with Gauss-Jordan solve + lattice quant scale 2^7 saturate ±64 + `alf_luma_coeff_delta_idx[]` row-deduplication + AlfCoeffL APS id=2; per-CTU 17-trial RDO `{off, fixed 0..15, learned APS}`) + APS-vs-fixed picture-bits trade-off RDO (SSE_Y + λ·8·aps_bytes); end-to-end ALF round-trip verified |
...
| **VP8** | ✅ 100% — entire 15-fixture corpus bit-exact | 🚧 ~97% — I+P + B_PRED + SPLIT_MV + alt-ref/golden + Lagrangian RDO + libvpx-shape Trellis + activity AQ + RFC 6386 §15.2 mode/ref deltas (static + opt-in adaptive ladder, ±6 cap) + opt-in 4×4 B_PRED RDO + opt-in UV-mode RDO + opt-in joint LF-RDO + opt-in SPLIT_MV partition RDO (with opt-in real-context second pass mirroring `emit_split_submvs`) + opt-in MV-cost-aware NEAREST/NEAR/NEW snap (Lagrangian extension to fixed-tolerance fast path) + opt-in Trellis context-rate (`nctx ∈ {0,1,2}`) + opt-in psy-RDO/ARNR |
...
| **WebP VP8** | ✅ 100% — via VP8 + bit-exact YUV→RGB + fancy chroma upsample + streaming `WebpAnimDecoder` (`new`/`next_frame`/`next_frame_borrowed` lifetime-clamped/`seek_to_frame`/`reset`/`info`/`done`; memory-tight lazy demux holds VP8/VP8L/ALPH as `(offset, length)` ranges into one owned body buffer; framework `Demuxer::next_packet` builds one OWEB payload at a time via `into_packets_iter`) | 🚧 ~96% — VP8 I-frame + ALPH + per-segment QP/LF + Trellis + animated ANIM/ANMF with file-level ICCP/EXIF/XMP + AVIF-style perceptual frame-merge encoder (`AnimFrameMode::Delta` with luminance-biased 8×8-block SAD cost model + ANMF blend-mode swap to DoNotBlend + bbox sub-rect emit; ~75% file reduction on 5%-changing-region fixture) + standalone `encode_vp8l_argb_with_metadata` + `encode_vp8_lossy_*` + registry-side `make_encoder_with_metadata` parity; dwebp cross-decode clean |
...
| **JPEG XL** | 🚧 ~85% — ISO/IEC 18181-1:2024 final core. 5 small lossless fixtures decode PIXEL-CORRECT. Modular path complete; VarDCT scaffold (Quantizer + LfCoefficients + F.1 LF dequant + F.2 adaptive smoothing + HfMetadata with nested `nb_transforms + TransformInfo[]` + DctSelect derivation + HfBlockContext custom branch + HfGlobal C.6.2 dequant-matrix encoding modes + GlobalModular ModularHeader §C.9.1 N=0 gate + F.3.1 single-TOC-entry section chaining on shared BitReader) wired into pipeline; r17 Auditor diagnostic isolated d1 Squeeze blocker to ~2.3 bits/sample over-consumption in `decode_uint_in_with_dist` per-token hybrid-uint extra-bits accounting (research artifact at `crates/oxideav-jpegxl/round17-d1-bisect.md`) — fix is r18 work; lacks HfPass C.7 + PassGroup HF C.8.3 + GetDCTQuantWeights materialisation + IDCT dispatch + XYB / CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **SVG** | ✅ ~98% — full shape set + path + g + defs + gradients + text/tspan + mask + clipPath + use/symbol + svgz + SMIL animate/set/animateTransform at arbitrary `t` (paced + spline calcMode) + CSS3 Selectors L3 cascade with `::before`/`::after` pseudo-elements + 8 stateful pseudo-classes (:hover/:focus/:active/:checked/:visited/:link/:disabled/:enabled — never match in static doc) + `@import` URL capture + SVG2 §9.3.2 `d`-as-CSS + filter primitive rasterisation graph (17 typed primitives — every short-name §11 primitive: feGaussianBlur / feOffset / feFlood / feComposite / feBlend / feMorphology / feColorMatrix / feMerge / feComponentTransfer / feDropShadow / feConvolveMatrix / feTurbulence / feDisplacementMap / feDiffuseLighting / feSpecularLighting / feImage / feTile) | ✅ ~85% — round-trips shape/stroke/fill/gradient/transform/mask/clipPath + PreservedExtras side-channel for `<style>`/`<filter>`/`<animate>`/`<foreignObject>` fragments |
| **PDF** | ✅ ~92% — bytes → Scene via legacy xref + cross-reference streams + object-stream resolver + recursive object parser + content-stream operator parser + `/Prev` chain incremental updates + `/Encrypt` decode all revisions (R=2/3/4 RC4 + AES-128, R=5/R=6 AES-256) + per-stream `/Crypt /Identity` opt-out + public-key security handlers `adbe.pkcs7.s3/s4/s5` decode (CMS EnvelopedData with both `IssuerAndSerial` + `SubjectKeyIdentifier` KTRI recipient forms; CMS KARI key-agreement variant decoder per RFC 5652 §6.2.2; per-CF recipient lists with separate permission masks; hand-rolled DER+CMS+X.509) | ✅ ~93% — PDF 1.4/1.5 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + xref-stream encoder + ObjStm encoder + incremental updates + Linearization (Annex F + F.4.2/F.4.3/F.4.4 hint-table headers) + ObjStm+encryption combined path + `/Encrypt` ENCODE all revisions + public-key ENCODE (`pkcs7_s4` / `pkcs7_s5_v4_aes128` / `pkcs7_s5_v5_aes256` + multi-CF emitter `write_pdf_from_scene_pubsec_multi_cf`); lacks text, JPEG passthrough, KARI unwrap (ECDH curve arithmetic) |
...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | DirectShow `IBaseFilter::Run` + `JoinFilterGraph` + `IPin::ReceiveConnection` all S_OK; `IMemInputPin::GetAllocator` waits on `NotifyAllocator` from upstream | DirectShow IBaseFilter wrapper (r25-r27): COM scaffolding (11 IIDs + GUID parser + ComObjectTable + vtable dispatch) + ole32 stubs + DllGetClassObject + IClassFactory::CreateInstance + full HostIFilterGraph (11 methods) + HostIPin (18 methods, output-pin role) + HostIEnumMediaTypes (7 methods) — vtables built in arena memory dispatched via existing `dispatch_stub` machinery. user32 cascade (r26): CreateWindowExA + DestroyWindow + IsWindow + UpdateWindow + GetMessageA + DispatchMessageA family — synthetic-HWND or no-op-success, no real window opened. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. |
...
MMX, segments + FPU zero-fill); clean disconnect on `vKill`/`D`.
Bind `:0` to pick a free port; chosen port logged at startup.
