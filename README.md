...
| **PDF** | ✅ ~68% — bytes → Scene via xref + recursive object parser (FlateDecode) + content-stream operator parser + `/Encrypt` decode (R=2 RC4-40, R=3 RC4-128, R=4 RC4-128 + AES-128 per ISO 32000-1 §7.6 + Algorithms 1-7); `/Info` → Metadata; lacks AES-256 R=5/R=6, cross-reference streams, public-key handlers | ✅ ~70% — PDF 1.4 multi-page + paths + gradients + strokes + transforms + opacity + clip + RGBA images + `/Info` dict; lacks text, JPEG passthrough, encryption-on-encode |
