...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate, 48 dB on HE-AACv1 reference. | ✅ AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS. Self-roundtrip ~17 dB on AM stereo. ffmpeg-interop still imperfect (SBR Huffman parse rejection — open). 150 tests. |
...
| **AC-3** (Dolby Digital) | ✅ Full decode + FFT IMDCT + §7.8 downmix. Sine 92.02 dB / transient 92.77 dB vs ffmpeg. | ✅ Encoder: rematrix + short-block emit + transient detection. ffmpeg cross-decodes our blksw=1 streams. Coupling + dba pending. |
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end (HF adjust + noise + tone + limiter + complex-covariance TNS, 78-81 dB round-trip). DRC + A-CPL + DE parser stacks complete with verbatim Huffman tables. Outer §4.2.14.1 metadata() walker. 288 tests. Pending: P92-94 sinusoid location, §5.7.7 A-CPL synthesis pipeline. | — |
...
| **MPEG-4 Part 2** | ✅ I + P + B-VOP, 4MV direct, half-pel + quarter-pel MC, field-MV + field-DCT + alt-vertical-scan for interlaced B-VOPs. Bit-exact within rounding on testsrc. | ✅ I+P+B-VOP encoder (B-VOP-aware `-bf N`). 1MV/4MV + intra-MB-in-P fallback + P-VOP quarter-pel encoder (verid=2 VOL, opt-in `qpel`). Self-roundtrip 44 dB; ffmpeg cross-decodes clean. 128 tests. |
| **Theora** | ✅ I+P frames | ✅ I+P frames incl. INTER_MV_FOUR (45 dB PSNR, 3.7× vs all-I) |
| **H.263** | ✅ I+P, half-pel MC, Annex J deblock, Annex D URMV, Annex F (4MV + OBMC), Annex E SAC. 5-frame testsrc QCIF: 60–69 dB vs ffmpeg. | ✅ I+P + diamond ME (46 dB on sliding-gradient). Annex F (4MV + OBMC) emit; SAC encoder/decoder for both I/P + Annex F paths (byte-identical SAC↔VLC roundtrip). 110 tests. |
...
| **MS-MPEG-4** (v1 / v2 / v3) | 🚧 Clean-room implementer handoff. v1+v2+v3 picture headers + MCBPCY decoders + v3 P-frame MV/residual pipeline + v1/v2 MV (with 33→65-entry spec correction). Full real-content PSNR gated on intra-AC + inter-AC VLC tables (spec OPEN-O4). 159 tests. | — |
| **H.264** | 🚧 **Spec-driven rewrite from scratch** (~45k LOC). All §7/§8/§9 core: NAL/SPS/PPS/slice parsing, FMO (all 7 map types), I/P/B-slice reconstruction, POC 0/1/2, DPB + MMCO + RPLM, 6-tap luma / bilinear chroma MC, weighted + spatial/temporal direct MV, deblock, CAVLC + CABAC, 7 SEI types, 4:2:0 + 4:2:2. 16 integration tests. Pending: 4:4:4 + MBAFF + Annex F/G/H. | 🚧 Clean-room Baseline I-only encoder: SPS+PPS+IDR + I_16x16 (4 modes) + I_NxN (9 modes) + luma AC residual + chroma residual + in-loop deblock + **Lagrangian RDO** (per-block + per-MB). Smooth content 47-51 dB; noisy 37-46 dB. **ffmpeg interop bit-exact on every fixture** (≤1 LSB). 774 lib + 11 integration tests. Next: P-slices. |
| **H.265 (HEVC)** | ✅ I/P/B decode for **8-bit + Main 10 + Main 12** in 4:2:0 + **4:2:2 (8/10/12-bit) intra + inter** all bit-exact (∞ dB) on gray fixtures. SAO + deblock + scaling lists. HEIF/HEIC opt-in. 138 tests. Pending: 4:4:4 12-bit, tiles+WPP, AMP, `interSplitFlag` desync. | ✅ Baseline CAVLC I+P + Main-profile CABAC P-slice encoder. I 45 / P 31 dB via our decoder; ffmpeg accepts at 26.82 dB. |
| **H.266 (VVC)** | 🚧 4:2:0 IDR pictures decode end-to-end with color. Full intra + inverse transform pipeline + chroma reconstruction. In-loop filters: deblock (short + long luma + chroma strong/weak) + SAO (EO/BO + CABAC parser) + ALF (luma + chroma apply + APS parser). 336 lib + 14 integration tests. Pending: ALF classification (currently filtIdx=0), CC-ALF, P/B inter slices, dual-tree, CCLM, MIP/ISP/BDPCM. | ✅ Encoder scaffold (VPS/SPS/PPS/PH/IDR parse-roundtrip through decoder front-end). No residual/CABAC/pixel output yet. |
...
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV list + inter-mode ctx + compressed-header probs. **Lossless bit-exact (66.77 dB)** for both keyframe and inter. **Lossy compound: 10.59 dB** (slowly rising over rounds 13-15). 154 tests. Pending: deep coef-tree probabilities, sub-8x8 chroma. | — |
| **AV1** | 🚧 OBU + range coder + coeff decode + partition + transforms + all intra predictors + CDEF + Loop Restoration + inter MC + palette mode + skipModeAllowed via DPB OrderHint + multi-ref DPB compound MC. **r15**: §8.2.6 `read_symbol` was inverted across rounds 1-14 (cancelled in degenerate cases, hid the bug). After fix: SVT-AV1 first 8×8 block 64 identical 128s → 213 distinct values; PAETH/SMOOTH/D203/palette/multiple TX sizes all decode end-to-end; 8.85 dB vs ffmpeg (still gated by chroma Subsampled_Size + palette finalization + loop-filter gaps). 342 tests. | — |
...
| **JPEG 2000** | ✅ **Bit-exact vs OpenJPEG.** Part-1 baseline + multi-tile + MQ + EBCOT + 5/3 + 9/7 IDWT + tier-2 + JP2 wrapper + all 5 progression orders + multi-layer + user precincts + POC marker + PPM/PPT (end-to-end). 110 tests. Pending: OPJ RGB MCT bit-exactness (~78% sample mismatches — likely RCT shift). | ✅ 5/3 lossless + 9/7 irreversible RGB + all 5 progression orders + POC + PPM/PPT. 13 new encoder tests bit-exact through opj_decompress. |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MOD** | ✅ 4-channel Paula-style mixer + full ProTracker 1.1B effect set. **r14-r15 PT-fidelity pass**: sample loop boundary, sample-swap-without-note defer-to-next-note, LED filter (E00/E01), extended period range (108-907 finetune extremes), vibrato sign verified, E6/Dxy + Fxx 0x20 boundary regression-tested. 88 unit + 33 integration tests. | — |
