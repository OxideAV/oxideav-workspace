...
| **Opus** | 🚧 ~26% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + r157 §3.1/§4.2 framing dispatch (SilkOnly/Hybrid/CeltOnly + Hybrid-pinned-WB + §4.2.2 frame count + §4.2.4 LBRR gate; 362 tests); CELT bands gated on #936 + #943 | 🚧 scaffold |
...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~90% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed MDCT + auto SM + outer-loop on Long/Short/Mixed/Start/End + r161 §2.4.3.4.10.3 auto-block-type mixed-block promotion (MixedClassifier + step_with_mixed; 575 tests); lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~89% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §13.4 fitter threaded into stream drivers + r161 §11 intra-within-inter picker widened to 4 Y × 4 UV whole-block grid (was DC-only r160; 533 tests) |
...
| **AV1** | 🚧 ~53% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.8/9/10/11/12/13/20/55/56 leaves + r161 §5.11.7 intra_frame_mode_info() prefix dispatcher (composes through read_delta_lf; 509 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~79% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + depth-3 lazy LZ77 + r161 §4.1 Shannon bit-cost mode chooser (strict-beats L1 by 2–113 B on 32/32 seeds; headline 10.4% predictor-stream reduction; 382 tests); lacks VP8-lossy encode (pending vp8 republish #1068) |
