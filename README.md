...
| **Vorbis** | 🚧 r3 (post-2026-05-20 orphan) — §4.2.2 identification + §5 comment + §3.2.1 codebook-header parse (sync / dimensions / entries / lengths dense+sparse+ordered + lookup table); lacks Huffman tree builder / setup-header walker / floors / residues / audio packets | 🚧 scaffold |
| **Opus** | 🚧 r3 (post-2026-05-20 orphan) — RFC 6716 §3.1 TOC + §3.2 frame packing + §4.1 range decoder (init / decode / dec_bit_logp / dec_icdf / dec_bits / dec_uint / tell / tell_frac); lacks SILK / CELT band machinery | 🚧 scaffold |
...
| **CELT** | 🚧 r3 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 frame-header prefix (silence/post-filter/transient/intra + deferred anti-collapse); lacks band decode / PVQ / MDCT / energy envelope | 🚧 scaffold |
...
| **MPEG-2 video** | 🚧 r2 (post-2026-05-18 orphan) — §6.2.2.1 sequence_header + §6.2.2.3 sequence_extension parse with composed 14-bit dim / 30-bit bit_rate / 18-bit VBV via Mpeg2Sequence::from_buf; lacks GOP / picture / slice / IDCT | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 r2 (post-2026-05-18 orphan) — §6.2 VS/VO/VOL + §6.2.4 GOV + §6.2.5 VOP header parse (vop_coding_type / modulo_time_base + vop_time_increment / vop_quant / fcode_fwd-bwd); lacks MB / DCT / MV decode | 🚧 scaffold |
| **Theora** | 🚧 r2 (post-2026-05-20 orphan) — §6.1 + §6.2 identification + §6.3 comment header (vendor + KEY=value with case-insensitive lookup); lacks setup header + frame decode | 🚧 scaffold |
...
| **VP8** | 🚧 r3 (post-2026-05-20 orphan) — RFC 6386 §7 boolean decoder + §9.1 uncompressed header + §19.2 boolean-coded header through prob_skip_false (segmentation / loop-filter knobs / mb_lf_adjustments / quant_indices / token_prob_update with §13.4 COEFF_UPDATE_PROBS / refresh-copy-sign-bias ladder); lacks mv_prob_update / intra-mode probs / MB decode / IDCT | 🚧 scaffold |
| **VP9** | 🚧 r3 (post-2026-05-20 orphan) — full §6.2 walk + §9.2 Boolean decoder + §6.3.1 read_tx_mode walk; lacks §6.3.3 diff_update_prob chain / coef probs / inter / loop filter | 🚧 scaffold |
| **AV1** | 🚧 r3 (post-2026-05-20 orphan) — §5.3 OBU + §5.5 sequence header + §5.9.2 uncompressed_header prefix (show_existing..refresh_frame_flags; 16/16 fixtures bit-exact on 12 frame-header columns); lacks frame_size / tile decode / MV / transforms / loop filters / film grain | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | 🚧 r3 (post-2026-05-20 orphan) — RFC 9649 §2.3-§2.7 container walker + typed VP8X + §2.7.1.2 ALPH info-byte (Rsv/P/F/C typed enums) + §2.7.1.1 ANIM (BGRA + loop u16); lacks ANMF field decode, ALPH/VP8/VP8L bitstream | 🚧 scaffold |
...
| **JPEG 2000** | 🚧 r3 (post-2026-05-20 orphan) — T.800 main-header + §A.4.2 SOT/SOD walker + typed COC/QCC/POC/RGN/PLT/PPT tile-part markers (PLT Iplt VLQ decoded); lacks tier-1 EBCOT / tier-2 packets / wavelet / dequant / MCT / JP2 / HTJ2K | 🚧 scaffold |
