...
| **AAC** | 🚧 ~84% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR NQ/max_sfb bounds + ICS gain_control acceptance + SBR hf_adjust monotonic-grid guard; lacks LD/ELD raw_data_block, USAC frame body | 🚧 ~78% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy default-on |
...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal-method compositor (§23 None/Keep/RestoreBackground/RestorePrevious) — clean-room rebuilt from CompuServe spec text + Welch 1984 only | ✅ 100% — per-frame palettes |
...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | ✅ **Multi-frame DirectShow decode** — r42 (vfw r38) drove the same `SandboxedDshowDecoder` instance through both an I-frame and a P-frame at 176×144 (1→2 frames; r41 had only ever driven one packet). Larger 352×288 GOP-30 fixture surfaces 1/6 frames Video and identifies two R39 blockers via diagnostic blob: (1) output-allocator side trap at MPG4DS32 RVA 0x4064d4 (codec walks slot 7 of output allocator's vtable with `ecx=output_alloc_vtbl0`, vtable address not `this`) + (2) sample-release cycle gap exhausts host pool after 4 unreleased samples (`VFW_E_NOT_COMMITTED` 0x80040211 on frames 4-5). Path landed r25-r41: IBaseFilter Run/Pause/GetState + JoinFilterGraph + IPin::ReceiveConnection (synth AMT fallback) + HostIMemAllocator committed-state machine + IMemInputPin::GetAllocator + IID_IMediaSample2 host stub (r39) + esp-discipline forensic (r40) + `IMemAllocator::GetBuffer` arg_dwords 4→5 fix (r41 — missing `dwFlags` arg left stack 4 bytes off). | DirectShow IBaseFilter wrapper: COM scaffolding + ole32 stubs + DllGetClassObject + full HostIFilterGraph + HostIPin (output+input) + HostIEnumMediaTypes + HostIMemAllocator (committed state-machine) + HostIMediaSample + EnumMediaTypes walker + IMediaFilter Pause/Run/GetState. user32 cascade: synthetic-HWND no-op family. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. |
