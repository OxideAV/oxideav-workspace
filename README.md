...
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM + §2.4.3.1 CRC-16 verify + mp2 frame-level decode loop; lacks bit-exact PSNR + LSF (#1076) + Annex D psy | 🚧 ~62% — Layer I encoder + Layer II §C.1.5.2.7 bit-allocation core + Table C.5 SNR ladder + r157 mp2 §2.4.2.3 frame-header writer (Layer II syncword/ID/layer/bitrate/sf/mode/emphasis pack; 14×3×4 matrix round-trip); lacks Layer II analysis filterbank + Table C.4 SCFSI + Table C.6 quantizer + Annex D psy |
...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~88% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed MDCT + auto block-type SM + outer-loop on Long/Short + r159 §C.1.5.4.3 mixed-block primitive (long sfb 0..=7 × short sfb 3..=11 composition, 547 tests); lacks Annex D psy (#1048) + intensity-stereo + transition-skeleton outer loop + auto-SM Mixed dispatch + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~86% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §13.4 keyframe+inter observed-counts fitter + r159 fitter threaded into Vp8KeyframeStreamEncoder / Vp8InterStreamEncoder stream drivers (−14 to −52 B / P-frame on synthetic, 529 tests) |
...
| **AV1** | 🚧 ~51% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.10/11/12/13/20/55/56 leaves + r159 §5.11.9 read_segment_id + SegmentIds[] + neg_deinterleave (494 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~77% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + §5.2.2 depth-3 lazy LZ77 + r159 §4.1 entropy-image-aware per-block tie-break (residuals bit-identical, sub-image collapses on tied blocks; 370 tests); lacks VP8-lossy encode (pending vp8 republish #1068) |
