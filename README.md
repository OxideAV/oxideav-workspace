...
| **Musepack** | 🚧 r197 — SV7 §2.5/§2.6 requantiser constants + SV7/SV8 stream-magic recognisers + SV8 packet outer-frame walker + r197 **SV7 mpc_huffman tables (10 tables / 384 entries) + CNS PRNG** (build.rs CSV-to-Rust generator + 256-byte parity table + 6 scalar PRNG constants + `Sv7BitReader` + `Error::HuffmanNoMatch`; vendored tables/ snapshot for standalone+crates.io builds; 56 tests); lacks SV7 fixed-header field map + SV8 canonical-Huffman entropy layer (31 staged `sv8-canonical-*` length tables + 31 symbol maps) + 32-band synthesis (ISO Layer-II tables) | 🚧 scaffold |
