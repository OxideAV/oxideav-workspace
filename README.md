...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate, 48 dB on HE-AACv1 reference. | ✅ AAC-LC (mono+stereo + PNS + IS + pulse data) + HE-AACv1 mono+stereo + HE-AACv2 PS. **r14**: time-direction differential (`iid_dt=1`) + multi-envelope (`num_env=1/2/4`) PS coding with DF-vs-DT cost compare. Real stereo bit-different from mono (96% post-warmup samples). vs Apple afconvert reference: 9.0/6.1 dB on mixed AM stereo (-6 to -12 dB gap, baseline for tuning). 150 tests. |
...
| **AC-3** (Dolby Digital) | ✅ Full decode + FFT IMDCT + §7.8 downmix. Sine 92.02 dB vs ffmpeg. **r14**: transient fixture stuck at 15.55 dB for 6 rounds was a **test-harness bug**, not the decoder — `tests/ffmpeg_fixture.rs` cross-correlation lag-search clamped at `.min(4096)` settled on lag=252 instead of true lag=0 (440 Hz periodic ambiguity), misaligning burst envelope. 7-line fix → **PSNR 15.55 → 92.77 dB** (decoder binary unchanged). | ✅ Encoder lifted via mantissa-emitter + D15 + bit-budget fixes (sine 21 dB, speech 32 dB, stereo L/R 23/45 dB, ffmpeg-decode clean). **r13**: §7.5.3 rematrixing landed (R-channel ~10 → 44.57 dB self / 34.34 dB ffmpeg). Short-block encode + coupling still pending. |
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end (18 Huffman codebooks + master-freq + QMF analysis/synthesis, 78-81 dB round-trip). HF adjustment / noise gen / tone gen / limiter / complex-covariance TNS all complete (P72-P101, §5.7.6.4.1-2). DRC + A-CPL + DE parser stacks (§4.2.13/14) wired with all official Huffman tables transcribed verbatim from `ts_10319001v010401p0-tables.c`. **r14**: A.4 DE codebooks (Tables A.58-A.61) + `parse_dialog_enhancement` (Table 76) + `parse_de_data` (Table 78). 268 tests. Remaining: P92-94 sinusoid location, §5.7.7 A-CPL synthesis math, outer `metadata()` walker. | — |
...
| **MPEG-4 Part 2** | ✅ I + P + B-VOP, 4MV direct, half-pel + quarter-pel MC (B-VOPs), decode→display reorder; field-MV + field-DCT + alt-vertical-scan for interlaced B-VOPs. Bit-exact within rounding on frame-7 testsrc fixture. | ✅ I+P+B-VOP encoder (B-VOP-aware `-bf N`). 1MV/4MV decision; B-VOP 4MV-direct activates from co-located P 4MV. **r14**: intra-MB-in-P fallback (§6.3.7) for scene changes — when `inter_sad > intra_mad + 384 + 128`, re-encode through I-VOP intra wrapped in `not_coded=0` + Table B-13 rows 4..7. Scene-change cut frame **19.06 → 38.89 dB** through ffmpeg. Overall self-roundtrip 44 dB on testsrc, ffmpeg cross-decodes clean. 124 tests. |
| **Theora** | ✅ I+P frames | ✅ I+P frames incl. INTER_MV_FOUR (45 dB PSNR, 3.7× vs all-I) |
| **H.263** | ✅ I+P pictures, half-pel MC, Annex J deblock, Annex D URMV (baseline + PLUSPTYPE Table D.3 UUI range), Annex F (4MV + OBMC), Annex E SAC core. 5-frame testsrc QCIF: 60–69 dB vs ffmpeg. Long-clip ~1 dB/P-frame IDCT-LSB drift documented. | ✅ I+P + diamond-pattern ME (46 dB on sliding-gradient). Annex F (4MV + OBMC) emit; ffmpeg decodes our output at 40+ dB. **r14**: Annex E SAC encoder/decoder complete for both I- and P-pictures — 100% of MB-layer syntax routes through §E.2/§E.5/§E.7/§E.8 in the no-4MV baseline. New `SacPPictureWriter/Reader` + GOB resync (`encoder_flush` + PSC_FIFO drain + `decoder_reset`). Bonus fix: `level_index_to_byte` off-by-one in escape-coded levels (>±12) — masked in r13 by low-quant intra. SAC + Annex F deferred. 106 tests. |
| **H.261** | ✅ I + P pictures on QCIF / CIF (integer-pel MC + optional loop filter); ffmpeg-PSNR harness: >66 dB intra, >68 dB clean P-chain. **r14**: chained-P decoder bug fixed (GOB-end start-code peek bailed when `bits_remaining<16` even though start code IS ≥16 bits — last MB of GOB 5 was being skipped). | ✅ Full Baseline I + P encoder with integer-pel ME ±15 pels + FIL (loop filter) MTYPEs + MQUANT delta rate control. testsrc QCIF: **39.37 dB / 8517 B** byte-tight vs ffmpeg. Spec correction landed: half-pel is H.263+ not H.261 (§3.2.2). 77 tests. |
| **MS-MPEG-4** (v1 / v2 / v3) | 🚧 Clean-room implementer handoff per `docs/video/msmpeg4/` (spec/99 + 40 tables). v1 + v2 + v3 picture headers + MCBPCY decoders + v3 P-frame MV/residual pipeline + intra-AC plumbing behind `AcSelection::Candidate`. v1/v2 MV decoder lands with **spec correction**: spec/06 §2.3 + spec/07 §3.3 say "33 entries" — actually **65** (bias arithmetic requires raw idx [0,64]); full 16384-byte LUT recovered (SHA-256 verified) vs the 4096-byte truncation in docs. Real-content PSNR gated on intra-AC + inter-AC VLC tables which are fused in a 68KB tiered-Huffman tree (spec/99 §9 OPEN-O4) — needs further clean-room binary RE. 159 tests. | — |
| **H.264** | 🚧 **Spec-driven rewrite from scratch** (~45k LOC). All §7/§8/§9 core layers: NAL/SPS/PPS/slice parsing, FMO (all 7 map types), I/P/B-slice reconstruction, POC types 0/1/2, DPB + MMCO + RPLM, 6-tap luma / bilinear chroma MC, weighted pred, spatial + temporal direct MV, 4×4/8×8/Hadamard transforms, deblock (§8.7), CAVLC + CABAC engines, 7 SEI types, 4:2:0 + 4:2:2 chroma. **16 integration conformance tests**; recent JVT-vector batches **5/14/14/12 pixel-exact vs ffmpeg**. Deferred: 4:4:4 + MBAFF + CABAC I_PCM term; Annex F SVC / G MVC / H 3D-AVC are long-term. | 🚧 Clean-room Baseline encoder: SPS+PPS+IDR + all 4 I_16x16 modes (V/H/DC/Plane, SAD decision §8.3.3) + all 4 Intra_Chroma modes + chroma residual + I_NxN (all 9 §8.3.1.2 modes, per-MB I_16x16-vs-I_NxN decision) + luma AC residual (`cbp_luma=15`). **r14**: in-loop §8.7 deblocking on local recon via `encoder::deblock` adapter (per-MB `MbDeblockInfo` carrying 4×4 AC nonzero bitmaps for bS=2) — gradient **42.58 → 48.05 dB (+5.47 dB)**, mixed bars 50.11 → 51.72 dB. Oriented-edges fixture: **I_NxN picked at 41.25 dB vs ~22 dB I_16x16-only**. Noisy 64×64: 37.63 dB. **ffmpeg interop bit-exact maintained on every fixture** (≤1 LSB tolerance). 768 lib + 11 integration tests. Next: Lagrangian RDO mode decision, P-slices. |
| **H.265 (HEVC)** | ✅ I/P/B slice decode, **8-bit + Main 10 + Main 12** for 4:2:0; **4:2:2 intra + P/B inter** also bit-exact (∞ dB) on gray fixtures vs ffmpeg, 23 dB on textured. SAO + deblock + scaling lists. AMVP POC scaling + TMVP BR/center 16×16 snap. HEIF/HEIC decode (opt-in `heif` feature). **r14**: `Yuv422P12Le` + `Yuv444P12Le` added to oxideav-core — clears the cross-crate gate so a future round can lift 4:2:2/4:4:4 12-bit `Unsupported`. 42 integration + 85 unit tests. Gaps: tiles+WPP, AMP, 12-bit 4:2:2/4:4:4, `interSplitFlag` CABAC desync. | ✅ Baseline CAVLC I+P + Main-profile CABAC P-slice encoder — integer-pel MVD, 2Nx2N only, DCT+flat-quant, local recon matches our decoder pixel-exact. I 45 / P 31 dB via our decoder; ffmpeg accepts with zero errors at 26.82 dB. |
| **H.266 (VVC)** | 🚧 Full VVC front-end + CTU walker + leaf-CU reader. **Reconstruction**: PLANAR/DC/cardinal angular intra (modes 2/18/34/50/66) + flat dequant + separable inverse 2D DCT-II (incl. size-2 + size-64) + reconstruct/clip + chroma at 4:2:0 half-res. **In-loop filters**: §8.8.3 deblock (short-tap + long luma 5/7-tap + chroma strong/weak) + §8.8.4 SAO (EO/BO with §7.3.11.3 CABAC parser). 4:2:0 IDR pictures decode end-to-end with color. CABAC residual + dequant scaling lists complete (§9.3.4.2.8 / §8.7.3). 316 lib + 10 integration tests. Pending: asymmetric long luma, ALF, P/B inter slices, dual-tree intra, CCLM, MIP/ISP/BDPCM. | ✅ Encoder scaffold — forward-bitstream emitter (VPS/SPS/PPS/PH/IDR) that parse-roundtrips through the decoder's front-end byte-for-byte. 8-bit 4:2:0, CTB=128, all tool flags off, empty coded-slice payload. No residual / CABAC / pixel output yet. |
| **VP6** | ✅ Full FLV playback (845/845 frames of sample decode cleanly; range coder + MB-types + IDCT + MC + loop filter + vp6a alpha). | ✅ VP6F keyframe encoder: forward 2D DCT-II + AC quantisation + zig-zag + bool-coded VP6_PCR_TREE walker + coefficient-model state mirroring decoder. Gradient content Y 36-40 dB at QP 4-32 through ffmpeg decode. P-frame skip encoder (flat-to-flat 63 dB). MV encode / loop-filter emission / Huffman path pending. |
| **VP8** | ✅ I+P frames (6-tap sub-pel + MV decode + ref management) | ✅ I + P frames, all 5 intra modes + SPLIT_MV + loop filter (42-51 dB PSNR) |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate §6.5 MV list + inter-mode ctx + §6.3 compressed-header probs + §8.10 saved slots + neighbour-ctx tracking. **Lossless: bit-exact (66.77 dB)** for both keyframe and inter — partition-context bsl inversion + §9.2.1 marker bit + §9.3.2 inter-ref contexts all root-caused over rounds 10-12. **Lossy compound: 10.49 dB** (slowly rising; r13 fixed 4 keyframe residual bugs +2.36 dB; r14 added §8.5.1 above-row extension gating + D45 boundary +0.20 dB). 154 tests. Next: deep coef-tree probabilities, sub-8x8 chroma. | — |
| **AV1** | 🚧 OBU + range coder/CDFs + coeff decode + partition + transforms + all intra predictors (incl. filter-intra §7.11.2.3) + §7.15 CDEF + Loop Restoration (Wiener + SGR) + inter MC (NEWMV/GLOBALMV) + ref-scaling. Palette mode end-to-end (§5.11.46/49/50, §7.11.4). All 9+13 default CDFs fixed for survival-form (`P(X>i) = 32768 - cdf_spec`). §5.11.10 `read_skip_mode` + §5.11.19 `inter_segment_id` + `skipModeAllowed` derived from DPB OrderHint trail (§5.9.21/§7.9.2). **r14**: multi-ref DPB carries `Arc<FrameState>` per slot — SKIP_MODE compound MC dispatches to `compound_mc_luma_u8/_u16` (average MC). Gray P-frame Y 43.10 dB. PSNR on SVT-AV1 fixtures bounded by upstream intra gap (intra → all-128 luma). 339 tests. Pending: var-tx residual-per-TU exercise, ref-MV list (§7.10), real intra recovery. | — |
| **Dirac / VC-2** | ✅ VC-2 LD + HQ intra end-to-end + Dirac core-syntax intra (VLC and AC paths) + core-syntax inter + OBMC motion comp + full IDWT (7 wavelets) + arithmetic coder + 10/12-bit output. ffmpeg-interop bit-exact: 8-bit 4:2:2 + 8-bit 4:4:4 + 10-bit 4:2:0. Gaps: VC-2 v3 asymmetric transforms (SMPTE ST 2042-1 ed-2 not in docs/), `Yuv422P12Le` / `Yuv444P12Le` (now in oxideav-core as of r14 — wiring pending). | ✅ HQ + LD intra encoders. HQ bit-exact through ffmpeg decode (∞ dB). LD testsrc 49.63 dB through ffmpeg (was stuck 14.27 dB until §1.3 `intlog2` convention vs ffmpeg's bit-width reader was reconciled). Comprehensive regression matrix: all 7 wavelets × 3 chromas + non-square dims + multi-stream. 132 tests. |
...
| **JPEG 2000** | ✅ **Bit-exact vs OpenJPEG on all test fixtures.** Part-1 baseline + multi-tile (§B.3) + MQ + EBCOT + 5/3 + 9/7 IDWT + tier-2 + JP2 wrapper. **All 5 progression orders** (LRCP/RLCP/RPCL/PCRL/CPRL) + **multi-layer** (§B.10) + **user precincts** (§A.6.1 Spprecinct) + **POC marker** (§A.6.6 / §B.12.2-3) + **PPM/PPT** (§A.7.4-5, end-to-end via body+header splitter). 110 tests. Pending: encoder-side POC/PPM/PPT, OPJ RGB MCT bit-exactness (~78% sample mismatches — likely RCT shift). | ✅ 5/3 lossless + 9/7 irreversible RGB (forward RCT/ICT; JP2 box wrapper). LRCP only. |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MOD** | ✅ 4-channel Paula-style mixer + full ProTracker 1.1B effect set. **Round 14 fidelity pass**: fixed sample loop boundary (was wrapping at `pcm.len()` instead of `loop_start+loop_length`, leaking tail data on attack+sustain layouts), sample-swap-without-note now defers PCM swap to next note-on per PT (only loads vol+finetune immediately), defensive loop-metadata clamp. Documented remaining PT-vs-spec gaps in new "Real-world MOD fidelity" README section: LED filter, vibrato sign convention, period-clamp range, E6/Dxy ordering. 73→77 unit + 33 integration tests. | — |
