...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~89% — Phase-2 PCM→MP3 + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed MDCT + auto block-type SM + outer-loop on Long/Short/Mixed + r160 §C.1.5.4.3 long-family transition-skeleton wiring (Long/Start/End share the long primitive; 554 tests); lacks Annex D psy (#1048) + intensity-stereo + auto-SM Mixed dispatch + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~88% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + §13.4 fitter threaded into stream drivers + r160 §11/§12.2 intra-within-inter MB picker (DC_PRED Y+UV opt-in, prob_intra fitted; 532 tests) |
...
| **AV1** | 🚧 ~52% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.9/10/11/12/13/20/55/56 leaves + r160 §5.11.8 intra_segment_id + §6.8.2 LosslessArray lookup (501 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~78% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + color-indexing + multi-meta-prefix + histogram-distance clusterer + §5.2.2 depth-3 lazy LZ77 + r160 §4.1 slack-cost tie-break variant (non-regressing extension of r159; strict-beats by 1–36 B on 12/20 seeds; 375 tests); lacks VP8-lossy encode (pending vp8 republish #1068) |
