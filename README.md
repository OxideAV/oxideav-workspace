...
| **MP3** | ✅ ~100% — MPEG-1/2/2.5 frame walker + Layer III main-data Huffman + requantize + stereo (MS+intensity) + IMDCT + polyphase synthesis; end-to-end bitstream→PCM bit-exact vs mpg123 on test corpora; ID3v2/Xing demuxer w/ seek | 🚧 ~70% — Phase-2 encoder: PCM→MP3 driver + outer loop + bit-reservoir + true-VBR + Xing TOC + CRC-16 + stereo + r146 joint-stereo MS (84/85 dB @ 192 kbit/s) + r147 §C.1.5.4.3 scalefac_scale 0→1 escalation; ffmpeg + mpg123 accept output; lacks Annex D psy (#1048) + intensity-stereo encode + preemphasis + LSF |
...
| **MPEG-4 Part 2** | 🚧 ~45% (post-2026-05-18 orphan) — I-VOP intra decode through §7.4.5 IDCT + Figure 7-5 predictor gathering + r147 §6.2.5 video_packet_header (rectangular shape); lacks Figure 7-34 MV-predictor (#1125) + inter+B-VOP + encoder | 🚧 scaffold |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures (incl. ARNR, golden refresh, SPLITMV recon) | 🚧 ~60% — Phase-2 I+P stream encoder; r147 §16.4 SPLITMV picker (16/16 SPLITMV on per-quadrant translation, 39.7 dB); lacks GOLDEN/ALTREF + multi-partition inter + §9.4 mode/ref delta |
...
| **AV1** | 🚧 ~30% — orphan-rebuild; full uncompressed header + msac symbol decoder + §9.4 default-CDF families + §8.3.2 ctx selectors + r147 §5.11.49 palette_tokens walker; lacks tile-content reconstruction + ref-frame update | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate (BT.601 YCbCr→RGBA); `oxideav_core::Decoder` registered | 🚧 ~50% (post-r144 Hat-3 rebuild) — VP8L lossless encoder w/ LZ77 + color-cache + r146 §4.1 spatial-predictor + r147 §3.5.2/§4.2 color-transform (-13% on correlated noise); container + metadata + animation builders; lacks VP8-lossy encode + 10 r131-r144 pieces (#1120) |
