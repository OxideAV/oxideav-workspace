...
| **Lagarith** | ✅ ~95% — all 11 wire types + modern range coder with spec/02 §5 three-way fast path + legacy adaptive-CDF + Fibonacci-Zeckendorf prefix + JPEG-LS Median + G-pivot decorr + zero-run RLE + pair-packed 513-entry CDF (Strategy F, decode-only for proprietary type-7 streams) + modern RGB(A) first-column predictor Rule B (spec/06 §3.2, byte-exact vs ffmpeg lagarith decoder) | 🚧 ~76% — encoder for SOLID/RGB/RGBA/YV12/YUY2/legacy-RGB + spec/02 §5 Step-A + Step-B + Step-C `freqs[]` cache (1.08× on Step-C-heavy fixtures, 244 MSym/s) + r135 modern-coder q≥1 frequency rescale (>TOP-pixel planes now encodable); byte-exact vs proprietary encoder Auditor-blocked |
...
| **JPEG XL** | 🚧 ~86% — ISO/IEC 18181-1:2024 final core. 7 small lossless fixtures decode PIXEL-CORRECT. Modular path + ISOBMFF `FF 0A` strip + 1..16 bpp pack + §F.3 zero-pad single-TOC fast path; VarDCT scaffold with Annex I.2 IDCT + GetDCTQuantWeights + 17-slot dequant set + §C.7.1 HfPass + §C.8.3 PassGroup HF + §F.3 HF dequantisation + §I.2.5 LLF-from-LF math step + §C.7.1 DecodePermutation (non-natural coefficient orders); lacks §C.7.2 histograms + per-block coefficient loop + CfL / Gaborish / EPF | — retired |
