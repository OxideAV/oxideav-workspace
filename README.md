...
| DVD-Video | ✅ | — | — | `oxideav-dvd` Phase 3b — ISO 9660 + UDF 1.02 mount + VIDEO_TS walk + IFO body parser (VMGI/VTSI + TT_SRPT + VTS_PTT_SRPT + PGCI [+ PGC subpicture colour-LUT + pre/post/cell nav command table] + VTS_C_ADT + chapter materialiser) + VOB demux (MPEG-PS pack/PES + Nav-Pack PCI/DSI [+ PCI highlight + DSI typed sections] + DVD substream router for AC-3/DTS/LPCM/subpicture) + VOB → MKV mux (`mkv-output` feature; per-PES PTS preserved + ChapterAtom per `DvdChapter` via RFC 9559 §5.1.7) + `dvd://` URI handler + r172 typed NavInstruction VM disassembler (Phase 3c precursor: full Link family + 13-entry link-subset + Jump/Call SS + Set arithmetic + Type 4..6 classifier). Lacks VM execution (interpreter over SPRMs/GPRMs/RSM/PC), CSS auth (Phase 3c + `oxideav-css`) |
...
| **Speex** | 🚧 r6 — Ogg stream-header + NB (Table 9.1 + CELP body bit-reader) + WB high-band (Table 10.1 + body bit-reader, modes 0..=4) + §5.5 in-band signalling + r165 typed packet → frame iterator composing rounds 2..r160 (89+ tests); lacks LSP-VQ + pitch/innovation codebooks (#969) + LSP→LPC + synthesis + UWB framing | 🚧 scaffold |
...
| **WavPack** | 🚧 r11 (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + run-length n-decoder + Golomb (base,add) interval + per-sample value reconstruction + single-call `decode_sample` + EntropyInfo→Medians bridge + block-header accessor coverage (lossless / sample-rate sentinel / experimental / effective bit-depth / audio-block / payload-bytes) + r130 MD5 typed view + walker finders + payload-kind predicates + r172 per-term `decorrelation_sample_count` + flat-payload partitioner (refuses stereo/reserved per-term-count docs gap); 150 tests; lacks median-adaptation amount (#992) / stereo per-term sample count / prediction loop / float+multichannel / CRC / encoder | 🚧 scaffold |
| **DTS** (Core) | 🚧 ~25% — frame-sync header (104/120-bit window) + 14↔16-bit pack/unpack + four-encoding round-trip + iter_frames_resync multi-frame walker + r165 find_next_sync first-byte gate (252/256 short-circuit; 163 tests); lacks §5.3.1 alloc/scfac (#928) + §5.4 polyphase filterbank (#1055) + 14-bit container-byte advance (#1084) | — |
...
| **FFV1** | 🚧 ~72% — RFC 9043 decoder + demux + decode_frame driver (YCbCr + RGB Y/Cb bit-exact; RGB Cr divergence open) | 🚧 ~30% — Slice Footer + Slice Header + Golomb-Rice primitives + frame-level Golomb-Rice + YCbCr encoder + r164 range-coded SliceContent encoder (RangePlaneEncoder + encode_frame_range_coder round-trip via decode_frame; 337 tests); lacks Decoder registration (#904) + RGB Cr fix + coder_type==2 table swap + RGB encode path |
...
| **MPEG-2 video** | 🚧 ~44% — §6.2.x sequence/GOP/picture/slice + macroblock_type + cbp + macroblock_modes + motion_vectors + §7.6.3.x PMV + §7.6.4 forming-predictions pel reader + §7.6.7 combine + §7.6.8 add-and-saturate + r165 §7.6 per-macroblock pipeline driver (490+ tests); lacks residual VLCs / IDCT (#1110) | 🚧 scaffold |
...
| **Theora** | 🚧 ~35% — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3-default tables + §6.4.x quant + DCT-token Huffman + §7.1–§7.5 frame walk + r160 §7.5 motion vectors (Table 7.23 + LAST1/2 + INTER_MV_FOUR PF=0/2/4 chroma averaging) + r165 §7.6 block-level qi decode (207+ tests); §6.4.1 LFLIMS body blocked (#944) | 🚧 scaffold |
...
| **H.266 (VVC)** | 🚧 ~67% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine sub-block MC + PROF + AMVP + SbTMVP + merge_subblock + §7.3.11.7 non-merge inter affine-syntax dispatcher composing inter_affine_flag + cu_affine_type_flag → MotionModelIdc via §8.5.5.2 eq. 160 (~1047 tests); lacks non-merge inter CU walker call-site | 🚧 ~86% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred + r177 §7.3.11.7 non-merge affine-syntax encoder dispatcher (Table 84/85 ctx slots; gate-closed = zero-bin per §7.4.12.7) — see crate README |
...
| **Cinepak** (CVID) | ✅ ~98% — frame header + multi-strip + V1/V4 codebooks + intra/inter + grayscale + Sega FILM demuxer + Saturn/3DO deviant + cargo-fuzz (4 targets) + criterion benches | ✅ ~98% — stateful encoder + rolling codebooks + RDO + LBG + 3-axis grid picker + bitrate-target rate-control + keyframe-interval + r155 ffmpeg multi-frame inter cross-decode (34.18 dB PSNR) + r160 profiling driver + baseline (decode 4.4 GiB/s, stateful GOP 13.5 ms/frame) |
| **SVQ1/SVQ3** (Sorenson) | 🚧 r6 (orphan rebuild) — SVQ1 frame-header + framework registry (SVQ1/svqi FourCC) + SVQ3 SEQH + slice + MB-type tree + residual coefficient walker + r179 inter-MB motion-vector precision selector (Fullpel/Halfpel/Thirdpel, P-frame consumes 0-2 bits, B-frame short-circuits to halfpel); 156 tests; SVQ1 pixel decode blocked on docs (§14.10/§14.11 codebook bytes #429) + SVQ3 MV-VLC table | — |
...
| **`oxideav-http`** | HTTP / HTTPS source driver | ✅ `http://` + `https://` via pure-Rust `ureq` + `rustls` + `webpki-roots`; Range-request seeking; `HttpConfig` policy layer (timeouts, redirect cap, custom headers) + r171 RFC 7233 §4.2 Content-Range validation on 206 + §3.1 200-fallback prefix-drop |
...
| **`oxideav-rtmp`** | RTMP ingest + push | ✅ Server + client; AMF0/AMF3 parser/builder; Enhanced-RTMP v1 video + v2 audio + ModEx; pluggable key-verification; `rtmp://` PacketSource; symmetric teardown + client `poll_event` + r164 injection-robust parser surface (PRNG-fuzzed AMF0/AMF3/FLV/ChunkReader/handshake + 64-level nested-container depth caps) |
