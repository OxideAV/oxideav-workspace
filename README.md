...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | DirectShow IBaseFilter::Run = S_OK; IPin::ReceiveConnection probed (codec returns VFW_E_NO_TYPES on staged MP43 AM_MEDIA_TYPE, awaiting graph-aware negotiation) | DirectShow IBaseFilter wrapper (r25): COM scaffolding (11 IIDs + GUID parser + ComObjectTable + vtable dispatch) + ole32 stubs + DllGetClassObject + IClassFactory::CreateInstance + IBaseFilter Stop/Pause/Run/EnumPins all S_OK. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. user32 cascade (r26): CreateWindowExA + DestroyWindow + IsWindow + UpdateWindow + GetMessageA + DispatchMessageA family — all return synthetic-HWND or no-op-success, no real window opened |
