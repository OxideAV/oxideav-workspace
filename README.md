...
| **AAC** | ✅ AAC-LC (mono+stereo, M/S) + HE-AACv1 SBR + HE-AACv2 PS spec-accurate, 48 dB on HE-AACv1 reference. **r17: `bs_limiter_gains` wired** (Table 4.176 `{0,1,2,3}`→`{0.5,1.0,2.0,1e10}`) — parser was extracting the field but `apply_envelope_with_limiter` hard-coded the 2.0× cap. | ✅ AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS. **r17 audit: the README's "PCM-scale convention" hypothesis was wrong** — switching both sides to float [-1,1] left v2 ffmpeg PSNR unchanged at 9/6 dB and regressed v1 afconvert path 48→22 dB. The actual ffmpeg-interop blocker is structural ("No quantized data read for sbr_dequant" rejection), not numerics. 152 tests. |
...
| **AC-3** (Dolby Digital) | ✅ Full decode + FFT IMDCT + §7.8 downmix. Sine 92.02 dB / transient 92.77 dB vs ffmpeg. | ✅ Encoder: rematrix + short-block emit + transient detection + **§7.4 channel coupling** (5 bands, ~half the HF mantissa cost). ffmpeg cross-decodes coupled streams. dba pending. |
| **AC-4** (Dolby) | 🚧 Full A-SPX front-end + DRC + DE + outer metadata walker. **r17: ASPX_ACPL_2 wired into Ac4Decoder** — `acpl_data_1ch` parsed, IIR delay-line + diff state survive across frames, Pseudocodes 115/116/121 + Tables 197/203-206 + Table 61 `sb_to_pb`. 319 tests. Pending: ASPX_ACPL_1 joint-MDCT residual layer, ASPX_ACPL_3 multichannel. | — |
...
| **H.264** | 🚧 **Spec-driven rewrite** (~45k LOC). All §7/§8/§9 core layers; I/P/B-slice reconstruction, POC 0/1/2, DPB + MMCO + RPLM, 6-tap luma / bilinear chroma MC, weighted + direct MV, deblock, CAVLC + CABAC, 4:2:0 + 4:2:2. 16 integration tests. Pending: 4:4:4, MBAFF, Annex F/G/H. | 🚧 Clean-room Baseline encoder: I-only modes + I_NxN + luma/chroma AC + deblock + Lagrangian RDO + **P-slice support** (P_Skip + P_L0_16x16 + **integer + half-pel ME**). I+P self-roundtrip & ffmpeg interop bit-exact (50.28 dB on 4-pixel-shift, 49.19 dB on ½-pel-shift fixture). 900 tests. Next: quarter-pel ME, 4MV, B-slices. |
| **H.265 (HEVC)** | ✅ I/P/B decode 8-bit + Main 10/12 in 4:2:0 + 4:2:2 (8/10/12-bit) intra+inter, bit-exact. SAO + deblock + scaling lists. HEIF/HEIC opt-in. **r17: AMP (§7.4.9.5) confirmed already wired end-to-end** — the "AMP rejected" claim was doc-only; smoke test on libx265 `--rect --amp` clip decodes 3 frames cleanly with bit-exact post-P IDR. `interSplitFlag` spec-correct fix attempted, reverted (regressed main10/12 inter EGk). 46 tests. Pending: 4:4:4 12-bit, tiles+WPP, `interSplitFlag` desync root cause. | ✅ Baseline CAVLC I+P + Main-profile CABAC P-slice encoder. I 45 / P 31 dB via our decoder; ffmpeg accepts at 26.82 dB. |
| **H.266 (VVC)** | 🚧 4:2:0 IDR decodes end-to-end with color. Full intra + inverse transform + chroma reconstruction. Deblock + SAO + ALF (luma + chroma + 25-class × 4-transpose). **r17: CC-ALF apply (§8.8.5.7) + AlfFixFiltCoeff/AlfClassToFiltMap (§7.4.3.18 eqs. 90/91) + §8.8.5.2 fixed-filter wiring** (was previously surfaced as "luma off"); fixed `AlfPicture::is_all_off` bug that short-circuited CC-ALF-only CTBs. 375 lib + 14 integration tests. Pending: P/B inter slices, dual-tree, CCLM, MIP/ISP/BDPCM. | 🚧 Forward CABAC + forward DCT-II + flat quant landed (round-trip bit-identical against decoder). Residual emit + pixel output wiring still pending — r17 scope-trimmed in favour of CC-ALF. |
| **VP6** | ✅ Full FLV playback (845/845 sample frames). | ✅ VP6F keyframe + skip P-frame encoder. **r17: inter-frame encoder with integer-pel SAD ME** (§10 PMBT + §11 short-MV tree) — own decoder roundtrips +8 dB Y on 4-px-shift fixture; ffmpeg `vp6f` decoder still rejects our inter frames (FLV mux structurally correct per ffprobe; divergence in inter-frame probability-model layer). Latent skip-frame coeff-prob bug fixed (was emitting at prob 0 vs decoder's 0x80 baseline). 53 tests. Loop-filter emission + Huffman path still pending. |
| **VP8** | ✅ I+P frames (6-tap sub-pel + MV decode + ref management) | ✅ I + P frames, all 5 intra modes + SPLIT_MV + loop filter (42-51 dB PSNR) |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV list + inter-mode ctx + compressed-header probs. **Round-17 audit: the 66.77 dB lossless number was a degenerate measurement** — the gray fixture's reference is a constant plane, so any "approximately gray" output trivially scores >60 dB. New `vp9_lossless_pattern` test against an `ffmpeg testsrc -lossless 1` reference shows the real lossless decode at **9.69 dB Y / 10.96 dB U / 9.26 dB V** (≈ every byte differs). Lossy compound also ~10.6 dB. 155 tests. Major pending: lossless WHT path (§8.7.2) does not reproduce ffmpeg's output bit-for-bit; lossy keyframe / inter reconstruction has a structural defect that flattens std-dev and shifts colour means. | — |
| **AV1** | 🚧 OBU + range coder + coeff decode + partitions + transforms + all intra predictors + CDEF + LR + inter MC + palette + skipModeAllowed + multi-ref DPB compound MC. **r17: §5.9.2 `ref_order_hint` gating fix** (was over-reading up to 56 bits per inter frame on non-error-resilient streams) **+ §5.9.20 `lr_unit_extra_shift` inversion fix**. SVT-AV1 fixture: pkt1 0/5→5/5 Frame OBUs parse cleanly; whole-fixture 6/30→21/30. Sacred SkipMode-compound + gray-P 41.97 dB invariants preserved. 344 tests. Pending: 9 visible-frame Frame OBUs trip in `global_motion_params`, palette finalization, irregular TX shapes (64×56, 32×41), loop-filter gaps, intrabc. | — |
...
| **AVIF** | 🚧 End-to-end decode works (HEIF box walker → AV1 OBU handoff → `VideoFrame`). Flat-content fixtures decode cleanly; rich content gated by AV1 quality. **r17: investigation closed on `bbb_alpha` and `kimono_rotate90`** — AVIF wrapper passes well-formed OBUs; the prior `symbol.rs:105` panic is gone, AV1 now surfaces a clean `Unsupported: TX 64×56` / `TX 32×41` (irregular bottom-edge chroma TX shapes). New end-to-end `irot` round-trip + `transforms_for` filtering tests. 44 tests. Pending: gated on oxideav-av1 TX-shape coverage + intrabc. | — |
