...
| **`oxideav-http`** | HTTP / HTTPS source driver | ✅ `http://` + `https://` via pure-Rust `ureq` + `rustls` + `webpki-roots`; Range-request seeking; `HttpConfig` policy layer (timeouts, redirect cap, custom headers) + RFC 7233 §4.2 Content-Range validation on 206 + §3.1 200-fallback prefix-drop + r179 RFC 9110 §15.5.17 + §14.4 416 unsatisfied-range surfacing |
