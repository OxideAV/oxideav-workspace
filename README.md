...
| AMV       | ✅ | ✅ | — | Chinese MP4-player format — demuxer + muxer + seek + strict-mode validators + §4b IMA/DVI-ADPCM audio decode (1116 blocks → 93.0 s) + fuzz harness |
...
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + ETSI codebooks + ACPL cfg0..3 + SAP + IMS bitstream walker; lacks object/A-JOC | 🚧 ~80% — IMS v0/v2 mono → 7.1, all eleven ACPL coupling layers real (α/β/γ incl. β₃) + TIME-direction A-SPX envelope DPCM + 5_X ACPL_3 live real QMF-derived ASPX envelope + multi-envelope (transient-driven num_env) live ASPX emission + live 5_X ASPX_ACPL_2 real `aspx_data_1ch()` centre-carrier envelope + live 7_X ASPX_ACPL_2 real ASPX envelopes; lacks mono multi-env live path + 7_X Table-202 back-pair Lb/Rb |
...
| **DTS** (Core) | 🚧 ~57% — frame header + 14↔16-bit pack/unpack + side-information subframe walker + Annex C reconstruction primitives + Annex D codebooks (incl. §D.6 block-code books + §C.2.1 table-look-up decoder) + §D.8 FIR tables + fused 32-band synthesis QMF driver (§C.2.5 QMFInterpolation, per-frame multi-channel) + §D.2 step-size tables + §5.5 inverse-quant scale composition + §D.5.7–§D.5.9 13/17/25-level audio books; lacks audio-array walk → PCM | — |
