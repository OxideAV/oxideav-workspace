...
| **MP1** | 🚧 clean-room rebuild underway (2026-05-24) — Layer I frame-header parse + frame sync + frame-length math + CRC-presence wiring + Layer I audio-data decode to requantized subband samples (bit allocation, scalefactor index, §2.4.3.2 requant; mono/stereo/joint-stereo; 38 tests, ISO/IEC 11172-3 only); lacks scalefactor rescale + polyphase synthesis (blocked on missing Annex B, #1016) + CRC verification | 🚧 not started (rebuild) |
...
| **MP3** | 🚧 clean-room rebuild underway (2026-05-24) — MPEG-1/2 frame-header parser + per-frame length + resyncing frame-walker + Layer III side-info parser for both MPEG-1 (2-granule) and MPEG-2/2.5 LSF (1-granule) layouts (§2.4.1.7; typed `SideInfo`/`GranuleChannel`; 38 tests, ISO/IEC 11172-3/13818-3 only); lacks main-data Huffman/requantize/IMDCT (Annex B codebooks missing from staged PDF, #1016), encoder | 🚧 not started (rebuild) |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + B-pyramid POC + **35 SEI types** (+content_colour_volume §D.1.33 type 149 in r107; +spare_pic §D.1.10 in r103; +3 in r99: sub_seq_info / sub_seq_layer_characteristics / sub_seq_characteristics §D.1.11–13) + fuzz-hardened slice/MC/SPS bounds + r91 strictness fixes — fuzz CI green; lacks MBAFF, SVC/3D/MVC | 🚧 ~82% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite; ffmpeg PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 r6 (post-2026-05-18 orphan) — Annex B + §7.3.1.2 NAL + §7.3.2.1 VPS + §7.3.3 PTL + §7.3.2.2 full SPS body (prefix + PCM + §7.3.7 short-term RPS both explicit + inter-RPS-prediction forms + long-term RPS + MVP/smoothing + opaque VUI/ext tail) + §7.3.2.3.1 PPS (tiles + deblocking-control + lists-mod/parallel-merge tail, se(v)) + §7.3.6.1 slice header (both IDR and non-IDR independent I-slice end-to-end through byte_alignment; POC + short-term RPS sps-flag/inline/idx + long-term RPS array; P/B ref-list/pred-weight bodies opaque); r7, 65 tests; lacks scaling_list_data / VUI body decode / ext bodies / CABAC (#444) | 🚧 scaffold |
...
| **glTF 2.0** (+ .glb) | ✅ ~92% — JSON + .glb + full PBR + KHR_lights_punctual + KHR_materials_unlit + KHR_materials_emissive_strength + KHR_materials_ior + KHR_materials_specular (decode + encode + §3.12 validator) + skin + skeletal animation (LINEAR/STEP/CUBICSPLINE) + sparse accessors + morph-targets + 11 spec-MUST validators + JSON fuzz hardening; lacks KHR_audio_emitter / KHR_materials_clearcoat/sheen/transmission/volume / KHR_texture_transform (further KHR specs newly-staged at `docs/3d/gltf/extensions/`) | ✅ ~91% — symmetric + sparse-encoding heuristic + signed+unsigned normalised-int quantisation + KHR_materials_unlit emit |
