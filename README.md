...
| Matroska  | ✅ | ✅ | ✅ | MKV/MKA/MKS; Cues seek; SeekHead/Chapters/Attachments/subtitles; opt-in block lacing on write; EBML + per-Cluster CRC-32; typed Tag/TrackOperation/ContentEncodings/Video FlagInterlaced/FieldOrder/geometry quartet/Colour master + RFC 9559 §5.1.4.1.28 scalar children + MasteringMetadata (§5.1.4.1.28.30..40 read + mux + bt2020_d65_hdr10 helper)/SMPTE 2086/StereoMode/Projection/AlphaMode/UncompressedFourCC/BlockAdditionMapping read+write |
...
| AVI       | ✅ | ✅ | ✅ | AVI 1.0 + OpenDML 2.0 demux/mux; AVIX/dmlh/vprp + 2-field interlaced + VBR audio + LIST INFO + WAVEFORMATEXTENSIBLE + ODML keyframe seek + typed PaletteChange/TextChunk/AvihFlags/Idx1Flags + AVISUPERINDEX sub-type + per-stream strh fields (handler/SuggestedBufferSize/SampleSize/dwStart/wPriority/ChannelMask/Length/Flags AVISF_DISABLED+AVISF_VIDEO_PALCHANGES) + dmlh.dwTotalFrames master frame-count override |
...
| **CELT** | 🚧 ~31% (post-2026-05-20 orphan) — RFC 6716 range decoder + §4.3 prefix + coarse-energy scaffold + bit-allocation fields + tf_change/select/spread + post-filter + de-emphasis + Walsh-Hadamard + cache_caps50 + dynamic-band-boost + initial-reservations walk + per-band minimums + trim_offsets + Table 55 + Table 57 static-allocation matrix + static-allocation search + §4.3.4.2 PVQ codebook + per-band shape decoder + §4.3.4.1 bits-to-pulses search + balance accumulator + §4.3.4.3 spreading rotation chain (forward/reverse + pre-rotation + multi-block); blocked on docs Laplace | 🚧 scaffold |
