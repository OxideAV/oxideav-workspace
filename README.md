...
| **Opus** | 🚧 ~25% — RFC 6716 range decoder + full SILK pipeline (TOC + frame packing + NLSF→LPC + LTP + excitation + synthesis filters + stereo unmixing + resampler delay budget) + r153 §4.3 Table 56 CELT pre-band header (silence + post-filter params + transient + intra; 349 tests); CELT bands gated on #936 (Laplace) + #943 (alloc) | 🚧 scaffold |
...
| **H.261** | ✅ ~98% — I+P QCIF/CIF + integer-pel + loop filter + BCH FEC + Annex B HRD + RFC 4587 RTP payload + SDP rtpmap/fmtp + r153 cargo-fuzz daily decoder harness (5 seeds across QCIF/CIF intra+inter) | ✅ ~98% — spiral+diamond ME + GQUANT-from-bitrate + BCH framing + RTP wrap + RFC 3550 RTCP SR/RR/SDES/BYE/APP + compound packet build/parse; 45 dB at 64 kbit/s QCIF |
