...
| **H.265 (HEVC)** | ✅ ~99% — conformance surface complete (16/16 staged + 44-stream tool-axis sweep byte-exact, Rext/SCC tail applied, 2 documented reference-decoder deviations); r460 133/134 real-world HEIC stills byte-exact across 3 producers (8–12-bit, 4:2:0/4:2:2/4:4:4/mono) + conformance-window output cropping; lacks SCC reconstruction exactness on the 15 official streams | 🟢 ~90% — quadtree + RDOQ/SDH/RQT + WP + scaling lists + WPP/tiles + pyramid VBV/HRD RC; r460 any-size Main Still Picture stills at −1.4% BD-rate vs x265 on 12 MP (8.3 s on 8 workers), intra `rd` 0..2, VUI range/colour + parameter-set ids, `YuvJ420P` input; lacks 10-bit/4:4:4/SCC encode, WPP-parallel pass 1 |
...
| **HEIF / HEIC** (ISO/IEC 23008-12 + MIAF) | ✅ ~95% — real-world HEIC/AVIF from Apple ImageIO / libheif / ImageMagick byte-exact vs black-box readers (grids, alpha, 8–12-bit, `iscl`, full-range `YuvJ*` labelling, ISO 21496-1 gain maps within 1 code); wired into `oxideav-meta` + CLI (`oxideav probe/convert x.heic`); lacks `lhvC`/`avcC` items | ✅ ~95% — `oxideav convert` png→heic/avif; HEVC stills (pcm/intra, Main Still Picture, VUI range/colour, alpha by parameter-set ids, `rd`/`tiles`) + AV1 stills (lossless or quality 0–100, native 8–12-bit, mono alpha) open in sips/libheif/ImageMagick/ffmpeg; grids, thumbnails, transforms, Exif/XMP/ICC, `msf1` sequences; lacks AVIF-profile migration, `lhvC`/`avcC` |
