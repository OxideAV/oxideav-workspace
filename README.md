...
| **Opus** | 🚧 ~42% — RFC 6716 range decoder + full SILK pipeline + CELT side through coarse energy (per-LM inter α,β), allocation search, PVQ codebook + spreading + framing incl. Appendix-B self-delimiting + §4.3.7 inverse-MDCT overlap window + §4.3.4.1 pulse-cost cache (unblocks PVQ reallocation) | 🚧 ~5% — scaffold |
...
| **CELT** | 🚧 ~31% (post-2026-05-20 orphan) — range decoder + coarse-energy decode COMPLETE (RFC 6716 Appendix-A carve-out) + full allocation chain + PVQ codebook/spreading/split geometry + IMDCT/WOLA synthesis primitives + §4.3 frame-prefix decode driver (Table 56 walk to the fine-energy boundary) + §4.3.4.1 bit-exact pulse-cost cache (unblocks §4.3.3 reallocation); 0.2.0 release pending pin sweep (#1648) | 🚧 ~5% — scaffold |
...
| **WMA** | 🚧 ~13% — patent-disclosed primitives (analysis/synthesis windows + codebook grid + quantization-band layout + §4 energy-derived quantization matrix + §5 open-loop stereo channel-coding decision + §8 decoder-tail stereo time-domain reconstruction); lacks Huffman codeword tables + exponent partition + sign layout (docs-gapped) | — |
...
| **SVQ1/SVQ3** (Sorenson) | 🚧 ~42% — SVQ1 codebook payloads + leaf stage-accumulation reconstruction (spec/04 §4.5) + SVQ3 transform/dequant/intra/interp primitives + chroma DC full-dequant pipeline; lacks full block-reconstruction composition (5 precise docs gaps filed) | — |
...
| **DDS** | ✅ ~99% — header + DXT10 + BC1-7 + BC6H all modes + cubemaps/arrays/volumes + 16-bit/float + packed R11G11B10_FLOAT + R9G9B9E5_SHAREDEXP + R10G10B10A2_UINT + sub-sampled packed R8G8_B8G8/G8R8_G8B8 HDR uncompressed surfaces + daily fuzz | ✅ ~96% — uncompressed + BC1-7 + BC6H + mip chains + cubemap/array |
