...
| **HuffYUV** / FFVHuff | 🚧 ~75% — clean-room rebuild from `docs/video/huffyuv/` (orphan master 2026-05-07; pre-rebuild history on `old` branch). Round 1: file-header + method dispatch (HFYU + FFVH FourCCs registered) + frame-layout (YUY2 / RGB24 / RGB32) + Left / Median / Gradient predictors with R-G/G/B-G decorrelation + classic-tables RLE record format + per-plane canonical Huffman built longest-length-first from `tables/06..09` (length-set-a/b + codes-set-a/b loaded via `include_str!`); 29 self-roundtrip tests green; lacks 10/12-bit FFVHuff family, interlaced field-stride=2, custom (non-classic-blob) length-table generation | — |
| **Lagarith** | 🚧 ~75% — clean-room rebuild from `docs/video/lagarith/` (orphan master 2026-05-07; pre-rebuild history on `old` branch). Round 1 spec/00..06 implemented: frame layout + dispatcher table; modern range coder (TOP=2^23 / init=2^31 / four-byte init / cross-byte LSB-rotated refill) + Fibonacci-Zeckendorf probability prefix with second-Fibonacci zero-run subcode + Left + JPEG-LS clamped Median predictor + cross-plane G-pivot decorrelation + residual zero-run RLE escape with 256-entry permutation LUT (loaded via `include_str!` from `tables/01..02`) + channel-header dispatcher (incl. u32-length-overflow fall-back); types 1/2/4/5/6/8/9 (Uncompressed + arithmetic RGB24/RGB32/RGBA + literal Solid Grey/RGB/RGBA) decode; types 3 (YUY2) / 7 (legacy RGB) / 10 (YV12) / 11 (reduced-res) reject as `Error::UnsupportedFrameType`; 42 tests green; legacy ARITH (`spec/07` adaptive-CDF range coder) deferred | — |
| **Ut Video** | ✅ ~95% — clean-room rebuild from `docs/video/utvideo/` (orphan master 2026-05-07; pre-rebuild history on `old` branch). Round 1 spec/00..05 implemented: 5 native FourCCs (ULRG / ULRA / ULY0 / ULY2 / ULY4) × full FourCC + extradata parse + plane walk + slice end-offset table + single-symbol special case + frame-info dword + 4 predictors (None / Left / Gradient / Median) with per-slice +128 first-pixel seed + RGB inter-plane decorrelation forward + inverse for ULRG/ULRA + RFC-1951-mirrored canonical Huffman build (length DESC, sym DESC) + 32-bit-LE-word MSB-first bitstream + zero-padded slice data; AVI carriage now lives in `oxideav-avi` per the codec/container split; 49 tests green (5 FourCCs × 4 predictors × {1,2,3,4,7,8} slice counts + spec-byte-trace + single-symbol fast-path + non-square dimensions) | ✅ ~95% — codec-internal encoder mirrors the decoder for self-roundtrip across all 5 × 4 (FourCC × predictor) combinations; lacks UQ 10-bit / UM SymPack / interlaced encoders, raw-mode (Huffman bit clear) emit |
