...
| MOV (QuickTime) | ✅ | ✅ | ✅ | QTFF + ISO BMFF fragments / edit lists / `cmov` / muxer (unchanged); r461 HEIF/HEIC layer retired to `oxideav-heif` (−8.5k LOC, HEIF-family brands never probed as mov); full QuickTime Metadata atom both directions (`ctry`/`lang` locale sets, `itif`/`name`/`mhdr`, well-known types); fuzz clean |
...
| **AAC** | 🚧 ~96% — LC/HE-v1/v2/LD/SSR/BSAC decode + SBR/PS over the 960 core; lacks the deployed ER-LD TNS filter record | ✅ ~99% — LC + HE-AAC v1/v2 encoders with ≥3-onset VARVAR grids, 960 SBR layout, PS Auto elections, PCE layouts, equal-rate harness vs the black-box reference (NMR within −0.2..−2 dB); lacks a 960-line LC core, 22.2 height layouts (docs) |
| **CELT** | ✅ ~98% — reference-lockstep + PLC, fuzzed (RFC 8251 §8 energy-cap fix), ~20× realtime decode | ✅ ~98% — ahead of the RFC listing's encoder at every LM × rate × material point (mean +1.0–1.6 dB), custom modes 18/18, measured-cost election; lacks an encode-throughput bench |
...
| **GSM 06.10** | ✅ — FR bit-exact both directions + 06.11 RX lost-frame legs; HR (06.20) decoder 0.99 corr vs the ETSI outputs (lattice sign fix); bit-exact blocked on the unstaged 06.06 arithmetic | ✅ — FR bit-exact + DTX; HR (06.20) encoder complete (INT_LPC/MODE 91%, unvoiced codes ~80%, voiced 20%, lags ±1 66%), registry `gsm-hr`; lacks HR bit-exactness (06.06) + HR DTX/VAD (06.41/42 unstaged) |
...
| **G.728** | ✅ 100% | ✅ 100% |
| **G.729** | 🟢 ~92% — fixed-point §4.1 + §4.2 cascade + Annex A postfilter on the clause-5 grid; r461 exact §4.2.5 inversion oracle, eq (78) ×2 fix, eq (90) re-pin: corr ≥0.9988 on every clean vector, FIXED 57% byte-exact; lacks the eq (90) pole / §4.2.2 LSB schedule (#440) | 🟡 ~78% — fixed-point clause-3 chain + Annex B DTX/SID; locked-VAD frame-exact 27.5%; lacks bit-exact wire (same LSB gaps) + §B.3 VAD |
...
| **AC-3** (Dolby Digital) | ✅ ~97% — AC-3 + full Annex E decode + E-AC-3 JOC/OAMD object reconstruction with opt-in stereo presentation (community #15, provenance-reviewed) | ✅ ~98% — all Annex E tools + fractional syncframes + TPNP; equal-rate harness vs the reference encoder within 0.5–3 dB at 192–384 kbps (exponent-sharing bound + strategy election fixed a 12 dB floor); lacks low-rate coupling strategy, high-rate transient coding, AHT RD tuning |
| **AC-4** (Dolby) | 🚧 ~99% — channel / immersive / object decode incl. A-JOC + dialogue enhancement; r461 5.X/7.X A-CPL ACPL_1/2/3 synthesis on the coded carriers (the r456 silence finding closed: out/in RMS 0.78–1.12 per channel), fuzz-hardened; lacks §5.6 `d_pcm` output delay, LFE alignment on non-A-CPL A-SPX routes | 🚧 ~96% — framework Encoder, all layouts; r461 parametric 5.X/7.X A-CPL (ACPL_1/2, 5.X ACPL_3, `acpl` option); lacks short-frame ASF, rate control, encoder-side companding |
| **MIDI** (SMF) | ✅ ~99% — SMF 0/1/2 → PCM + soundfonts (SF2 LFOs/modulators/global zones) + native MIDI 2.0 mixer (16/32-bit, per-note bend/controllers, RC/NRPN, MIDI-CI profiles), .midi2 native playback | ✅ ~97% — SMF + .midi2 clip writers (fixed-point proven) + synthesis; lacks live-CC SF2 modulators + Standard Profile specs (unstaged) |
...
| **WMA** | ✅ ~95% — vendor v2 decode 1763/1763 boundaries, LSP-envelope family bit-exact (3 931 vendor conversions), validated dequantisers + cutoff/LCG; lacks v1 stream, window/abs-scale read | 🟢 ~90% — vendor-wire encoder: 47 catalogue cells black-box accepted (VLC + LSP envelopes), masking/M-S/noise-sub, reservoir RC; lacks LSP short-block/stereo vendor evidence |
...
| **MS-MPEG-4** (v1/v2/v3) | 🚧 ~95% — all three Microsoft fixtures decode end-to-end: spec/19 exact integer IDCT (I-frames ±1 vs black-box), DIV3 intra-in-P, 50/50 P-frames; fuzzed; lacks the vendor-plane 100% proof + inter-arm/chroma-MV/rounding traces | ✅ ~92% — v1/v2/v3 I+P decoder-verified, exact IDCT/DC rules mirrored |
...
| **H.265 (HEVC)** | ✅ ~99% — conformance surface complete (16/16 staged + 44-stream tool-axis sweep byte-exact, Rext/SCC tail applied); 133/134 real-world HEIC stills byte-exact; r462 Annex F/G/H multi-layer: MV-HEVC both views byte-exact (macOS-produced stream), SHVC resampling self-pinned, `layer`/`view`/`ols` options, two latent RPL bugs fixed; lacks SCC recon exactness, CGS, conformance streams for MV-HEVC/SHVC (#442) | 🟢 ~90% — quadtree + RDOQ/SDH/RQT + WP + WPP/tiles + VBV/HRD RC; 8-bit 4:2:0 Main Still Picture stills at −1.4% BD-rate vs x265; r462 every HEIC layout as lossless PCM (Gray8–16, 4:2:0/4:2:2/4:4:4 8–12-bit); lacks lossy 10-bit/4:4:4 intra (u8 coder), WPP-parallel pass 1 |
...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | ✅ ~97% — 41/41 corpus byte-exact, tile-parallel decode; registry carries the full 12-format §7.2.2 matrix incl. 4:4:0 (core 0.1.35) | ✅ ~99% — §8.4 writer-side adaptation, switchable filter incl. sub-8x8/scaled leaves, two-pass RC over structured + every format, scene-cut planning, RDOQ over the token tree (−4.8% bytes), 4 byte-identical speedups; lacks pareto-tail forward-update scoring, downscaled first pass |
| **AV1** | ✅ 100% — 155-stream corpus + r460 114/114 real-world AVIF/HEIF stills byte-exact (libaom/aomenc/SVT/ImageMagick, 1×1–12 MP, 8/10/12-bit, all layouts, alpha); `av1C` extradata seeding; decode peak RSS −35% | 🟢 ~99% — GOP drivers (pyramid, superres, SVC, grain) corpus 164 byte-exact; r460 still pictures (reduced header, any extent, quality dial, 3 speed presets, tile-parallel) at +19.8% BD-rate vs libaom s6, 12 MP in 37 s; lacks compound/opener inter-layer, pyramid mid-GOP superres |
...
| **ProRes** | ✅ 100% | ✅ 100% |
| **EVC** (MPEG-5) | 🟢 ~90% — Baseline + Main tools, §8.3 RPL bookkeeping, errata #313; 23094-4 gate red on filed entropy asks | 🟢 ~88% — intra/P/B + hierarchical B, BTT, IQT/ATS, ALF, EIPD, exact-bit RD + RDOQ, one-/two-pass RC (ADCC writer lands default-off); lacks CU QP delta/AQ, ADCC trellis |
...
| **Lagarith** | ✅ 100% — vendor corpus 187/188 byte-exact (solid-plane 0xff rule, Rule B, lazy RLE zero-count, pow2 normaliser 189/189), 42/52 vendor-lossy reproduced, fuzzed; lacks W≥4 DIB pad bytes + odd-width YUY2 vendor validation | ✅ ~95% — all 11 wire types, 45/179 vendor-identical (never larger than the vendor), NULL emission, PTS passthrough |
...
| **SVQ1/SVQ3** (Sorenson) | 🚧 ~97% — SVQ1 complete + 9-target fuzz; SVQ3 I/P decode byte-exact 10/10 fixture AUs incl. the vendor post-reconstruction edge filter (spec/09) + extended-mode read; lacks B slices / extended-mode MB layer (docs-unspecified) | ✅ SVQ1 encoder (unchanged) |
| **Indeo 3** (IV31/IV32) | ✅ ~95% — both IV32 fixtures 16/16 frames pixel-exact (intra + inter with MC, cell-geometry banks, families A/E), fuzzed; lacks families C/D + a >320-wide fixture | — |
| **Indeo 2/4/5** | 🚧 ~75% — IV50 intra byte-exact host buffer (YUY2 153 600/153 600) + 8/8 checksums on both fixtures (quantiser banks 288/288, tile phases, DC chain), fuzzed; lacks an IV50 inter fixture + IV20/IV41 specs | — |
...
| **TIFF** (6.0) | ✅ 100% — full JPEG-in-TIFF (incl. tiled §22 tables-form) + all baseline features; lacks Exif/GPS tag semantics (docs ask) | ✅ ~97% — chunky/planar/tiled all photometrics + CCITT + JPEG-in-TIFF encode (SOF0/12-bit SOF1/lossless SOF3, strips/tiles/planar, JPEGTables, DRI); lacks Exif/GeoTIFF/DNG semantics (#408) |
...
| **AVIF** | ✅ ~99% — container by `oxideav-heif` since r461 (parse / compose / `avis` / `av1C`); grid / iovl / iden / alpha / transforms at 8–12-bit, layered `lsel`/`a1op`/`a1lx`, AVIF §2–§8 audits on top; lacks `tmap`/`sato` composition (#439) | ✅ ~97% — stills / grid / overlay / iden / layered / alpha / depth / Exif / XMP through `HeifWriter` with AVIF layout, `avis` sequences; opens in heif-info / ImageMagick / ffmpeg / avifdec |
| **HEIF / HEIC** (ISO/IEC 23008-12 + MIAF) | ✅ ~98% — HEIC / AVIF / AVC / L-HEVC-base items, grids, overlays, `tmap` gain maps per 23008-12 Amd 1, alpha stills + alpha tracks, sequences, 14496-12 8th-ed HDR boxes; byte-exact vs ffmpeg on 67/67 comparable producer cells (sips, heif-enc x265/aom, magick, ffmpeg); CLI hostile suite 6 004 runs / 0 panics; lacks L-HEVC enhancement layers (h265 release) | ✅ ~97% — HEVC (pcm / intra, VUI, Main Still, ids) + AV1 (quality 0–100) stills, grids, thumbnails, alpha, Exif/XMP/ICC, transforms, tmap authoring, alpha-track sequences; opens in sips / heif-convert / magick / ffmpeg / heif-info; 12 MP baseline decode 0.30 s, HEVC encode 2.7 s, AV1 90 s (optimization next) |
...
| **OpenEXR** | ✅ 100% — scanline + tiled + deep + multi-part, all codecs; framework path: true-HDR F32, Y/RY/BY → RGB, layered/multi-view mapping, DWA decode ×1.9 | ✅ ~99% — scanline/tiled/deep/multi-part write; registry encoder: F32 family, Y/RY/BY, layer prefix, tile/level/line-order shapes; lacks per-layer multi-frame emission (core ask) + deep/multi-part registry emission |
| **Farbfeld** | ✅ 100% | ✅ 100% |
...
| **PICT** (Apple QuickDraw) | ✅ ~99% — v1 + v2 vector / raster / text / regions; r461 `$8200` CompressedQuickTime images render (`jpeg` via oxideav-mjpeg, `raw ` built-in, `cvid` via registry; matrix / matte / mask compositor shared with `$8201`), Palette Manager bit-14 sequential rule, ImageMagick-emitted PICTs byte-identical; lacks `rle `/`rpza`/`smc ` decoders, `raw ` ≤8-bit CLUT (docs gap) | ✅ ~97% — v1 / v2 rasters, patterns, QuickTime wrappers |
...
| **IFC** (BIM, ISO 16739) | ✅ Phase 3 complete / Phase 4 ~90% — STEP/P21 + EXPRESS typing + tessellation: profiles, sweeps, curved advanced Breps, non-convex mesh–mesh Booleans + curved half-spaces, p-curve-bounded + sectioned surfaces, CSG trees, Delaunay-trimmed faces, WHERE rules, georeferencing, fuzzed; lacks torus half-spaces + both-periodic implicit outers | — |
...
| **XM** (FastTracker 2) | ✅ ~98% — structural parse + full playback; 38/38 black-box oracle gates (Amiga table, LFOs, autovibrato, envelope order, FT2 order-flow machine); lacks envelope Scale (docs) | — |
| **IT** (Impulse Tracker) | ✅ ~95% — decode + full player: NNA voices, envelopes, Axx…Zxx + Sxx, 25/25 black-box oracle gates, steered fuzz; lacks 2.14 compressed samples, filters, carry/S9x/Zxx (docs) | — |
