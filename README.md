...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek | 🚧 ~83% — Phase-2 PCM→MP3 + outer loop + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + r151/r152 short+mixed forward MDCT toggles; lacks Annex D psy (#1048) + intensity-stereo + LSF + block-type auto-decision |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~78% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + §9.7/§9.8 RefreshControls + §9.4 LoopFilterDeltas + r152 P-frame pixel lockstep fix (inter MB dequant on raw coeffs before recon; closes #1140) |
...
| **AV1** | 🚧 ~43% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + Partition_Subsize tables + §5.11.4 decode_partition + r152 §5.11.11 read_skip() (~398 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~72% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + simple code-length + color-indexing + multi-meta-prefix + r152 histogram-distance per-region clusterer (2.4-5.7% smaller on noisy sweep); lacks VP8-lossy encode + 6 r131-r144 pieces (#1120) |
