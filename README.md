...
| **JPEG XL** | 🚧 ~85% — ISO/IEC 18181-1:2024 final core. 5 small lossless fixtures decode PIXEL-CORRECT. Modular path complete; VarDCT scaffold wired into pipeline (Quantizer + LfCoefficients + F.1 LF dequant + F.2 adaptive smoothing + HfMetadata + HfGlobal + GlobalModular). Auditor d1-Squeeze bisect through r17..r22 has falsified per-token hybrid-uint accounting, cluster_map uniformity, state init, prelude bits, "267-bit overshoot", per-cluster distributions, alias self-map, and WP rounding bias — divergence localised to Y' sample 22 / one MA-tree leaf-flip; ANS final state never hits §D.3.3 sentinel. r24 then byte-traced cluster 0+1 D[] arrays (sum=4096, alias-routed total=4096; 0 invariant violations across 3072 calls). Bug now confirmed NOT in the per-call ANS pipeline below the prelude — must be either upstream (D[] decoded from prelude differs from encoder's intent despite bit-correct boundaries) or in cluster/ctx selection ordering. Diagnostics in `round17..24-d1-*.md`; lacks HfPass + PassGroup HF + GetDCTQuantWeights + IDCT dispatch + XYB / CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~24 filters registered as factories (blur / edge / resize / sharpen / unsharp / gamma / brightness / contrast / level / normalize / posterize / solarize / threshold / negate / sepia / modulate / grayscale / motion-blur / emboss / flip / flop / rotate / crop / vignette / colorize / equalize / auto-gamma) |
