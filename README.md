...
| Matroska  | ✅ | ✅ | ✅ | MKV/MKA/MKS; DocType-aware probe; Cues seek; SeekHead emit; Chapters + Attachments + subtitle tracks surfaced; RFC 9559 §5.1.8 target-scoped Tags (tag:track:N:* / tag:chapter:N:* / tag:attachment:N:* / tag:edition:N:*) |
...
| **Vorbis** | 🚧 r4 (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook header + §3.2.1 canonical Huffman tree builder & entry decoder (worked-example bit-exact, errata-20150226); lacks setup-header walker / VQ vector unpack / floors / residues / audio packets | 🚧 scaffold |
| **Opus** | 🚧 r4 (post-2026-05-20 orphan) — RFC 6716 §3.1 TOC + §3.2 frame packing + §4.1 range decoder + §4.2.7.1–§4.2.7.5.1 SILK header (stereo pred / mid-only / frame-type+qoff / LSF stage-1); lacks SILK gains+LSF-2+LTP+excitation, CELT band machinery | 🚧 scaffold |
...
| **CELT** | 🚧 r4 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 frame-header prefix + §4.3.2.1 coarse-energy scaffold (21-band layout + intra prediction filter α=0/β=4915 Q15); Laplace decoder + e_prob_model table blocked on docs (#936) | 🚧 scaffold |
...
| **NSF** (NES) | 🚧 ~88% — full 6502 (incl. unofficial opcodes) + IRQ/NMI dispatch (incl. APU frame + DMC IRQ on CPU IRQ line) + 5/5 2A03 APU + DMC DMA + six expansion chips (VRC6/MMC5/S5B/N163/VRC7-coarse/FDS) + NSF v1 + NSFe + NSF v2 + NSFe extended chunks (auth/tlbl/taut/text/time/fade/plst/psfx/mixe/regn/RATE/VRC7); VRC7 still a 2-op approximation pending OPLL operator-table docs | — synthesis only |
| **Shorten** (.shn) | 🚧 r1 (post-2026-05-18 orphan) — `ajkg` magic + v2/v3 ulong parameter-block decode (spec/01 + spec/02); lacks per-block command stream + predictor + Rice residuals | 🚧 scaffold |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~96% — baseline + progressive 4:2:0/4:2:2/4:4:4/grey + SOF9 arithmetic + lossless SOF3 grey P=2..16 + 3-comp RGB P=8 | ✅ ~94% — baseline + progressive + lossless SOF3 grey P=2..16 + 3-comp RGB P=2..16 (all 7 Annex H predictors) |
| **FFV1** | 🚧 r1 (post-2026-05-18 orphan) — RFC 9043 §4.2/§4.3 Configuration Record parser + range coder + ur/sr/br symbol decoder; 3 v3 extradata fixtures decode; lacks slice / pixel decode | 🚧 scaffold |
...
| **MPEG-2 video** | 🚧 r3 (post-2026-05-18 orphan) — §6.2.2.1 sequence_header + §6.2.2.3 sequence_extension + §6.2.2.6 group_of_pictures_header (25-bit time_code + closed_gop + broken_link); lacks picture / slice / IDCT | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 r3 (post-2026-05-18 orphan) — VS/VO/VOL with promoted §6.2.3 trailing fields (interlaced / sprite_enable / quant_precision / scalability) + GOV + VOP header + VopHeader::from_vol one-call entry; lacks MB / DCT / MV decode | 🚧 scaffold |
| **Theora** | 🚧 r3 (post-2026-05-20 orphan) — §6.1 + §6.2 + §6.3 comment + §6.4 setup-header entrypoint + MSb-first BitReader (§5.2); lacks loop-filter limit table + AC/DC scale + base matrices + Huffman + VOP | 🚧 scaffold |
...
| **VP8** | 🚧 r4 (post-2026-05-20 orphan) — RFC 6386 §7 boolean decoder + §9.1 uncompressed header + complete §19.2 boolean-coded header (segmentation / loop-filter / quant_indices / token_prob_update + §13.4 / mv_prob_update + §17.2 MV_DEFAULT_PROBS / intra-Y + intra-UV mode probs / prob_intra+last+gf); lacks MB decode / IDCT / loop filter | 🚧 scaffold |
| **VP9** | 🚧 r4 (post-2026-05-20 orphan) — full §6.2 walk + §9.2 Boolean decoder + §6.3.1 read_tx_mode + §6.3.3 diff_update_prob chain (§6.3.4 decode_term_subexp + §6.3.5 inv_remap_prob + §6.3.6 inv_recenter_nonneg + 255-entry inv_map_table); lacks coef probs / inter / loop filter | 🚧 scaffold |
| **AV1** | 🚧 r4 (post-2026-05-20 orphan) — §5.3 OBU + §5.5 sequence header + §5.9.2 prefix + §5.9.5/§5.9.6/§5.9.8/§5.9.9 frame_size sub-syntax (frame_width / render / superres / mi_cols / mi_rows; 16/16 fixtures × 17 trace columns bit-exact); lacks ref-frame state, tile decode, MV, transforms | 🚧 scaffold |
...
| **Ut Video** | ✅ ~97% — 5 native FourCCs (ULRG/ULRA/ULY0/ULY2/ULY4) × 4 predictors + RGB inter-plane decorrelation + LUT-accelerated canonical Huffman + word-aligned bit reader + **slice-parallel decode (2.87×→5.63× speedup 320×240→1280×720 on 8 cores via std::thread::scope, threshold-gated)** | ✅ ~95% — codec-internal encoder mirrors decoder for self-roundtrip |
...
| **SVQ1** (Sorenson) | 🚧 r2 (orphan rebuild) — frame-header parse + oxideav-core framework integration (Decoder/Encoder/probe registered via SVQ1/svqi FourCC, default-on `registry` feature); decode returns Unsupported on pixel data — blocked on docs (§14.10/§14.11 codebook bytes #429) | — |
...
| **WebP** (VP8 + VP8L) | 🚧 r4 (post-2026-05-20 orphan) — RFC 9649 §2.3-§2.7 walker + VP8X + ALPH + ANIM + §2.7.1.5 ANMF per-frame header (Frame X/Y + width/height + duration + typed Blending/Disposal enums); lacks ALPH/VP8/VP8L bitstream decode + encoder | 🚧 scaffold |
...
| **JPEG 2000** | 🚧 r4 (post-2026-05-20 orphan) — T.800 main-header + §A.4.2 SOT/SOD walker + typed COC/QCC/POC/RGN/PLT/PPT markers + JP2 ISO BMFF box wrapper (jP/ftyp/jp2h:ihdr+bpcc+colr/jp2c); lacks tier-1 EBCOT / tier-2 packets / wavelet / dequant / MCT / HTJ2K | 🚧 scaffold |
...
| **HDR** (Radiance RGBE) | ✅ ~97% — new-RLE + old-RLE + 8 axis-flag combos full round-trip + shared-exponent + multi-record EXPOSURE/COLORCORR stacking + typed COLORCORR / PRIMARIES headers (incl. P3-D65 / BT.2020) | ✅ ~98% — new-RLE + old-RLE + auto-RLE + all 8 axis-flag combos + XYZE↔RGB + 8 tonemap ops (Reinhard / ReinhardLuminance / Reinhard-Extended / Hable / Drago / ACES / Linear / Gamma) |
