...
| **MP3** | ✅ ~100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~98% — Layer III + Annex D psychoacoustic: Phase-2 bit-allocation loop closed + Model 1 Steps 1-7 end-to-end + Model 2 COMPLETE (§D.2.4 steps a)–n) chained, full per-call SMR walk); lacks Layer III Model-2 adaptations (§C.1.5.3.2.1 window switching + step m pre-echo) + intensity-stereo |
...
| **NSF** (NES) | 🚧 ~97% — full 6502 + IRQ/NMI + 5/5 2A03 APU + DMC DMA + six expansion chips + NSF v1/v2/NSFe + Dendy + Namco 163 + VRC7 OPLL pipeline + register semantics + KSR (Key Scale of RATE) + §4 KSL byte base table (YM2413 Table III-5) + Table III-7 decay-rate per-sample Q16 step + Decay/percussive-Sustain/Release lookup + Table III-7 attack-time per-sample Q16 step (closes the Attack column of the per-RATE envelope ladder) + MMC5 PCM Mode/IRQ + VRC6 sawtooth 14-step + E-clear accumulator zero + NSFDRV driver-ID tag; lacks §7 LFO AM/VIB step arrays + rhythm mode | — synthesis only |
...
| **AV1** | 🚧 ~35% — intra decode: standalone `decode_av1` + 4:2:0/monochrome multi-superblock to 128×128 + full base_q_idx; lacks inter prediction, loop filter, CDEF, loop restoration, film grain, superres, scalability | 🚧 ~30% — intra encode: `encode_av1` 4:2:0 YUV→IVF + 14-mode picker + all §5.11 TX_SIZES_ALL shapes + lossy/lossless + §5.11.23 `write_inter_block_mode_info` write side complete (ref_frames→mode→drl→assign_mv + interp_filter + §5.11.27-29 tail fold + decode tail-order fix + §5.11.26 intrabc PredMv + §5.11.7 fully threaded into decode_block_syntax — both arms live: composite intra else-arm (palette/CFL/filter-intra feeding §5.11.33/34 chroma+residual ctx) + intrabc find_mv_stack/assign_mv MV chain + §5.11.49 palette_tokens) + encoder partition-tree write threading of the same §5.11.7 body (sentinel-lockstep + 11-grid mirror vs the decode walker); lacks §5.11.34 residual write threading + RD picker + inter reconstruction/encode chain |
