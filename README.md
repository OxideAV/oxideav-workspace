...
| AVI       | ✅ | ✅ | ✅ | AVI 1.0 + OpenDML 2.0 demux/mux; AVIX/dmlh/vprp + 2-field interlaced + VBR audio + LIST INFO + typed PaletteChange/TextChunk/AvihFlags/Idx1Flags + ODML keyframe seek + WAVEFORMATEXTENSIBLE + `strn`/`strd` + CBR-audio validator + dmlh.dwTotalFrames + IDIT/ISMP/rcFrame + per-stream `wLanguage`/`dwInitialFrames`/`dwQuality`/`wPriority`/`dwStart` + r163 typed `dwChannelMask`/`Speaker`/`ChannelLayout` + r197 OpenDML AVISUPERINDEX `bIndexSubType` surface + r210 per-stream `strh.fccHandler` (4-byte driver-handler FourCC at AVISTREAMHEADER byte 4; `AviDemuxer::stream_handler(idx) -> Option<[u8;4]>` + `AviMuxOptions::with_stream_handler(idx, fourcc)`; printable-ASCII vs `0xHHHHHHHH` metadata fallback; video packaging mirrors `BITMAPINFOHEADER.biCompression`, audio defaults to all-zero; 13 round-trip tests) |
...
| WebP      | ✅ | ✅ | — | RIFF/WEBP (lossy + lossless + animation; ANIM + ANMF emit) + r207 §4.2 `inverse_color` per-block CTE hoist + r210 §4.4 `inverse_color_indexing` per-bundle hoist (palette-2 −22.4% on the new criterion bench; 526 tests) |
