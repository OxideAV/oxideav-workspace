...
| **AV1** | 🚧 ~88% — full §5.11.5 decode_block syntax-walker end-to-end + complete §7.11 prediction layer (all 13 intra modes + all §7.11.3 inter MC sample-gen kernels + §7.11.3.1 predict_inter driver SIMPLE single-ref arm) + §5.11.34 residual + §7.13 inverse transform + §7.12.{2,3} dequant + frame-buffer merge + **complete per-frame post-processing pipeline** (§7.14 deblock + §7.15 CDEF + §7.16 superres + §7.17 LR Wiener+SGR + §7.18.3 film grain) (1082 tests); decoder produces real sample output for all-intra + SIMPLE-single-ref-inter frames; full post-filter chain runs end-to-end. Lacks §7.11.3.1 driver wiring for compound/WARP/OBMC arms (kernels done) + §9.5.3 Quantizer_Matrix + §7.10.2.5/.6 temporal-MV scan | 🚧 scaffold |
