...
- **3D scenes & assets** — typed `oxideav-mesh3d` (Scene3D / Mesh /
  ...
  trace + fuzz-resistant header detection + `EncodeStats` vertex
  uniqueness + configurable float precision), `oxideav-obj` (+ MTL
  Phong + Wavefront-PBR + smoothing groups + multi-name groups + MTL
  Tf/Ke/sharpness extras + negative-index encoder + `p` point elements
  + `mg` merging groups + display attributes (bevel/c_interp/d_interp/
  lod) + MTL `map_*` option flags + `d -halo` + polyline rejoin),
  ...
  heuristic + normalised-int animation outputs (BYTE/UBYTE/SHORT/USHORT)
  + multi-scene), `oxideav-usdz` (ZIP STORED walker + writer + USDA
  reader + writer + UsdPreviewSurface + per-mesh `xformOp:translate/orient/
  scale/transform` + multi-primitive sibling-Mesh emit-and-fold;
  `ZipStoredAsset` + `EncodeReport.pass_through_textures` confirm
  zero-re-encode USDZ→USDZ texture passthrough). `oxideav convert in.obj
  out.gltf` honours `-stl-format ascii|binary` and `-gltf-format
  glb|embedded|external` flags.
...
| **AC-3** (Dolby Digital) | ✅ ~95% — full decode + downmix + WAVE_FORMAT_EXTENSIBLE channel reorder for acmod 3/5/7 (corrects bitstream-slot → mask order; +78 dB PSNR jump on 3.0 + 5.1 fixtures); 90+ dB vs ffmpeg | 🚧 ~92% — acmod 1/2/3/6/7 + LFE + DBA + 5-fbw coupling + E-AC-3 indep+dep substream |
...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | DirectShow infra wired end-to-end (r25-r34): IBaseFilter Run/Pause/GetState + JoinFilterGraph + IPin::ReceiveConnection (synth AMT fallback) + downstream HostIBaseFilter + HostIPin + HostIMemInputPin Receive callback + HostIMemAllocator committed-state machine + pin_with_direction walker + real MP43 keyframe (176×144) wired through trait path + IMemInputPin::GetAllocator dispatch with `CodecAllocatorNegotiation` capture. r35 mints host-side `CLSID_MemoryAllocator {1E651CC0-…}` class factory — codec's CoCreateInstance now S_OK + GetAllocator returns non-NULL allocator; r36 traces NULL+0x1c trap to MPG4DS32 RVA 0x7184 inside an inlined `IsEqualGUID(stack_local[+0x8c]+0x1c, &kZeroGuid)`; r37 wires HostIPin::QueryPinInfo + ConnectedTo + HostIBaseFilter::QueryFilterInfo with real PIN_INFO/FILTER_INFO structs but trap remains at same site — codec doesn't call those during Receive. r38 path: disasm RVA 0x2da7/0x6473/0x6560 + suspect `IMediaSeeking` / `IBaseFilter::QueryVendorInfo` / `IPin::QueryId` (all currently E_NOTIMPL) | DirectShow IBaseFilter wrapper: COM scaffolding + ole32 stubs + DllGetClassObject + full HostIFilterGraph + HostIPin (output+input) + HostIEnumMediaTypes + HostIMemAllocator (committed state-machine) + HostIMediaSample + EnumMediaTypes walker + IMediaFilter Pause/Run/GetState. user32 cascade: synthetic-HWND no-op family. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. |
