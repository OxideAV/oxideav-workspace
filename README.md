...
| MOV (QuickTime) | ✅ | — | ✅ | Apple QTFF + ISO BMFF meta + HEIF/HEIC item-properties + grid/iovl/tmap + symmetric muxer + fragmented-MP4 seek + DASH sidx/styp + stbl + traf saiz/saio sample-aux + r157 pnot preview-poster preflight + r182 ISO 14496-12 §4.2/§11.1 `uuid` User-Type Box parser (file_uuids + RFC 4122 string + ISO reserved-namespace detector); ffprobe-accepted |
...
| FLV       | ✅ | — | — | Flash Video — MP3/AAC/H.264 audio + VP6f/VP6a/H.264 video + Enhanced RTMP ExVideoTagHeader + AMF0 onMetaData/onXMPData/onCuePoint + Annex F encryption + E-FLV ModEx walk + multitrack body splitter + HDR colorInfo metadata + `audiosamplesize` → sample_format + AMF0 TypedObject/XMLDocument/Unsupported + r161 injection-robustness suite + 16 MB OOM-lever guard + r182 onMetaData catch-all preserves Date/Null/StrictArray/AMF3-nested via flatten_amf_value; lacks muxer |
...
| **Opus** | 🚧 ~28% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + §3.1/§4.2 framing dispatch + r162 §3.4 R1..R7 malformed-input audit + r182 celt_band_layout (21-band MDCT partition + Hybrid §4.3 first-17-bands rule + celt_band_at_hz reverse lookup; 401 tests); CELT bands gated on #936 + #943 | 🚧 scaffold |
...
| **AC-3 / AC-4** (Dolby Digital / Dolby AC-4) | ✅ ~97% — AC-3 full decode + E-AC-3 SPX (§E.3.6 HF regen) + transient pre-noise (§E.3.7.2 TPNP) + multichannel fbw+LFE+coupling AHT (§3.4 Adaptive Hybrid Transform incl. LFE-channel mantissas + lfeahtinu synthesis + interleaved cplahtinu coupling-channel mantissas) + §7.8.2 LtRt matrix-encoded stereo downmix + r126 Annex D §2.3 alternate-syntax mix-level params + WAVE_FORMAT_EXTENSIBLE + r172 §3.6.4.2.3 SPX-attenuation border notch + r182 §7.10.1 opt-in decoder CRC verifier (residue-form crc1/crc2; encoder crc2 augmented-form follow-up); AC-4 ~98% decoder + IMS encoder ~65% (mono/stereo/5.0/5.1/7.1 Cfg3Five + 5_X ASPX_ACPL_3 + 7.1 3/4/0.1 SIMPLE/ASPX_ACPL_2 LFE multichannel + r126 7.0 SIMPLE/Cfg3Five) | 🚧 AC-3 ~95% — acmod 1/2/2.1/3/6/7 + LFE + DBA + 5-fbw coupling + E-AC-3 indep+dep + per-channel PSNR gates + r95 two-stage equalise + spread-cap greedy for per-channel `fsnroffst[ch]` (≤ ~1.5 dB spread; closes r91 cheap-mantissa runaway) |
...
| **MIDI** (SMF) | ✅ ~99% — SMF Type 0/1/2 → PCM via 32-voice mixer + SF2/SFZ/DLS + DLS `art1`/`art2` + SF2 EG2 + 2-pole resonant low-pass biquad on shared SamplePlayer + SFZ filter EG (`cutoff` / `resonance` / `fil_type` covering all 6 SFZ v1 shapes + `fileg_*` envelope opcodes) + MPE v1.1 + RPN 0/1/2/5/6 + CA-25 Master Tuning + MIDI Tuning Standard (per-key + scale/octave microtuning) + Universal Master Volume SysEx + Master Balance SysEx + GM2 Global Parameter Control (CA-024 reverb/chorus) + Data Inc/Dec (CC 96/97, RP-018) + `SmfFile::time_signatures()` iterator (FF 58, stable-merge across tracks) + r125 `SmfFile::tempo_map()` iterator (FF 51, BPM-cached, stable-merge across tracks) + r128 `SmfFile::key_signatures()` iterator (FF 59, circle-of-fifths) + r172 cargo-fuzz harness (smf/sf2/dls/sfz, 30M+ panic-free) + r182 `SmfFile::lyrics()` iterator (FF 05 karaoke `.kar` syllable stream, stable-merge across tracks) | — synthesis only |
| **NSF** (NES) | 🚧 ~92% — full 6502 + IRQ/NMI + 5/5 2A03 APU + DMC DMA + six expansion chips + NSF v1/v2/NSFe + Dendy region + mixe per-device gain + plst/psfx playlist + region-aware noise + FDS modulation/envelope/sound-enable/$4090..=$4097 + r154 Namco 163 per-channel timer accumulators + r182 VRC7 internal patch ROM (16 instruments) + Vrc7Patch bit-field decoder + per-channel patch_index/sustain from $2X/$3X registers; VRC7 OPLL synth core still pending #861 | — synthesis only |
...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz harness (3 panic-free targets, r183 adds direct §5.1/§6.1.1/§6.3/§5.3 header-parser fuzz) + r161 profiling driver + baseline (decode 296–727 MiB/s, encode 121–326 MiB/s) | ✅ ~95% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + multi-frame rate-control + 12-bit cross-decode + ffmpeg cross-decode acceptance (8 cases 58.0-63.8 dB luma PSNR) |
