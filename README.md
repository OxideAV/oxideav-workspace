...
| **Ut Video** | ✅ ~92% — ULRG + ULY2 with NONE/LEFT/MEDIAN/GRADIENT (incl. interlaced field-parity-aware predictor) + UQ 10-bit (Pro variant: frame_info-at-start + per-plane Huffman lengths + mod-1024 LEFT seed 0x200) + UM SymPack (LeBitReader + 3-bit control sign-flip mapping); UL/UQ/UM 8/10-bit families bit-exact across 11 ffmpeg fixtures | ✅ ~85% — classic-family encoder ULRG / ULRA / ULY0 / ULY2 / ULY4 × {NONE, LEFT, GRADIENT, MEDIAN} (5 FourCCs × 4 predictors = 20 combos); per-frame predictor pick by entropy-cost RDO (caller can pin via `with_predictor`); slice-row striping; per-plane canonical Huffman with ≤16-bit length-limit; ffmpeg cross-decode bit-exact on every (FourCC, predictor); lacks UQ 10-bit / UM SymPack / interlaced encoders |
| **MagicYUV** | ✅ ~95% — clean-room rebuild from `docs/video/magicyuv/` (orphan master 2026-05-06; pre-rebuild history on `old` branch). All 17 native v7 FOURCCs (8-bit + 10/12/14-bit M0/M2/M4 family) decode byte-exact self-roundtrip; modular 8-bit Median + JPEG-LS Median for high bit depths per spec/04 §4.4 audit-corrected; non-RFC-1951 longest-length-first cumulative Huffman per spec/05 §2.0; raw-mode fallback; interlaced field-stride=2; minimal RIFF/AVI demuxer; `--features trace` JSONL emitter (9 event kinds, jq-line-diff-compatible with the cleanroom Python ref); Auditor round-1 PASS (56/56 PCM-lockstep against Python ref, `OxideAV/docs@0f70ba2`); lacks OpenDML 2.0 super-index for AVI > 1 GB | ✅ ~85% — `encode_frame` / `encode_avi` / `EncodeOptions` covering all 17 FOURCCs across Left / Gradient / Median + Huffman / raw; produces well-formed v7 frames; doesn't pursue proprietary "Dynamic" strategy or proprietary byte-budget raw fallback (encoder-side per spec/04 §3) |
