...
- **Vector graphics + text** — `oxideav-svg` (read+write SVG; rounds 1-3
  ...
  selection, AGL glyph-name→Unicode), `oxideav-otf` (CFF / Type 2 charstrings incl. CID-keyed ROS/FDArray/FDSelect, cubic outlines),
...
| **Vorbis** | 🚧 r8 (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook + Huffman tree + full §4.2.4 setup-header walker + §3.2.1/§3.3 VQ vector unpack (entry → vector via multiplicands + minimum_value + delta_value + sequence_p) + §8.6 residue decode (formats 0/1/2); lacks floor curve / audio packets | 🚧 scaffold |
| **Opus** | 🚧 r8 (post-2026-05-20 orphan) — RFC 6716 §3.1 TOC + §3.2 frame packing + §4.1 range decoder + §4.2.7.1–§4.2.7.5.1 SILK header + §4.2.7.4 subframe gains + §4.2.7.5.2 LSF Stage-2 residual + §4.2.7.5.3 NLSF reconstruction (Tables 23/24 cb1_Q8 + IHMW w_Q9 in [1819, 5227] + final NLSF_Q15 clamp) + §4.2.7.5.4 NLSF stabilization (RFC 8251 §7 erratum); lacks §4.2.7.5.5 interpolation + LTP + excitation + CELT | 🚧 scaffold |
...
| **MP3** | ✅ ~96% — MPEG-1 Layer III M/S + free-format streams (§2.4.2.3; measure_free_format_size scan + CBR-fast seek + lazy-scan seek) + Xing/Info encoder-version surfaced as `encoder` metadata | 🚧 ~88% — CBR + VBR + M/S + intensity + Annex D Psy-1 + per-region big-value Huffman + per-granule count1 picker + demand-weighted CBR bit allocator |
| **AAC** | 🚧 ~87% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened bounds + gapless `iTunSMPB` + §4.6.18.5 SBR boundary upsample + AAC-LD (AOT 23) `er_raw_data_block` bootstrap for channelConfiguration 1 (SCE) and 2 (CPE common-window + independent-ICS) at 512/480-sample frame lengths + multi-RDB ADTS decode (1..4 raw_data_blocks/frame, CRC + no-CRC); lacks LTP, multichannel LD, low-overlap window, ELD, USAC | 🚧 ~84% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy + TNS (CPE + SCE) + perceptual M/S decision §6.6.1.3 with Johnston binaural masking + PE-based VETO/PROMOTE gates |
...
| **MPEG-1 video** | 🚧 clean-room rebuild (post-audit 2026-05-18) — sequence/GOP/picture/slice headers + §6.2.5 macroblock_address_increment + §6.2.5.1 macroblock_type (Annex B Tables B-2/B-3/B-4); macroblock body / motion / IDCT pending | 🚧 scaffold |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + B-pyramid POC + **33 SEI types** (+3 in r99: sub_seq_info / sub_seq_layer_characteristics / sub_seq_characteristics §D.1.11–13) + fuzz-hardened slice/MC/SPS bounds + r91 strictness fixes — fuzz CI green; lacks MBAFF, SVC/3D/MVC | 🚧 ~82% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite; ffmpeg PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 r4 (post-2026-05-18 orphan) — Annex B + §7.3.1.2 NAL + §7.3.2.1 VPS + §7.3.3 PTL + §7.3.2.2 full SPS body (prefix + PCM + §7.3.7 short-term RPS both explicit + inter-RPS-prediction forms + long-term RPS + MVP/smoothing + opaque VUI/ext tail) + §7.3.2.3.1 PPS (tiles + deblocking-control + lists-mod/parallel-merge tail, se(v)); 52 tests; lacks slice header / scaling_list_data / VUI body decode / ext bodies / CABAC (#444) | 🚧 scaffold |
...
| **VP8** | 🚧 r8 (post-2026-05-20 orphan) — RFC 6386 §7 bool decoder + §9.1 + §19.2 + §11 KF MB mode layer + §12 intra-prediction kernels + §13 DCT-coefficient token decoder (coeff_tree walker + §13.2 EOB-skip + §13.5 default coef_probs[4][8][3][11] table + extra-bits decode through CAT6 11-bit DCTextra) + §14 dequant + inverse WHT/DCT + summation; 104 tests; lacks §14.2 per-MB orchestration / §15 loop filter / §16 inter modes / §17 MV decode | 🚧 scaffold |
| **VP9** | 🚧 r8 (post-2026-05-20 orphan) — full §6.2 walk + §9.2 Boolean decoder + §6.3.1/§6.3.2/§6.3.3/§6.3.7/§6.3.8 compressed-header sweeps + §6.4.24 / §6.4.26 coefficient-token decoder (token_tree walker + §9.3.2 pareto helper + §10.3 128×8 PARETO_TABLE verbatim + extra-bits decode for CAT1..CAT6 at 8/10/12-bit) + §8.6.1 dequant (dc/ac qlookup + get_qindex + get_dc/ac_quant); 100 tests; lacks §6.4.21 residual driver / §8.7 inverse transforms / inter §6.3.9+ / loop filter | 🚧 scaffold |
| **AV1** | 🚧 r8 (post-2026-05-20 orphan) — §5.3 OBU + §5.5 sequence + §5.9.2 prefix + §5.9.5–§5.9.9 frame_size + §5.9.3 allow_intrabc + §5.9.15 tile_info + §5.9.12 quantization_params (streaming) + §5.9.14 segmentation_params (typed `SegmentationParams { enabled, update_map, temporal_update, seg_id_pre_skip, last_active_seg_id, feature [8][8] }`, Table 5.9.14 SEGMENTATION_FEATURE_BITS / SIGNED / MAX); lacks delta_q / delta_lf / cdef / lr / tile data / inter ref-frame state | 🚧 scaffold |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **PNG / APNG** | ✅ 100% — 5 colour types × 8/16-bit + APNG + sBIT/pHYs/tIME/bKGD/hIST/eXIf round-trip | ✅ 100% |
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal compositor + structured Application Extensions (NETSCAPE2.0 / ANIMEXTS1.0 / XMP / ICC / Exif) + Plain Text Extension + lenient-decoder mode + lazy `Playback` + §18.c.viii pixel-aspect-ratio accessors; clean-room from CompuServe spec + Welch 1984 | ✅ 100% — per-frame palettes + `optimize_color_tables()` GCT/LCT hoisting + §7 Required Version enforcement (rejects 89a-only blocks under Gif87a header) + `upgrade_version_if_needed()` |
| **WebP** (VP8 + VP8L) | 🚧 r8 (post-2026-05-20 orphan) — RFC 9649 §2.3-§2.7 walker + VP8X + ALPH + ANIM + ANMF + §2.3/§2.4/§2.7.1 RIFF builder + typed §2.5 `VP8 ` routing handle + typed §2.6 `VP8L` routing handle (WebP Lossless §3.4/§7.1 image-header peek: width/height/alpha_is_used/version; no oxideav-vp8/oxideav-vp8l runtime dep) + §2 LSB-first bit-reader + §4 transform-list header reader; 133 tests; lacks §5 entropy decode + full pixel decode + encoder | 🚧 scaffold |
| **JPEG** (still) | ✅ ~95% — via MJPEG | ✅ ~90% — via MJPEG |
| **TIFF** (6.0) | ✅ ~97% — II/MM + BigTIFF read + 6 photometrics + 1/4/8/16-bit + None/PackBits/LZW/Deflate/CCITT-MH/T.4-1D + FillOrder + tiles + multi-page + JPEG-in-TIFF + PlanarConfiguration=2 (separate component planes across strips/tiles + chunky re-interleave + Predictor=2 driven per-plane); lacks CCITT T.4 2-D / T.6 (#874), CMYK-JPEG, JPEG-in-TIFF + planar=2 | ✅ Gray8/16/RGB24/Palette8 — None/PackBits/LZW/Deflate + Predictor=2 + Bilevel CCITT-MH / T.4-1D, single+multi-page |
...
| **OBJ** (+ MTL) | ✅ ~97% — full Wavefront grammar + MTL (Phong + Wavefront-PBR + map_* options + typed refl) + smoothing/display attrs + free-form geometry pass-through + `xyzrgb` per-vertex colour + Bezier + B-spline / NURBS / Cardinal (Catmull-Rom) / Taylor `curv` + Bezier `surf` 2D-surface tessellation (tensor-product de Casteljau); lacks non-Bezier `surf` surfaces, multi-patch decomposition, trim/hole loops | ✅ ~96% — symmetric + negative-index encoder + polyline rejoin |
