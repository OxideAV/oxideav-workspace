...
| **MP2** | ✅ ~90% — Layer II decode → PCM validated END-TO-END to the ISO conformance bound (≤1 LSB) across the full mode×rate matrix (MPEG-1 + LSF, per-frame) + 32-stream whole-file matrix ≤0.025 LSB float + free-format decode/encode (`to_free_format`/`rewrite_to_free_format`) | ✅ ~95% — psychoacoustically driven at ALL SIX rates: both Annex D models (13818-3 LSF masking tables D.1d–f/D.2d–f + Model-2 LSF partitions; +3 dB vs the rate-driven fallback at 64 kbit/s stereo) + joint/intensity (Annex G.1) + VBR + CRC + emphasis (50/15µs + J.17) + 43-cell independently validated conformance corpus + r434 J.17 absolute-gain RULED (Table-1 analytic lock tests, per-rate cascades via the Rec's own acceptance procedure) + §2.4.1.8 ancillary both directions (fuzzed length identity) + r447 §2.5 MULTICHANNEL ENCODER (matrixing procedures '00'/'01'/'11' across all §2.5.2.15 configs + LFE + B.2a/B.2b MC allocation + §2.5.3.2.1.3 prediction + selectable tc_allocation; official-programme round-trip 13.1–32.2 dB @384k) + MC registry surface both directions via core ChannelLayout (mc=off/on/auto, no core change needed) + r441 official ISO/IEC 13818-4 sweep PASSED (§2.5.4 normative accuracy at ~70× headroom on both accuracy bitstreams; 15/15 16-bit comparison cells ≤1 LSB; all multichannel base streams decode) + r444 §2.5 MULTICHANNEL extension decode COMPLETE (all 7 configs, dynamic crosstalk, MC prediction, dematrixing as exact downmix inverses, LFE + multilingual; official vectors 20/20 ≤1 LSB, 5-channel accuracy 80× headroom); lacks MC encode |
...
| **AAC** | 🚧 ~95% — LC/HE-v1/v2/LD/SSR/BSAC decode; lacks the deployed ER-LD TNS filter record | 🟢 ~90% — LC all Table-1.19 layouts + HE-AAC v1 SBR encoder (ADTS/ASC/LOAS, black-box PCM parity 0.00 dB/band); lacks PS encode, PCE custom layouts |
| **CELT** | ✅ ~98% — r451 downsampled output 8-24 kHz (108 dB oracle lockstep) + reduced-rate encode input (±0.1 dB parity) + NB/WB/SWB end-bands + packet-loss concealment (pitch-LPC + comfort noise) + registry resample/end_band/loss options; decode REFERENCE-EXACT: 133–137 dB float SNR at every frame size, mono+stereo, symbol traces bit-identical to the staged §A.1-listing oracle (embedded-listing posture user-ratified 2026-07-17; full §4.3 chain — allocation + reallocation walk + PVQ + anti-collapse + IMDCT/WOLA + stereo synthesis — gated on the staged raw-frame fixture sets); lacks hybrid (start=17)/downsampled rates | ✅ ~92% — BEATS the reference listing at every measured rate (+1.2..+11 dB mono, +0.2..+3.2 dB stereo): λ-priced Viterbi TF RD + listing-exact alloc-trim/dynalloc analysis + two-pass coarse-energy RD + §5.3.1 pitch prefilter + stereo dual/MS verdicts + consec-transient anti-collapse + VBR (drift controller, 2-byte silence, constrained reservoir) — every stream cross-decodes bit-lockstep through our decoder AND the §A.1 listing decoder (99.4–133 dB, all 8 LM×channel combos), registry dual-API + r434 VBR/CVBR vs oracle (silence positions exact on all 6 staged sets, CVBR totals byte-equal, mean rate within 1.9%) + hybrid-mode CELT layer (start=17 both directions) + 192/384 kb/s above-listing; + r442 CUSTOM MODES 8–96 kHz (full Appendix-A mode construction, 48 kHz tables reproduced bit-exact as the special case; nine configs symbol-exact both directions vs oracle, CVBR byte-equal) — last lacks CLOSED |
...
| **G.728** | ✅ 100% | ✅ 100% |
| **G.729** | 🟢 ~90% — clause-5 fixed-point chain, frame-0 startup byte-exact on every clean vector, Annex B CNG with SID-LSP spectral envelope, corr 0.99+; lacks whole-vector bit-exact (§4.2 residual) | 🚧 ~70% — clause-3 encoder on the fixed-point front end (LSF 90.7% locked); lacks bit-exact wire |
...
| **Musepack** | ✅ ~97% — SV7+SV8 stream→PCM ±1 LSB vs oracles incl. CNS noise waveform (native-oracle-proven; the r405 irreproducibility was the oracle generator's) + SV8 SO/ST seek layer (ST byte-exact vs reference on all 7 corpus streams; entry-aligned random access ±1 LSB) + fuzz-hardened (SV8 drain DoS + ST overflow fixed) | ✅ ~92% — from-PCM SV7 AND SV8 encoders at 1-LSB reference parity (76–85 dB) + seek-layer emission (third-party seek honours it) + opt-in CNS (−85..89% rate on tonal content) + RD posture election; lacks psymodel |
...
| **WMA** | 🟢 ~80% — r450 variable-size lapped reconstruction + calibrated dequant (1 dB/step gain ladder): 18–27 dB SNR / corr² ≥0.99 on the closing envelope-coded families + core-registered dual-API decoder (sample-exact vs direct chain, 11.4M samples) + fuzz through synthesis; r439 vendor-bitstream decode stack on the Extractor-6 staging: all 8 vendor VLC trees exact + §0 stream config + §1 superframe/reservoir-carry assembly (holds on all 1 769 staged vendor packets) + §2–§4 frame/block parse + exponent-band partitions + §5 mid/side inverse + staged-ladder dequant/synthesis to PCM — vendor frame-layer closure 1705/1763 (96.7%) — five of six families 100% byte-exact (mono8k/stereo22k/apollo8/vbr_q75/wmp12_96k; r446 closes the 44.1k high-rate residual), correlated PCM vs black-box reference (corr² 0.96 / 13.6 dB best); vendor-measured errata recorded (F1 one-ahead pipeline, per-block B2 envelope-reuse bit on two-channel short blocks — r446 overturns the earlier no-B2 reading, channel-scoped ALT, zero-§1-carry ⇒ padded previous packet); lacks dequant composition + mono-VBL F1 latch trace (asks filed) | 🚧 ~35% — full encoder mirror (§3 window+forward-MLT + §4 quantizer/Bark masking/matrix side-info + §5 stereo fold + §6 run-level + canonical-Huffman bit layer + §8 FIG.5 channel encoder; decode(encode(PCM))≡PCM in-crate) + staged vendor tables live: coef VLCs modes 1/3 bit-exact-canonical vs CSVs + real critical-band geometry + 113-step gain ladder driving the §8 chains + full wire bit layer (corrected mode-2 + run-level ramps + scale/gain VLCs + staged frame/superframe layout, PCM→bits→PCM byte-exact self-roundtrip mono+stereo) — dec ~55%; §4b class-selector thresholds wired (bit-exact pins + typed regions); r433 all 14 staged tables consumed + staged-pinned MSB-first bitio + fast MLT + 4-target fuzz; vendor streams await validator-staged semantics (9 asks filed) |
...
| **APE** (Monkey's Audio) | ✅ ~90% — whole-file PCM decode BYTE-EXACT vs the vendor corpus (all 7 fixtures, per-frame CRCs + whole-file CRC-32 pins): range coder + §1 header/tail (both eras) + vendor frame layer + r448 §6 predictor cascade (nn-filter FIR + §6.7 offset predictors + first-order stage + level-5000 quirk) + §6.9 stereo decorrelation + 8/16/24-bit reassembly, framework-registered ('MAC ' payload-magic) + encode mirrors locking source-only branches + corruption/truncation sweeps; lacks <v3.93 era + ≥3-channel (fixture-asked) | 🚧 ~10% — encode mirrors for round-trip testing only |
| **DTS** (Core) | ✅ ~97% — §5.4.1 DYNF/RANGE (§D.4 multiplier, post-QMF) + §C.2.5 streaming QMF synthesis → PCM (inter-frame filter tail; reference-shape-exact) + CPF/SICRC side-info tail (JOINX==0) + frame header + 14↔16-bit pack/unpack + side-information subframe walker + Annex C reconstruction primitives + Annex D codebooks (incl. §D.6 block-code books + §C.2.1 table-look-up decoder) + r446 §D.10 VQ index-space closure (all 1024 HF + 4096 ADPCM vectors bit-exact through the real bitstream path; 480-vector black-box coverage fixture; staged-CSV digest integrity pin on all 49 152 values) + §D.8 FIR tables + fused 32-band synthesis QMF driver (§C.2.5 QMFInterpolation, per-frame multi-channel) + §D.2 step-size tables + §5.5 inverse-quant scale composition + §D.5.7–§D.5.9 13/17/25-level audio books + §5.5 Primary Audio Data Arrays decode walk + §D.5.10/.11/.12 ABITS 8/9/10 audio-data Huffman code books + end-to-end raw-16-bit Core frame → planar S32 PCM decode (§5.3→§5.4→§5.5→§C.2.5, JOINX/DYNF/CPF==0 common case) wired into the registry `Decoder` + §C.2.6 InterpolationFIR LFE polyphase + §5.5 LFE phase dequant (LFE channel emitted in the planar S32 frame) + §5.3.1 14-bit-packed container frames → PCM (both byte orders, bit-exact) + §C.2.4 sum/difference matrixing (front L/R + surround) + §5.6/§5.7 optional-info (Table 5-30 walk off the real decode cursor, §D.11 downmix tables unit-verified, §5.7.1 aux + §5.7.2 Rev2 chunks, embedded-downmix PCM fold) + Annex B CRC-16 verify (AUX/Rev2AUX spans; HCRC family untested per spec) + signed-Q2 dynrng→dB DRC (off-by-127 DYNF gain fixed) + §5.7.2.2 CRC-gated Rev2AUX DRC override + 5.1+LFE black-box regression (all six planes corr 1.000000); r429 JOINX joint-intensity + r430 termination-frame PSC valid-prefix decode (§5.4.1, SSC×PSC grid battery; reference parser skips FTYPE=0 — prefix pinned shape-exact) — side-info sweep complete + r439 §D.10 VQ/ADPCM code books built in (staged-table transcription, SHA-pinned; HF-VQ + ADPCM-prediction decode by default, every frame class shape-identical to reference at 94–98 dB SNR) — Core-profile decode COMPLETE; extensions (EXSS/XCH/XXCH/X96/XLL) out of scope | — |
...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | ✅ ~97% — 41/41 corpus byte-exact, tile-parallel decode; PrevSegmentIds refresh gate fixed | ✅ ~95% — alt-ref pyramid + show_existing_frame, all 4 SEG_LVL features, tile cols+rows, pixel-accurate scaled-ref, intra-only frames (29 streams black-box byte-exact); lacks writer-side §8.4 adaptation, 2-pass RC |
| **AV1** | ✅ ~99% — 153-stream corpus byte-exact; fuzz OOM closed (per-TB TxTypes copy) + Annex A picture-size cap | 🟢 ~98% — conformance-grade all §6.4.1 pairings; short ref signaling twin, film grain on pyramid/adaptive drivers; lacks pyramid short-signaling (lag-3 grain measured, no win) |
...
| **ProRes** | ✅ 100% | ✅ 100% |
| **EVC** (MPEG-5) | 🟢 ~90% — Baseline + Main tools, §8.3 RPL bookkeeping, errata #313; 23094-4 gate red on filed entropy asks | 🚧 ~55% — intra + P/B GOPs, multi-ref, frame-level rate control (±1%), conformance cropping; lacks RDOQ, 2-pass |
...
| **OpenEXR** | ✅ ~99% — scanline + tiled + deep + multi-part across all 4 part types + mip/ripmap + PXR24 & B44/B44A decode+encode across scanline / tiled (ONE_LEVEL/MIPMAP/RIPMAP) / multi-part (edge-aware HALF blocks + pLinear inverse-log) + mixed multi-part files carrying multi-level (MIPMAP/RIPMAP) flat AND deep tiled parts incl. PXR24/B44/B44A compression + typed attribute inspectors + fuzz-hardened readers (9 overflow/OOB/div-zero panics fixed, hostile-window sweeps on every parse entry point, 1.6M+ fuzz execs clean) + typed envmap/preview/floatvector/deepImageState attributes + sub-sampled luminance/chroma (PXR24/B44/B44A, reference-validated) + hoisted scatter/gather dispatch (2.6–19.7 GiB/s decode) + multipart displayWindow now file-global (8 encoders fixed; unequal-sized parts were reader-unreadable) + exrinfo/exr2aces independent-reader cross-validation + full lineOrder conformance (DECREASING_Y/RANDOM_Y read+write, canonically-keyed offset tables; PXR24 native-bytes raw fallback + deep-multipart sample-order bugs fixed) + parse_flat fuzz + r439 PIZ + DWAA/DWAB decode from the staged observer spec (PIZ bit-exact both ways; DWA bit-exact vs the reference decode on all stress cases) — full 10-code compression matrix + r446 perf pass (buffered 64-bit Huffman I/O, monomorphized wavelets, sparse DWA fill: PIZ decode +33–49%, PIZ encode +14–17%, DWA +8–14%, all bit-exact) + r450 DWA-decode fuzz hardening (2 hostile-reservation OOMs + over-subscribed Huffman code-length OOB fixed; 14.8M execs clean) | ✅ ~97% — scanline (+PXR24 + B44/B44A) + tiled + deep + multi-part mixed write (incl. multi-level flat + deep tiled) + sub-sampled lossy + multilevel lossy + all lineOrder variants + r439 PIZ + DWAA/DWAB encode (chunks accepted and decoded bit-identically by the reference) |
| **Farbfeld** | ✅ 100% | ✅ 100% |
...
| **glTF 2.0** (+ .glb) | ✅ ~98% — JSON + .glb + full PBR + 12+ KHR extensions (incl. KHR_gaussian_splatting typed splat-field decode + ellipse-kernel attribute + SH colour evaluator + SH-degree conformance) + skins/animations/morph targets + KHR_texture_transform across every textureInfo (incl. material-extension slots) + spec-MUST validators (index-reference + structural-minimum + animation-sampler + image-source + morph-weights + closed-enum/componentType schema passes); meshopt codec both ways (decode since r316) + r442 full Object Model pointer registry (121+32 rows, §Operation MUSTs) + r446 mesh3d-0.0.5 typed adoption (Option-shaped sampler filters — undefined no longer coerced — + TextureRef::transform on all five PBR slots + Node::weights with animation&gt;node&gt;mesh precedence; round-trip + cross-surface suites); Draco bitstream + splat await docs staging | ✅ ~95% — symmetric round-trip incl. XMP + KHR_meshopt_compression write — full v1 bitstream (all control modes) + edge-reuse triangles + two-baseline indices + all 4 Appendix-B forward filters |
| **USDZ** (+ USDA) | ✅ ~96% — typed MaterialExt via published mesh3d 0.0.4 (ior/clearcoat/specular shims retired) + primvar-reader + <UDIM> inputs (were hard decode failures) + validate()-clean matrix4d boundary (row/column transpose bug fixed at all six crossings) + ZIP walker + USDA composition (LIVRPS / variants / references) + `.usdc` Crate parser with resolved-spec join (SPECS↔FIELDSETS↔FIELDS) + §4.6 spec pathIndex bounds-check + §3b int-coded buffer trailing-payload reject + §4.3 value-rep flag decode + value-region resolution (inline scalars / uncompressed+compressed arrays incl. §3b compressed-integer materialisation) + ZIP64-sentinel reject (read + fallible write); + UsdSkel skinning (joint tree + inverse bind matrices + influences + SkelAnimation TRS + BlendShape morph targets) + full UsdPreviewSurface/UVTexture/PrimvarReader material network; lacks §4.5 PATHS tail (docs gap) + inbetweens/GeomSubset; + r438 full §16.3 USDC read (value-type table IDs 1–59, path construction, typed value decoding, .usdc default layers → validate()-clean Scene3D incl. skeleton+animation; corrected 2 observer guesses + found 2 spec errata) + §16.4 container rules + r446 mesh3d-0.0.5 adoption (static blendShapeWeights→Node::weights incl. §1.4.1 inbetween expansion + animationSource scoping + BlendState synthesis; texture-transform bake into shared-index st channels on encode; undefined sampler filters preserved) | ✅ ~88% — symmetric writer + pass-through + composition arcs + verified round-trip fixed point (bare-mesh collapse + empty-Materials-root + geometry-hint-leak fixed; 8/8 fidelity channels) + r442 GeomSubset per-face material splits + full binding resolution (direct + §15 CollectionAPI + §3.4 precedence) + UsdSkel in-between shapes (exact bake + closed-form inverse) + Crate Variant/VariantSet spec forms |
| **FBX** | 🚧 ~95% — full §7 section set (Documents/References + Definitions census + §7b property templates) + depth-capped fuzz-swept parsers (3 hostile-input aborts fixed) + binary + ASCII + object graph + mesh/animation/deformers + Properties70 grammar + class-default resolution + multi-LayerElementNormal layers + FBXHeaderExtension authoring metadata + static Model node local transforms (Lcl T/R/S → Trs, reduced-chain guard) + all LayerElement mapping modes (ByPolygonVertex/ByVertex/ByPolygon/AllSame) + compcol bounded deflate; + Edges array + LayerElementSmoothing (ByEdge/ByPolygon, per-corner resolved, round-tripped) + r433 byte-faithful whole-file re-encode closure (C bool wire-token fix, IndexToDirect identity, footer parse) + r439 GlobalSettings typed axis conventions + InheritType world transforms + full Constraint grammar round-trip + KeyAttrFlags raw catalogue + §3 template bodies + c byte-array code — ~98%; + r442 multi-target blend shapes round-trip (per-channel curve bucketing onto one strided sampler, static DeformPercent, document-order determinism); + r446 mesh3d-0.0.5 adoption both directions (UVSet→uv_set label joins, authored texture TRS→typed transform, DeformPercent node weights) + ASCII integer-narrowed-array widening fix (dropped layer payloads recovered) + §4a axis-triple coherence guard + one-BlendShape-deformer-per-geometry writer fix + r449 NURBS evaluation/tessellation ENGINE (rational curves + tensor-product surfaces, open/closed/periodic, analytic quadric ground truths, DoS-capped → mesh3d primitives); lacks the NURBS wire payload grammar (docs-asked) + KeyAttrFlags bit semantics + texture-placement composition order (docs asks) | ✅ ~80% — Scene3D→FBX binary+ASCII `Mesh3DEncoder` (geometry / Lcl-transforms / materials / textures / unit+axis / animation curves) + symmetric binary + ASCII writer + opt-in deflate; + decode-parity drive (multi-UV/colour/tangent layers + material slots + skin/blend-shape deformers + light/camera attributes + Takes/header metadata + GlobalSettings) + r433 binary footer emission (byte-faithful 17200-byte fixture re-encode) + BindPose round-trip + r436 full node-transform chain BOTH directions (pivots/pre-post-rotation/all 7 RotationOrders, animation re-composition + writer de-composition; 2 latent curve bugs fixed) + r439 Constraint emission + InheritType + axis records — ~98%; + r442 multi-target morph-animation emission (one curve per channel slot); lacks KeyAttrFlags re-emission (same docs ask) |
| **IFC** (BIM, ISO 16739) | 🚧 Phase 3–4 — STEP/P21 + EXPRESS typing + tessellation incl. named I/L/T/U/Z/C profiles, tapered sweeps, WHERE rules, georeferencing (map conversion); lacks curved breps + directrix sweeps | — |
