...
| **MagicYUV** | 🚧 ~40% — clean-room rebuild from `docs/video/magicyuv/` (orphan master 2026-05-06; pre-rebuild history on `old` branch). v7 file header + slice table + plane mapping + three predictors (Left / Gradient / modular 8-bit Median) + non-RFC-1951 longest-length-first cumulative Huffman + raw-mode fallback + minimal RIFF/AVI demuxer; all 7 8-bit native FOURCCs (M8RG / M8RA / M8Y4 / M8Y2 / M8Y0 / M8YA / M8G0) byte-exact self-roundtrip across Huffman + raw modes; lacks 10/12/14-bit (M0/M2/M4 family), interlaced field-stride, OpenDML, encoder | — |
