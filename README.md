...
| **AC-4** (Dolby) | 🚧 Sync / TOC / presentation / substream parse + full A-SPX front-end incl. 18 Annex A.2 Huffman codebooks + `aspx_ec_data()` parser. **Round 7**: §5.7.3 QMF analysis + **§5.7.4 QMF synthesis** (78-81 dB round-trip PSNR on sine/PRNG) + §5.7.6.3.1.4 patch-tables + §5.7.6.4.1.4 simplified HF tile copy + flat envelope gain. **ASPX decode now produces non-silent PCM** end-to-end. Full per-envelope adjustment + TNS chirp/alpha + noise/tone generators + cross-frame `ts_offset_hfgen` continuity pending. | — |
...
| **FFV1** | ✅ v3: 4:2:0/4:4:4 YUV + RGB via JP2K RCT + `extra_plane` alpha + 9..16-bit RGB (§3.7.2.1 BGR exception) + 10-bit YUV. Range-coded `coder_type=1` + state-transition delta `coder_type=2` + Golomb-Rice `coder_type=0`. **Round 6**: cross-frame state retention for `intra=0` (`PersistentFrameState` per-slice per-plane-ctx buffers, keyframe bit drives reset per §3.8.1.3 / §3.8.2.5) + Golomb-Rice 10-bit decode (`coder_type=0 + bits_per_raw_sample>8`). ffmpeg interop: long-GOP intra=0 bit-exact, Golomb 10-bit bit-exact. | ✅ v3 range-coded YUV + multi-slice + 10-bit YUV (round 5) + **RGB 8-bit encode via JP2K RCT** (round 6: forward RCT `Cb=B-G / Cr=R-G / Y=G+(Cb+Cr)>>2`, our RGB → MKV → ffmpeg decode bit-exact). |
| **MPEG-1 video** | ✅ I+P+B frames | ✅ I+P+B frames (half-pel ME, FWD/BWD/BI B-modes, 43 dB PSNR) |
| **MPEG-4 Part 2** | ✅ I + P + B-VOP with 4MV direct mode, half-pel MC, quarter-pel MC in B-VOPs, decode→display reorder. Field-MV decode + field-DCT residual reorder + alt-vertical-scan for interlaced B-VOPs (round 5). **Round 6**: field-sample MC landed (`field_predict_luma_mb` / `field_predict_chroma_mb` per §7.6.2.2 — bit-exact interleaving on synthetic refs). Frame-7 MB(3,3) divergence hand-traced 2 rounds; spec-level VLC bit-exact on both sides (ffmpeg shows the row as `D D D D` coded-direct, we read MBTYPE=Backward MV(0,-16)); root cause unresolved. | ✅ I+P-VOP (41-43 dB PSNR, 21% vs all-I) |
...
| **MS-MPEG-4** (v1 / v2 / v3) | 🚧 **Docs UNBLOCKED (round 7)**: `docs/video/msmpeg4/` is now a clean-room Implementer handoff (9 spec docs + ~40 extracted tables). v3 I-frame first-light: `ffmpeg -c:v msmpeg4v3` DIV3 AVI → `Frame::Video` emits with non-zero luma. `build.rs` loads `tables/region_05eac8.csv` (MCBPCY VLC) — decoder uses `include!`, no hand-typed numerics. H.263 §6.2.2.1 dequant. DC-only reconstruction for now — real intra-AC TCOEFs need the `0x3df40` shared tiered-Huffman walker tree (no Extractor session has resolved that yet). P-frames + v1/v2 I-frame also pending. | — |
...
| **H.266 (VVC)** | 🚧 Full VVC front-end + **CTU walker scaffold** (round 6: `CtuLayout` / `CtuPos` / `NeighbourAvail` / `SliceQpY` / `SliceCabacState` / `CtuWalker` orchestrator) + **leaf-CU syntax reader** (round 7: `pred_mode_flag` / `intra_mip_*` / `intra_luma_ref_idx` / ISP / `intra_luma_mpm_*` / `intra_chroma_pred_mode`) + **§8.4.2 MPM list** (all 7 branches) + **§8.4.3 Table 20 chroma derivation**. 211 tests. `reconstruct_leaf_cu` still `Unsupported` — next: CBF + residual coding + transforms. | — |
| **VP6** | ✅ Full FLV playback (845/845 frames of sample decode cleanly; range coder + MB-types + IDCT + MC + loop filter + vp6a alpha) | ✅ **Round 7 scaffold**: VP6F keyframe encoder — DC-only, simple profile, bool-coded coefficients. Encoder state mirrors decoder byte-for-byte. **ffmpeg decodes our output** (pixel-exact with our decoder on flat content). AC coefficients + P-frames / MV encode / loop-filter emission pending. |
...
| **AV1** | 🚧 OBU parse + range coder/CDFs + coeff decode + partition + transforms + all intra predictors w/ edge filter + **§7.15 CDEF** + Loop Restoration (Wiener+SGR) + inter MC (NEWMV/GLOBALMV) + ref-scaling. Round-5 chroma fix: gray U/V 12 → 48/45 dB. **Round 6**: `read_block_tx_size()` (§5.11.16) + `read_skip` ordering per §5.11.5/.7. **Round 7**: `read_delta_qindex` (§5.11.12) + `read_delta_lf` (§5.11.13) + `use_intrabc` (§5.11.7, emits explicit `Unsupported` on true instead of silent desync) + `filter_intra_mode_info` (§5.11.24, stored on MI grid, predictor uses DC_PRED fallback). Testsrc luma ~11 dB — `palette_mode_info` + var-tx inter path + `read_var_tx_size` + per-block `DeltaLF[]` apply still pending. | — |
| **Dirac / VC-2** | ✅ VC-2 LD + HQ intra end-to-end + Dirac core-syntax intra (VLC and AC paths) + core-syntax inter + OBMC motion comp + full IDWT (7 wavelets) + arithmetic coder + 10/12-bit output + frame-rate-aware timebase + pts passthrough. ffmpeg-interop tests: 8-bit 4:2:2 + 8-bit 4:4:4 + 10-bit 4:2:0. Gaps: VC-2 v3 asymmetric transforms (SMPTE ST 2042-1 ed-2 not in docs/), `Yuv422P12Le` / `Yuv444P12Le` variants not yet in oxideav-core. | ✅ **VC-2 HQ + LD intra encoders** (round 7 added LD on top of existing HQ) — forward 5/3 DWT + forward arithmetic coder + slice packing with funnel-bounded streams. Self-roundtrip bit-exact (∞ dB) at qindex=0 on 64×64 4:2:0 gradient. ffmpeg-interop gaps: encoder sets profile=3 (HQ) for LD too (should be 2); slice_y_length arithmetic audit pending. |
