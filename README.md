...
| **Opus** | 🚧 ~88% — TOC + CELT + SILK NB/MB; libopus interop 10-26 dB | ✅ ~87% — CELT full-band + SILK NB/MB/WB + Hybrid + NLSF stage-1 codebook search §4.2.7.5.1 + stage-2 quantisation §4.2.7.5.6 (29.9% open-loop residual reduction on vowel /a/); ffmpeg + libopus cross-decode clean |
...
| **AAC** | 🚧 ~85% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR/ICS/ADTS bounds; lacks LD/ELD, USAC, SBR upsampling at output boundary (#771) | 🚧 ~84% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy + TNS (CPE + SCE) + perceptual M/S decision §6.6.1.3 with Johnston binaural masking + PE-based VETO/PROMOTE gates (+2.50 dB R / +0.03 dB L PSNR at -1.14% bytes on centred-stereo fixture) |
...
| **FFV1** | ✅ 100% — bit-exact ffmpeg | ✅ 100% — bit-exact ffmpeg + range-coded YUVA + 12-bit YUV (yuv{420,422,444}p12le single + multi-slice, ffmpeg bit-exact cross-decode); config-record + decoder shape gates lifted to 8..=16 bits per RFC 9043 §3.8 |
...
| **MS-MPEG-4** (v1/v2/v3) | 🚧 ~33% — clean-room scaffold; v3 intra 3-tier ESC + custom intra-DC VLC + G0..G3 LMAX/RMAX wired + synthetic-VLC end-to-end (633 round-trips across all four alphabets); still lacks G0..G3 primary canonical-Huffman bit-length array (spec/99 §10 OPEN) + alt-MV VLC re-extract. VfW-sandboxed mpg4c32.dll runs in parallel — see Windows codec sandbox below | — |
...
| **H.266 (VVC)** | 🚧 ~55% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + chroma 4-tap sub-pel + DMVR §8.5.3.2.4 (5×5 integer + 3-point parabolic half-pel refinement; bit-exact on symmetric-bipred fixture); lacks PROF, affine, full mvd_coding | 🚧 ~85% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B slice + sub-pel MC ½/¼-pel (luma + chroma) + multi-ref DPB + weighted bi-pred — see crate README |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle | 🚧 ~67% — keyframe + intra-mode RDO + P-frame inter with multi-ref (LAST_FRAME + GOLDEN_FRAME per-CU RDO via §6.4.5 single-ref tree; +1.51 dB on 4-frame A→B→A→A GOP) + ½/¼-pel ME + quadtree partitions to 8×8 with full four-way RDO at every level (down to B4x4 via §6.4.16 walk) |
| **AV1** | 🚧 ~73% — OBU + range coder + all intra preds + CDEF + LR + inter MC + palette + multi-ref compound + super-res + tx_type read-gating + §5.11.4 partition force-split + §5.11.39 sign-loop + COEFF_CDF_Q_CTXS=4 outer-dim threaded through coefficient CDFs per §9.4 (q_ctx > 0 slices now spec-correct); SVT-AV1 48/48; lacks intrabc | 🚧 ~60% — forward range coder + DCT-II 8/16/32 + coefficient emitter + partition/mode/TX emit; **dav1d 1.5.x cross-decodes every single-SB square 8×8..64×64** via spec-derived `bsl_ctx` per §5.11.4 + chroma `txb_skip_ctx=7` per §5.11.39 |
...
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~110 filter types / 137 factory names — see crate README for the catalogue |
