...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~51% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header primitives + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §6.3.12 frame_reference_mode + §6.3.16 mv_probs outer sweep + §6.4.1 get_tile_offset + §6.4.2 decode_tile + §6.3 inter-arm `parse_compressed_header_inter` + §6.4 lines 2306-2311 tile-payload sizer + §8.8.1 loop_filter_frame_init builder + §8.8.3 filter_size( ) per-edge primitive; lacks §6.2.5 inter-frame + §6.4.4 decode_block_apply + §8.8.2..§8.8.5 superblock walk | 🚧 scaffold |
| **AV1** | 🚧 ~95% — decoder feature-complete + standalone `decode_av1` entry + §6.7.2 Y-only monochrome + multi-SB dyn-Y dispatch up to 128×128 | 🚧 ~42% encoder — pixel-space YUV→IVF + 14-mode intra picker + §7.13.3 forward 2D + WHT lossless + forward quantize + §7.11.5.3 UV_CFL_PRED + base_q_idx>0 lossy + rectangular extents + monochrome encoder dyn driver + multi-super-block tiling (128 cap) + 4:2:0 YUV multi-super-block + §8.2.6 post-renormalisation probes + §7.13.3 rectangular TX up to short-side-32 (TX_4X8/TX_8X4/TX_8X16/TX_16X8/TX_16X32/TX_32X16/TX_32X64/TX_64X32 with × 2896 per-row post-scale + §7.12.3 dqDenom=4 on 64-axis pair); lacks remaining 6 rectangular TX sizes (|log2W − log2H| == 2) + §5.11.18 inter mode_info + RD picker |
