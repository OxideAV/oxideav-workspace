...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~93% — Phase-2 + cross-channel-MS agreement + r164 §2.4.3.4.10 finer attack-detector knobs (per-instance leak via AttackDetectorParams + with_params; 596 tests); lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~92% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + r164 §13.4 fitter composed with §9.4 deltas on stream refresh path (549 tests) |
...
| **AV1** | 🚧 ~56% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + §5.11.4 decode_partition + §5.11.7-13/19-21/55/56 leaves + r164 §5.11.7 use_intrabc syntax element + DEFAULT_INTRABC_CDF (539 tests); lacks §5.11.5 decode_block body + §5.11.18 inter_frame_mode_info dispatcher | 🚧 scaffold |
