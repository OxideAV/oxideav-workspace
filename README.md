...
| **CELT** | 🚧 r7 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 prefix + §4.3.2.1 coarse-energy scaffold + §4.3.2.2 fine-energy + finalize + §4.3.3 bit-allocation fields + §4.3.4 tf_change/tf_select; 93 tests; blocked on docs #936 (Laplace) + #943 (cache_caps50 / LOG2_FRAC_TABLE / alloc loop) | 🚧 scaffold |
...
| **H.263** | 🚧 ~80% (post-2026-05-18 orphan) — §5.1-§5.4 baseline + §6 IDCT/MV/half-pel/INTER + Annex J §J.3 deblock + Annex I §I.2/§I.3 AIC + Annex D §D.2 UMV + Annex F §F.2 4-MV candidate + Table F.1 chroma snap + Annex F §F.3 OBMC + §5.1.4 PLUSPTYPE + Annex I §I.3 INTRA-coef VLC + Annex K §K.2 Slice-Structured header + Annex F INTER4V driver wiring (Figure-5 mvs4 + §F.3 per-block OBMC dispatch + Table F.1 chroma) into decode_picture + r174 Annex I §I.3 modified-dequant + `clipAC` + `oddifyclipDC` primitives (308 tests); lacks Annex K driver wiring + PB-frames + PLUSPTYPE-gated driver + AIC §I.3 prediction-reconstruction + AIC absorbed-INTRADC | 🚧 scaffold |
...
| **`oxideav-generator`** | Synthetic media source (`generate://...` URIs) + zero-input filters | ✅ audio synth (sine + chirp/FM/AM/ringmod/DTMF/multitone/ADSR/formant) + image (xc/gradient/pattern/fractal/plasma/noise/label) + video (testsrc/smptebars/fractal_zoom/gradient_animate/zoneplate); ImageMagick/sox shorthands in `convert` verb (vector text → raster via scribe + raster) |
