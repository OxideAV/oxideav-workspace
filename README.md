...
| AVI       | ✅ | ✅ | ✅ | AVI 1.0 + OpenDML 2.0 demux/mux; AVIX/dmlh/vprp + 2-field interlaced + VBR audio + LIST INFO + WAVEFORMATEXTENSIBLE + ODML keyframe seek + typed PaletteChange/TextChunk/AvihFlags/Idx1Flags + AVISUPERINDEX sub-type + per-stream strh fields (handler/SuggestedBufferSize/SampleSize/dwStart/wPriority/ChannelMask/Length/Flags AVISF_DISABLED+AVISF_VIDEO_PALCHANGES + dwScale/dwRate timebase) + dmlh.dwTotalFrames master frame-count override |
...
| **GSM 06.10** | 🚧 ~35% — clean-room §5.3 RPE-LTP decoder + §4.4 in-band homing + §5.1 norm/div primitives | 🚧 §5.2.0..§5.2.14 + §5.2.18 — pre-processing + LPC analysis (autocorrelation/reflection/Schur) + §5.2.6 piecewise breakpoints + §5.2.7 LAR quantisation + coding + short-term analysis filter + LTP analysis clause + §5.2.13 weighting filter + §5.2.14 RPE grid selection; `make_encoder` still `Unsupported` until §5.2.15..§5.2.17 + §1.7 packer |
...
| **AV1** | 🚧 ~95% — decoder feature-complete + standalone `decode_av1` entry + §6.7.2 Y-only monochrome + multi-SB dyn-Y dispatch up to 128×128 + public docstring synced to actual accepted scope ([8,128]/axis 4:2:0+monochrome+multi-SB+full base_q_idx) | 🚧 ~45% encoder — **public `encode_av1` graduated from `NotImplemented` to working 4:2:0 YUV→AV1 delegation** (Y‖U‖V planar split → `encode_intra_frame_yuv_dyn`) + pixel-space YUV→IVF + 14-mode intra picker + §7.13.3 forward 2D + WHT lossless + forward quantize + §7.11.5.3 UV_CFL_PRED + base_q_idx>0 lossy + rectangular extents + monochrome encoder dyn driver + multi-super-block tiling (128 cap) + 4:2:0 YUV multi-super-block + §8.2.6 post-renormalisation probes + §7.13.3 rectangular TX up to short-side-32 (TX_4X8/TX_8X4/TX_8X16/TX_16X8/TX_16X32/TX_32X16/TX_32X64/TX_64X32 with × 2896 per-row post-scale + §7.12.3 dqDenom=4 on 64-axis pair) + `Error #[non_exhaustive]`; lacks remaining 6 rectangular TX sizes (|log2W − log2H| == 2) + §5.11.18 inter mode_info + RD picker |
