...
| **MP3** | ✅ ~100% — bit-exact decode + ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~96% — Phase-2 + long + pure-short + mixed-block per-band threshold-in-quiet + trait-API one-shot Annex D threshold-in-quiet factory + §D.1 Step 3 caller-supplied dB offset path + §D.1 Step 6 vf masking-function + Step 7 LTg global masker + §D.1 Step 4 Tables D.2a-f + §D.1 Step 4 masker-at-band placement + Step 5 decimation + Step 7 nearby-masker Bark-window pre-filter + §C.1.5.3.2.1 spreading function + Annex D Table D.5 Layer I/II coder partition table + Table D.5 dual-role ωhigh_n/ωlow_{n+1} accessors + Table D.5 partition FFT-line range accessor (composed `(ωlow_n, ωhigh_n)`) + Table D.5 `width_n` column accessor + Table D.5 composed `CoderPartitionD5Span` descriptor + Table D.5 inclusive-line membership predicate `partition_n_contains_line(n, ω)` + Table D.5 row-order iteration helper + Table D.5 `first_partition_containing_line(ω)` inverse lookup + `coder_partition_d5_omega_iter(n)` per-partition FFT-line walker + §D.1 Step 8 `coder_partition_d5_ltg_min` per-partition LTg minimum reducer + `coder_partition_d5_ltg_min_row_order` 32-element broadcast across Table D.5 row-order spans + `coder_partition_d5_width_row_order` 32-element broadcast `[0×12, 1×20]` (closes Step-8 row-order pair `(LTmin_n, width_n)` for bit-allocation loop input) + `coder_partition_d5_reduction_row_order` paired `[CoderPartitionD5Reduction { ltmin_db, width_n }; 32]` zip (Phase-2 step 61) + `coder_partition_d5_reduction_row_order_by_width` Phase-2 step 62 width-gated split `CoderPartitionD5ReductionByWidth { narrow_band[12], wide_band[20] }` at Table D.5 single 0→1 `width_n` transition (per-row branch for Layer I/II bit-allocation loop) + `coder_partition_d5_ltmin_db_row_order_by_width` Phase-2 step 63 width-gated `LTmin_n` column projection `CoderPartitionD5LtminDbByWidth { narrow_band[12], wide_band[20] }` (strict projection of step 62 onto ltmin_db field) + `coder_partition_d5_ltmin_linear_row_order_by_width` Phase-2 step 64 width-gated linear-energy projection `CoderPartitionD5LtminLinearByWidth { narrow_band[12], wide_band[20] }` via `10^(db/10)` of step 63 (linearises masking threshold for downstream Step 9/Step 10 consumers) + `coder_partition_d5_ltmin_log2_row_order_by_width` Phase-2 step 65 width-gated `log2(LTmin_lin_n)` column projection `CoderPartitionD5LtminLog2ByWidth { narrow_band[12], wide_band[20] }` via `f64::log2` of step 64 (bit-budget proxy: every factor-of-two in linear masking energy = one unit on the log2 axis; exact identity with step 63's dB column via `log2(10)/10 ≈ 0.33219 × dB`) + `coder_partition_d5_ltmin_log2_wide_band_bit_budget_total<F>(ltg_per_line) -> f64` Phase-2 step 66 wide-band signed bit-budget total `Σ_{n=1..=32} width_n · log2(LTmin_lin_n)` over Table D.5 — algebraically collapses to the 20-element f64 sum of step 65's `wide_band` subarray (Table D.5 structural invariant `width_n = [0; 12]` then `[1; 20]`) + `coder_partition_d5_ltmin_log2_narrow_band_bit_budget_total<F>(ltg_per_line) -> f64` Phase-2 step 67 narrow-band complementary signed bit-budget `Σ (1-width_n) · log2_n` collapsing to the unweighted 12-cell sum of step 65's `narrow_band` subarray (partition identity `narrow_total + wide_total = Σ row-order log2_n` over the disjoint width split) + `coder_partition_d5_ltmin_log2_paired_bit_budget_totals<F>(ltg_per_line) -> (f64, f64)` Phase-2 step 68 fused single-pass `(narrow_total, wide_total)` accessor (calls step 65 once then independently sums both subarrays — halves FFT-line callback fan-out vs steps 67+66 back-to-back; bit-identical to the two standalone totals) + Phase-2 step 69 §D.1 Step 9 width-gated signal-to-mask ratio `SMR_n = Lsb(n) − LTmin_n` over Table D.5 (the step previously believed docs-blocked — derivable from the staged Annex D prose after all) + Phase-2 step 70 §D.1 Step 9 row-order SMR vector `[SMR_1…SMR_32]` (Step-59 LTmin composed with the Step-9 subtraction, bit-identical to step 69's width-gated split read in row order) + Phase-2 step 71 §D.1 Step 9 paired `(SMR_n, width_n)` row-order vector `CoderPartitionD5Smr` (index-aligned zip of step-70 SMR with step-60 width column; SMR analogue of step 61's `(LTmin_n, width_n)` pair) + Phase-2 step 72 §C.1.5.2.7 per-partition mask-to-noise ratio `MNR_n = SNR_n − SMR_n` row-order vector `CoderPartitionD5Mnr` (first arithmetic of the Layer I/II bit-allocation loop; caller-injected SNR_n per Table C.5) + Phase-2 step 73 §C.1.5.2.7 minimal-MNR subband selection `coder_partition_d5_min_mnr` (row-order argmin over the 32-partition MNR vector → the bit-allocation loop's greatest-benefit subband; deterministic lowest-index tie-break, NaN-skip) + Phase-2 step 74 §C.1.5.2.7 next-higher-entry quantization promotion (the loop's increment step after the minimal-MNR pick — promotes the chosen subband to the next finer quantization-accuracy entry) + Phase-2 step 75 §C.1.5.2.7 recompute-MNR-after-promotion (re-evaluates `MNR_n = SNR_n − SMR_n` for the just-promoted subband with its new finer-quantization SNR so the bit-allocation loop re-enters argmin selection); lacks Steps 1-3 FFT/SPL/tonality classifier + Model 2 + intensity-stereo |
...
| **NSF** (NES) | 🚧 ~97% — full 6502 + IRQ/NMI + 5/5 2A03 APU + DMC DMA + six expansion chips + NSF v1/v2/NSFe + Dendy + Namco 163 + VRC7 OPLL pipeline + register semantics + KSR (Key Scale of RATE) + §4 KSL byte base table (YM2413 Table III-5) + Table III-7 decay-rate per-sample Q16 step + Decay/percussive-Sustain/Release lookup + Table III-7 attack-time per-sample Q16 step (closes the Attack column of the per-RATE envelope ladder) + MMC5 PCM Mode/IRQ + VRC6 sawtooth 14-step + E-clear accumulator zero; lacks §7 LFO AM/VIB step arrays + rhythm mode | — synthesis only |
...
| **AV1** | 🚧 advancing toward full AV1 — clean-room rebuild from `docs/video/av1/av1-spec.pdf`. Decoder shipped: standalone `decode_av1` entry + §6.7.2 Y-only monochrome + multi-SB dyn-Y dispatch up to 128×128 + 4:2:0 chroma + multi-super-block + full `base_q_idx`. Lacks: inter prediction + reconstruction, loop filter, CDEF, loop restoration, film grain, superres, multi-layer / scalability. | 🚧 advancing toward full AV1 — encoder shipped: working `pub fn encode_av1` (4:2:0 YUV → AV1 via planar Y‖U‖V → `encode_intra_frame_yuv_dyn`) + pixel-space YUV→IVF + 14-mode intra picker + §7.13.3 forward 2D + WHT lossless + forward quantize + §7.11.5.3 UV_CFL_PRED + `base_q_idx>0` lossy + rectangular extents + monochrome encoder dyn driver + multi-super-block tiling (128 cap) + 4:2:0 YUV multi-super-block + §8.2.6 post-renormalisation probes + §7.13.3 rectangular TX through `\|log2W-log2H\|==1` family (TX_4X8/TX_8X4/TX_8X16/TX_16X8/TX_16X32/TX_32X16/TX_32X64/TX_64X32) + all 12 §5.11 rectangular `TX_SIZES_ALL` shapes — both `\|log2W-log2H\|==2` endpoints (TX_16X64/TX_64X16 + TX_4X16/TX_16X4) + mid-family TX_8X32/TX_32X8 closing the family + § 7.12.3 dqDenom=4 routing + `Error #[non_exhaustive]` + §5.11.20 `write_is_inter` (first inter-arm §5.11.18 mode_info scalar writer) + §5.11.10 `write_skip_mode` gating Arm 1 of §5.11.18 inter_frame_mode_info + §5.11.9 `write_segment_id` shared §5.11.8 / §5.11.19 primitive (with O(1) algebraic `neg_interleave` inverse replacing O(8) search) + §5.11.19 `write_inter_segment_id(preSkip)` six-arm dispatcher + §5.11.18 `write_inter_frame_mode_info_prefix` dispatcher composing all 5 sub-writers (pre-skip + post-skip segment_id, skip_mode, skip, is_inter) + §5.11.18 lines 18-20 leaf writers (`write_cdef` per §5.11.56 inverse + `write_delta_qindex` per §5.11.12 + `write_delta_lf` per §5.11.13; analytic escape-ladder inverse `n = FloorLog2(abs_value − 1)` covers `0..=511`) + composed into `write_inter_frame_mode_info_prefix` via `InterFrameDeltaSiteInputs` (cdef_idx / reduced_delta_q_index / reduced_delta_lf surfaced on `InterFrameModeInfoPrefix`) + §5.11.22 intra_block_mode_info leaf writers: `write_intra_angle_info_y` (§5.11.42 inverse — TileAngleDeltaCdf[YMode-V_PRED] gated on MiSize≥BLOCK_8X8 ∧ directional YMode) + `write_intra_angle_info_uv` (§5.11.43 — short-circuit on UV_CFL_PRED=13) + `write_filter_intra_mode_info` (§5.11.24 outer-gate + inner-arm) + §5.11.46 no-palette `write_palette_mode_info` leaf (has_palette_y/uv outer gate + TilePaletteYModeCdf/TilePaletteUVModeCdf §8.3.2 selections; non-zero palette → `Error::PaletteEntriesUnsupported`) + §5.11.22 `write_intra_block_mode_info` dispatcher composing all four sub-writers (`UV_CFL_PRED=13` line-8 CFL arm excluded as caller-bug rejection) + §5.11.46 full palette-entries writers — luma `write_palette_entries_y` (r262: per-slot L(1) cache-merge + first-new L(BitDepth) literal + L(2) `palette_num_extra_bits_y` + L(paletteBits) delta loop with `paletteBits = Min(paletteBits, CeilLog2(range))` per-step refinement) + chroma `write_palette_entries_uv` (r263: U no-`delta++` delta loop with `range = (1<<BitDepth) − palette_colors_u[idx]`; V two-arm direct-literal/signed-delta with modular-wrap candidate picker `pick_v_signed_delta` over `cur−prev` and `cur−prev±maxVal`; 8/10/12-bit round-trip bit-exact vs `read_palette_entries_uv`) + §5.11.22 entries-aware dispatcher `write_intra_block_mode_info_with_palette` (r264: lifts the no-palette precondition — threads palette_size_y/uv_minus_2 S() against TilePaletteYSizeCdf/TilePaletteUVSizeCdf[bsizeCtx] + delegates to write_palette_entries_y/uv + closes §5.11.24 filter_intra outer-gate mechanically when PaletteSizeY != 0) + r265: §5.11.22 line-8 CFL arm folded into the dispatcher — `cfl_alpha_u/v: Option<i8>` threaded between `write_intra_uv_mode` (line 6) and `write_intra_angle_info_uv` (line 10) matching reader interleave; caller-bug rejection for missing alphas on CFL arm, surplus alphas on non-CFL arm, !has_chroma+alphas, and `UV_CFL_PRED` with `cfl_allowed == false` + r266: §5.11.23 inter_block_mode_info bootstrap — `write_inter_block_mode_info_bootstrap` covers the three §5.11.25 no-bit arms of `read_ref_frames()` (arm 1 skip_mode → SkipModeFrame[0..2]; arm 2 SEG_LVL_REF_FRAME → (seg_data, NONE); arm 3 SEG_LVL_SKIP || SEG_LVL_GLOBALMV → (LAST_FRAME, NONE); arm 4 comp_mode S() ladder deferred as caller-bug) + r267: §5.11.25 arm-4 first symbol `write_comp_mode` — folds `bw4/bh4` from §9.3 Num_4x4_Blocks_{Wide,High}[MiSize] then mirrors `if (reference_select && Min(bw4,bh4) >= 2) comp_mode S() else SINGLE_REFERENCE` (explicit path: one §8.2.6 S() over TileCompModeCdf[ctx]; suppressed path emits no bit) + r268: §5.11.25 arm-4 COMPOUND_REFERENCE body `write_compound_ref_frames` — exact inverse of the full 9-symbol comp_ref_type / uni_comp_ref{,_p1,_p2} / comp_ref{,_p1,_p2} / comp_bwdref{,_p1} S() cascade (comp_ref_type via the §8.3.2 `is_samedir_ref_pair` predicate; per-symbol ctx derived internally from the §5.11.18 neighbour-state octet; all 16 reachable compound pairs round-trip with CDF-table equality). + r269: §5.11.25 arm-4 SINGLE_REFERENCE body `write_single_ref_frames` (the single_ref_p1..p6 S() cascade over TileSingleRefCdf, §8.3.2 count_refs/ref_count_ctx derived internally) + composed four-arm `write_ref_frames` dispatcher over r266 bootstrap + r267 comp_mode + r268 COMPOUND + r269 SINGLE — `read_ref_frames` is now fully writable + r270: §5.11.23 single-prediction inter-mode writer `write_inter_single_mode` (new_mv/zero_mv/ref_mv branch bits → YMode; S() cascade over Tile{NewMv,ZeroMv,RefMv}Cdf with consulted-ctx-only validation) + r271: §5.11.23 compound-prediction inter-mode writer `write_compound_mode` (compound_mode = YMode − NEAREST_NEARESTMV, one S() over TileCompoundModeCdf[ctx]) + r272: §5.11.23 `drl_mode` DRL-index writer `write_drl_mode` (re-derives the per-slot binary `drl_mode` S() cascade over TileDrlModeCdf[DrlCtxStack] from the chosen RefMvIdx; one loop body for both the NEWMV/NEW_NEWMV start=0 arm and the has_nearmv() start=1 arm; rejects RefMvIdx unreachable at NumMvFound) + r273: §5.11.31/§5.11.32 `read_mv`/`read_mv_component` MV-difference writer `write_read_mv` (mv_joint quadrant from `diff = Mv − PredMv`, then per-non-zero-component mv_sign + mv_class + class-0 bit/fr/hp or higher-class integer ladder + mv_fr/mv_hp S() cascade; MV_INTRABC_CONTEXT under use_intrabc; force-integer and no-high-precision arms enforced as caller-bug rejects) + r274: §5.11.26 `assign_mv` PredMv derivation `assign_mv_pred_mv` (selects the PredMv feeding `write_read_mv` from the §7.10.2 find_mv_stack outputs — RefStackMv[RefMvIdx] for NEW*MV / NEAR*MV vs §7.10.2.10 GlobalMv for *GLOBALMV* / ZeroMv; the find_mv_stack ctx surface flagged in r273 was already complete decode-side). Lacks: wiring write_read_mv + write_drl_mode + assign_mv_pred_mv into the inter mode-info bootstrap; RD picker, inter encode chain. |
