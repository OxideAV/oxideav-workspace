...
| **HEIF / HEIC** (ISO/IEC 23008-12 + MIAF) | ✅ ~95% — real-world HEIC/AVIF from Apple ImageIO / libheif / ImageMagick byte-exact vs black-box readers (grids, alpha, 8–12-bit, `iscl`, full-range `YuvJ*` labelling, ISO 21496-1 gain maps within 1 code); wired into `oxideav-meta` + CLI (`oxideav probe/convert x.heic`); lacks `lhvC`/`avcC` items | ✅ ~90% — `oxideav convert in.png out.heic` works (RGB/RGBA/16-bit/grey in; sips, libheif, ImageMagick, ffmpeg open it, alpha included); HEVC pcm/intra + lossless AV1 items, grids, thumbnails, transforms, Exif/XMP/ICC, `msf1` sequences; lacks lossy AV1 items (adopt av1 0.1.19), AVIF-profile migration |
