...
| **AAC** | 🚧 ~85% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR NQ/max_sfb bounds + ICS gain_control acceptance + SBR hf_adjust monotonic-grid guard + ADTS/ASC sf_index 13/14/15 rejection + multi-frame predictor_data_present tolerance; lacks LD/ELD raw_data_block, USAC frame body, SBR upsampling at output boundary (#771) | 🚧 ~78% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy default-on |
...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal-method compositor (§23 None/Keep/RestoreBackground/RestorePrevious) + structured Application Extension parsing (NETSCAPE2.0 loop+buffer, Adobe XMP, ICC profile §26.c.iv/v) — clean-room rebuilt from CompuServe spec text + Welch 1984 only | ✅ 100% — per-frame palettes |
...
|--------|--------|--------|
| **STL** (ASCII + binary) | ✅ ~98% — both formats + per-face attribute round-trip + 16-bit colour extension (VisCAM + Materialise conventions) + Materialise binary header `COLOR=`/`MATERIAL=` + multi-`solid` ASCII + JSONL trace + `share_stats` event + fuzz-resistant header detection (BOM / leading-whitespace / `solid by Microsoft` 80-byte traps) + opt-in `validate` API (§6.5: facet orientation / unit normal / watertight / SLA-octant) + `bbox` API | ✅ ~98% — both formats + per-face attribute pass-through + auto `stl:unique_vertex_count` extras + `EncodeStats` (bit-exact + tolerance + uniform-grid spatial-index dedup) + configurable float precision |
| **OBJ** (+ MTL) | ✅ ~95% — full Wavefront grammar (v/vt/vn/f/g/o/s/mtllib/usemtl) + MTL (Phong + Wavefront-PBR Pr/Pm/Pc/Ps + map_Pr/map_Pm + Tf spectral/xyz + sharpness + `d -halo` + map_* option flags `-blendu`/`-bm`/`-clamp`/`-imfchan`/`-o`/`-s`/`-t`/`-texres` + typed `refl -type cube_*/sphere`) + smoothing groups + multi-name `g` + display attrs (`bevel`/`c_interp`/`d_interp`/`lod`) + `p` point + `mg` merging-group + LineStrip/Loop promotion + free-form geometry (`vp`/`cstype`/`deg`/`curv`/`surf`/`parm`/`trim`/`hole`/`scrv`/`sp`/`end`/`bzp`/`bsp` via Scene3D::extras) + `xyzrgb` per-vertex colour (MeshLab/libigl/Meshroom) + `v xyzw` 4th rational-weight preserved through round-trip; lacks NURBS/Bezier tessellation evaluator | ✅ ~95% — symmetric + negative-index encoder + polyline rejoin + `with_mtl_basename` |
...
| **USDZ** (+ USDA) | ✅ ~85% — own ZIP STORED walker (rejects DEFLATE entries; validates 64-byte alignment) + USDA tokenizer + prim-tree parser + UsdGeomMesh → Mesh+Primitive + UsdPreviewSurface → PBR + UsdUVTexture (`ZipStoredAsset` exposes `raw_storage("zip-stored")` for pass-through) + upAxis + metersPerUnit + per-mesh `xformOp:translate/orient/scale/transform` + multi-primitive sibling-Mesh emit-and-fold + `usd:no_fold` flag + UsdMediaSpatialAudio reader (`auralMode`/`gain`/`startTime`) + `variantSet` structured parsing (BTreeMap<name, BTreeMap<variant, Variant>>) + line:column error reporting; lacks `.usdc` binary backend (#754), UsdSkel*, UsdGeomSubset, composition arc evaluation | ✅ ~85% — symmetric ZIP STORED writer with 64-byte alignment + USDA writer + `EncodeReport.pass_through_textures`/`pass_through_audio` confirm zero-re-encode USDZ→USDZ for textures + audio when source `raw_storage("zip-stored")` matches archive scheme + strips/fans tessellation + `UsdGeomBasisCurves`/`Points` for line/point topologies |
| **FBX** | 🚧 ~30% — NEW `oxideav-fbx` r1 (binary container ✓: 32/64-bit auto-detect, 13 property type codes incl. zlib-deflated arrays; object-graph ✓: Objects walk + Connections OO wiring; mesh extraction ✓: Vertices + PolygonVertexIndex fan-triangulation + LayerElementNormal/UV with ByPolygonVertex/ByVertex/Direct/IndexToDirect mapping). Spec basis: Blender Foundation binary writeup (public domain) + ufbx project docs (MIT/Unlicense, docs only — never source) at `docs/3d/fbx/`. Lacks: ASCII FBX, animation surface, deformer (Skin/Cluster/BlendShape) surface, encoder | — |
...
Cross-format integration: `oxideav-cli-convert` exposes a 3D conversion path through `oxideav_meta::populate_mesh3d_registry` — `oxideav convert in.obj out.gltf` (or `--probe` for structural inspection). `crates/oxideav-tests/tests/mesh3d_*.rs` runs the cross-format roundtrip suite (79 tests across stl ↔ obj ↔ gltf ↔ usdz pairs + extras side-channel preservation audit + multi-material pool stress + Mesh3DRegistry lookup contract). Convert verb gained the 6 ImageMagick `-resize` geometry-modifier suffixes (`!` Force / `^` Fill / `>` Shrink-only / `<` Grow-only / `%` Percent / `@` Area), `-thumbnail WxH[!^<>%@]` (resize+strip), and `-define KEY[=VALUE]` in round 42; round 43 added the **USDZ encoder** to the routing list AND an MVP **3D→raster renderer** (`oxideav convert in.gltf out.png` — auto-framed 60° perspective, half-space edge-function rasteriser with z-buffer, flat shading off material base_color, `-render flat|wireframe`). Black-box oracles in `tests/mesh3d_{usdz_apple,blender_assimp}_oracle.rs` cross-validate against Apple `usdzconvert` (USDZ) + Blender + assimp (STL/OBJ/glTF/GLB/FBX).
...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | ✅ **Full GOP DirectShow decode** — r43 (vfw r39) closed both R38 blockers and now drives the full 6/6 GOP at 352×288 through one `SandboxedDshowDecoder` plus 20/20 frames through 10× back-to-back I+P cycles past the 4-slot pool, validating the recycle path. (1) output-allocator pool-walk traps neutered via `alloc_get_buffer` sanity-checking every pool pointer; corrupted next-link returns `VFW_E_TIMEOUT`. (2) sample-release cycle closed: new `sample_release` thunk replaces generic `release` on `IMediaSample::Release` and clears `in_use` at `+36` on refcount 1→0, mirroring `CMediaSample`'s `pAllocator->ReleaseBuffer` callback. `alloc_get_buffer` now FORCES issued sample's refcount to exactly 1 so the codec's standard one-AddRef + one-Release reliably reaches 0; `receive_frame` calls `IMemAllocator::ReleaseBuffer` on the input allocator after `IMemInputPin::Receive` returns. Earlier path landed r25-r42: IBaseFilter Run/Pause/GetState + JoinFilterGraph + IPin::ReceiveConnection + HostIMemAllocator committed-state machine + IMemInputPin::GetAllocator + IID_IMediaSample2 host stub + esp-discipline forensic + `IMemAllocator::GetBuffer` arg_dwords 4→5 fix; first I+P at 176×144 in r42. | DirectShow IBaseFilter wrapper: COM scaffolding + ole32 stubs + DllGetClassObject + full HostIFilterGraph + HostIPin (output+input) + HostIEnumMediaTypes + HostIMemAllocator (committed state-machine) + HostIMediaSample + EnumMediaTypes walker + IMediaFilter Pause/Run/GetState. user32 cascade: synthetic-HWND no-op family. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. |
...
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~50 filter types / 60 factory names (blur / edge / resize / sharpen / unsharp / gamma / brightness / contrast / level / normalize / posterize / solarize / threshold / negate / sepia / modulate / grayscale / motion-blur / emboss / flip / flop / rotate / crop / vignette / colorize / equalize / auto-gamma / tint / sigmoidal-contrast / implode / swirl / despeckle / wave / spread / charcoal / convolve (custom N×N kernel) / polar (Cartesian↔polar) / morphology (dilate/erode/open/close, 3×3 square or cross) / perspective (4-corner homography, bilinear) / distort (barrel/pincushion radial-polynomial) / tilt-shift (selective Gaussian masked by horizontal in-focus band) / composite (16 Porter-Duff + PDF §11.3.5 ops: over/in/out/atop/xor/plus/multiply/screen/overlay/darken/lighten/difference + hardlight/softlight/colordodge/colorburn) / roll (circular shift) / shave (uniform border trim) / extent (re-window onto fixed canvas) / trim (auto-crop via `-fuzz N%` Chebyshev) / channel-extract (R/G/B/A/Y/U/V → Gray8) / morphology-edge{in,out,magnitude} (gradient operators) / evaluate (per-pixel Add/Sub/Mul/Div/Pow/Max/Min/Set/And/Or/Xor/Threshold) / cycle (modular value rotation) / statistic (Median/Min/Max/Mean WxH window) / affine (6-coefficient warp w/ bilinear) / srt (Scale/Rotate/Translate composite affine)) |
