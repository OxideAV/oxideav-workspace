...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz + `idct8x8_dc_only` fast path + SHA-256 lockstep pin on 9 fixtures + 128×128 interlaced apcn + FIPS 180-4 §B.1/§B.2 self-check + §6.1.1 Tables 5/6/7 color-metadata reverse helpers + typed `FrameHeader::color_primaries_kind()` + `matrix_coefficients_kind()` Table 5/6 + `transfer_characteristic_kind()` §6.1.1 OETF accessors (Bt1886/St2084/Hlg) | ✅ ~97% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + ffmpeg cross-decode + SHA-256 lockstep pin across every encoder entry + SHA-256 pin on 10/12-bit encoder paths |
...
| **ILBM** (Amiga IFF) | ✅ ~94% — BMHD/CMAP/CAMG/BODY + ByteRun1 RLE + EHB + HAM6/HAM8 + PBM + SHAM + PCHG + ANIM op-0/op-5 + CRNG/CCRT + DRNG (DPaint IV extended range) + Sham typed row-palette accessors (`row_palette` / `palette_at_line` / `is_empty` / `rows`); lacks ANIM op-7/op-8, DEEP true-colour | ✅ ~84% — IlbmMuxer parity + masking + ANIM op-5 + CRNG/CCRT/DRNG encoder |
