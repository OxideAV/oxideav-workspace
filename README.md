...
| **Speex** | 🚧 ~52% — NB decode to first PCM (LSP→LPC + LPC synthesis filter on raw excitation) + log-domain excitation-gain reconstruction grid + §8.4 gain-scaled innovation c[n] + gain-scaled adaptive-codebook (pitch) contribution in float signal domain + §8.4 excitation composition e[n]=p[n]+c[n] + WB header/LSP/pitch chain + WB high-band synthesis assembly + UWB framing recursion; lacks mode-4 HB codebook binding + HB LSP interp | 🚧 ~5% — scaffold |
...
| **G.729** | 🚧 ~33% — tables + serial parser + full §4.1 per-frame parameter chain (LSP / pitch / FCB / gains incl. parity concealment) + §4.1.6 LP synthesis (adaptive/fixed excitation → first reconstructed-speech PCM) over 18 222 conformance frames + §4.2.2 short-term postfilter H_f(z) + §4.2.3 tilt compensation + §4.2.4 adaptive gain control + §4.2.1 long-term postfilter (integer-delay eqs 78–83) + §4.2.5 output high-pass H_h2(z) + ×2 up-scaling (full §4.2 post-processing cascade wired into decode chain) + §4.4 frame-erasure concealment (voicing classifier + gain/predictor attenuation eqs 93–96 + periodic pitch-delay repeat); lacks §4.2.1 1/8 fractional pass (tab_hup phase layout docs-gapped) + Annex B DTX | 🚧 ~5% — scaffold |
...
| **DTS** (Core) | 🚧 ~60% — frame header + 14↔16-bit pack/unpack + side-information subframe walker + Annex C reconstruction primitives + Annex D codebooks (incl. §D.6 block-code books + §C.2.1 table-look-up decoder) + §D.8 FIR tables + fused 32-band synthesis QMF driver (§C.2.5 QMFInterpolation, per-frame multi-channel) + §D.2 step-size tables + §5.5 inverse-quant scale composition + §D.5.7–§D.5.9 13/17/25-level audio books + §5.5 Primary Audio Data Arrays decode walk; lacks full subband → PCM reconstruction | — |
...
| **MPEG-1 video** | 🚧 ~50% — headers + macroblock walk + dct_coeff + dequantiser + P1180-conformant IDCT + quant-matrix state machines + §6.2.4 I-picture decode driver (real-fixture full-frame); lacks motion compensation + P/B reconstruction driver | 🚧 ~5% — scaffold |
