...
| Ogg       | ✅ | ✅ | ✅ | Vorbis/Opus/Theora/Speex pages + comments; page-granule bisection + page-level seek index (`open_indexed`); chained-link-aware duration (RFC 3533 §4); page-loss/hole detection via `page_sequence_number` (RFC 3533 §6, `hole_count()`) + continued-flag framing-consistency check (`framing_error_count()`) + page-sync recapture after parsing errors (RFC 3533 §3 + §6, `resync_count()`) |
...
| **WavPack** | 🚧 r6 (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + run-length n-decoder + Golomb (base,add) interval + per-sample value reconstruction + single-call `decode_sample` + EntropyInfo→Medians bridge; 92 tests; lacks median-adaptation amount (#992) / prediction loop / float+multichannel / CRC / encoder | 🚧 scaffold |
...
| **Theora** | 🚧 r5 (post-2026-05-20 orphan) — §6.1 + §6.2 + §6.3 + §6.4 setup-header entrypoint + Appendix B.2/B.3 VP3-default LFLIMS/ACSCALE/DCSCALE tables (vp3_defaults constructor) + §6.4.2 quantization-parameters decode (ACSCALE/DCSCALE/base matrices/quant ranges) + §6.4.3 quant-matrix compute + §6.4.4 DCT-token Huffman tables (80-table binary-tree decode, iterative DFS); 99 tests; §6.4.1 LFLIMS body blocked (#944) | 🚧 scaffold |
...
| **VP6** | 🚧 r4 (post-2026-05-18 orphan) — §9 raw-bit frame-header prefix + §15 inverse-quantization (DctQMask→DC/AC dequant tables, full 64-coeff block) + §16 inverse DCT (14-bit fixed-point separable IDCT, row+column passes) + §17.1 intra-block reconstruction (level-shift +128 + 0..255 clip, BoolCoder-independent) + §11.4 fractional-pixel interpolation filters + §17.2/§17.3/§17.4 inter-block reconstruction (shared §17 sum + 0..=255 clip; §17.2 zero-MV co-located + §17.3 full-pel MV + §17.4 prediction-fetch); 75 tests; §7.3 BoolCoder b(n) blocked (#930) | 🚧 scaffold |
