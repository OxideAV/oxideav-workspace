...
| Layer | Status | Notes |
|-------|:-------|-------|
| AACS  | ✅ Common 0.953 + BD-Prerecorded 0.953 | `oxideav-aacs` clean-room — KEYDB.cfg + MKB_RO/Unit_Key_RO parsers, Subset-Difference walk, Device-Key → PK → MK → VUK, AES-128-CBC Aligned Unit decryption, Title Key unwrap + Phase B SCSI MMC + Phase C Drive-Host AKE (ECDSA AACS curve + SHA-1 + AES-128-CMAC) + READ_DISC_STRUCTURE Format 0x81/0x82/0x83/0x84 (Data Keys, Common §4.14.3.5 Table 4-19) + REPORT KEY Binding Nonce 0x20/0x21 + MKB ECDSA verify + BD-Prerecorded §2.3 Content Hash Table + signed Content Certificate verify + CRL parse/verify/lookup (PVB §2.7 Tables 2-2..2-5). Lacks AACS 2.0 |
...
| **Vorbis** | 🚧 ~85% (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook + §4.2.4 setup walker + §3.2.1/§3.3 VQ unpack + §8.6 residue (0/1/2) + §7.2 floor 1 + §6.2 floor 0 LSP + Vorbis window + §4.3.5 channel coupling + §4.3.7 IMDCT + streaming overlap-add | 🚧 ~38% — identification/comment/codebook/floor 1/floor 0/residue/mapping/mode header WRITE + §4.3.1 audio-packet header WRITE primitive + §4.3.7 forward-MDCT cosine-summation kernel (`mdct::mdct_naive`); lacks audio-packet body WRITE + setup-header splice |
| **Opus** | 🚧 ~37% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + §3.1/§4.2 framing dispatch + §4.5 state-reset + §4.5.1.4 redundant-frame params + §4.3.2.1 CELT coarse-energy Laplace E_PROB_MODEL + §4.3.3 CACHE_CAPS50 + alloc-trim PDF + §4.3.3 per-band allocation-trim offsets + §4.3.3 band-boost decoder + §4.3.3 reservation block + §4.3.3 per-band `thresh[]` minimum-allocation vector + signalling gate; per-LM inter-mode `(α,β)` deferred | 🚧 scaffold |
...
| **AC-3 / AC-4** (Dolby Digital / Dolby AC-4) | ✅ ~97% — AC-3 + E-AC-3 SPX/TPNP/AHT + §7.8.2 LtRt downmix + Annex D mix-level + WAVE_FORMAT_EXTENSIBLE + §7.10.1 CRC + augmented crc2 + typed BitStreamMode + E-AC-3 chanmap routing + CompressionGain + xbsi2/Surround EX/Headphone/AD-converter + AudioProductionInfo + TimeCode1/2/Presence + CopyrightInfo + typed DialNorm + typed AdditionalBitStreamInfo + typed §2.3.1.2 `StereoDownmixPreference` (dmixmod, base + Annex E) | 🚧 AC-3 ~95% + AC-4 IMS — 5_X SIMPLE/ASPX_ACPL_3 real γ₁..γ₄ per-band + write_aspx_data_{1,2}ch_real_envelope builders |
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/1/2/3 + LFE + SSF/SNF + SAP + Pseudocode 121 + IMS bitstream walker; lacks ETSI fixture RMS audit + object/a-joc | 🚧 IMS ~77% — v0/v2 TOC + mono/stereo/joint M/S + 5.0/5.1/7.1 SIMPLE Cfg3Five + 5_X SIMPLE/ASPX_ACPL_1/2/3 + real per-band α+β/α1+α2+β1+β2/γ5+γ6 + 7.0/7.1 SIMPLE/ASPX_ACPL_2 + encoder-side ASPX envelope extractor + §5.7.6.4.2.1 QMF energy aggregator + encoder-side `chparam_info()` / `sap_data()` builders; lacks γ1..γ4 + 7_X ACPL_3 β + Table-181 SAP residual + back-pair Lb/Rb |
| **MIDI** (SMF) | ✅ ~99% — SMF Type 0/1/2 → PCM via 32-voice mixer + SF2/SFZ/DLS + FF 01..07 text-meta iterator + smpte_offsets/FrameRate + channel-state seek primitive + sequencer_specifics (FF 7F) + sequence_numbers (FF 00 02) + midi_ports (FF 21) + channel_prefixes (FF 20) + sysex_events (F0/F7) iterators; cargo-fuzz panic-free | ✅ SmfFile::to_bytes() / Track::to_bytes_chunk() mux-side writer + synthesis |
...
| **TTA** (True Audio) | ✅ ~98% — TTA1 fmt=1/2 + password + ID3v1/APEv2 trailer + streaming + random-access decode API + ECMA-182 CRC-64 + duration-keyed player-API quartet (total_duration/seek_to_time/frame_iter_from_time/decode_from_time) + sample_range cargo-fuzz target + typed `StreamHeader` sub-accessor sweep complete (format / bits_per_sample / channels / sample_rate / total_samples) | ✅ ~96% — TTA1 fmt=1/2 + password; bit-exact self-roundtrip |
...
| **Cook** (RealMedia) | 🚧 ~28% — flavor table + cookie parser + 8 DSP parameter tables + open-time `DecodeConfig` + wire-level real-stream integration test + cookie→flavor multi-match API + selector-family classification + typed per-family GAP errors + stateful `CallSession` RADecode call-counter + structural `Driver`/`PreparedCall`/`decode_call` orchestrator + typed `CategoryIndex`/`CategoryParameters` parallel-table bundle + structural cookie-geometry guard + typed `Descriptor::recover_samples_per_frame` + typed `FlavorRecord::is_sentinel`/`iter_playable_flavor_records` discriminator + `RA_GET_NUMBER_OF_FLAVORS{,2}_ADVERTISED` audit-anchored API-surface constants; lacks backend frame-decode | — |
