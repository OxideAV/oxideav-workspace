...
| **Vorbis** | ✅ ~95% — all residue types per RFC 5215 | 🚧 ~78% — BitrateTarget {Low/Medium/High/HighTail} bank with mu-law-companded non-uniform multiplicand axis on lookup_type=1 (+1.5 dB Laplacian SNR); per-target point-stereo crossover (3/4/6 kHz); per-frame floor0/floor1 dispatch infrastructure (5 codebooks + 4 floors + 4 mappings + 4 modes); ffmpeg cross-decodes |
| **Opus** | 🚧 ~88% — full TOC + RFC 7845 pre-skip + output_gain + §4.3.7.2 CELT de-emphasis + §4.3.5 CELT auto-scale (libopus interop 10-26 dB on 5 fixtures) + §4.2.7.4 integer silk_log2lin + §4.2.7.8 Q23 shell-pulse + §4.2.7.8.6 LCG dither + §4.2.7.5.5 NLSF stage-2 residual decoding (Tables 17-22/25-30; SILK NB/MB libopus interop 16-17 dB); lacks LSF interpolation §4.2.7.5.5 + Q12-saturated Levinson §4.2.7.5.7/8 + rewhitening §4.2.7.9.1 (last gap to 20+ dB) | ✅ ~85% — CELT full-band + SILK NB/MB/WB + Hybrid mono/stereo at 10/20 ms; ffmpeg + libopus cross-decode clean |
...
| **H.264** | 🚧 ~75% — I/P/B + 4:2:0/4:2:2 + CAVLC + CABAC + DPB + B-pyramid POC + 8 SEI payload types + Level 1b sizing; lacks 4:4:4, MBAFF, SVC/3D/MVC | 🚧 ~72% — I + P (1MV/4MV, ¼-pel) + B 16x16/16x8/8x16/B_8x8 + per-cell mixed B_8x8 sub_mb_type ∈ {1,2,3} + B_Skip/B_Direct_16x16 (§8.4.1.2.2 spatial-direct) + weighted pred + 4:2:2/4:4:4 IDR + CABAC I/P/B with real ME (mvp §6.4.11.7); ffmpeg bit-exact PSNR_Y 44.20 dB; lacks B-CABAC intra fallback wiring, P/B CABAC at 4:4:4 |
...
| **VP8** | ✅ ~99% — I+P + 6-tap luma/chroma + per-MB loopfilter slab + persistent ref/mode_deltas + correct §16.3 split_mv_tree + RFC 6386 §17.1 default_mv_context high-bit probs; lacks small loopfilter ±1 / sub-pel rounding gap on 4 ReportOnly | 🚧 ~84% — I+P + B_PRED intra-in-P + SPLIT_MV + alt-ref/golden + Lagrangian RDO + segment QP/LF + per-frequency AC/DC deltas; small-roi-segmentation 78.92% inter-MB recon |
...
| **Dirac / VC-2** | ✅ ~90% — VC-2 LD + HQ intra + Dirac core-syntax intra/inter + OBMC + 7 wavelets + 10/12-bit; ffmpeg bit-exact 8-bit 4:2:2/4:4:4 + 10-bit 4:2:0 | 🚧 ~83% — HQ + LD intra + Dirac core-syntax intra + core-syntax inter (OBMC + LeGall 5/3 wavelet residue, ffmpeg cross-decode 34 dB on translating square / 53 dB on camera-pan qpel; bit-exact lossless self-roundtrip) + 2-ref bipred B-picture (~42 dB ffmpeg) |
...
| **EVC** (MPEG-5) | 🚧 ~58% — NAL + SPS/PPS/APS + §9.3 CABAC (Baseline + 51 Main init tables) + §8 intra (5-mode Baseline) + DCT-II + Baseline P/B inter + cbf!=0 residual + deblock + RPL non-IDR (full §7.3.7/§7.4.8 ref_pic_list_struct) + HMVP (§8.5.2.7 LRU + §8.5.2.4.4 derive_default_mv) + multi-reference DPB + HMVP-as-AMVP fallback + POC reordering; lacks spatial-neighbour MV grid AMVP, LTRP entries, ALF, DRA, IBC | — |
...
| **WebP VP8L** | ✅ 100% — full lossless; 7/7 BitExact vs `dwebp` | ✅ ~95% — subtract-green + predictor + colour transform + RDO + K=16 meta-Huffman + cache-bits {0,4,6,7,8,9,10,11} + LZ77 16384-px window/256-deep chains + near-lossless + palette RDO bias + two-pass cost-modelled LZ77 + predictor-tile-bits RDO sweep; beats cwebp on natural fixtures (-46% on 128×128 natural); lacks EntropyImage transform + smart tile-boundary switching |
