...
| **AV1** | 🚧 ~94% — decoder feature-complete + **standalone `decode_av1` public entry** (1353 tests + integration roundtrips) | 🚧 ~18% encoder — pixel-space YUV→IVF driver (16×16 4:2:0 intra) with DC_PRED + forward 4×4 DCT + forward WHT (§7.13.3 lossless) + forward quantize + §5.11.4 partition + §5.11.36 transform_tree + §5.11.39 coefficient encode. **Full encoder→`decode_av1`→pixels lossless roundtrip via public API on Y+U+V at base_q_idx=0**. Lacks forward DCT 8/16/32/64 + ADST/FLIPADST/IDTX + §5.11.18 inter mode_info + RD |
