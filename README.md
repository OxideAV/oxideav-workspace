...
| **MP3** | ✅ MPEG-1 Layer III M/S | 🚧 CBR + VBR mono+stereo + **MS-stereo joint mode (44.8% smaller on mono fold-down)** + **short/start/stop block selection on transients (per-channel energy-ratio detector + 1-granule lookahead block-type machine; isolated-transient pre-onset PSNR 54.37 → ~300 dB, +245 dB pre-echo elimination)**; lacks Bark-partition spreading psy-1, intensity-stereo encode, LSF MPEG-2/2.5 |
| **AAC** | 🚧 AAC-LC + HE-AACv1 SBR + HE-AACv2 PS; lacks LD/ELD, USAC | 🚧 AAC-LC + HE-AACv1 mono+stereo + HE-AACv2 PS + **ffmpeg SBR warning closed via r26 3-bug fix (QMF matrix + envelope clamp + flush SBR FIL)** + **PNS (63.93% smaller on noise-rich)** + **5.1 multichannel (chcfg=6) + 7.1 multichannel (chcfg=7, [SCE C, CPE L,R, CPE Ls,Rs, CPE Lb,Rb, LFE], all 8 channels ffmpeg cross-decode clean at PSNR 22-36 dB)**; psychoacoustic model basic; lacks gapless padding tuning |
...
| **MPEG-4 Part 2** | ✅ I+P+B-VOP, 4MV direct, half/quarter-pel, field-MV/DCT/alt-vertical-scan, GMC (S(GMC) routing wired), data partitioning (ARTS@L1, DC/MOTION marker decode) | 🚧 I+P+B + 1MV/4MV + intra-MB-in-P + quarter-pel + single-warp GMC + data partitioning + per-VOP-type quantiser knobs + GOP knob + **intra-in-P MBs under DP (PMbDp::{Inter, Intra} pre-pass, intra MCBPC in part-1, ac_pred + cbpy + 6 DC differentials in part-2, intra AC in part-3; scene-change fixture roundtrips at 38.69 dB Y through ffmpeg)**; ffmpeg cross-decodes DP at 39-44 dB; lacks RVLC, mid-VOP packet splits, Inter4MV under DP, DP+GMC, DP+B-VOPs, multi-warp GMC, ACE Sprite VOP, MPEG-4 Studio profile, scalability layers |
| **Theora** | ✅ I+P (Theora has no B-frames) | ✅ I+P incl. INTER_MV_FOUR |
| **H.263** | 🚧 I+P + half-pel + Annex J + D + F + E + N + G (PB-frames) + **Annex I (advanced INTRA + AC prediction)** + **Annex M (Improved PB-frames, per-MB Table M.1 MODB VLC selecting bidir / fwd-only / bwd-only B-block prediction)**; lacks K-W | 🚧 I+P + diamond ME + Annex F/J/D/N + Annex G PB-frames + **r15 B-block residual (28.8→55-57 dB B-half)** + **Annex I (~21% smaller intra-rich)** + **Annex M (Lagrangian RDO across bidir / fwd / bwd shapes, -52% on mixed-motion fixture; ffmpeg lacks in-band Annex M signal so cross-decode is informational)**; lacks intra-in-P MVD, PB+Annex F, plus K, L, P, Q, R, S, T, U, V |
