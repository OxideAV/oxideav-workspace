...
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~46% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header primitives chain complete + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + r199 §6.3.12 `frame_reference_mode` + r205 §6.3.16 `mv_probs()` + r209 §6.4.4 **`decode_block( )` driver** (pure-state fan-out primitive consuming `mode_info()` + `residual()` outputs; new `decode_block_apply()` + `DecodedBlockResult` + `Vp9FrameState` `MiRows × MiCols` write-back arrays; §6.4.4 skip-rewrite rule + 10-cell per-MI fan-out across `num_8x8_blocks_*_lookup[ subsize ]`; 428 lib tests); lacks §6.2.5 inter-frame branch of uncompressed-header walker + §6.4.3 driver wire-up of `decode_block_apply` + §8.4 probability adaptation + §8.8 loop filter | 🚧 scaffold |
...
| **Ut Video** | ✅ ~97% — 5 native FourCCs × 4 predictors + RGB inter-plane decorrelation + LUT-accelerated canonical Huffman + slice-parallel decode (5.63× at 720p) + criterion baseline + r186 `Decoder` trait factory + r196 Gradient/Median per-row branch-hoist + r203 row-strided None + Left predictor refactor | ✅ ~97% — slice-parallel encode (3.28×) + content-fixture corpus + r161 cargo-fuzz oracle + r209 **`Encoder` trait wiring from `CodecParameters`** (`CodecCapabilities::with_encode()` + `registry::make_encoder` factory; FourCC derives from `tag` else from `pixel_format` Yuv420P/422P/444P; ULY0/ULY2 dim constraints + extradata parsing surface at factory time; `send_frame`/`receive_packet` repacks stride-padded plane buffers tight; all emitted packets carry `flags.keyframe = true`; tests/round17_encoder_trait_integration.rs 26 tests pin trait-path byte-equality vs direct `encode_frame`, state-machine contract, factory rejections, end-to-end round-trip through both traits) |
...
| **BMP** | ✅ ~97% — 1/4/8/16/24/32-bit + V4/V5 + OS/2 + RLE4/RLE8 + 3 fuzz targets + 31-test property sweep + r205 **V4/V5 colour-space + embedded ICC profile decode/encode** (`decode_bmp_with_metadata` returns `BmpMetadata` with `bV4CSType` / `CIEXYZTRIPLE` endpoints / R-G-B gamma / V5 rendering intent / profile offset+size / embedded ICC blob; new `BmpColorSpace`/`BmpRenderingIntent`/`BmpMetadata` types; +9 lib = 86; rule-E scrubs in same commit) | ✅ ~97% — top-down + `biClrUsed`-trimmed palette + r205 `encode_bmp_with_icc_profile` writes 124-byte BITMAPV5HEADER with `bV5CSType = PROFILE_EMBEDDED` for Rgba/Rgb24 |
...
| **PCX** (ZSoft) | ✅ ~97% — 1/2/4/8 bpp planar + packed-bits + 24 bpp RGB planar + grayscale flag + DCX multi-page + DCX `Demuxer` + r136 fuzz-hardened + r197 Criterion benches + r209 unpack-path rewrite (geo-mean −22.6% across 9 scenarios; 24bpp 1080p −24%, 2bpp CGA −39%; bit-identical output) | ✅ ~92% — 8 write paths + DCX; r185 framework `Encoder` widened to Rgba/Rgb24/Gray8 + Bgr24/Bgra/MonoBlack/MonoWhite |
