...
| **AAC** | 🚧 ~85% — LC + HE-AACv1 SBR + HE-AACv2 PS + LATM + PCE + fuzz-hardened SBR NQ/max_sfb bounds + ICS gain_control acceptance + SBR hf_adjust monotonic-grid guard + ADTS/ASC sf_index 13/14/15 rejection + multi-frame predictor_data_present tolerance + §4.6.18 implicit-SBR sf_index ∈ {9..12} reject (2·core_rate ∉ Table 1.16) + §4.4.2.3 SBR-FIL pre-SCE/CPE reject + ADTS-declared ch ∈ {1,2} HE-AAC gate; lacks LD/ELD raw_data_block, USAC frame body, SBR upsampling at output boundary (#771) | 🚧 ~78% — LC + HE-AACv1/v2 + PNS + 5.1/7.1 + Bark psy default-on |
...
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + B-pyramid POC + 8 SEI types + fuzz-hardened §7.4.3 first_mb_in_slice ≤ PicSizeInMbs−1 + §8.4.2 zero-dim chroma reject in luma/chroma MC interp + §7.4.2.1.1 + Annex A.3.1 max_num_ref_frames ≤ 16; lacks MBAFF, SVC/3D/MVC | 🚧 ~80% — I+P (1MV/4MV, ¼-pel) + B 16x16/16x8/8x16/B_8x8 + per-cell mixed B_8x8 + B_Skip/B_Direct + weighted pred + CABAC at all chroma layouts; ffmpeg PSNR_Y 44.20 dB |
...
| **AV1** | 🚧 ~72% — OBU + range coder + all intra preds + CDEF + LR + inter MC + palette + multi-ref compound + super-res + §5.11.47 tx_type read-gating against §5.11.39 set_idx (Y midgrey 128→129); SVT-AV1 48/48; lacks intrabc | 🚧 ~55% — forward range coder + forward DCT-II 8/16/32 + full coefficient emitter + partition/mode/TX emit; self-roundtrip bit-exact via own decoder, dav1d still rejects 64×64 |
...
| **GIF** | ✅ 100% — 87a/89a + LZW + interlaced + animation + disposal-method compositor (§23) + structured Application Extension parsing (NETSCAPE2.0 + ANIMEXTS1.0 loop+buffer, Adobe XMP, ICC profile §26.c.iv/v, Exif 2.3 §4.7.2) + Plain Text Extension §25 (per-cell glyph emit on flatten) + lenient-decoder mode (recoverable LZW/dimension faults) + lazy `Playback` / `LoopingFrameIter` (NETSCAPE2.0 semantics) + `decode_first_frame()` fast-path — clean-room rebuilt from CompuServe spec + Welch 1984 only | ✅ 100% — per-frame palettes + `optimize_color_tables()` GCT/LCT hoisting |
...
|--------|--------|--------|
| **STL** (ASCII + binary) | ✅ ~98% — both formats + per-face attribute round-trip + 16-bit colour extension (VisCAM + Materialise conventions) + Materialise binary header `COLOR=`/`MATERIAL=` + multi-`solid` ASCII + JSONL trace + `share_stats` event + fuzz-resistant header detection (BOM / leading-whitespace / `solid by Microsoft` 80-byte traps) + opt-in `validate` API (§6.5: facet orientation / unit normal / watertight / SLA-octant) + `bbox` API + topology module (Euler χ + boundary edges) + ASCII comment preservation | ✅ ~98% — both formats + per-face attribute pass-through + auto `stl:unique_vertex_count` extras + `EncodeStats` (bit-exact + tolerance + uniform-grid spatial-index dedup) + configurable float precision |
| **OBJ** (+ MTL) | ✅ ~95% — full Wavefront grammar (v/vt/vn/f/g/o/s/mtllib/usemtl) + MTL (Phong + Wavefront-PBR Pr/Pm/Pc/Ps + map_Pr/map_Pm + Tf spectral/xyz + sharpness + `d -halo` + map_* option flags `-blendu`/`-bm`/`-clamp`/`-imfchan`/`-o`/`-s`/`-t`/`-texres` + typed `refl -type cube_*/sphere`) + smoothing groups + multi-name `g` + display attrs (`bevel`/`c_interp`/`d_interp`/`lod`) + `p` point + `mg` merging-group + LineStrip/Loop promotion + free-form geometry (`vp`/`cstype`/`deg`/`curv`/`surf`/`parm`/`trim`/`hole`/`scrv`/`sp`/`end`/`bzp`/`bsp` via Scene3D::extras) + `xyzrgb` per-vertex colour (MeshLab/libigl/Meshroom) + `v xyzw` 4th rational-weight preserved through round-trip + Bezier `curv`/`surf` tessellation evaluator (de Casteljau, adaptive subdivision); lacks NURBS tessellation | ✅ ~95% — symmetric + negative-index encoder + polyline rejoin + `with_mtl_basename` |
| **glTF 2.0** (+ .glb) | ✅ ~90% — JSON + binary container (magic `glTF`) + full PBR (base_color, metallic, roughness, normal+scale, occlusion+strength, emissive, alphaMode Mask{cutoff}, doubleSided) + KHR_lights_punctual (Directional / Point{range} / Spot{inner,outer}) + BufferViewAsset (BIN chunk → AssetSource) + skin + skeletal animation (LINEAR / STEP / CUBICSPLINE × translation/rotation/scale/weights) + sparse accessors + multi-scene + morph-targets + accessor min/max bounds validation + normalised-int animation outputs (BYTE/UBYTE/SHORT/USHORT) + 6 spec-MUST validators (vertex-attribute byteOffset/byteStride alignment §3.6.2.4 + per-primitive accessor count parity §3.7.2.1 + index restart-sentinel rejection + TANGENT.w=±1 + COLOR_0 [0,1] range) + extensionsRequired/extensionsUsed consistency validator + animation sampler/channel target-path validator + JSON fuzz hardening (depth + size caps + duplicate-key reject); lacks KHR_audio_emitter / KHR_materials_* / KHR_texture_transform (blocked on docs/3d/gltf/extensions/ mirror, task #714) | ✅ ~90% — symmetric + sparse-encoding heuristic for animation outputs / MAT4 IBM / mesh attributes (POSITION/NORMAL/TANGENT/COLOR/WEIGHTS) + signed + unsigned normalised-int quantisation + TANGENT excluded from sparse (zero-base would yield non-conformant w=0) |
| **USDZ** (+ USDA) | ✅ ~90% — own ZIP STORED walker + USDA tokenizer + prim-tree parser + UsdGeomMesh → Mesh+Primitive + UsdPreviewSurface → PBR + UsdUVTexture (`ZipStoredAsset` `raw_storage("zip-stored")` pass-through) + upAxis + metersPerUnit + per-mesh `xformOp:translate/orient/scale/transform` + multi-primitive sibling-Mesh emit-and-fold + `usd:no_fold` flag + UsdMediaSpatialAudio reader + `variantSet` structured parsing + **variant-selection composition with LIVRPS strength** + composition-arc round-trip (references / payloads / subLayers / inherits / specializes preserved through Node::extras) + line:column error reporting; lacks `.usdc` binary backend (#754), UsdSkel*, UsdGeomSubset | ✅ ~88% — symmetric ZIP STORED writer + USDA writer + `EncodeReport.pass_through_*` confirms zero-re-encode USDZ→USDZ + strips/fans tessellation + `UsdGeomBasisCurves`/`Points` for line/point + **variant writer** (round-trips `variantSet` blocks via `Node::extras["usd:variantSets"]`) + composition-arc writer (LIVRPS-ordered emit) |
| **FBX** | 🚧 ~60% — `oxideav-fbx` r3 (binary container ✓: 32/64-bit auto-detect, 13 property type codes incl. zlib-deflated arrays; object-graph ✓: Objects walk + Connections OO wiring; mesh ✓: Vertices + PolygonVertexIndex fan-triangulation + LayerElement Normal/UV ByPolygonVertex/ByVertex/Direct/IndexToDirect; **animation ✓**: AnimationStack/Layer/CurveNode/Curve → mesh3d Animation TRS+DeformPercent channels with Hamilton-product quaternions from XYZ Euler degrees; **deformer ✓**: Skin (top-4 normalised joint+weight per corner) + Cluster (inverse(TransformLink)·Transform inverse-bind) + BlendShape sparse Indexes → MorphTarget). Spec basis: Blender Foundation binary writeup (PD) + ufbx docs (MIT/Unlicense, docs only — never source). Lacks: ASCII FBX (clean-room grammar trace blocked, #785) | ✅ ~55% — binary FBX writer symmetric with decoder: 32/64-bit FBXHeaderExtension + Definitions/Objects/Connections + mesh Vertices/PolygonVertexIndex/LayerElement + AnimationCurve nodes + Skin/Cluster/BlendShape — Blender/ufbx-readable round-trip |
...
Cross-format integration: `oxideav-cli-convert` exposes a 3D conversion path through `oxideav_meta::populate_mesh3d_registry` — `oxideav convert in.obj out.gltf` (or `--probe` for structural inspection). `crates/oxideav-tests/tests/mesh3d_*.rs` runs the cross-format roundtrip suite (79 tests across stl ↔ obj ↔ gltf ↔ usdz pairs + extras side-channel preservation audit + multi-material pool stress + Mesh3DRegistry lookup contract). Convert verb evolution: r42 added 6 IM `-resize` geometry-modifier suffixes + `-thumbnail` + `-define KEY[=VALUE]`; r43 added USDZ encoder + MVP 3D→raster renderer; r44 added Gouraud + Phong shading + `-light` / `-camera` / `-projection` / `-fov` / `-bg` + FBX decoder wired; r45 added `-render normal-debug|depth-debug` + `-aa N` (1..=8) supersampling; r46 added multi-size ICO writer via `-define icon:auto-resize=16,32,48,256` (single .ico containing all listed mipmaps, BMP/PNG per-entry). Black-box oracles in `tests/mesh3d_{usdz_apple,blender_assimp}_oracle.rs` cross-validate against Apple `usdzconvert` + Blender + assimp.
...
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~70 filter types / 82 factory names (blur / edge / resize / sharpen / unsharp / gamma / brightness / contrast / level / normalize / posterize / solarize / threshold / negate / sepia / modulate / grayscale / motion-blur / emboss / flip / flop / rotate / crop / vignette / colorize / equalize / auto-gamma / tint / sigmoidal-contrast / implode / swirl / despeckle / wave / spread / charcoal / convolve / polar / morphology / perspective / distort / tilt-shift / composite (16 Porter-Duff + PDF §11.3.5) / roll / shave / extent / trim / channel-extract / morphology-edge{in,out,magnitude} / evaluate / cycle / statistic / affine / srt / clamp / auto-level / contrast-stretch / linear-stretch / color-matrix / function (Polynomial/Sinusoid/ArcSin/ArcTan) / laplacian / canny / blue-shift (moonlight tint) / frame (3-D bevel border) / shade (directional Lambertian relief from azimuth+elevation) / paint (oil-paint modal-bucket vote) / quantize (uniform-grid cbrt(N)-per-axis) / clut (1-D LUT) / hald-clut (Hald CLUT trilinear cube) / sketch (DoG+pencil-stroke) / deskew (Hough-line angle estimator) / hough-lines (peak-vote line overlay) / barrel-inverse (lens-undistortion solve) / stegano (LSB embed/extract) / stereo (anaglyph offset)) |
