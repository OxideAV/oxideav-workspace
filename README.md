...
| MOV (QuickTime) | ✅ | — | ✅ | Native `oxideav-mov` crate — Apple QTFF + chan layout + faststart + udta + dref (incl. external file refs + multi-hop alias chains + cross-platform `file://`) + tkhd matrix→rotation + chapter resolution + gmhd/text/tmcd + meta-only files open without moov + ISO BMFF meta (pitm + iinf + iloc + idat + iref) + HEIF/HEIC item-properties (colr/pixi/auxC alpha-plane/clli/mdcv/cclv HDR) + derived images (grid/iovl/iden) with TransformChain cascade + per-tile ispe validation + 29-variant BrandClass + `is_heic`/`is_avif`/`is_miaf` + styl/ftab text-sample style trailers; rejects fragmented MP4 (use `oxideav-mp4`) |
| AVI       | ✅ | ✅ | ✅ | OpenDML 2.0 super-index + AVIX + dmlh + vprp + 2-field interlaced + 02ix mid-`movi` + truncated-head recovery + VBR audio (with VBR/CBR `dwSampleSize` validator) + LIST INFO emit/read (hdrl-nested + top-level + multi-value FourCC accessors) + typed `PaletteChange` + `TextChunk` round-trip (eager + lazy iterators) + typed `AvihFlags` + fluent muxer flag builders + computed `dwSuggestedBufferSize`/`dwMaxBytesPerSec` (audio-only fallback) + `avi:over_budget` metadata; idx1 + ODML keyframe seek with `KeyframeSeekResult` |
...
| **AC-4** (Dolby) | 🚧 ~98% — A-SPX + DRC + 60+ ETSI codebooks + 5_X/7_X ACPL_1/2/3 + cfg0/1/2/3 trailer-aware + LFE + SSF/SNF + SAP Tables 181/183 (sap_mode 0/1/3) + Pseudocode 121 companding with multi-channel cross-channel exact `g_synch` + ACPL_3 stereo-CPE M=2 synced + IMS bitstream_version≥2 walker; lacks ETSI fixture RMS audit, object/a-joc substreams | 🚧 IMS scaffold ~25% — v0 + v2 TOC round-trip through `Ac4Decoder` via `Ac4ImsEncoder` + arbitrary-PCM mono SIMPLE/ASF body (forward MDCT + KBD + scalefactor + HCB5; 1 kHz/multi-tone/silence round-trip, sine SNR >40 dB); lacks HCB1..11 codebook search, stereo/multichannel |
...
| **VP8** | ✅ 100% — entire 15-fixture corpus bit-exact | 🚧 ~97% — I+P + B_PRED + SPLIT_MV + alt-ref/golden + Lagrangian RDO + libvpx-shape Trellis + activity AQ + RFC 6386 §15.2 mode/ref deltas; ~12 opt-in advanced flags off-by-default (adaptive LF cap ladder + variance-driven cap + UV-channel deltas + per-MB / spatial 4-means segment_lf_deltas + 4×4 B_PRED RDO + UV-mode RDO + joint LF-RDO + SPLIT_MV partition RDO with first-pass real-context + MV-cost-aware NEAREST/NEAR/NEW + sub-pel partition refinement + Trellis context-rate + psy-RDO/ARNR) |
...
| **SVG** | ✅ ~98% — full shape set + path + gradients + text/tspan + mask + clipPath + use/symbol viewport mapping + svgz + SMIL animate/set/animateTransform at arbitrary `t` (paced + spline calcMode + parent-id-tracked re-attachment) + CSS3 Selectors L3 cascade with pseudo-classes + `@import` resolve_imports (cycle detection + depth cap 8) + `@font-face` + `@keyframes` capture + `@keyframes` runtime evaluation at `t_seconds` (lerps transform/opacity/colour) + Media Queries L4 `@media` parse + evaluation + viewBox + non-uniform preserveAspectRatio + `<image>` data-URI + external href + `<script>` graceful capture + 17 typed filter primitives | ✅ ~86% — round-trips full shape graph + PreservedExtras side-channel for `<style>`/`<filter>`/`<animate>`/`<foreignObject>`/`<script>`/`<image>` |
| **PDF** | ✅ ~94% — bytes → Scene via xref + xref-streams + ObjStm + content-stream parser + `/Prev` incremental updates + `/Encrypt` all revisions (R=2/3/4 RC4 + AES-128, R=5/6 AES-256) + per-stream `/Crypt /Identity` + public-key handlers `adbe.pkcs7.s3/s4/s5` incl. KARI ECDH (P-256/384/521/X25519 with X9.63 + RFC 8418 HKDF KDFs + AES-KW 128/192/256) + `TrustStore` for long-term-cert KARI originators + read-only RC2 / 3DES envelopes for PDF 2.0 legacy; lacks text extraction, JPEG passthrough, X448 | ✅ ~96% — PDF 1.4/1.5 multi-page + paths + gradients + strokes + opacity + clip + RGBA images + xref-stream + ObjStm + incremental updates + Linearization (Annex F lin-dict + F.4.1 hint table) + ObjStm+encryption combined path + `/Encrypt` ENCODE all revisions + public-key ENCODE (pkcs7_s4 / pkcs7_s5_v4_aes128 / pkcs7_s5_v5_aes256 + multi-CF + symmetric KARI writer across all 4 curves + 3 HKDF binds) |
...
| MSMPEG4 v3 DShow | `mpg4ds32.ax` | winxp | DirectShow infra wired end-to-end (r25-r34): IBaseFilter Run/Pause/GetState + JoinFilterGraph + IPin::ReceiveConnection (synth AMT fallback) + downstream HostIBaseFilter + HostIPin + HostIMemInputPin Receive callback + HostIMemAllocator committed-state machine + pin_with_direction walker + real MP43 keyframe (176×144) wired through trait path + IMemInputPin::GetAllocator dispatch with `CodecAllocatorNegotiation` capture. mpg4ds32 returns `CLASS_E_CLASSNOTAVAILABLE` from GetAllocator — its internal `CoCreateInstance(CLSID_MemoryAllocator)` fails because host doesn't register that CLSID. r35 path: mint host-side `CLSID_MemoryAllocator {1E651CC0-…}` class factory | DirectShow IBaseFilter wrapper: COM scaffolding + ole32 stubs + DllGetClassObject + full HostIFilterGraph + HostIPin (output+input) + HostIEnumMediaTypes + HostIMemAllocator (committed state-machine) + HostIMediaSample + EnumMediaTypes walker + IMediaFilter Pause/Run/GetState. user32 cascade: synthetic-HWND no-op family. CLSID `{82CCD3E0-F71A-11D0-9FE5-00609778EA66}`. |
