...
| **H.266 (VVC)** | 🚧 ~50% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD; lacks DMVR/PROF, affine, full mvd_coding | 🚧 ~72% — forward CABAC + DCT-II + per-CTU SAO RDO + APS-signalled luma ALF Wiener RDO + chroma residual + spec-shaped coding_tree shells + cu_qp_delta + 128×128 forced-QT-split + MTT BT+TT picker RDO + multi-row `CuNeighbourMap` + inter-frame P-slice scaffold (single-ref DPB + integer-pel SAD + spatial MVP + `mvd_coding` + `tu_y_coded_flag`; 78.23 dB PSNR_Y on 4-px translation); lacks sub-pel MC, B-slice, multi-ref DPB |
...
| **VP9** | 🚧 ~85% — keyframe + inter + segmentation + COMPOUND_PRED + INTERINTRA + per-frame CDF; chroma bit-exact + version-robust fuzz oracle (per-plane uniform-fill envelope + libavcodec-version probe) | 🚧 ~40% — keyframe + simple P-frame + per-block luma + chroma intra-mode RDO ({DC, V, H, TM} per §6.4.6) + early-termination DC-probe pruning; chroma gradient U 52.34 dB |
...
| **JPEG XL** | 🚧 ~85% — ISO/IEC 18181-1:2024 final core. 7 small lossless fixtures decode PIXEL-CORRECT (incl. alpha-64x64 RGBA + bit-depth-16 LE-packed). Modular path complete + ISOBMFF `FF 0A` strip + 1..16 bpp pack convention; VarDCT scaffold with Annex I.2 IDCT primitive + non-DCT helpers wired. d1-Squeeze localised to upstream prelude D[] / TOC boundaries; lacks HfPass + PassGroup HF + GetDCTQuantWeights + CfL / Gaborish / EPF | — retired; will re-author after decoder forward progress |
...
| **`oxideav-audio-filter`** | Audio effects & conversions (streaming) | ✅ Volume, NoiseGate, Echo, Resample (polyphase windowed-sinc), Spectrogram, Biquad (LP/HP/BP/notch/peaking/shelving), Compressor, Limiter (look-ahead), DcBlocker, StereoWidener, Reverb (Schroeder), Tremolo, LoudnessITU (BS.1770/EBU R128), PitchShift (SOLA) |
| **`oxideav-image-filter`** | Single-frame image effects (stateless) | ✅ ~83 filter types / 99 factory names — see crate README for the catalogue |
