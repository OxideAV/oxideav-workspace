...
| Matroska  | ✅ | ✅ | ✅ | MKV/MKA/MKS; DocType-aware probe; Cues-based seek; SeekHead emit; Chapters + Attachments surfaced via metadata; subtitle tracks via `MediaType::Subtitle` |
| WebM      | ✅ | ✅ | ✅ | First-class: separate fourcc, codec whitelist (VP8/VP9/AV1/Vorbis/Opus); inherits Matroska Cues seek |
| MP4       | ✅ | ✅ | ✅ | mp4/mov/ismv brands, faststart, iTunes ilst metadata; sample-table seek; fragmented (moof/mfhd/tfhd/trun) demux + mux for DASH/HLS/CMAF; sidx/mfra/tfra random-access indexes; AC-3/E-AC-3/DTS sample-entry FourCCs |
...
| **H.264** | 🚧 ~75% — I/P/B + 4:2:0/4:2:2 + CAVLC + CABAC + DPB + MMCO/RPLM + B-pyramid POC + SEI T.35/pan-scan/itu_t_t35/film_grain_characteristics/post_filter_hint/full_frame_freeze/release/snapshot/deblocking_filter_display_preference + Level 1b DPB sizing; lacks 4:4:4, MBAFF, SVC/3D/MVC | 🚧 ~55% — I + P (1MV/4MV, ¼-pel) + B-slice (16×16/8×8 mixed) + weighted pred + 4:2:2/4:4:4 IDR + CABAC for I/P + level_idc auto-derive; ffmpeg cross-decode 50-52 dB on B-frames; lacks B-slice CABAC encode, P/B at 4:2:2/4:4:4, multi-ref temporal direct, full mvp under CABAC |
| **H.265 (HEVC)** | 🚧 ~68% — I/P/B 8-bit + Main 10/12 + 4:2:0/4:2:2/4:4:4 + SAO + deblock + scaling lists + bit_depth-aware Main 10/12 dequant/IDCT/prediction/deblock; HEIF/HEIC opt-in (corpus 6/14 BitExact via tolerance tier — still-image-overlay layer-0 corner drift fixed); 4:2:2 cu_qp_delta CABAC desync open | 🚧 ~60% — Baseline CAVLC I+P + Main CABAC P + B-slice (mini-GOP=2) + per-CU SAD-RDO + AMP partitions + Main 10/12 IDR + Main 4:4:4 8/10/12 IDR (45 dB ffmpeg); lacks Nx2N/2NxN, mvd_l1_zero_flag, mini-GOP > 2 at 10/12-bit, P/B at 4:4:4 |
| **H.266 (VVC)** | 🚧 4:2:0 IDR intra (BDPCM, MIP, CCLM, ISP) + ALF/SAO/CC-ALF + P/B merge+skip + fractional-pel MC + HMVP + temporal merge + pairwise-average merge + MMVD merge (uni + equal-POC bi + asymmetric-POC bi §8.5.2.7) + CIIP merge + BCW (CU-level weighted bi-pred §8.5.6.6.2 eq. 981) + PH-carried pred_weight_table (§7.3.8); lacks GPM, AMVR, BDOF/DMVR/PROF, dual-tree, full inter residual decode, affine, CIIP-with-MMVD | 🚧 forward CABAC + DCT-II + flat quant only; residual emit + pixel output pending |
...
| **VP8** | ✅ ~98% — I+P + 6-tap luma + 6-tap chroma (profile 0) + spec-correct find_near_mvs + per-MB loopfilter slab + sharpness-gated interior_limit + coef-prob persistence gated on refresh_entropy_probs; 4 ReportOnly P-frames remaining on SPLIT_MV+SKIP at mb_y≥3 | 🚧 ~82% — I+P + all 5 intra modes + B_PRED in intra-in-P + SPLIT_MV + loop filter + alt-ref/golden-ref + Lagrangian RDO with bool-coded bit accumulator + per-mode SSE for Intra (+2.47 dB / -42.5% bytes BD-rate win) + segment maps + per-segment QP/LF + scene-cut + alt-ref temporal-filter; ffmpeg cross-decode clean |
| **VP9** | 🚧 Keyframe + inter + segmentation + bit-accurate MV + compressed-header probs + §6.2.2.1 found_ref break + show_existing_frame DPB emit; lossless-pattern Y 47.70 dB + chroma bit-exact (r22 §6.4.3 fix); sub-8×8 inter mode-info per-4×4 iteration + within-MB §6.5.14 + between-MB §6.5.11 sub-block MV mixing (both §6.5 asymmetry axes closed per spec) | — |
| **AV1** | 🚧 ~72% — OBU + range coder + coeff + partitions + transforms + all intra preds + CDEF + LR + inter MC + palette + multi-ref DPB compound + spec-correct update_cdf + HasChroma/cfl/angle-delta-ctx extended to inter intra-within-inter + super-resolution clipped/unclipped block-size threading; SVT-AV1 chain 48/48; lacks intrabc, full primary_ref_frame=4 chain | 🚧 ~30% — forward range coder + intra DC + 4×4 DCT + coeff emit + streaming-precarry V-tracking (11-byte state, no O(N) memory) + partition/intra-mode/TX-type emit + dav1d cross-validation (profile 0 8-bit 4:2:0, ≤64×64); lacks B-frames, full inter |
...
| **WebP VP8L** | ✅ ~100% — full lossless; 7/7 BitExact corpus vs libwebp | ✅ ~85% — subtract-green + predictor + colour transform + 32-trial RDO + K=4 meta-Huffman + near-lossless smoothing + simple-Huffman + palette RDO bias; 1.17× cwebp on photo, 0.98× on palette |
| **WebP VP8** | ✅ via VP8 decoder + bit-exact YUV→RGB + fancy chroma upsample (libwebp interop); +1 LSB loopfilter rounding gap on a few P-frame edges | 🚧 ~75% — VP8 I-frame + ALPH + per-segment QP tuning by quality + per-segment LF deltas; animated encode + per-frame mixed lossy/lossless (Auto picks smaller); webpinfo + dwebp cross-decode clean |
...
| **JPEG XL** | 🚧 Signature + SizeHeader + ImageMetadata + ISOBMFF `jxlp` container + 2019 committee-draft Modular pixel decode + 2021 FDIS migration through round 10 (ANS + FrameHeader + TOC + ImageMetadata + LfGlobal + GlobalModular + modular_fdis + cl_code early-terminate + kRCT/kPalette/kSqueeze transforms); next blocker: docs gap on inverse-palette negative-index branch (FDIS L.6 vs djxl) | 🚧 Round 4 — lossless modular + Gradient predictor + ANS-coded symbol stream + multi-group + libjxl cross-validation; encoder-side alias-table workaround for FDIS bijection issue |
...
| **SVG** | ✅ Rounds 1-3: full shape set + `<path>` + `<g>` + `<defs>` + linear/radial gradients + `<text>`/`<tspan>` (via scribe, opt-in `text` feature) + `<filter>` graceful pass-through + `<mask>` + `<clipPath>` + `<use>`/`<symbol>` + `.svgz` inflate + `<animate>`/`<set>` snapshot at t=0; CSS L3 colours + transforms; hand-rolled SAX parser | ✅ Round-trip preserves shape/stroke/fill/gradient/transform/mask/clipPath; auto-generated `<defs>` ids on encode |
