...
| **WavPack** | 🚧 ~82% (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + Golomb (base,add) interval + r186 `parse_block` aggregate + r191 `AdaptiveMedians` §3.2 + r194 **first PCM-producing API** `decode_packed_samples_mono` + r199 **stereo per-sample decode loop** (`decode_sample_stateful_stereo` + `decode_packed_samples_stereo` per spec §2 channel-alternation + §4.2-stream-level BOTH-channel zero-run + per-channel holding-state independence; truncation-recoverable cursor via parity gate; 252 tests = +13); lacks hybrid 0x0B+0x0C / float / multichannel / CRC / encoder | 🚧 scaffold |
