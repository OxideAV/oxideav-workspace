...
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + r155 MPEG-2.5 framing (Fraunhofer extension, parse+writer round-trip; bit-exact decode still MPEG-1-only) | 🚧 ~84% — Phase-2 PCM→MP3 + outer loop + bit-reservoir + true-VBR + CRC-16 + stereo/MS + auto MS/LR + short+mixed forward MDCT toggles + linbits-reach filter; lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain + block-type auto-decision |
...
| **VP8** | ✅ 100% — RFC 6386 key+inter decode, bit-exact vs vpx/ffmpeg on 10+ multi-frame fixtures | 🚧 ~81% — Phase-2 I+P + SPLITMV + GOLDEN/ALTREF + multi-partition + RefreshControls + LoopFilterDeltas + P-frame pixel lockstep + §9.4 filter_type knob + r155 §13.4 caller-driven token-prob update layer (keyframe path; 504 tests) |
...
| **AV1** | 🚧 ~45% — uncompressed header + msac + §9.4 default-CDF + §8.3.2 ctx selectors + §5.11.49 palette_tokens + §9.3 block-size + Partition_Subsize tables + §5.11.4 decode_partition + §5.11.11 read_skip + §5.11.10 read_skip_mode + r155 §5.11.12 read_delta_qindex (CurrentQIndex + DELTA_Q_SMALL escape ladder, 433 tests); lacks §5.11.5 decode_block body | 🚧 scaffold |
...
| **WebP** (VP8 + VP8L) | ✅ ~95% — RFC 9649 container + VP8L lossless + ALPH + animated; VP8-lossy via sibling crate | 🚧 ~73% — VP8L lossless encoder + LZ77 + spatial/color-transform + color-cache + simple code-length + color-indexing + multi-meta-prefix + histogram-distance per-region clusterer + r155 §4.1 predictor size_bits two-value sweep (default-4 + maximal-9); lacks VP8-lossy encode (pending vp8 republish #1068) |
