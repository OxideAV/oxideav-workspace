...
| IFF / 8SVX| ✅ | ✅ | — | Amiga IFF with NAME/AUTH/ANNO/CHRS; r178 ILBM 24-bit literal-RGB true-colour read+write (raw + ByteRun1; HasMask/HAM/EHB rejected at 24bpp) |
...
| TIFF      | ✅ | ✅ | — | TIFF 6.0 single-image + r177 BigTIFF write (magic 43 / 8-byte offsets / LONG8 strip+tile arrays) |
...
| ICO / CUR | ✅ | ✅ | — | Windows icon + cursor — multi-resolution, BMP and PNG sub-images; r178 body-dim `(0,256]` reject (closes fuzz CUR-biHeight probe-vs-render panic) |
...
| **FLAC** | ✅ 100% — bit-exact vs RFC 9639 + CUESHEET → Chapter API + r163 RFC 9639 §8.8 typed PICTURE accessor (parse + write; 92 tests) | ✅ 100% — bit-exact roundtrip + LPC order/window/precision search + closed-form Rice estimate + flamegraphs + §8.6 PADDING writer + composable block-header serialiser + opt-in PADDING reservation |
| **Vorbis** | 🚧 r8 (post-2026-05-20 orphan) — identification + comment + §3.2.1 codebook + Huffman tree + full §4.2.4 setup-header walker + §3.2.1/§3.3 VQ vector unpack (entry → vector via multiplicands + minimum_value + delta_value + sequence_p) + §8.6 residue decode (formats 0/1/2) + §7.2.3/§7.2.4 floor type 1 packet decode + curve computation + §6.2.2/§6.2.3 floor type 0 LSP per-packet decode + curve computation + §1.3.2/§4.3.1 Vorbis window + §4.3.5 inverse channel coupling + §4.3.3 nonzero-vector propagate + §4.3.6 floor×residue dot product + §4.3.1–§4.3.6 audio-packet driver + §4.3.8 overlap-add primitive + r180 §4.3.7 IMDCT + §4.3.6 window wired into per-packet driver via `decode_audio_packet_windowed` (~88/0 → ~92/0; full §4.3 pipeline reachable end-to-end, fixture-derived `imdct_scale` knob deferred per #1051) | 🚧 scaffold |
| **Opus** | 🚧 ~27% — RFC 6716 range decoder + full SILK pipeline + §4.3 Table 56 CELT pre-band header + §3.1/§4.2 framing dispatch + r162 §3.4 R1..R7 malformed-input rejection audit (382 tests, 20 integration); CELT bands gated on #936 + #943 | 🚧 scaffold |
| **MP1 / MP2** | ✅ Layer I + Layer II decode to PCM + §2.4.3.1 CRC-16 verify + mp2 frame-level decode loop; lacks bit-exact PSNR + LSF (#1076) + Annex D psy | 🚧 ~65% — Layer I encoder + Layer II §C.1.5.2.7 bit-allocation + Table C.5 SNR + r160 §2.4.2.3 mp2 frame-header writer (168-cell ladder×Fs×mode matrix round-trips through FrameHeader::parse; 158 tests); lacks Layer II ALLOC/SCFSI/sample writers + Table C.4 SCFSI + Table C.6 quantizer + Annex D psy |
| **MP2** | 🚧 ~28% (post-2026-05-24 orphan) — §2.4.1.3/§2.4.2.3 Layer II header parser + §2.4.3.1 frame sizing + Annex B Table 3-B.1/3-B.2a..d/3-B.4 + joint-stereo allocation + scfsi + §2.4.3.3.4 sample requantizer + §2.4.3.1 CRC-16 + r162 malformed-input property suite (32-bit bit-flip exhaustion + prefix-truncation + sync-search robustness; 121 tests); lacks §2.4.3.2 polyphase synthesis + encoder | 🚧 scaffold |
| **MP3** | ✅ ~100% — bit-exact vs mpg123; ID3v2/Xing seek + MPEG-2.5 framing | 🚧 ~93% — Phase-2 + cross-channel-MS agreement + r165 §2.4.3.4.10 attack-detector DEFAULT_AMBIENT_LEAK empirical-corpus calibration (596 tests); lacks Annex D psy (#1048) + intensity-stereo + LSF audio-chain |
| **AAC** | 🚧 Phase 1 — ADTS + raw_data_block walker + AudioSpecificConfig + program_config_element + r177 §4.4.1 GASpecificConfig extensionFlag body (Table 4.1: AOT-22 BSAC layer pair / ER-AOTs resilience triplet) + Table 1.15 trailing epConfig 2-bit field for 11 ER AOTs (333 tests); decoder body still pending | 🚧 scaffold — Phase-2 writers: section_data + ics_info + pulse_data + tns_data + scale_factor_data + DPCM accumulator pair + r160 §4.4.2.1 raw_data_block frame assembler (SCE/CPE/CCE/LFE/FIL/DSE/END byte-align) + r165 Pce::write + FrameAssembler::push_pce (294+ tests) |
| **CELT** | 🚧 r8 (post-2026-05-20 orphan) — RFC 6716 §4.1 range decoder + §4.3 prefix + §4.3.2.1 coarse-energy scaffold + §4.3.3 bit-allocation fields + §4.3.4 tf_change/tf_select + r181 §4.3.4.3 spread parameter + Table 56/59 f_r; 112 tests; blocked on docs #936 (Laplace) + #943 (cache_caps50 / LOG2_FRAC_TABLE / alloc loop) | 🚧 scaffold |
| **Speex** | 🚧 r7 — Ogg stream-header + NB (Table 9.1 + CELP body bit-reader) + WB high-band (Table 10.1 + body bit-reader, modes 0..=4) + §5.5 in-band signalling + r165 typed packet → frame iterator + r179 MSB-first `BitWriter` (symmetric companion to BitReader, encoder-shape infrastructure; 127 tests); lacks LSP-VQ + pitch/innovation codebooks (#969) + LSP→LPC + synthesis + UWB framing | 🚧 scaffold |
...
| **AC-3 / AC-4** (Dolby Digital / Dolby AC-4) | ✅ ~97% — AC-3 full decode + E-AC-3 SPX (§E.3.6 HF regen) + transient pre-noise (§E.3.7.2 TPNP) + multichannel fbw+LFE+coupling AHT (§3.4 Adaptive Hybrid Transform incl. LFE-channel mantissas + lfeahtinu synthesis + interleaved cplahtinu coupling-channel mantissas) + §7.8.2 LtRt matrix-encoded stereo downmix + r126 Annex D §2.3 alternate-syntax mix-level params (ltrtcmixlev/ltrtsurmixlev/lorocmixlev/lorosurmixlev via xbsi1, reserved-code resolution per Tables D2.3-D2.6) + WAVE_FORMAT_EXTENSIBLE + r172 §3.6.4.2.3 SPX-attenuation border notch (Table E3.14); AC-4 ~98% decoder + IMS encoder ~65% (mono/stereo/5.0/5.1/7.1 Cfg3Five + 5_X ASPX_ACPL_3 + 7.1 3/4/0.1 SIMPLE/ASPX_ACPL_2 LFE multichannel + r126 7.0 SIMPLE/Cfg3Five) | 🚧 AC-3 ~95% — acmod 1/2/2.1/3/6/7 + LFE + DBA + 5-fbw coupling + E-AC-3 indep+dep + per-channel PSNR gates + r95 two-stage equalise + spread-cap greedy for per-channel `fsnroffst[ch]` (≤ ~1.5 dB spread; closes r91 cheap-mantissa runaway) |
...
| **MIDI** (SMF) | ✅ ~99% — SMF Type 0/1/2 → PCM via 32-voice mixer + SF2/SFZ/DLS + DLS `art1`/`art2` + SF2 EG2 + 2-pole resonant low-pass biquad on shared SamplePlayer + SFZ filter EG (`cutoff` / `resonance` / `fil_type` covering all 6 SFZ v1 shapes + `fileg_*` envelope opcodes) + MPE v1.1 + RPN 0/1/2/5/6 + CA-25 Master Tuning + MIDI Tuning Standard (per-key + scale/octave microtuning) + Universal Master Volume SysEx + Master Balance SysEx + GM2 Global Parameter Control (CA-024 reverb/chorus) + Data Inc/Dec (CC 96/97, RP-018) + `SmfFile::time_signatures()` iterator (FF 58, stable-merge across tracks) + r125 `SmfFile::tempo_map()` iterator (FF 51, BPM-cached, stable-merge across tracks) + r128 `SmfFile::key_signatures()` iterator (FF 59, circle-of-fifths label resolver) + r172 cargo-fuzz harness (smf/sf2/dls/sfz, 30M+ inputs panic-free post sfz octave-overflow fix) | — synthesis only |
...
| **WavPack** | 🚧 r12 (post-2026-05-18 orphan) — v4 block/metadata/decorrelation/entropy parse + LSB bit-reader + run-length n-decoder + Golomb (base,add) interval + per-sample value reconstruction + single-call `decode_sample` + EntropyInfo→Medians bridge + block-header accessor coverage + r172 per-term `decorrelation_sample_count` + flat-payload partitioner + r179 `PackedSamples` typed view of `0x0A` sub-block + `BitReader` cursor accessors + channel-indexed `Medians::from_entropy`; 173 tests; lacks median-adaptation amount (#992) / stereo per-term sample count / prediction loop / float+multichannel / CRC / encoder | 🚧 scaffold |
...
| Codec | Decode | Encode |
|-------|--------|--------|
| **MJPEG** | ✅ ~97% — baseline + progressive 4:2:0/4:2:2/4:4:4/grey + 12-bit YUV + SOF9 arithmetic + lossless SOF3 P∈{2..16} 3-component decode (P=8 Rgb24, P∈{10,12,14} planar GbrpNLe, else Rgb48Le) + RFC 2435 RTP/JPEG depacketization + packetization + docs-corpus CI tier-gated + 6 cargo-fuzz targets | ✅ ~95% — baseline + progressive + lossless SOF3 grey/RGB (all 7 Annex H predictors) + DRI/RSTn restart markers + non-zero point transform Pt 0..15 |
...
| **MPEG-2 video** | 🚧 ~48% — §6.2.x sequence/GOP/picture/slice + macroblock_type + cbp + macroblock_modes + motion_vectors + §7.6.3.x PMV + §7.6.4 forming-predictions pel reader + §7.6.7 combine + §7.6.8 add-and-saturate + r165 §7.6 per-macroblock pipeline driver + r179 §7.4 inverse-quantisation pipeline (Tables 7-4/7-5/7-6 + intra DC + weighting + reconstruction + saturation + sum-parity; 498 lib tests); lacks residual VLCs / IDCT (#1110) | 🚧 scaffold |
| **MPEG-4 Part 2** | 🚧 ~52% — I-VOP intra + inter texture pipeline (§6.2.7 inter block(i) → §7.4.x non-intra chain through §7.4.5 IDCT to residual) + §6.2.5 video_packet_header + §7.8.7.3 GMC + §6.2.2 VisualObject() + r163 §6.2.7 / §7.3 inter block driver (no MV-pred dep; 405 tests); lacks Figure 7-34 MV-predictor (#1125) + motion compensation + B-VOP + encoder | 🚧 scaffold |
| **Theora** | 🚧 ~37% — §6.1–§6.4 setup-header + Appendix B.2/B.3 VP3-default tables + §6.4.x quant + DCT-token Huffman + §7.1–§7.5 frame walk + r160 §7.5 motion vectors + r165 §7.6 block-level qi decode + r179 §7.7.1 EOB Token decode (all 7 EOB tokens, residual EOBS run-length output for §7.7.3 driver; 238 tests); §6.4.1 LFLIMS body blocked (#944) | 🚧 scaffold |
| **H.263** | 🚧 ~82% (post-2026-05-18 orphan) — §5.1-§5.4 baseline + §6 IDCT/MV/half-pel/INTER + Annex J §J.3 deblock + Annex I §I.2/§I.3 AIC + Annex D §D.2 UMV + Annex F §F.2 4-MV candidate + Annex F §F.3 OBMC + §5.1.4 PLUSPTYPE + Annex I §I.3 INTRA-coef VLC + Annex K §K.2 Slice-Structured header + r151 Annex F INTER4V driver wiring + r181 Annex I §I.3 absorbed-INTRADC INTRA-block parser (323 tests); lacks Annex K driver wiring + PB-frames + PLUSPTYPE-gated driver + AIC §I.3 DC/AC prediction reconstruction | 🚧 scaffold |
...
| **MS-MPEG-4** (v1/v2/v3) | 🚧 ~37% — clean-room scaffold; v3 intra 3-tier ESC + custom intra-DC VLC + G0..G3 LMAX/RMAX wired + synthetic-VLC end-to-end + v1/v2 CBPY VLC binary↔H.263 Table 8 / MPEG-4 Part 2 Table B-6 cross-check + spec/15 §3 (count_A, count_B) provenance-pinned single-source-of-truth table + inter (P-frame) AC residual decode + r181 `GFamily::subclass_of` / `chroma_selector` / `luma_selector` const-fn accessors per spec/13 §2 + spec/14 §3.1 (274 tests); still lacks G0..G3 primary canonical-Huffman bit-length array (spec/99 §10 OPEN) + alt-MV VLC re-extract. VfW-sandboxed mpg4c32.dll runs in parallel | — |
| **H.264** | 🚧 ~80% — I/P/B + 4:2:0/4:2:2/4:4:4 + CAVLC + CABAC + DPB + 41 SEI types + fuzz-hardened (r164 `sei_payload` cargo-fuzz target + 2772-cell SEI panic-free sweep; r177 §D.2.20 motion_constrained_slice_group_set count-bound + per-id range check closes #1044 1 GiB OOM); lacks MBAFF, SVC/3D/MVC | 🚧 ~83% — I+P (1MV/4MV, ¼-pel) + B + CABAC at all chroma layouts + Trellis-quant RDOQ-lite + IDR Intra_16x16 luma AC trellis + chroma AC trellis (1227 tests); ffmpeg PSNR_Y 44.20 dB |
| **H.265 (HEVC)** | 🚧 ~42% — VPS+SPS+PPS bodies + scaling-list + scan + §9.3 CABAC engine + slice header through r163 §7.3.6.3 pred_weight_table() in-place wiring (universal base-profile; weighted-pred-gated P/B end-to-end through byte_alignment; 201 tests); lacks ref_pic_lists_modification() in-place wiring + §9.3.4.2 binarization+ctxIdx (#444) + §7.4.8 inter-RPS-prediction + residual/IDCT | 🚧 scaffold |
| **H.266 (VVC)** | 🚧 ~68% — 4:2:0 IDR intra + ALF/SAO/CC-ALF + P/B merge+skip + HMVP + MMVD + CIIP + BCW + BDOF + GPM + AMVR + HBD + DMVR + affine sub-block MC + PROF + AMVP + SbTMVP + merge_subblock + §7.3.11.7 non-merge inter affine-syntax dispatcher → MotionModelIdc via §8.5.5.2 eq. 160 + r181 oxideav-bitstream structural VPS §7.3.2.3 (single-layer; 102 tests) (~1047 tests); lacks non-merge inter CU walker call-site + multi-layer VPS | 🚧 ~86% — forward CABAC + DCT-II + SAO/ALF/cu_qp_delta + MTT BT+TT RDO + P+B + sub-pel MC + multi-ref DPB + weighted bi-pred + r177 §7.3.11.7 non-merge affine-syntax encoder dispatcher (Table 84/85 ctx slots; gate-closed = zero-bin per §7.4.12.7) — see crate README |
| **VP6** | 🚧 r8 (post-2026-05-18 orphan) — §9 raw-bit frame-header prefix + §15 inverse-quantization + §16 inverse DCT + §17.x intra/inter reconstruction + §11.3/§11.4/§11.5 deblock/interp/border + §14 DC prediction + §10 mode tables + §13 DCT-token static tables + §7.2 Huffman tree construction + r179 §13.3.3 AC zero-run static surface + §13.3.3.2 Huffman conversion; §7.3 BoolCoder b(n) blocked (#930) | 🚧 scaffold |
| **VP8** | ✅ 100% | ✅ 100% |
| **VP9** | 🚧 ~39% — §6.2 walk + §9.2 Bool decoder + §6.3 compressed-header sweeps + §6.4.24 coeff + §8.6 dequant + §8.7 inverse transforms + §8.5.1 intra pred + §8.6.2 reconstruct + §6.4.3 decode_partition + §6.4.13 read_is_inter + §6.3.9/10/11/14 prob sweeps + r163 §6.3.13 frame_reference_mode_probs (385 tests); lacks §6.3.12 frame_reference_mode + §6.4.4 decode_block + §8.4 loop filter | 🚧 scaffold |
...
| **ProRes** | ✅ ~96% — RDD 36 entropy + 8/10/12-bit + 4:4:4:4 alpha + interlaced + RAW refused; ffmpeg interop 60-68 dB + cargo-fuzz harness (2 panic-free targets) + r161 profiling driver + baseline (decode 296–727 MiB/s, encode 121–326 MiB/s) | ✅ ~95% — RDD 36 all 6 profiles + interlaced + alpha + perceptual quant matrices + multi-frame rate-control + 12-bit cross-decode + ffmpeg cross-decode acceptance (8 cases 58.0-63.8 dB luma PSNR) |
...
| **Ut Video** | ✅ ~97% — 5 native FourCCs × 4 predictors + RGB inter-plane decorrelation + LUT-accelerated canonical Huffman + slice-parallel decode (5.63× at 720p) + criterion baseline (decode 146 MiB/s 1080p ULRG; encode 160 MiB/s) | ✅ ~96% — slice-parallel encode (3.28×, byte-identical to serial) + content-fixture corpus + r161 second cargo-fuzz target encode_utvideo_frame with bit-exact encode→decode roundtrip oracle (11-test stable-CI mirror) |
...
| **Indeo 3** (IV31/IV32) | 🚧 r12 — clean-room codec-frame header + bitstream header + spec/02 picture-layer + spec/03 macroblock-layer + spec/04 VQ codebook + spec/06 byte-level entropy + spec/07 output-reconstruction + four cell-shape kernels (A/B/C/D) + spec/02 strip-context array + spec/03 per-cell sub-array wiring + r181 spec/05 §1 indeo3::mc_table per-plane packed-MV table layout + `MvTableParserArm` §1.2 dispatch + `MvIndexFetch` §1.3 INTER-leaf indexing + `MvIndexValidity` §1.4 classifier (212 tests); lacks cell-stack pre-population (spec/03 §6 Q4) / pixel-buffer edge fix-up / MC dispatch (spec/05 §2..§5) | — |
...
| **AVIF** | 🚧 ~82% — HEIF→AV1 + grid + imir/clap/colr/pixi/pasp + HDR metadata + AV1 wrap pass-through + DoS caps + HEIF item-properties (infe v2/v3 mime/uri tail + thmb/cdsc/prem iref + Exif/XMP item resolver) + auxC URN routing (Alpha / Depth / HDR-gain-map) + rloc / lsel / iovl / grpl parsers + `mif1` compliance audit + a1op/a1lx AV1 layered-image properties + essential-property enforcement + r130 tmap av1-avif §4.2.2 file-shape audit (paired altr group + hidden inputs compliance checker) + r172 av1-avif §7 grid-derivation transformative-property audit (per-tile clap/irot/imir prohibited in dimg chain); AV1 pixel decode gated on sibling rebuild | — |
...
| **FBX** | 🚧 ~67% — binary container (32/64-bit) + object-graph + mesh + animation (TRS+DeformPercent) + deformers (Skin / Cluster / BlendShape) + Material / Texture / Video surfacing via Connections walker (embedded Video.Content R-blobs + OP typed PBR routing for DiffuseColor / NormalMap / EmissiveColor / metallic / occlusion) + bind pose (Pose/BindPose → node extras + skeleton inverse-bind refine) + r178 multi-material slot table via LayerElementMaterial. Lacks: ASCII FBX (#785), Properties70 P-record grammar + Light/Camera NodeAttribute | ✅ ~58% — symmetric binary writer + opt-in zlib deflate; Blender/ufbx-readable round-trip |
...
|--------|----------|--------|--------|-------|
| **`oxideav-videotoolbox`** | macOS (Apple Silicon + Intel Macs) | 🚧 H.264 + HEVC + ProRes + MJPEG + MPEG-2 + VP9 | 🚧 H.264 + HEVC + ProRes + MJPEG | Roadmap: AV1 / MPEG-4 Pt 2. MPEG-2 decode-only ~61 dB PSNR-Y. H.264 ~51 dB PSNR-Y, HEVC ~54 dB, ProRes ~52 dB, MJPEG ~36 dB. r178 VP9 wired (HW M1+; ffmpeg-cross-decode PSNR_Y ≥ 30 dB). AV1 hardware needs M3+. |
| **`oxideav-audiotoolbox`** | macOS | 🚧 AAC LC + HE-AAC v1/v2 + AAC-LD/ELD + ALAC | 🚧 AAC LC + HE-AAC v1/v2 + AAC-LD/ELD + ALAC | AAC LC 36.7 dB @ 128 kbit/s; HE-AAC v1 ~11 dB @ 64 kbit/s stereo; HE-AAC v2 ~10 dB @ 32 kbit/s stereo (PS requires stereo); ALAC bit-exact 190,464 / 192,000 samples; r178 AAC encoder reports actual quantised bitrate via AudioConverterGetProperty read-back (downstream ADTS buffer_fullness / MP4 btrt now accurate). Roadmap: FLAC, Opus, AMR-NB/WB, iLBC. |
...
|-----------|------|--------|
| **`oxideav-source`** | URI resolution + file reader + prefetching BufferedSource | ✅ `file://` + `mem://` + `data:` (RFC 2397 inline base64/percent) + `concat:` (`|`-separated `file://` segments → one seekable stream) + r178 `slice:<offset>+<length>!<inner-uri>` byte-window driver (composes recursively over file/mem/data) + `FileScope` allow-list policy; generic `SourceRegistry` for pluggable schemes |
| **`oxideav-http`** | HTTP / HTTPS source driver | ✅ `http://` + `https://` via pure-Rust `ureq` + `rustls` + `webpki-roots`; Range-request seeking; `HttpConfig` policy layer (timeouts, redirect cap, custom headers) + r171 RFC 7233 §4.2 Content-Range validation on 206 + §3.1 200-fallback prefix-drop + r179 RFC 9110 §15.5.17 + §14.4 416 (Range Not Satisfiable) handling (surfaces server's authoritative resource length) |
