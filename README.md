...
| **MP3** | ✅ 100% — bit-exact decode + free-format frames + ID3v2/Xing seek + 13818-3 Table B.2 LSF bands (reference-PCM-exact) | 🚧 ~99.8% — full Layer III + joint stereo + MPEG-2 LSF/MPEG-2.5 encode (CBR/VBR/MS/CRC) + §2.4.3.2 LSF + §2.4.3.4.9.3 short-block (per-window) intensity-stereo incl. auto-MS + short + intensity combined + auto-block-type short granules with intensity + §C.1.5.2 LSF/MPEG-2.5 auto block-type + §C.1.5.3.2.1 Model-2-driven (pe>1800) block-type switching + §C.1.5.3 scfsi scalefactor-selection-info + §C.1.5.4.4.6 band-aligned SUBDIVIDE + Model 2 psychoacoustic threshold in the outer loop; lacks only MPEG-2.5 band tables (docs ask) |
| **AAC** | 🚧 ~48% — ADTS + full channel-body parse + §4.6.1.3/§4.6.2.3.3 dequantisation + §4.6.11 IMDCT/sine-KBD filterbank + overlap-add (streaming TDAC perfect-reconstruction) + §4.6.8.1 M/S stereo de-matrix + §4.6.13 PNS synthesis; lacks intensity synthesis for byte-exact PCM | 🚧 ~20% — Phase-2 writers for every syntax element + TNS decode tool; SBR pending QMF |
